import RicciFlower.Analysis.ODE.FlowVariationalLinearMap
  ...
  LinearMap.mkContinuousOfExistsBound
    ...
    (by
      ...
      have h_normDA_cont : ContinuousOn (fun s => ‖fderiv ℝ (fun y => A y s) x‖)
          (Set.Icc α β) :=
        (continuous_norm (E := F →L[ℝ] (G →L[ℝ] G))).comp_continuousOn hDAx_cont_Icc
      obtain ⟨σP, _, hP_bd⟩ := hIcc_cpt.exists_isMaxOn hIcc_ne h_normDA_cont
      let Pv : ℝ := ‖fderiv ℝ (fun y => A y σP) x‖
      have hPv_nn : 0 ≤ Pv := ContinuousLinearMap.opNorm_nonneg _
...
theorem linearODESolution_hasFDerivAt_param
  ...
  have hDAnorm_cont_K : ContinuousOn
    ...
    (continuous_norm (E := F →L[ℝ] (G →L[ℝ] G))).comp_continuousOn hDA_cont_K
  ...
  have h_taylor : ∀ ε > (0 : ℝ), ∃ δ_ε > (0 : ℝ), δ_ε ≤ δ₀ ∧
    ...
    intro ε hε
    have hUC_norm :
        @UniformContinuousOn (F × ℝ) (F →L[ℝ] (G →L[ℝ] G)) _ PseudoMetricSpace.toUniformSpace
          (fun p : F × ℝ => fderiv ℝ (fun z => A z p.2) p.1) K := by
      exact hUC
    rw [Metric.uniformContinuousOn_iff_le] at hUC_norm
    obtain ⟨δ', hδ'_pos, hδ'_unif⟩ := hUC_norm ε hε
    ...
    have h_pt_bd : ∀ y ∈ Metric.closedBall x ‖h‖,
      ...
      rw [dist_eq_norm
        (fderiv ℝ (fun z => A z s) y)
        (fderiv ℝ (fun z => A z s) x)] at hunif
      exact hunif
