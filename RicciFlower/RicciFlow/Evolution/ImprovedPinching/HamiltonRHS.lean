...
theorem pinchEvol_setup
    ...
    (htfLap : forall (t : Realized.RealTimeInterval.RegularTime D) x,
      tfNormLap (t : Real) x =
        Realized.laplacianAt (I := I) G (t : Real)
          (tfRicNormSq scalar ricciNormSq (t : Real)) x)
    (hscalarLap : forall (t : Realized.RealTimeInterval.RegularTime D) x,
      scalarLap (t : Real) x =
        Realized.laplacianAt (I := I) G (t : Real)
          (scalar (t : Real)) x)
    (htfDiff : forall (t : Realized.RealTimeInterval.RegularTime D) y,
      MDifferentiableAt I 𝓘(Real, Real)
        (tfRicNormSq scalar ricciNormSq (t : Real)) y)
    (hscalarDiff : forall (t : Realized.RealTimeInterval.RegularTime D) y,
      MDifferentiableAt I 𝓘(Real, Real) (scalar (t : Real)) y)
    (htfNonneg : forall (t : Realized.RealTimeInterval.RegularTime D) y,
      0 <= tfRicNormSq scalar ricciNormSq (t : Real) y)
    (hscalarPos : forall (t : Realized.RealTimeInterval.RegularTime D) y,
      0 < scalar (t : Real) y)
    (hgradTf : forall (t : Realized.RealTimeInterval.RegularTime D) x,
      MDiffAt (T% fun y : M =>
        Realized.gradientFun (I := I) (G.metric (t : Real))
          (tfRicNormSq scalar ricciNormSq (t : Real)) y) x)
    (hgradScalar : forall (t : Realized.RealTimeInterval.RegularTime D) x,
      MDiffAt (T% fun y : M =>
        Realized.gradientFun (I := I) (G.metric (t : Real))
          (scalar (t : Real)) y) x)
    (hgradScalarPow : forall (t : Realized.RealTimeInterval.RegularTime D) y,
      MDiffAt (T% fun z : M =>
        Realized.gradientFun (I := I) (G.metric (t : Real))
  ...
  fun t x =>
    normSq0S (I := I) (g t) x 3
      (ricciGradCoupleAt (I := I)
        ...
        (Realized.differential1FormFun (I := I) (scalar t) x))
    ...
    calc
      inner0S (I := I) (G.metric t) x 1
          ...
          = (G.metric t).inner x
              ...
              simpa [Realized.gradientAt] using
                Realized.inner0S_differential1FormFun_pair_eq_grad_inner
...
theorem ricciMixed_eq_gradNorm
    ...
    (gInv : Idx -> Idx -> Real)
    (hinv : MetricInverseInBasis (I := I) (G.metric t) x basis gInv)
    (hmc : RicciFlower.Connection.IsMetricCompatible
    ...
    (hdu :
      Realized.DuFieldRealizes (I := I)
        (fun y : M => Realized.normSq02 (I := I) (G.metric t) y (RicSec y))
        duRicNorm) :
    2 * inner0S (I := I) (G.metric t) x 3 (nablaRicSec x)
      ...
      (G.metric t).inner x
        (Realized.gradientAt (I := I) G t
          (fun y : M =>
            Realized.normSq02 (I := I) (G.metric t) y (RicSec y)) x)
        (Realized.gradientAt (I := I) G t (scalar t) x) := by
      ...
      inner0S (I := I) (G.metric t) x 3 (nablaRicSec x)
        ...
        inner0S (I := I) (G.metric t) x 2
          ((tensor0S_curry (I := I) (𝕜 := Real) (M := M) 2 x
              (nablaRicSec x))
            (cotangentSharp (I := I) (G.metric t) x
              (Realized.differential1FormFun (I := I) (scalar t) x)))
          (RicSec x) := by
    exact inner0S_three_product_right (I := I) (G.metric t) x
      basis gInv hinv (nablaRicSec x)
      (Realized.differential1FormFun (I := I) (scalar t) x) (RicSec x)
  have hsharp :
      cotangentSharp (I := I) (G.metric t) x
          (Realized.differential1FormFun (I := I) (scalar t) x) = W := by
    simpa [W, Realized.gradientAt] using
      Realized.cotangentSharp_differential1FormFun_eq_gradientFun
        (I := I) (G.metric t) (scalar t) x
  have hduNorm :=
    Realized.du_norm02 (I := I) (G.connection t) (G.metric t) hmc
  ...
  have hinner_du :
    ...
    rw [hduNorm]
    simp [Realized.inner02, Realized.tensor02FreezeNabla_eq_curry]
  have hdu_grad :
      duRicNorm x (fun _ : Fin 1 => W) =
        (G.metric t).inner x
          (Realized.gradientAt (I := I) G t normFun x) W := by
    rw [hdu x]
    change
      Realized.differential1FormFun (I := I) normFun x
          (fun _ : Fin 1 => W) =
        (G.metric t).inner x
          (Realized.gradientAt (I := I) G t normFun x) W
    simpa [Realized.gradientAt] using
      Realized.differential1FormFun_apply_eq_inner_gradientFun
  ...
  calc
    ...
    _ = (G.metric t).inner x
          (Realized.gradientAt (I := I) G t normFun x) W := hdu_grad
    _ = (G.metric t).inner x
        (Realized.gradientAt (I := I) G t
          (fun y : M =>
            Realized.normSq02 (I := I) (G.metric t) y (RicSec y)) x)
        (Realized.gradientAt (I := I) G t (scalar t) x) := rfl
...
theorem ricciMixed_eq_tfGrad
    ...
    (hricNorm : forall y : M,
      ricciNormSq t y =
        Realized.normSq02 (I := I) (G.metric t) y (RicSec y))
    (hnormDiff : MDifferentiableAt I 𝓘(Real, Real)
      (fun y : M => Realized.normSq02 (I := I) (G.metric t) y (RicSec y)) x)
  ...
  have hgradSq :
      Realized.gradientAt (I := I) G t sqFun x =
        (2 * R x) • Realized.gradientAt (I := I) G t R x := by
    unfold Realized.gradientAt
    exact Realized.gradientFun_mul_self (I := I) (G.metric t) hscalarDiff
  have hgradSqFun :
      Realized.gradientFun (I := I) (G.metric t) sqFun x =
        (2 * R x) • Realized.gradientFun (I := I) (G.metric t) R x := by
    simpa [Realized.gradientAt] using hgradSq
  have hgradTf :
      Realized.gradientAt (I := I) G t
          (tfRicNormSq scalar ricciNormSq t) x =
        Realized.gradientAt (I := I) G t normFun x -
          (((2 : Real) / 3) * R x) •
            Realized.gradientAt (I := I) G t R x := by
    rw [htfFun]
    unfold Realized.gradientAt
    rw [Realized.gradientFun_sub (I := I) (G.metric t) hnormDiff hthirdDiff]
    rw [Realized.gradientFun_const_smul (I := I) (G.metric t)
      (1 / 3 : Real) hsqDiff]
    change
      Realized.gradientFun (I := I) (G.metric t) normFun x -
          (1 / 3 : Real) • Realized.gradientFun (I := I) (G.metric t) sqFun x =
        Realized.gradientFun (I := I) (G.metric t) normFun x -
          (((2 : Real) / 3) * R x) •
            Realized.gradientFun (I := I) (G.metric t) R x
    rw [hgradSqFun]
    simp [smul_smul, R]
    ring_nf
  have hgradNorm :
    ...
    _ = scalar t x *
        ((G.metric t).inner x
          ...
          (Realized.gradientAt (I := I) G t (scalar t) x) +
            ((2 : Real) / 3) * scalar t x * gradScalarNormSq t x) := by
          rw [hgradNorm]
          simp [R, Realized.gradientAt, hgradScalarSq, smul_eq_mul,
            mul_assoc, mul_comm]
...
theorem pinchDrift_exp
  ...
  have hmul :
      Realized.gradientAt (I := I) G t (fun y : M => phi y * Rpow y) x =
        phi x • Realized.gradientAt (I := I) G t Rpow x +
          Rpow x • Realized.gradientAt (I := I) G t phi x := by
    exact Realized.gradientAt_mul (I := I) G t
      (f := phi) (h := Rpow) (x := x) htfDiff hRpowDiff
  have hpow :
      Realized.gradientAt (I := I) G t Rpow x =
        (-(2 - epsilon) * R x ^ (-(2 - epsilon) - 1)) •
          Realized.gradientAt (I := I) G t R x := by
    exact Realized.gradientAt_rpow (I := I) G t
  ...
  unfold Realized.gradientAt Realized.gradientFun Realized.metricSharp at hinnerSymm
  rw [hinnerSymm]
...
theorem pinchEvol_book_of_couple
    ...
    (hscalar : forall (t : Realized.RealTimeInterval.RegularTime D) x,
      0 < scalar (t : Real) x)
    (hgradScalarSq : forall (t : Realized.RealTimeInterval.RegularTime D) x,
      gradScalarNormSq (t : Real) x =
        (G.metric (t : Real)).inner x
          (Realized.gradientAt (I := I) G (t : Real)
            (scalar (t : Real)) x)
          (Realized.gradientAt (I := I) G (t : Real)
            (scalar (t : Real)) x))
    ...
    (hcouple : forall (t : Realized.RealTimeInterval.RegularTime D) x,
      coupleSq (t : Real) x =
...
theorem pinchEvol_book_of_mixed
    ...
    (hscalarDiff : forall (t : Realized.RealTimeInterval.RegularTime D) x,
      MDifferentiableAt I 𝓘(Real, Real) (scalar (t : Real)) x)
    (basis : forall (_t : Realized.RealTimeInterval.RegularTime D) (x : M),
      Module.Basis Idx Real (TangentSpace I x))
    (gInv : forall (_t : Realized.RealTimeInterval.RegularTime D) (_x : M),
      Idx -> Idx -> Real)
    (hinv : forall (t : Realized.RealTimeInterval.RegularTime D) (x : M),
      MetricInverseInBasis (I := I) (G.metric (t : Real)) x
        (basis t x) (gInv t x))
    (hnabla : forall (t : Realized.RealTimeInterval.RegularTime D) (x : M),
    ...
    (hric : forall (t : Realized.RealTimeInterval.RegularTime D) (x : M),
      ricciNormSq (t : Real) x =
        normSq0S (I := I) (G.metric (t : Real)) x 2
          (Ric (t : Real) x))
    (hmixed : forall (t : Realized.RealTimeInterval.RegularTime D) (x : M),
      2 * scalar (t : Real) x *
...
theorem pinchEvol_sec
    ...
    (hric : forall (t : Realized.RealTimeInterval.RegularTime D) (x : M),
      ricciNormSq (t : Real) x =
        normSq0S (I := I) (G.metric (t : Real)) x 2
          (RicSec (t : Real) x))
    (hmc : forall (t : Realized.RealTimeInterval.RegularTime D),
      RicciFlower.Connection.IsMetricCompatible
        (I := I) (G.connection (t : Real)) (G.metric (t : Real)))
    (hRicNabla : forall (t : Realized.RealTimeInterval.RegularTime D),
      TotalNabla0SRealizes (𝕜 := Real) (E := E) (H := H) (I := I)
        (M := M) 2 (G.connection (t : Real))
        (RicSec (t : Real)) (nablaRicSec (t : Real)))
    (hdu : forall (t : Realized.RealTimeInterval.RegularTime D),
      Realized.DuFieldRealizes (I := I)
        (fun y : M =>
          Realized.normSq02 (I := I) (G.metric (t : Real)) y
            (RicSec (t : Real) y))
        (duRicNorm (t : Real)))
    (hricNorm : forall (t : Realized.RealTimeInterval.RegularTime D) y,
      ricciNormSq (t : Real) y =
        Realized.normSq02 (I := I) (G.metric (t : Real)) y
          (RicSec (t : Real) y))
    (hnormDiff : forall (t : Realized.RealTimeInterval.RegularTime D) x,
      MDifferentiableAt I 𝓘(Real, Real)
        (fun y : M =>
          Realized.normSq02 (I := I) (G.metric (t : Real)) y
            (RicSec (t : Real) y)) x) :
