...
import RicciFlower.Riemann.Basic.Sections
...
private theorem directionalDeriv_pullback_pushFwd
    ...
    (A P Q : ContMDiffSection I E (∞ : WithTop ℕ∞)
      (TangentSpace I : M -> Type _)) (x : M) :
    LeviCivita.directionalDeriv (I := I) (fun p : M => A p)
        (fun y : M =>
          (Diffeomorph.pullbackMetric (I := I) g Phi).inner y (P y) (Q y)) x =
      LeviCivita.directionalDeriv (I := I)
  ...
  rw [hfun]
  unfold LeviCivita.directionalDeriv
  dsimp only
  ...
  have hG_diff :
    ...
    (Riemann.CovariantDerivative.metric_inner_contMDiffAt (I := I) (M := N) g
...
private theorem koszulScalar_pullback_pushFwd
    ...
    (A B C : ContMDiffSection I E (∞ : WithTop ℕ∞)
      (TangentSpace I : M -> Type _)) (x : M) :
    LeviCivita.koszulScalar (I := I) (Diffeomorph.pullbackMetric (I := I) g Phi)
        (fun p : M => A p) (fun p : M => B p) (fun p : M => C p) x =
      LeviCivita.koszulScalar (I := I) g
        (fun q : N => pushFwdSection (I := I) Phi A q)
        (fun q : N => pushFwdSection (I := I) Phi B q)
        (fun q : N => pushFwdSection (I := I) Phi C q) (Phi x) := by
  unfold LeviCivita.koszulScalar
  dsimp only
...
private theorem metricCov_pullback_pushFwd_apply
  ...
  have hkoszul_h :
    ...
    exact LeviCivita.leviCivitaConnectionOfMetric_inner_eq_koszulScalar
...
private theorem connectionRiemannCurvatureField_pullback_pushFwd
  ...
  let ZYh : ContMDiffSection I E (∞ : WithTop ℕ∞) (TangentSpace I : M -> Type _) :=
    ⟨fun p : M => (covh (fun q : M => Z q) p) (Y p),
      fun p => Riemann.CovariantDerivative.cov_smooth_apply_contMDiffAt
...
theorem metricRm04Std_pullback
  ...
  unfold metricRm04At
  have hleft :=
    Riemann.CovariantDerivative.riemannCurvature04At_apply_smooth
      (I := I)
  ...
  rw [show Riemann.CovariantDerivative.riemannCurvature04At
        (Diffeomorph.pullbackMetric (I := I) g Phi)
        (metricCov (I := I) (M := M) (Diffeomorph.pullbackMetric (I := I) g Phi))
