...
proof.  The invariant input is still `Realized.canScalHess`; this file packages
...
geometric proof is `Realized.canScalHess`. -/
...
theorem frameInvMetric_real
    ...
    {x₀ : M} {C : LocalChartAt (I := I) x₀} (F : C.Frame)
    {x : M} (hx : x ∈ F.domain) :
    MetricInverseInBasis (I := I) (M := M) g x (F.basisAt hx)
