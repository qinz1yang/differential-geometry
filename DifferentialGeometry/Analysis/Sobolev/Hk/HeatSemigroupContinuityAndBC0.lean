...
`heatSemigroupHkExt g σ : ℝ → scalarHs g σ →L[ℝ] scalarHs g σ`
is strongly continuous on the non-negative half-line: for every
`u : scalarHs g σ`, the map `t ↦ heatSemigroupHkExt g σ t u` is
...
* the operator-norm contraction
  `‖heatSemigroupHkExt g σ t‖ ≤ 1` for `t ≥ 0`
  (`heatSemigroupHkExt_opNorm_le_one`), and
* the semigroup law on `t, s ≥ 0`
  (`heatSemigroupHkExt_add`),

which together yield, for any `t, t₀ ≥ 0`,

  `‖heatSemigroupHkExt g σ t u − heatSemigroupHkExt g σ t₀ u‖`
  `  ≤ ‖heatSemigroupHkExt g σ |t − t₀| u − u‖`.

Hence strong continuity at every `t₀ ≥ 0` reduces to right-continuity at
`0`, i.e. `heatSemigroupHkExt g σ τ u → u` as `τ ↓ 0`. Right-continuity
...
3. handle the finite-rank piece `heatSemigroupHkExt g σ τ u' − u'`
...
* `heatSemigroupHkExt_continuousOn` — strong continuity of the heat
  semigroup on `[0, ∞)`.
...
`heatSemigroupHkExt g σ t u − heatSemigroupHkExt g σ t₀ u`
has norm bounded by `‖heatSemigroupHkExt g σ |t − t₀| u − u‖`. The proof
writes the larger of `t, t₀` as the sum of the smaller and `|t − t₀|`,
applies the semigroup law `heatSemigroupHkExt_add`, and finishes by
contractivity `heatSemigroupHkExt_opNorm_le_one`. -/

private lemma norm_heatSemigroupHkExt_sub_le_diff
    {g : SmoothRiemannianMetric I M} {σ : ℝ}
    {t t₀ : ℝ} (ht : 0 ≤ t) (ht₀ : 0 ≤ t₀)
    (u : scalarHs (I := I) (M := M) g σ) :
    ‖heatSemigroupHkExt (I := I) (M := M) g σ t u -
        heatSemigroupHkExt (I := I) (M := M) g σ t₀ u‖ ≤
      ‖heatSemigroupHkExt (I := I) (M := M) g σ |t - t₀| u - u‖ := by
  ...
  · -- `t ≥ t₀`. Write `t = t₀ + (t − t₀)`.
    ...
    have h_law :
        heatSemigroupHkExt (I := I) (M := M) g σ t =
          (heatSemigroupHkExt (I := I) (M := M) g σ t₀).comp
            (heatSemigroupHkExt (I := I) (M := M) g σ (t - t₀)) := by
      have h_add :=
        heatSemigroupHkExt_add (I := I) (M := M)
    ...
    have h_sub_eq :
      ...
      rw [← (heatSemigroupHkExt (I := I) (M := M) g σ t₀).map_sub]
    rw [h_sub_eq]
    have h_op_le :=
      ContinuousLinearMap.le_opNorm
        (heatSemigroupHkExt (I := I) (M := M) g σ t₀)
        (heatSemigroupHkExt (I := I) (M := M) g σ (t - t₀) u - u)
    have h_op_le_one :
        ‖heatSemigroupHkExt (I := I) (M := M) g σ t₀‖ ≤ 1 :=
      heatSemigroupHkExt_opNorm_le_one
        (I := I) (M := M) (g := g) (σ := σ) ht₀
    have h_norm_nn :
        0 ≤ ‖heatSemigroupHkExt
            (I := I) (M := M) g σ (t - t₀) u - u‖ := norm_nonneg _
    calc ‖heatSemigroupHkExt (I := I) (M := M) g σ t₀
              (heatSemigroupHkExt
                (I := I) (M := M) g σ (t - t₀) u - u)‖
        ≤ ‖heatSemigroupHkExt (I := I) (M := M) g σ t₀‖ *
            ‖heatSemigroupHkExt
              (I := I) (M := M) g σ (t - t₀) u - u‖ := h_op_le
      _ ≤ 1 * ‖heatSemigroupHkExt
              (I := I) (M := M) g σ (t - t₀) u - u‖ :=
            mul_le_mul_of_nonneg_right h_op_le_one h_norm_nn
      _ = ‖heatSemigroupHkExt
...
`heatSemigroupHkExt g σ τ u' − u'` has coordinates
...
`heatSemigroupHkExt g σ τ u' − u'` is a finite sum of squared spectral
terms. -/
private lemma sq_norm_heatSemigroupHkExt_sub_self_of_finite
    {g : SmoothRiemannianMetric I M} {σ : ℝ}
    {τ : ℝ} (hτ : 0 ≤ τ)
    {u' : scalarHs (I := I) (M := M) g σ}
    (hu' : u' ∈ scalarHs.finiteSupportSubmodule
      (I := I) (M := M) g σ) :
    ‖heatSemigroupHkExt (I := I) (M := M) g σ τ u' - u'‖ ^ 2 =
  ...
  have h_norm_sq := scalarHs.norm_sq_eq_tsum
    (I := I) (M := M)
    (heatSemigroupHkExt (I := I) (M := M) g σ τ u' - u')
  -- Coefficient of the difference at index `i`.
  have h_diff_coeff : ∀ i,
      (heatSemigroupHkExt (I := I) (M := M) g σ τ u' - u').coeff i =
        (Real.exp (-(EigenIdx.lambda (I := I) (M := M) i) * τ) - 1) *
          u'.coeff i := by
    intro i
    have h_sub : (heatSemigroupHkExt (I := I) (M := M) g σ τ u' - u').coeff i =
        (heatSemigroupHkExt (I := I) (M := M) g σ τ u').coeff i - u'.coeff i := by
      change (fun j => (heatSemigroupHkExt
            (I := I) (M := M) g σ τ u').coeff j - u'.coeff j) i = _
      rfl
    rw [h_sub, heatSemigroupHkExt_coeff (I := I) (M := M)
      (g := g) (σ := σ) hτ u' i]
    ring
  -- Rewrite the tsum summand using the explicit difference coefficient.
  have h_tsum_eq :
      ∑' i, scalarSobolevWeight (I := I) (M := M) i σ *
          ((heatSemigroupHkExt (I := I) (M := M) g σ τ u' - u').coeff i) ^ 2 =
      ∑' i, scalarSobolevWeight (I := I) (M := M) i σ *
          ((Real.exp (-(EigenIdx.lambda (I := I) (M := M) i) * τ) - 1) *
            u'.coeff i) ^ 2 := by
...
`τ ↦ heatSemigroupHkExt g σ τ u'` is continuous at `0` within `Ici 0`. -/
private lemma tendsto_heatSemigroupHkExt_of_finite
    {g : SmoothRiemannianMetric I M} {σ : ℝ}
    {u' : scalarHs (I := I) (M := M) g σ}
    (hu' : u' ∈ scalarHs.finiteSupportSubmodule
      (I := I) (M := M) g σ) :
    Tendsto (fun τ : ℝ =>
        heatSemigroupHkExt (I := I) (M := M) g σ τ u')
  ...
  suffices h_norm_to_zero :
    ...
    have h_diff_to_zero :
      ...
      (tendsto_zero_iff_norm_tendsto_zero
        (f := fun τ : ℝ =>
          heatSemigroupHkExt (I := I) (M := M) g σ τ u' - u')).mpr h_norm_to_zero
    have h_added :=
  ...
  have h_sq_to_zero :
    ...
    have h_rewrite :
        (fun τ : ℝ =>
          ‖heatSemigroupHkExt (I := I) (M := M) g σ τ u' - u'‖ ^ 2) =ᶠ[𝓝[Set.Ici (0 : ℝ)] 0]
        (fun τ : ℝ =>
      ...
      simpa [hF_def] using
        sq_norm_heatSemigroupHkExt_sub_self_of_finite
  ...
  have h_norm_eq_sqrt :
      ∀ τ : ℝ,
        ‖heatSemigroupHkExt (I := I) (M := M) g σ τ u' - u'‖ =
          Real.sqrt
            (‖heatSemigroupHkExt (I := I) (M := M) g σ τ u' - u'‖ ^ 2) := by
    intro τ
...
private lemma tendsto_heatSemigroupHkExt_at_zero
    (g : SmoothRiemannianMetric I M) (σ : ℝ)
    (u : scalarHs (I := I) (M := M) g σ) :
    Tendsto (fun τ : ℝ =>
        heatSemigroupHkExt (I := I) (M := M) g σ τ u)
      (𝓝[Set.Ici (0 : ℝ)] 0) (𝓝 u) := by
  classical
  -- Reduce to `‖heatSemigroupHkExt g σ τ u - u‖ → 0`.
  suffices h_norm_to_zero :
      Tendsto (fun τ : ℝ =>
          ‖heatSemigroupHkExt (I := I) (M := M) g σ τ u - u‖)
        (𝓝[Set.Ici (0 : ℝ)] 0) (𝓝 0) by
  ...
  have hu'_tendsto :=
    tendsto_heatSemigroupHkExt_of_finite
  ...
  have h_eventually :
      ∀ᶠ τ in 𝓝[Set.Ici (0 : ℝ)] 0,
        ‖heatSemigroupHkExt (I := I) (M := M) g σ τ u' - u'‖ < ε / 3 := by
  ...
  have h_decomp :
      heatSemigroupHkExt (I := I) (M := M) g σ τ u - u =
        heatSemigroupHkExt (I := I) (M := M) g σ τ (u - u') +
          (heatSemigroupHkExt (I := I) (M := M) g σ τ u' - u') +
            (u' - u) := by
    rw [(heatSemigroupHkExt (I := I) (M := M) g σ τ).map_sub]
    abel
  have h_first_norm :
      ‖heatSemigroupHkExt (I := I) (M := M) g σ τ (u - u')‖ ≤
        ‖u - u'‖ := by
    have h_le :=
      ContinuousLinearMap.le_opNorm
        (heatSemigroupHkExt (I := I) (M := M) g σ τ) (u - u')
    have h_op :
        ‖heatSemigroupHkExt (I := I) (M := M) g σ τ‖ ≤ 1 :=
      heatSemigroupHkExt_opNorm_le_one
        (I := I) (M := M) (g := g) (σ := σ) hτ_nn
    have h_nn : 0 ≤ ‖u - u'‖ := norm_nonneg _
    calc ‖heatSemigroupHkExt (I := I) (M := M) g σ τ (u - u')‖
        ≤ ‖heatSemigroupHkExt (I := I) (M := M) g σ τ‖ * ‖u - u'‖ := h_le
  ...
  have h_first_lt : ‖heatSemigroupHkExt (I := I) (M := M) g σ τ (u - u')‖
  ...
  have h_total_norm :
    ...
    calc ‖heatSemigroupHkExt (I := I) (M := M) g σ τ (u - u') +
        ...
        ≤ ‖heatSemigroupHkExt (I := I) (M := M) g σ τ (u - u') +
              (heatSemigroupHkExt (I := I) (M := M) g σ τ u' - u')‖ +
            ‖u' - u‖ := norm_add_le _ _
      _ ≤ ‖heatSemigroupHkExt (I := I) (M := M) g σ τ (u - u')‖ +
              ‖heatSemigroupHkExt (I := I) (M := M) g σ τ u' - u'‖ +
            ‖u' - u‖ := by
          gcongr
...
`t ↦ heatSemigroupHkExt g σ t u` is continuous on `Set.Ici 0`. -/
theorem heatSemigroupHkExt_continuousOn (g : SmoothRiemannianMetric I M)
    (σ : ℝ) (u : scalarHs (I := I) (M := M) g σ) :
    ContinuousOn (fun t : ℝ =>
        heatSemigroupHkExt (I := I) (M := M) g σ t u) (Set.Ici 0) := by
  intro t₀ ht₀
  ...
  · -- `t₀ > 0`: two-sided continuity via the contractive comparison.
    ...
    rw [show (𝓝 (heatSemigroupHkExt (I := I) (M := M) g σ t₀ u)) =
        𝓝 (0 +
          heatSemigroupHkExt (I := I) (M := M) g σ t₀ u) by rw [zero_add]]
    have h_diff_to_zero :
        Tendsto (fun t : ℝ =>
            heatSemigroupHkExt (I := I) (M := M) g σ t u -
              heatSemigroupHkExt (I := I) (M := M) g σ t₀ u)
      ...
      have h_bound_event : ∀ᶠ t in 𝓝[Set.Ici (0 : ℝ)] t₀,
        ...
        exact norm_heatSemigroupHkExt_sub_le_diff
          (I := I) (M := M) (g := g) (σ := σ)
      ...
      have h_strong :=
        tendsto_heatSemigroupHkExt_at_zero
          (I := I) (M := M) g σ u
      have h_compose :
          Tendsto (fun t : ℝ =>
              heatSemigroupHkExt (I := I) (M := M) g σ |t - t₀| u)
            (𝓝[Set.Ici (0 : ℝ)] t₀) (𝓝 u) :=
    ...
    have h_added :=
      h_diff_to_zero.add (tendsto_const_nhds
        (x := heatSemigroupHkExt (I := I) (M := M) g σ t₀ u))
    simpa using h_added
  · -- `t₀ = 0`: use right-continuity at `0`.
    subst ht₀_eq
    rw [ContinuousWithinAt]
    have h_at_zero :
        heatSemigroupHkExt (I := I) (M := M) g σ 0 u = u := by
      rw [heatSemigroupHkExt_zero]; rfl
    rw [h_at_zero]
    exact tendsto_heatSemigroupHkExt_at_zero
      (I := I) (M := M) g σ u
...
noncomputable def scalarHsBoundedC0Semigroup
  ...
  toFun := fun t => heatSemigroupHkExt (I := I) (M := M) g σ t
  apply_zero := heatSemigroupHkExt_zero (I := I) (M := M) g σ
  apply_add := fun _ _ ht hs =>
    heatSemigroupHkExt_add (I := I) (M := M) (g := g) (σ := σ) ht hs
  opNorm_le_one := fun _ ht =>
    heatSemigroupHkExt_opNorm_le_one (I := I) (M := M)
      (g := g) (σ := σ) ht
  continuousOn_apply := fun u =>
    heatSemigroupHkExt_continuousOn (I := I) (M := M) g σ u
...
theorem scalarHsBoundedC0Semigroup_apply
    ...
    scalarHsBoundedC0Semigroup (I := I) (M := M) g σ t =
      heatSemigroupHkExt (I := I) (M := M) g σ t := rfl
