...
private lemma repr_coord_abs_le (x : E) (i : Fin (Module.finrank ℝ E)) :
    |((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E).repr x) i| ≤ basisCoordSup (E := E) * ‖x‖ := by
...
private lemma basis_vec_norm_le (k : Fin (Module.finrank ℝ E)) :
    ‖(DifferentialGeometry.Tensor.Coordinates.chartModelBasis E) k‖ ≤ basisVecSup (E := E) := by
  exact Finset.le_sup' (f := fun k => ‖(DifferentialGeometry.Tensor.Coordinates.chartModelBasis E) k‖) (Finset.mem_univ _)
