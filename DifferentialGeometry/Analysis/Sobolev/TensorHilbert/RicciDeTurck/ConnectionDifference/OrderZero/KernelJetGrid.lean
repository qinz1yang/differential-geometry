...
theorem connectionDifferenceContrInsertionInnerFib_contMDiff (g₀ g₁ : SmoothRiemannianMetric I M) :
    ContMDiff I (I.prod 𝓘(ℝ, Tensor0SBundle.TensorRSModel 2 3 ℝ E)) ∞
      (fun x : M => TotalSpace.mk' (Tensor0SBundle.TensorRSModel 2 3 ℝ E)
        ...
        (show Tensor0SBundle.TensorRSSpace 2 3 I x from
          connContrCLM (I := I) 1 1 x ((connectionDifferenceSection (I := I) g₁ g₀).toSection x))) := by
...
def connectionDifferenceContrInsertionInnerField (g₀ g₁ : SmoothRiemannianMetric I M) :
    SmoothCcTensor g₀ 2 3 where
  toSection :=
    { toFun := fun x : M =>
        (show Tensor0SBundle.TensorRSSpace 2 3 I x from
          connContrCLM (I := I) 1 1 x ((connectionDifferenceSection (I := I) g₁ g₀).toSection x))
      contMDiff_toFun := connectionDifferenceContrInsertionInnerFib_contMDiff (I := I) g₀ g₁ }
  hasCompactSupport := HasCompactSupport.of_compactSpace _

omit [I.Boundaryless] in
omit [NeZero (Module.finrank ℝ E)] in
omit [SigmaCompactSpace M] in
@[simp] theorem connectionDifferenceContrInsertionInnerField_toSection
    (g₀ g₁ : SmoothRiemannianMetric I M) (x : M) :
    (connectionDifferenceContrInsertionInnerField (I := I) g₀ g₁).toSection x =
      (show Tensor0SBundle.TensorRSSpace 2 3 I x from
        connContrCLM (I := I) 1 1 x ((connectionDifferenceSection (I := I) g₁ g₀).toSection x)) := rfl
...
omit [NeZero (Module.finrank ℝ E)] in
omit [SigmaCompactSpace M] in
theorem connectionDifferenceContrInsertionInnerField_eq_reindex_slotExtend
  ...
  have hR : Tensor0SSpace.toModel
    ...
    have h1 : ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 3 I x from
      ...
      rw [hD'_def]
      exact reindexCoefficientInputSlotsFiber_apply (I := I) 2 3 innerContractionSwapPerm x _ D
    rw [h1]
...
theorem connectionDifferenceGradContrInsertionFib_contMDiff (g₀ g₁ : SmoothRiemannianMetric I M) :
  ...
  have h := connContrCLM_field_contMDiff (I := I) 1 2
    (fun x => (covGrad (I := I) (M := M) g₀ 1 2
      (connectionDifferenceSection (I := I) g₁ g₀)).toSection x)
    (covGrad (I := I) (M := M) g₀ 1 2 (connectionDifferenceSection (I := I) g₁ g₀)).toSection.contMDiff
    (fun x => Y x) Y.contMDiff
  refine h.congr (fun x => ?_)
...
def connectionDifferenceGradContrInsertionField (g₀ g₁ : SmoothRiemannianMetric I M) :
...
theorem connectionDifferenceGradContrInsertionField_eq_reindex_slotExtend
  ...
  have hL : Tensor0SSpace.toModel
      ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 4 I x from
        (connectionDifferenceGradContrInsertionField (I := I) g₀ g₁).toSection x) D) u =
      Tensor0SSpace.toModel D
        ![rs13ContrVec (I := I) (M := M) x
          (show Tensor0SBundle.TensorRSSpace 1 3 I x from
            (covGrad (I := I) (M := M) g₀ 1 2
              (connectionDifferenceSection (I := I) g₁ g₀)).toSection x) ![u 1, u 2, u 3], u 0] := by
    rw [show ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 4 I x from
        (connectionDifferenceGradContrInsertionField (I := I) g₀ g₁).toSection x) D) =
        connContrCLM (I := I) 1 2 x
          ((covGrad (I := I) (M := M) g₀ 1 2
            (connectionDifferenceSection (I := I) g₁ g₀)).toSection x) D from rfl]
  ...
  have hR : Tensor0SSpace.toModel
        ...
        slotExtendPointwise (I := I) (M := M) 1 3 x
          (show Tensor0SSpace 1 I x →L[ℝ] Tensor0SSpace 3 I x from
            (covGrad (I := I) (M := M) g₀ 1 2
              (connectionDifferenceSection (I := I) g₁ g₀)).toSection x) D' := by
      rw [hD'_def]
      exact reindexCoefficientInputSlotsFiber_apply (I := I) 2 4 innerContractionSwapPerm x _ D
    rw [h1]
    conv_lhs => rw [show u = Fin.cons (u 0) (Matrix.vecTail u) from (Fin.cons_self_tail u).symm]
    ...
    rw [TensorMultilinear.tensor0S_curry_toModel_apply (I := I) (M := M) (n := 1) D'
      ...
      (fun _ : Fin 1 => rs13ContrVec (I := I) (M := M) x
        (show Tensor0SBundle.TensorRSSpace 1 3 I x from
          (covGrad (I := I) (M := M) g₀ 1 2
            (connectionDifferenceSection (I := I) g₁ g₀)).toSection x) (Matrix.vecTail u))]
    rw [hD'_def, Tensor0SSpace.toModel_ofModel, ContinuousMultilinearMap.domDomCongr_apply]
    rw [hvec]
    congr 1
    funext j
...
theorem linearizedRicciConnectionDifferenceOrder0CoeffField_eq_ricciCometricFourTrace_comp_kernelField
    ...
    linearizedRicciConnectionDifferenceOrder0CoeffField (I := I) (M := M) g₀ g₁ =
      ccOperatorFieldComp (I := I) (M := M) g₀ 2 4 2
        ...
        (linearizedRicciConnectionDifferenceOrder0KernelField (I := I) g₀ g₁) := by
  apply SmoothCcTensor.ext
  apply ContMDiffSection.ext
  intro x
    ...
    linearizedRicciConnectionDifferenceOrder0KernelField (I := I) g₀ g₁ =
        ...
        + reindexCoefficientInputSlots (I := I) (M := M) g₀ 2 4
            (ccOperatorFieldComp (I := I) (M := M) g₀ 2 4 4
              ...
              (ccOperatorFieldComp (I := I) (M := M) g₀ 2 3 4
                ...
                (ccOperatorFieldComp (I := I) (M := M) g₀ 2 3 3
                  (slotPermCc0 (I := I) (M := M) g₀ kMid0Perm102)
                  (connectionDifferenceContrInsertionInnerField (I := I) g₀ g₁)))) innerContractionSwapPerm
        + ccOperatorFieldComp (I := I) (M := M) g₀ 2 4 4
      ...
      - reindexCoefficientInputSlots (I := I) (M := M) g₀ 2 4
          (ccOperatorFieldComp (I := I) (M := M) g₀ 2 4 4
            ...
            (connectionDifferenceGradContrInsertionField (I := I) g₀ g₁)) innerContractionSwapPerm := by
  apply SmoothCcTensor.ext
  apply ContMDiffSection.ext
  intro x
...
omit [NeZero (Module.finrank ℝ E)] [CompactSpace M] [I.Boundaryless] [BoundarylessManifold I M]
    [T2Space M] [SigmaCompactSpace M] in
private lemma o0RiemannianFiberNormSq_smul (g : SmoothRiemannianMetric I M) (r s : ℕ) (x : M)
...
theorem riemannianFiberNormSq_iteratedCovGrad_linearizedRicciConnectionDifferenceOrder0KernelField_diagonalProductGrid_le
  ...
  have hCQ_nn : ∀ l, 0 ≤ CQ l := by
    intro l
    simp only [hCQ_def]
    refine mul_nonneg (operatorFieldApplicationGdiag_nonneg (E := E) l) ?_
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 2 (3 + m) x
          ((iteratedCovGrad (I := I) g₀ 2 3 m
            (slotExtend (I := I) (M := M) g₀ 1 2
              (connectionDifferenceSection (I := I) g₁ g₀))).toSection x) := by
      rw [connectionDifferenceContrInsertionInnerField_eq_reindex_slotExtend (I := I) (M := M) g₀ g₁]
      exact riemannianFiberNormSq_iteratedCovGrad_reindexCoefficientInputSlots_eq (I := I) (M := M) g₀ 2 3
        (slotExtend (I := I) (M := M) g₀ 1 2 (connectionDifferenceSection (I := I) g₁ g₀))
        innerContractionSwapPerm m x
    rw [h0]
    refine le_trans (riemannianFiberNormSq_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 1 2
      (connectionDifferenceSection (I := I) g₁ g₀) m x) ?_
  ...
  have hCore34 : ∀ n : ℕ, riemannianFiberNormSq (I := I) (M := M) g₀ 3 (4 + n) x
    ...
    have h0 : riemannianFiberNormSq (I := I) (M := M) g₀ 3 (4 + n) x
      ...
      exact riemannianFiberNormSq_iteratedCovGrad_reindexCoefficientInputSlots_eq (I := I) (M := M) g₀ 3 4
        (slotExtend (I := I) (M := M) g₀ 2 3
          (slotExtend (I := I) (M := M) g₀ 1 2 (connectionDifferenceSection (I := I) g₁ g₀)))
        connectionDifferenceContrInsertionReindexPerm n x
    rw [h0]
    ...
    have h2 := riemannianFiberNormSq_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 1 2
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 2 (4 + q) x
          ((iteratedCovGrad (I := I) g₀ 2 4 q
            (slotExtend (I := I) (M := M) g₀ 1 3
              (covGrad (I := I) (M := M) g₀ 1 2
                (connectionDifferenceSection (I := I) g₁ g₀)))).toSection x) := by
      rw [connectionDifferenceGradContrInsertionField_eq_reindex_slotExtend (I := I) (M := M) g₀ g₁]
      exact riemannianFiberNormSq_iteratedCovGrad_reindexCoefficientInputSlots_eq (I := I) (M := M) g₀ 2 4
        (slotExtend (I := I) (M := M) g₀ 1 3
          (covGrad (I := I) (M := M) g₀ 1 2 (connectionDifferenceSection (I := I) g₁ g₀)))
        innerContractionSwapPerm q x
    rw [h0]
    refine le_trans (riemannianFiberNormSq_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 1 3
      (covGrad (I := I) (M := M) g₀ 1 2 (connectionDifferenceSection (I := I) g₁ g₀)) q x) ?_
    have hcomm : riemannianFiberNormSq (I := I) (M := M) g₀ 1 (3 + q) x
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 1 (2 + (q + 1)) x
          ((iteratedCovGrad (I := I) g₀ 1 2 (q + 1)
            (connectionDifferenceSection (I := I) g₁ g₀)).toSection x) :=
      riemannianFiberNormSq_iteratedCovGrad_covGrad_comm_rs (I := I) (M := M) g₀ 1 2 q
  ...
  have hB2 : riemannianFiberNormSq (I := I) (M := M) g₀ 2 (4 + l) x
      ...
      CQ l * Combinatorics.antidiagonalTupleGridWindow b (l + 3) := by
    rw [riemannianFiberNormSq_iteratedCovGrad_reindexCoefficientInputSlots_eq (I := I) (M := M) g₀ 2 4 _
...
theorem riemannianFiberNormSq_iteratedCovGrad_ricciCometricFourTraceCastG0_diagonalProductGrid_le
  ...
  have hCD_nn : ∀ n, 0 ≤ CD n := by
    intro n
    simp only [hCD_def]
    exact mul_nonneg (mul_nonneg (operatorFieldApplicationGdiag_nonneg (E := E) n)
  ...
  have hWgrid : ∀ q : ℕ, riemannianFiberNormSq (I := I) (M := M) g₀ 4 (4 + q) x
    ...
    intro q
    refine le_trans (riemannianFiberNormSq_iteratedCovGrad_slotInsertEndoCc_le_endo (I := I) g₀ 3
      (metricComparisonDifferenceEndomorphismField (I := I) g₀ g₁) q x) ?_
  ...
  have hD : riemannianFiberNormSq (I := I) (M := M) g₀ 4 (2 + n) x
    ...
    have hterm : ∀ a ∈ Finset.range (n + 1),
      ...
      refine mul_le_mul (hSΦ a x) hsum
        ...
        (hSΦ_nn a)
    refine le_trans (mul_le_mul_of_nonneg_left (Finset.sum_le_sum hterm)
      (operatorFieldApplicationGdiag_nonneg (E := E) n)) ?_
  ...
  have hpure : riemannianFiberNormSq (I := I) (M := M) g₀ 4 (2 + n) x
    ...
    have hxid : (iteratedCovGrad (I := I) g₀ 4 2 n
      ...
      rw [cometricDoubleTraceCoefficient_eq_doubleTrace_add_ccOperatorFieldComp (I := I) g₀ g₁,
  ...
  have hR1 : riemannianFiberNormSq (I := I) (M := M) g₀ 4 (2 + n) x
    ...
    riemannianFiberNormSq_iteratedCovGrad_reindexCoefficientInputSlots_eq (I := I) (M := M) g₀ 4 2
      (cometricDoubleTraceCoefficient (I := I) (M := M) g₀ g₁) fourTraceCyclePerm123 n x
  have hR2 : riemannianFiberNormSq (I := I) (M := M) g₀ 4 (2 + n) x
      ...
      riemannianFiberNormSq (I := I) (M := M) g₀ 4 (2 + n) x
        ((iteratedCovGrad (I := I) g₀ 4 2 n
          (cometricDoubleTraceCoefficient (I := I) (M := M) g₀ g₁)).toSection x) :=
    riemannianFiberNormSq_iteratedCovGrad_reindexCoefficientInputSlots_eq (I := I) (M := M) g₀ 4 2
      (cometricDoubleTraceCoefficient (I := I) (M := M) g₀ g₁) fourTraceSwap13Perm n x
  have hR3 : riemannianFiberNormSq (I := I) (M := M) g₀ 4 (2 + n) x
      ...
      riemannianFiberNormSq (I := I) (M := M) g₀ 4 (2 + n) x
        ((iteratedCovGrad (I := I) g₀ 4 2 n
          (cometricDoubleTraceCoefficient (I := I) (M := M) g₀ g₁)).toSection x) :=
    riemannianFiberNormSq_iteratedCovGrad_reindexCoefficientInputSlots_eq (I := I) (M := M) g₀ 4 2
      (cometricDoubleTraceCoefficient (I := I) (M := M) g₀ g₁) fourTraceDoubleTranspositionPerm n x
  ...
  rw [hsec, o0RiemannianFiberNormSq_smul (I := I) (M := M) g₀ 4 (2 + n) x]
