      ...
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        ...
        ∀ (i : ℕ), i ≤ a →
          ‖iteratedCovGrad (I := I) g₀ 1 (2 + 1) i
              (covGrad (I := I) (M := M) g₀ 1 2 (connectionDifferenceSection (I := I) g₁ g₀))‖ ^ 2 ≤
  ...
  · obtain ⟨x₀⟩ := hMne
    ...
    have hpt : ∀ x : M,
      ...
      have hsplit : riemannianFiberNormSq (I := I) (M := M) g₀ 1 (2 + (i + 1)) x
          ((iteratedCovGrad (I := I) g₀ 1 2 (i + 1) (connectionDifferenceSection (I := I) g₁ g₀)).toSection x) ≤
      ...
      have hrem_reshaped : riemannianFiberNormSq (I := I) (M := M) g₀ 1 (2 + (i + 1)) x
        ...
        exact connectionDifferenceResidualGridSum_le_boundedFactorGridWindow
          (fun l => riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + l) x
...
theorem covGradConnectionDifferenceSection_metricPerturbationPath_jetL2_perOrder_topOrderSeparated
    (g₀ : SmoothRiemannianMetric I M) (a : ℕ)
    (ha_super : 2 * Module.finrank ℝ E + 10 ≤ a) {R : ℝ} (hR : 0 ≤ R)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ Ktop : ℝ, 0 ≤ Ktop ∧ ∃ Kc : ℕ → ℝ, (∀ i, 0 ≤ Kc i) ∧
...
theorem covGradConnectionDifferenceSection_metricPerturbationPath_jetL2_summed_topOrderSeparated
    (g₀ : SmoothRiemannianMetric I M) (a : ℕ)
    (ha_super : 2 * Module.finrank ℝ E + 10 ≤ a) {R : ℝ} (hR : 0 ≤ R)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
  ...
  obtain ⟨Ktop, hKtop_nn, Kc, hKc_nn, hper⟩ :=
    covGradConnectionDifferenceSection_metricPerturbationPath_jetL2_perOrder_topOrderSeparated
      (I := I) (M := M) g₀ a ha_super hR hδ₀
