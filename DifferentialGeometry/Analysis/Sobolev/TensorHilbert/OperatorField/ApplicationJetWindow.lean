...
theorem operatorFieldApply_l2_le_of_pointwise_fiberNormSq_bound_left
  ...
  rw [← operatorFieldComposition_zero_eq_operatorFieldApply (I := I) (M := M) g r s Φ W]
...
private theorem inverseMetricDifferenceSlotCoefficient_perOrder_l2_tame
    (g₀ : SmoothRiemannianMetric I M) (a : ℕ)
    (ha_super : 2 * Module.finrank ℝ E + 10 ≤ a) {R₀ : ℝ} (hR₀ : 0 ≤ R₀)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ K : ℕ → ℝ, (∀ i, 0 ≤ K i) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        ...
        (∀ j : ℕ, j ≤ a + 2 → ‖iteratedCovGrad (I := I) g₀ 0 2 j P‖ ≤ R₀) →
        ∀ (i : ℕ),
          ‖iteratedCovGrad (I := I) g₀ 2 2 i (inverseMetricDifferenceSlotCoefficient (I := I) g₀ g₁)‖ ^ 2 ≤
  ...
  · intro g₁ P δ hδ_le hδ htie hPball i
    ...
    · obtain ⟨x₀⟩ := hM
      ...
      have hgridpt : ∀ x : M,
          riemannianFiberNormSq (I := I) (M := M) g₀ 2 (2 + i) x
              ((iteratedCovGrad (I := I) g₀ 2 2 i (inverseMetricDifferenceSlotCoefficient (I := I) g₀ g₁)).toSection x) ≤
      ...
      · subst hi0
        ...
        have key := normSq_le_integral_of_pointwise_fiberNormSq_le_rs (I := I) (M := M) g₀ 2 (2 + 0)
          (iteratedCovGrad (I := I) g₀ 2 2 0 (inverseMetricDifferenceSlotCoefficient (I := I) g₀ g₁))
          (fun x => Cgrid 0 * ∑ n ∈ Finset.range (0 + 1),
            ∑ e ∈ Finset.Nat.antidiagonalTuple n 0, ∏ m : Fin n,
              riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + e m) x
                ((iteratedCovGrad (I := I) g₀ 0 2 (e m) P).toSection x))
      ...
      · have hi1 : 1 ≤ i := Nat.one_le_iff_ne_zero.mpr hi0
        ...
        have key := normSq_le_integral_of_pointwise_fiberNormSq_le_rs (I := I) (M := M) g₀ 2 (2 + i)
          (iteratedCovGrad (I := I) g₀ 2 2 i (inverseMetricDifferenceSlotCoefficient (I := I) g₀ g₁))
          (fun x => Cgrid i * ∑ n ∈ Finset.range (i + 1),
            ∑ e ∈ Finset.Nat.antidiagonalTuple n i, ∏ m : Fin n,
              riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + e m) x
                ((iteratedCovGrad (I := I) g₀ 0 2 (e m) P).toSection x))
          ...
          have hinner : ∀ n ∈ Finset.range (i + 1),
              ...
              ∑ e ∈ Finset.Nat.antidiagonalTuple n i, ∫ x, ∏ m : Fin n,
                  riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + e m) x
                    ((iteratedCovGrad (I := I) g₀ 0 2 (e m) P).toSection x)
                ∂(riemannianVolumeMeasure (I := I) (M := M) g₀) :=
            fun n hn => MeasureTheory.integral_finsetSum _ (fun e he => (hPT n hn e he).1)
    ...
    · have hM' : IsEmpty M := not_nonempty_iff.mp hM
      have hz : ‖iteratedCovGrad (I := I) g₀ 2 2 i (inverseMetricDifferenceSlotCoefficient (I := I) g₀ g₁)‖ = 0 := by
...
theorem deTurckPrincipalCometricCoeff_perOrder_l2_tame_generic
  ...
  have hF_int : MeasureTheory.Integrable
    ...
    (MeasureTheory.integrable_finsetSum (Finset.range (i + 1))
      (fun j _ => integrable_riemannianFiberNormSq_toSection (I := I) (M := M) g₀ 2 (2 + j)
        (iteratedCovGrad (I := I) g₀ 2 2 j (inverseMetricDifferenceSlotCoefficient (I := I) g₀ g₁)))).const_mul (Cpo i)
  have key := normSq_le_integral_of_pointwise_fiberNormSq_le_rs (I := I) (M := M) g₀ 4 (2 + i)
    (iteratedCovGrad (I := I) g₀ 4 2 i (deTurckPrincipalCometricCoeff (I := I) (M := M) g₀ g₁))
    (fun x => Cpo i * ∑ j ∈ Finset.range (i + 1),
      riemannianFiberNormSq (I := I) (M := M) g₀ 2 (2 + j) x
...
theorem operatorFieldApplication_jet_l2Sq_le
