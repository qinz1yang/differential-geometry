...
open DifferentialGeometry.Analysis.Parabolic.TensorSpectral
  (connectionDifferenceCovDerivBiContrFib deTurckLieConnectionDifferenceDerivativeBiContrFib_contMDiff deTurckLieCovariantDerivativeInsertionFib deTurckLieCovariantDerivativeInsertionFib_contMDiff
    ...
    connectionDifference_gFibreNorm_le_iteratedCovGrad_of_lt_one)
...
theorem deTurckLieConnectionDifferenceDerivCoeffField_metricPerturbationPath_riemannianFiberNormSq_order0_ballUniform
  ...
  obtain ⟨Cq, hCq_nn, hCq⟩ :=
    exists_covDerivConnectionDifference_gQuadratic_le_of_jetEnvelope (I := I) (M := M) g₀
      (δ₀ := δ₁) hδ₁_lt B hB_nn
  ...
  have htie : ∀ (y : M) (v w : TangentSpace I y),
    ...
    intro y v w
    rw [hg₁, hP_def]
    exact metricPerturbationPath_inner_of_mem (I := I) g₀ T T' hδ hδ' hs_mem y v w
  ...
  have hquad : ∀ v w u : TangentSpace I x,
    ...
    intro v w u
    unfold covDerivConnectionDifferenceSqrt
  ...
  have hbase : ∀ u v : TangentSpace I x,
    ...
    change Real.sqrt (g₀.inner x (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x u v)
        (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x u v)) ≤
      Ca0 * tensorRSRiemannianNorm (I := I) (M := M) g₀ 0 3 x
          ((iteratedCovGrad (I := I) g₀ 0 2 1 P).toSection x) *
        Real.sqrt (g₀.inner x u u) * Real.sqrt (g₀.inner x v v) at h
    exact h
  exact deTurckLieConnectionDifferenceDerivCoeffField_fiberNormSq_le_of_scalar_bounds
