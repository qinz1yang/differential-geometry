...
theorem deTurckLieConnectionDifferenceDerivCoeffField_metricPerturbationPath_jetL2_perOrder_topOrderSeparated
  ...
  refine ⟨Ktop_field * diagonalGridGrowthFactor (E := E) a * diagonalGridGrowthFactor (E := E) a,
      mul_nonneg (mul_nonneg hKtop_field_nn (operatorFieldApplicationGdiag_nonneg (E := E) a))
        (operatorFieldApplicationGdiag_nonneg (E := E) a),
  ...
  have happa_nn : 0 ≤ diagonalGridGrowthFactor (E := E) a := operatorFieldApplicationGdiag_nonneg (E := E) a
...
theorem deTurckLieConnectionDifferenceDerivCoeffField_metricPerturbationPath_jetL2_summed_topOrderSeparated
    (g₀ g_bg : SmoothRiemannianMetric I M) (a : ℕ)
    (ha_super : 2 * Module.finrank ℝ E + 10 ≤ a) {R : ℝ} (hR : 0 ≤ R)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
  ...
  exact jetL2_sum_lowShift a 2 3 Ktop hKtop_nn Kc hKc_nn
    (fun i => ‖iteratedCovGrad (I := I) g₀ 2 2 i
      (deTurckLieConnectionDifferenceDerivCoeffField (I := I) g₀ (metricPerturbationPath
