...
theorem symmC0_riemannianFiberNormSq_le (g₀ : SmoothRiemannianMetric I M)
...
private lemma engineRem_le_deTurckLieConnectionDifferenceDerivativeGridWin (b : ℕ → ℝ) (hb : ∀ j, 0 ≤ b j) (j : ℕ) :
  ...
  have h1 : b (j - k) ≤ Combinatorics.antidiagonalTupleGrid b (j - k) :=
    single_le_antidiagonalTupleGridPartialSum b hb (j - k) (by omega)
...
private theorem exists_riemannianFiberNormSq_connectionDifferenceSection_topsep_deTurckLieConnectionDifferenceDerivative
  ...
  have hrem2 : riemannianFiberNormSq (I := I) (M := M) g₀ 1 (2 + j) x
    ...
    le_trans hrem (mul_le_mul_of_nonneg_left (engineRem_le_deTurckLieConnectionDifferenceDerivativeGridWin b hb j) (hKc0_nn j))
...
private theorem exists_riemannianFiberNormSq_deTurckLieConnectionDifferenceDerivativeKernelRaised_topsep (g₀ g_bg : SmoothRiemannianMetric I M)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
      ...
      ∀ (g₁ : SmoothRiemannianMetric I M) (T : SmoothCcTensor g₀ 0 2)
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 1 (3 + i) x
            ((iteratedCovGrad (I := I) g₀ 1 3 i
              (deTurckLieConnectionDifferenceDerivativeKernelRaisedCc (I := I) (M := M) g₀ g₁ g_bg)).toSection x) ≤
  ...
  obtain ⟨cc, hcc_nn, hcc⟩ := exists_fixedField_riemannianFiberNormSq_jet_deTurckLieConnectionDifferenceDerivative (I := I) (M := M) g₀ 1 2
    (connectionDifferenceSection (I := I) g_bg g₀)
  ...
  have hCQ1_nn : ∀ j, 0 ≤ CQ1 j := by
    intro j
    refine mul_nonneg (operatorFieldApplicationGdiag_nonneg (E := E) j) (Finset.sum_nonneg fun i' _ => ?_)
    refine mul_nonneg (mul_nonneg (Nat.cast_nonneg _) (hCA_nn i'))
      (Finset.sum_nonneg fun l _ => mul_nonneg (hCA_nn l) (antidiagonalTuplePairCount_nonneg _ _))
  ...
  have hCQ3_nn : ∀ j, 0 ≤ CQ3 j := by
    ...
    refine mul_nonneg (operatorFieldApplicationGdiag_nonneg (E := E) j) (Finset.sum_nonneg fun i' _ => ?_)
    refine mul_nonneg (mul_nonneg (Nat.cast_nonneg _) (hCA_nn i'))
      (Finset.sum_nonneg fun l _ => mul_nonneg (hcc_nn l) (antidiagonalTuplePairCount_nonneg _ _))
  ...
  have harm : ∀ i', i' ≤ i →
      riemannianFiberNormSq (I := I) (M := M) g₀ 1 (2 + i') x
        ((iteratedCovGrad (I := I) g₀ 1 2 i' (connectionDifferenceSection (I := I) g₁ g₀)).toSection x) ≤
      CA i' * antidiagonalTupleGridPartialSum b (i' + 2) :=
    fun i' _ => hCA g₁ T htie hδ_le hδ0 hbound i' x
  have hfix : ∀ i', i' ≤ i →
      riemannianFiberNormSq (I := I) (M := M) g₀ 1 (2 + i') x
        ((iteratedCovGrad (I := I) g₀ 1 2 i' (connectionDifferenceSection (I := I) g_bg g₀)).toSection x) ≤
      cc i' * antidiagonalTupleGridPartialSum b (i' + 2) := by
    intro i' _
    refine le_trans (hcc i' x) ?_
    have h1 : (1 : ℝ) ≤ antidiagonalTupleGridPartialSum b (i' + 2) := one_le_antidiagonalTupleGridPartialSum b hb
      (by omega)
    nlinarith [hcc_nn i']
  have hQ1 : riemannianFiberNormSq (I := I) (M := M) g₀ 1 (3 + i) x
      ((iteratedCovGrad (I := I) g₀ 1 3 i
        (deTurckLieConnectionDifferenceDerivativeQuadCc (I := I) (M := M) g₀ g₁ g₁)).toSection x) ≤ CQ1 i * W :=
    deTurckLieConnectionDifferenceDerivativeQuad_tower_of_factors (I := I) (M := M) g₀ g₁ g₁ i x b hb CA CA hCA_nn hCA_nn harm harm
  have hQ2 : riemannianFiberNormSq (I := I) (M := M) g₀ 1 (3 + i) x
      ((iteratedCovGrad (I := I) g₀ 1 3 i
        (deTurckLieConnectionDifferenceDerivativeQuadCc (I := I) (M := M) g₀ g_bg g₁)).toSection x) ≤ CQ2 i * W :=
    deTurckLieConnectionDifferenceDerivativeQuad_tower_of_factors (I := I) (M := M) g₀ g_bg g₁ i x b hb cc CA hcc_nn hCA_nn hfix harm
  have hQ3 : riemannianFiberNormSq (I := I) (M := M) g₀ 1 (3 + i) x
      ((iteratedCovGrad (I := I) g₀ 1 3 i
        (deTurckLieConnectionDifferenceDerivativeQuadCc (I := I) (M := M) g₀ g₁ g_bg)).toSection x) ≤ CQ3 i * W :=
    deTurckLieConnectionDifferenceDerivativeQuad_tower_of_factors (I := I) (M := M) g₀ g₁ g_bg i x b hb CA cc hCA_nn hcc_nn harm hfix
  ...
  have hA2 : riemannianFiberNormSq (I := I) (M := M) g₀ 1 (3 + i) x
    ...
    nlinarith [hcbg_nn i]
  set A1 := covGrad (I := I) (M := M) g₀ 1 2 (connectionDifferenceSection (I := I) g₁ g₀) with hA1_def
  set A2 := covGrad (I := I) (M := M) g₀ 1 2 (connectionDifferenceSection (I := I) g_bg g₀) with hA2_def
  set Q11 := deTurckLieConnectionDifferenceDerivativeQuadCc (I := I) (M := M) g₀ g₁ g₁ with hQ11_def
  set Qbg1 := deTurckLieConnectionDifferenceDerivativeQuadCc (I := I) (M := M) g₀ g_bg g₁ with hQbg1_def
  set Q1bg := deTurckLieConnectionDifferenceDerivativeQuadCc (I := I) (M := M) g₀ g₁ g_bg with hQ1bg_def
  ...
  have t1 := riemannianFiberNormSq_iteratedCovGrad_sub_le_deTurckLieConnectionDifferenceDerivative (I := I) (M := M) g₀ 1 3 i A1 A2 x
  have t2 := riemannianFiberNormSq_iteratedCovGrad_add_le_deTurckLieConnectionDifferenceDerivative (I := I) (M := M) g₀ 1 3 i (A1 - A2) Q11 x
  have t3 := riemannianFiberNormSq_iteratedCovGrad_sub_le_deTurckLieConnectionDifferenceDerivative (I := I) (M := M) g₀ 1 3 i (A1 - A2 + Q11) Qbg1 x
  have t4 := riemannianFiberNormSq_iteratedCovGrad_sub_le_deTurckLieConnectionDifferenceDerivative (I := I) (M := M) g₀ 1 3 i (A1 - A2 + Q11 - Qbg1) P1 x
  have t5 := riemannianFiberNormSq_iteratedCovGrad_add_le_deTurckLieConnectionDifferenceDerivative (I := I) (M := M) g₀ 1 3 i (A1 - A2 + Q11 - Qbg1 - P1) P2 x
  have t6 := riemannianFiberNormSq_iteratedCovGrad_sub_le_deTurckLieConnectionDifferenceDerivative (I := I) (M := M) g₀ 1 3 i
    (A1 - A2 + Q11 - Qbg1 - P1 + P2) P3 x
  have t7 := riemannianFiberNormSq_iteratedCovGrad_add_le_deTurckLieConnectionDifferenceDerivative (I := I) (M := M) g₀ 1 3 i
    (A1 - A2 + Q11 - Qbg1 - P1 + P2 - P3) P4 x
  have hKK : deTurckLieConnectionDifferenceDerivativeKernelRaisedCc (I := I) (M := M) g₀ g₁ g_bg =
...
private lemma gridSplit_deTurckLieConnectionDifferenceDerivative (G cΦ0 τ Wtop Wrem gridB : ℝ) (i : ℕ) (pΦ qW : ℕ → ℝ)
...
private lemma operatorFieldApplicationGrid_le_deTurckLieConnectionDifferenceDerivative (b : ℕ → ℝ) (hb : ∀ j, 0 ≤ b j) (i : ℕ)
    (pΦ cΦ qW cW : ℕ → ℝ)
  ...
  have hcell : ∀ i' ∈ Finset.range (i + 1),
    ...
    have hA1_rhs_nn : 0 ≤ cΦ i' * antidiagonalTupleGridPartialSum b (i' + 1) :=
      mul_nonneg (hcΦ_nn i') (antidiagonalTupleGridPartialSum_nonneg b hb (i' + 1))
...
private theorem exists_riemannianFiberNormSq_deTurckLieConnectionDifferenceDerivativeLowered_topsep (g₀ g_bg : SmoothRiemannianMetric I M)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ Ktop : ℝ, 0 ≤ Ktop ∧ ∃ Kc : ℕ → ℝ, (∀ i, 0 ≤ Kc i) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (T : SmoothCcTensor g₀ 0 2)
        (_htie : ∀ (y : M) (v w : TangentSpace I y),
  ...
  obtain ⟨Ktop, hKtop_nn, Kc, hKc_nn, hker⟩ :=
    exists_riemannianFiberNormSq_deTurckLieConnectionDifferenceDerivativeKernelRaised_topsep (I := I) (M := M) g₀ g_bg hδ₀
  ...
  have hbridge : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + i) x
      ...
      riemannianFiberNormSq (I := I) (M := M) g₀ 1 (3 + i) x
        ((iteratedCovGrad (I := I) g₀ 1 3 i
          (deTurckLieConnectionDifferenceDerivativeKernelRaisedCc (I := I) (M := M) g₀ g₁ g_bg)).toSection x) := by
    have h := riemannianFiberNormSq_iteratedCovGrad_cometricRaiseSlot0Field_eq (I := I)
      (M := M) g₀ 2
      (deTurckLieConnectionDifferenceDerivativeLoweredCc (I := I) (M := M) g₀ g₁ g_bg) i x
    rw [deTurckLieConnectionDifferenceDerivativeLoweredCc_raise_repr (I := I) (M := M) g₀ g₁ g_bg] at h
    exact h.symm
  rw [hbridge]
  exact hker g₁ T htie hδ_le hδ0 hbound i x

private theorem exists_riemannianFiberNormSq_deTurckLieConnectionDifferenceDerivativeSym_topsep (g₀ g_bg : SmoothRiemannianMetric I M)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ Ktop : ℝ, 0 ≤ Ktop ∧ ∃ Kc : ℕ → ℝ, (∀ i, 0 ≤ Kc i) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (T : SmoothCcTensor g₀ 0 2)
        (_htie : ∀ (y : M) (v w : TangentSpace I y),
  ...
  obtain ⟨KtopL, hKtopL_nn, KcL, hKcL_nn, hL⟩ :=
    exists_riemannianFiberNormSq_deTurckLieConnectionDifferenceDerivativeLowered_topsep (I := I) (M := M) g₀ g_bg hδ₀
  obtain ⟨CL, hCL_nn, hCL⟩ := exists_riemannianFiberNormSq_deTurckLieConnectionDifferenceDerivativeLowered_tgrid (I := I) (M := M) g₀ g_bg hδ₀
  ...
  have hCLT_nn : ∀ i, 0 ≤ CLT i := by
    intro i
    refine mul_nonneg (operatorFieldApplicationGdiag_nonneg (E := E) i) (Finset.sum_nonneg fun i' _ => ?_)
    exact mul_nonneg (hCP_nn i') (Finset.sum_nonneg fun l _ =>
      mul_nonneg (hCL_nn l) (antidiagonalTuplePairCount_nonneg _ _))
  refine ⟨8 * KtopL * (1 + cPer),
    ...
    fun i => by
      have h1 := hKcL_nn i; have h2 := hCLT_nn i; have h3 := operatorFieldApplicationGdiag_nonneg (E := E) i
      positivity, ?_⟩
  intro g₁ T htie δ hδ_le hδ0 hbound i x
  set b : ℕ → ℝ := fun l => riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + l) x
    ((iteratedCovGrad (I := I) g₀ 0 2 l T).toSection x) with hb_def
  ...
  have happ_nn : 0 ≤ diagonalGridGrowthFactor (E := E) i := operatorFieldApplicationGdiag_nonneg (E := E) i
  ...
  have hPfac : ∀ i', i' ≤ i →
      riemannianFiberNormSq (I := I) (M := M) g₀ 4 (4 + i') x
        ((iteratedCovGrad (I := I) g₀ 4 4 i'
          (endoSlotZeroCcTensor (I := I) (M := M) g₀ 3
            (deTurckLieConnectionDifferenceDerivativePerturbSharpEndoField (I := I) (M := M) g₀ T))).toSection x) ≤
      CP i' * antidiagonalTupleGridPartialSum b (i' + 1) := by
    intro i' _
    refine le_trans (riemannianFiberNormSq_iteratedCovGrad_slotInsert3_deTurckLieConnectionDifferenceDerivativePerturb_le (I := I) (M := M) g₀ T i' x) ?_
    ...
    | (m + 1) =>
        ...
        have h3 : Combinatorics.antidiagonalTupleGrid b (m + 1) ≤
            antidiagonalTupleGridPartialSum b ((m + 1) + 1) := antidiagonalTupleGrid_le_partialSum b hb (by omega)
        have hfac1 : (1 : ℝ) ≤ fr ^ 2 * δ₀ ^ 2 + 1 := le_add_of_nonneg_left (by positivity)
        have hwin_nn : 0 ≤ antidiagonalTupleGridPartialSum b
          ((m + 1) + 1) := antidiagonalTupleGridPartialSum_nonneg b hb _
  ...
  have hPer0 : riemannianFiberNormSq (I := I) (M := M) g₀ 4 (4 + 0) x
      ((iteratedCovGrad (I := I) g₀ 4 4 0
        (endoSlotZeroCcTensor (I := I) (M := M) g₀ 3
          (deTurckLieConnectionDifferenceDerivativePerturbSharpEndoField (I := I) (M := M) g₀ T))).toSection x) ≤ cPer := by
    refine le_trans (riemannianFiberNormSq_iteratedCovGrad_slotInsert3_deTurckLieConnectionDifferenceDerivativePerturb_le (I := I) (M := M) g₀ T 0 x) ?_
  ...
  have hPer : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + i) x
      ((iteratedCovGrad (I := I) g₀ 0 4 i
        (deTurckLieConnectionDifferenceDerivativeLoweredPerturbCc (I := I) (M := M) g₀ T g₁ g_bg)).toSection x) ≤
    ...
    refine le_trans
      ...
      (I := I) (M := M) g₀ i 0 4 4
      (endoSlotZeroCcTensor (I := I) (M := M) g₀ 3 (deTurckLieConnectionDifferenceDerivativePerturbSharpEndoField (I := I)
        (M := M) g₀ T))
      (deTurckLieConnectionDifferenceDerivativeLoweredCc (I := I) (M := M) g₀ g₁ g_bg) x) ?_
    exact gridSplit_deTurckLieConnectionDifferenceDerivative (diagonalGridGrowthFactor (E := E) i) cPer
      (riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + (i + 2)) x
        ((iteratedCovGrad (I := I) g₀ 0 2 (i + 2) T).toSection x))
      ...
      (le_trans (operatorFieldApplicationGrid_le_deTurckLieConnectionDifferenceDerivative b hb i
        (fun i' => riemannianFiberNormSq (I := I) (M := M) g₀ 4 (4 + i') x
          ((iteratedCovGrad (I := I) g₀ 4 4 i'
            (endoSlotZeroCcTensor (I := I) (M := M) g₀ 3
              (deTurckLieConnectionDifferenceDerivativePerturbSharpEndoField (I := I) (M := M) g₀ T))).toSection x))
  ...
  have hG1 : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + i) x
      ((iteratedCovGrad (I := I) g₀ 0 4 i
        (deTurckLieConnectionDifferenceDerivativeLoweredG1Cc (I := I) (M := M) g₀ T g₁ g_bg)).toSection x) ≤
      (2 * KtopL * (1 + cPer)) * diagonalGridGrowthFactor (E := E) i *
          riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + (i + 2)) x
            ((iteratedCovGrad (I := I) g₀ 0 2 (i + 2) T).toSection x) +
        (2 * KcL i + 2 * diagonalGridGrowthFactor (E := E) i * cPer * KcL i + 2 * CLT i) * W := by
    refine le_trans (riemannianFiberNormSq_iteratedCovGrad_add_le_deTurckLieConnectionDifferenceDerivative (I := I) (M := M) g₀ 0 4 i
      (deTurckLieConnectionDifferenceDerivativeLoweredCc (I := I) (M := M) g₀ g₁ g_bg)
      (deTurckLieConnectionDifferenceDerivativeLoweredPerturbCc (I := I) (M := M) g₀ T g₁ g_bg) x) ?_
  ...
  have hperm : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + i) x
    ...
    riemannianFiberNormSq_iteratedCovGrad_domDomCongrSection (I := I) (M := M) g₀
      (Equiv.swap (0 : Fin 4) 1) (deTurckLieConnectionDifferenceDerivativeLoweredG1Cc (I := I) (M := M) g₀ T g₁ g_bg) i x
  refine le_trans (riemannianFiberNormSq_iteratedCovGrad_add_le_deTurckLieConnectionDifferenceDerivative (I := I) (M := M) g₀ 0 4 i
    (domDomCongrSection (I := I) g₀ (Equiv.swap (0 : Fin 4) 1)
      (deTurckLieConnectionDifferenceDerivativeLoweredG1Cc (I := I) (M := M) g₀ T g₁ g_bg))
    (deTurckLieConnectionDifferenceDerivativeLoweredG1Cc (I := I) (M := M) g₀ T g₁ g_bg) x) ?_
  rw [hperm]
  calc 2 * riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + i) x
          ((iteratedCovGrad (I := I) g₀ 0 4 i
            (deTurckLieConnectionDifferenceDerivativeLoweredG1Cc (I := I) (M := M) g₀ T g₁ g_bg)).toSection x) +
...
theorem riemannianFiberNormSq_iteratedCovGrad_deTurckLieConnectionDifferenceDerivCoeffField_topOrderSeparated_le (g₀ g_bg : SmoothRiemannianMetric I M)
  ...
  have hCfield_nn : ∀ i, 0 ≤ Cfield i := by
    ...
    exact mul_nonneg (hCPT_nn i') (Finset.sum_nonneg fun l _ =>
      mul_nonneg (mul_nonneg hfr_nn (mul_nonneg hfr_nn (hCX_nn l))) (antidiagonalTuplePairCount_nonneg _ _))
  refine ⟨CPT 0 * fr ^ 2 * KtopS,
    ...
    fun i => by
      have h1 := hKcS_nn i; have h2 := hCfield_nn i; have h3 := operatorFieldApplicationGdiag_nonneg (E := E) i
      have h4 := hCPT_nn 0
      positivity, ?_⟩
  intro g₁ T htie δ hδ_le hδ0 hbound i x
  set b : ℕ → ℝ := fun l => riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + l) x
  ...
  have hXfr : ∀ l, riemannianFiberNormSq (I := I) (M := M) g₀ 2 (6 + l) x
    ...
    have hperm : riemannianFiberNormSq (I := I) (M := M) g₀ 2 (6 + l) x
      ...
      riemannianFiberNormSq_iteratedCovGrad_rs_eq_of_section_domDomCongr (I := I)
        (M := M) g₀ 2 6 deTurckLieConnectionDifferenceDerivativeInputPermutation
        (slotExtendIter (I := I) (M := M) g₀ 0 4 2 (deTurckLieConnectionDifferenceDerivativeSymCc (I := I) (M := M) g₀ T g₁ g_bg))
        (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 deTurckLieConnectionDifferenceDerivativeInputPermutation
          (slotExtendIter (I := I) (M := M) g₀ 0 4 2 (deTurckLieConnectionDifferenceDerivativeSymCc (I := I) (M := M) g₀ T g₁ g_bg)))
        (fun y d => by rw [rsDomDomCongrSection_toSection, toModel_rsDomDomCongr_apply]) l x
    rw [hperm]
    have h1 := riemannianFiberNormSq_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 1 5
      (slotExtendIter (I := I) (M := M) g₀ 0 4 1 (deTurckLieConnectionDifferenceDerivativeSymCc (I := I) (M := M) g₀ T g₁ g_bg)) l x
    have h2 := riemannianFiberNormSq_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 0 4
    ...
    calc riemannianFiberNormSq (I := I) (M := M) g₀ 2 (6 + l) x
      ...
      _ = fr ^ 2 * riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + l) x
            ((iteratedCovGrad (I := I) g₀ 0 4 l
              (deTurckLieConnectionDifferenceDerivativeSymCc (I := I) (M := M) g₀ T g₁ g_bg)).toSection x) := by ring
  have hPT0 : riemannianFiberNormSq (I := I) (M := M) g₀ 6 (2 + 0) x
      ((iteratedCovGrad (I := I) g₀ 6 2 0 (deTurckLieConnectionDifferenceDerivativePairTraceOperator (I := I) (M := M) g₀ g₁)).toSection x) ≤
      CPT 0 := by
  ...
  refine le_trans
    (riemannianFiberNormSq_iteratedCovGrad_ccTensorCompose_diagonalProductGrid_leftFactor_le
    (I := I) (M := M) g₀ i 2 6 2 (deTurckLieConnectionDifferenceDerivativePairTraceOperator (I := I) (M := M) g₀ g₁)
    (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 deTurckLieConnectionDifferenceDerivativeInputPermutation
      (slotExtendIter (I := I) (M := M) g₀ 0 4 2
        (deTurckLieConnectionDifferenceDerivativeSymCc (I := I) (M := M) g₀ T g₁ g_bg))) x) ?_
  refine le_trans (gridSplit_deTurckLieConnectionDifferenceDerivative (diagonalGridGrowthFactor (E := E) i) (CPT 0)
    (riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + (i + 2)) x
      ((iteratedCovGrad (I := I) g₀ 0 2 (i + 2) T).toSection x))
    (fr ^ 2 * KtopS * diagonalGridGrowthFactor (E := E) i) (fr ^ 2 * KcS i * W) (Cfield i * W) i
    (fun i' => riemannianFiberNormSq (I := I) (M := M) g₀ 6 (2 + i') x
      ((iteratedCovGrad (I := I) g₀ 6 2 i' (deTurckLieConnectionDifferenceDerivativePairTraceOperator (I := I) (M := M) g₀ g₁)).toSection x))
