...
theorem endoCov_gInvDiffRaisedField_apply
  ...
  have hsharpY_mdiff :=
    ((inverseMetricSharpFib_g0FlatY_contMDiff (I := I) g₀ g₁ Y) x).mdifferentiableAt
      (by norm_num)
  have hΛapply : (metricComparisonDifferenceEndomorphismField (I := I) g₀ g₁ : Π y : M, _) =
      fun y : M => metricComparisonDifferenceEndomorphism (I := I) g₀ g₁ y := rfl
  have hLeibniz := endoCovariantDerivative_apply (I := I) (M := M) g₀
    (metricComparisonDifferenceEndomorphismField (I := I) g₀ g₁) Y x v
  rw [hLeibniz]
  have hΛval : ∀ y : M, (metricComparisonDifferenceEndomorphismField (I := I) g₀ g₁ y) (Y y) =
      (inverseMetricSharpFib (I := I) g₁ y) (β y) - Y y := by
    intro y
    rw [hβdef]
    change metricComparisonDifferenceEndomorphism (I := I) g₀ g₁ y (Y y) = _
    rw [metricComparisonDifferenceEndomorphism_apply]
  have hΛx : (metricComparisonDifferenceEndomorphismField (I := I) g₀ g₁ x) gradY =
      (inverseMetricSharpFib (I := I) g₁ x) (g0FlatCLM (I := I) g₀ x gradY) - gradY := by
    change metricComparisonDifferenceEndomorphism (I := I) g₀ g₁ x gradY = _
    rw [metricComparisonDifferenceEndomorphism_apply]
  have hsplit : (LeviCivita (I := I) g₀)
    (fun y : M => (metricComparisonDifferenceEndomorphismField (I := I) g₀ g₁ y) (Y y)) x v =
      (LeviCivita (I := I) g₀).toFun (fun y : M => (inverseMetricSharpFib (I := I) g₁ y) (β y)) x v
        - (LeviCivita (I := I) g₀).toFun (fun y : M => Y y) x v := by
    have hfun : (fun y : M => (metricComparisonDifferenceEndomorphismField (I := I) g₀ g₁ y) (Y y)) =
        (fun y : M => (inverseMetricSharpFib (I := I) g₁ y) (β y)) - (fun y : M => Y y) := by
      funext y
      rw [Pi.sub_apply, hΛval y]
    have hop : (LeviCivita (I := I) g₀).toFun
          (fun y : M => (metricComparisonDifferenceEndomorphismField (I := I) g₀ g₁ y) (Y y)) x =
  ...
  rw [show (inverseMetricSharpFib (I := I) g₁ x) (β x) =
      metricComparisonEndomorphism (I := I) g₀ g₁ x (Y x) from by
    rw [hβdef, metricComparisonEndomorphism_apply]]
  abel
...
theorem sqrt_inner_endoCov_gInvDiffRaisedField_le
  ...
  set EC : TangentSpace I x :=
    ((endoCovariantDerivative (I := I) (M := M) g₀)
      (metricComparisonDifferenceEndomorphismField (I := I) g₀ g₁) x v) (Y x) with hEC_def
  set T2 : TangentSpace I x :=
    - PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x (metricComparisonEndomorphism (I := I) g₀ g₁ x w) v
    with hT2_def
  ...
  have hgir := sqrt_inner_metricComparisonEndomorphism_le (I := I) g₀ g₁
  ...
  have hT2_bound : Real.sqrt (g₀.inner x T2 T2) ≤ 2 * C₀ * G * Nv * Nw := by
    have hraw := hpw g₁ T h hδ hδ0 hbound x (metricComparisonEndomorphism (I := I) g₀ g₁ x w) v
    rw [← hNv_def] at hraw
    ...
    have hgir' : Real.sqrt (g₀.inner x (metricComparisonEndomorphism (I := I) g₀ g₁ x w)
        (metricComparisonEndomorphism (I := I) g₀ g₁ x w)) ≤ 2 * Nw := by
      refine hgir.trans ?_
      exact mul_le_mul_of_nonneg_right hinv_le hNw_nn
    calc C₀ * ‖((iteratedCovGrad (I := I) g₀ 0 2 1 T).toSection x :
              Tensor0SBundle.TensorRSSpace 0 3 I x)‖ *
            Real.sqrt (g₀.inner x (metricComparisonEndomorphism (I := I) g₀ g₁ x w)
              (metricComparisonEndomorphism (I := I) g₀ g₁ x w)) * Nv
        ≤ C₀ * G * (2 * Nw) * Nv := by
          rw [← hG_def]
          gcongr
  ...
  have hT3_bound : Real.sqrt (g₀.inner x T3 T3) ≤ 2 * C₀ * G * Nv * Nw := by
    set Dfun : TangentSpace I x →L[ℝ] ℝ :=
      (-(cotangentToCLM (I := I) (g0FlatCLM (I := I) g₀ x w)).comp
          ((PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x).flip v)) with hDfun_def
    ...
    have hpp_val : g₀.inner x p p =
        - g₀.inner x w (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v) := by
      rw [hpz p, hDval p]
    have hconn_p := hpw g₁ T h hδ hδ0 hbound x p v
    rw [← hNv_def, ← hNpdef] at hconn_p
    have hconnG : Real.sqrt (g₀.inner x (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v)
          (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v)) ≤ C₀ * G * Np * Nv := hconn_p
    have hcs := abs_metric_inner_le_sqrt_metric_quadratic (I := I) (M := M) g₀ x w
      (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v)
    rw [← hNw_def] at hcs
    have hNp_le : Np ≤ C₀ * G * Nv * Nw := by
      have hpp_le : g₀.inner x p p ≤
          Nw * Real.sqrt (g₀.inner x (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v)
            (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v)) := by
        rw [hpp_val]
        calc - g₀.inner x w (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v)
            ≤ |g₀.inner x w (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v)| := neg_le_abs _
          _ ≤ Nw * Real.sqrt (g₀.inner x (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v)
              (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v)) := hcs
      have hKnn : 0 ≤ C₀ * G * Nv * Nw :=
        mul_nonneg (mul_nonneg (mul_nonneg hC₀0 hG_nn) hNv_nn) hNw_nn
      have hchain : Nw * Real.sqrt (g₀.inner x (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v)
            (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v)) ≤
          Nw * (C₀ * G * Np * Nv) :=
