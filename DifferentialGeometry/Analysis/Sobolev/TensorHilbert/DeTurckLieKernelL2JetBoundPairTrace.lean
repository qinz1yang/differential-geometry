...
private lemma pureDeTurckTrace_eq_metricComparisonTrace (g₀ g₁ : SmoothRiemannianMetric I M)
  ...
  have hGrep : ∀ a : Fin (Module.finrank ℝ E),
    ...
    rw [g₁.symm x (smoothOrthoFrame (I := I) g₁ x c x)
      (metricComparisonEndomorphism (I := I) g₀ g₁ x (smoothOrthoFrame (I := I) g₀ x a x))]
    rw [g1_inner_metricComparisonEndomorphism_left_deTurckLieConnectionDifferenceDerivative (I := I) (M := M) g₀ g₁ x
...
private lemma deTurckLieConnectionDifferenceDerivativePairTraceOperator_toModel (g₀ gm : SmoothRiemannianMetric I M)
    ...
    Tensor0SSpace.toModel
        ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 2 I x from
          (ccOperatorFieldComp (I := I) (M := M) g₀ 2 6 2 (deTurckLieConnectionDifferenceDerivativePairTraceOperator (I := I) (M := M) g₀ gm)
...
def deTurckLieConnectionDifferenceDerivativeSymCc (g₀ : SmoothRiemannianMetric I M) (T : SmoothCcTensor g₀ 0 2)
...
private lemma deTurckLieConnectionDifferenceDerivativeSymCc_unitModel_apply (g₀ : SmoothRiemannianMetric I M)
  ...
  rw [ContinuousMultilinearMap.domDomCongr_apply]
  rw [deTurckLieConnectionDifferenceDerivativeLoweredG1Cc_unitModel_apply (I := I) (M := M) g₀ T g₁ g_bg htie x
    (fun i => m ((Equiv.swap (0 : Fin 4) 1) i))]
  rw [deTurckLieConnectionDifferenceDerivativeLoweredG1Cc_unitModel_apply (I := I) (M := M) g₀ T g₁ g_bg htie x m]
...
private theorem exists_riemannianFiberNormSq_pureDT_tgrid (g₀ : SmoothRiemannianMetric I M) (s : ℕ)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ C : ℕ → ℝ, (∀ j, 0 ≤ C j) ∧
  ...
  rw [pureDeTurckTrace_eq_metricComparisonTrace (I := I) (M := M) g₀ g₁ s]
  refine le_trans
  ...
  have hzero : ∀ i' ∈ Finset.range (j + 1), i' ≠ 0 →
    ...
    rw [iteratedCovGrad_succ_cometricDT_zero_deTurckLieConnectionDifferenceDerivative (I := I) (M := M) g₀ s m]
  ...
  have hsumS : (∑ l ∈ Finset.range (j + 1 - 0),
    ...
    exact antidiagonalTupleGrid_le_partialSum b hb (by omega)
...
theorem exists_deTurckLieConnectionDifferenceDerivativePairTraceOperator_fiberNormSq_antidiagonalTupleGrid_bound (g₀ : SmoothRiemannianMetric I M)
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 6 (2 + j) x
          ...
          C j * antidiagonalTupleGridPartialSum
            (fun l => riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + l) x
              ((iteratedCovGrad (I := I) g₀ 0 2 l T).toSection x)) (j + 1) := by
  classical
  obtain ⟨C2, hC2_nn, hC2⟩ := exists_riemannianFiberNormSq_pureDT_tgrid (I := I) (M := M) g₀ 2 hδ₀
  obtain ⟨C4, hC4_nn, hC4⟩ := exists_riemannianFiberNormSq_pureDT_tgrid (I := I) (M := M) g₀ 4 hδ₀
  refine ⟨fun j => diagonalGridGrowthFactor (E := E) j * ∑ i' ∈ Finset.range (j + 1),
    ...
    fun j => mul_nonneg (operatorFieldApplicationGdiag_nonneg (E := E) j)
      (Finset.sum_nonneg fun i' _ => mul_nonneg (hC2_nn i')
        (Finset.sum_nonneg fun l _ => mul_nonneg (hC4_nn l) (antidiagonalTuplePairCount_nonneg _ _))), ?_⟩
  intro g₁ T htie δ hδ_le hδ0 hbound j x
  set b : ℕ → ℝ := fun l => riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + l) x
    ((iteratedCovGrad (I := I) g₀ 0 2 l T).toSection x) with hb_def
  have hb : ∀ l, 0 ≤ b l :=
  ...
  have hW_nn : 0 ≤ W := antidiagonalTupleGridPartialSum_nonneg b hb (j + 1)
  refine le_trans
    (riemannianFiberNormSq_iteratedCovGrad_ccTensorCompose_diagonalProductGrid_leftFactor_le
    (I := I) (M := M) g₀ j 6 4 2
    (cometricDoubleTraceSmoothCcTensor (I := I) (M := M) g₀ g₁ 2)
  ...
  have hcell : ∀ i' ∈ Finset.range (j + 1),
    ...
    have hpair : antidiagonalTupleGridPartialSum b (i' + 1) * antidiagonalTupleGridPartialSum b
      ...
      antidiagonalTupleGridPartialSum_mul_le b hb (i' + 1) (l + 1) (j + 1) (by omega)
...
theorem deTurckLieConnectionDifferenceDerivCoeffField_eq_pairTrace
      ...
      (-1 : ℝ) • ccOperatorFieldComp (I := I) (M := M) g₀ 2 6 2
        ...
        (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 deTurckLieConnectionDifferenceDerivativeInputPermutation
          (slotExtendIter (I := I) (M := M) g₀ 0 4 2
            (deTurckLieConnectionDifferenceDerivativeSymCc (I := I) (M := M) g₀ T g₁ g_bg))) := by
  classical
  apply SmoothCcTensor.ext
  apply ContMDiffSection.ext
  intro x
  ...
  have hsmul : ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 2 I x from
      (((-1 : ℝ) • ccOperatorFieldComp (I := I) (M := M) g₀ 2 6 2
        ...
        (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 deTurckLieConnectionDifferenceDerivativeInputPermutation
          (slotExtendIter (I := I) (M := M) g₀ 0 4 2
            (deTurckLieConnectionDifferenceDerivativeSymCc (I := I) (M := M) g₀ T g₁ g_bg)))).toSection x)) D) =
      (-1 : ℝ) • ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 2 I x from
        ((ccOperatorFieldComp (I := I) (M := M) g₀ 2 6 2 (deTurckLieConnectionDifferenceDerivativePairTraceOperator (I := I) (M := M) g₀ g₁)
          (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 deTurckLieConnectionDifferenceDerivativeInputPermutation
            (slotExtendIter (I := I) (M := M) g₀ 0 4 2
              (deTurckLieConnectionDifferenceDerivativeSymCc (I := I) (M := M) g₀ T g₁ g_bg)))).toSection x)) D) := by
