...
private lemma connectionDifferenceResidualGridSum_le_boundedFactorGridWindow (b : ℕ → ℝ) (hb : ∀ j, 0 ≤ b j) (j : ℕ) :
...
theorem connectionDifferenceContravariantInsertionField_perOrder_l2_topOrderSeparated_generic
  ...
  · obtain ⟨x₀⟩ := hMne
    ...
    have hpt : ∀ x : M,
      ...
      have htrans : riemannianFiberNormSq (I := I) (M := M) g₀ 3 (4 + i) x
        ...
        have h0 : riemannianFiberNormSq (I := I) (M := M) g₀ 3 (4 + i) x
          ...
          exact riemannianFiberNormSq_iteratedCovGrad_reindexCoeffGen_eq
            ...
            connectionDifferenceContrInsertionReindexPerm i x
        have h1 := riemannianFiberNormSq_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 2 3
          (slotExtend (I := I) (M := M) g₀ 1 2 (connectionDifferenceSection (I := I) g₁ g₀)) i x
        have h2 := riemannianFiberNormSq_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 1 2
          (connectionDifferenceSection (I := I) g₁ g₀) i x
        rw [h0]
        calc riemannianFiberNormSq (I := I) (M := M) g₀ 3 (4 + i) x
            ...
            ≤ fr * riemannianFiberNormSq (I := I) (M := M) g₀ 2 (3 + i) x
                ((iteratedCovGrad (I := I) g₀ 2 3 i
                  (slotExtend (I := I) (M := M) g₀ 1 2
                    (connectionDifferenceSection (I := I) g₁ g₀))).toSection x) := h1
      ...
      have hsplit : riemannianFiberNormSq (I := I) (M := M) g₀ 1 (2 + i) x
          ...
          2 * riemannianFiberNormSq (I := I) (M := M) g₀ 1 (2 + i) x
            ((iteratedCovGrad (I := I) g₀ 1 2 i (connectionDifferenceSection (I := I) g₁ g₀) -
              Hd).toSection x) := by
        have hadd := riemannianFiberNormSq_add_le (I := I) (M := M) g₀ 1 (2 + i) x
          (Hd.toSection x)
          ((iteratedCovGrad (I := I) g₀ 1 2 i (connectionDifferenceSection (I := I) g₁ g₀) - Hd).toSection x)
        have key :
            (iteratedCovGrad (I := I) g₀ 1 2 i (connectionDifferenceSection (I := I) g₁ g₀)).toSection x =
              Hd.toSection x +
                (iteratedCovGrad (I := I) g₀ 1 2 i (connectionDifferenceSection (I := I) g₁ g₀) -
      ...
      have hrem_reshaped : riemannianFiberNormSq (I := I) (M := M) g₀ 1 (2 + i) x
        ...
        refine le_trans (connectionDifferenceResidualGridSum_le_boundedFactorGridWindow
          (fun l => riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + l) x
    ...
    refine le_trans (normSq_le_integral_of_pointwise_fiberNormSq_le_rs (I := I) (M := M) g₀ 3
      (4 + i) (iteratedCovGrad (I := I) g₀ 3 4 i (connectionDifferenceContravariantInsertionField (I := I) g₀ g₁))
...
theorem connectionDifferenceContravariantInsertionField_metricPerturbationPath_jetL2_perOrder_topOrderSeparated
  ...
  have hmain := hgen (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s)
...
theorem connectionDifferenceContravariantInsertionField_metricPerturbationPath_jetL2_summed_topOrderSeparated
  ...
  exact jetL2_sum_lowShift a 1 2 Ktop hKtop_nn Kc hKc_nn
    (fun i => ‖iteratedCovGrad (I := I) g₀ 3 4 i
      (connectionDifferenceContravariantInsertionField (I := I) g₀ (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s))‖ ^ 2)
