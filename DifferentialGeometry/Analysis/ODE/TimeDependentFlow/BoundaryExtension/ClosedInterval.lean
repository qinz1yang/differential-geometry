...
theorem flowValid_chain_step
  ...
  have hoverlap : ∀ x : M, ∀ s ∈ Set.Ioo a₀ hi, Φ s x = Ψ (Φ t₁ x) s := by
    ...
    exact integral_curves_eqOn_of_jointC1 (a := a₀) (b := hi) (t₀ := t₁)
...
theorem global_flow_on_closed_interval_of_closed_manifold
    (X : ℝ → ∀ x : M, TangentSpace I x)
    (hX : ContMDiff (𝓘(ℝ,ℝ).prod I) (I.prod 𝓘(ℝ,E)) ∞
      (fun q : ℝ × M => (TotalSpace.mk' E q.2 (X q.1 q.2) : TangentBundle I M)))
    (T : ℝ) (hT : 0 < T) :
...
theorem global_flow_with_reverse_on_closed_interval_of_closed_manifold
    (X : ℝ → ∀ x : M, TangentSpace I x)
    (hX : ContMDiff (𝓘(ℝ,ℝ).prod I) (I.prod 𝓘(ℝ,E)) ∞
      (fun q : ℝ × M => (TotalSpace.mk' E q.2 (X q.1 q.2) : TangentBundle I M)))
    (T : ℝ) (hT : 0 < T) :
