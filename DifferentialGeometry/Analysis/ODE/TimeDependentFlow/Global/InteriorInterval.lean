...
theorem integralCurves_eqOn_Icc_of_agree_at_left
  ...
  have hflowXt' : ∀ t ∈ Set.Icc a b,
    ...
    intro t ht
    rw [hXt_eq t (hsub ht) (Φ' t x')]; exact hflow' t ht
  exact forward_integral_curves_eqOn_of_jointC1 Xt hXt_auto Φ Φ' x x' hflowXt hflowXt' hstart
...
theorem integral_curves_eqOn_Ico_of_local_lipschitz
  ...
  obtain ⟨delta, hdelta, hlocal⟩ :=
    integral_curves_eventuallyEq_zero_of_lipschitz (I := I) X (Φ 0 x) ha hdelta₀ hlip
...
theorem integral_curves_eqOn_Ico_of_smooth
    (X : ℝ → ∀ x : M, TangentSpace I x) (T : ℝ) (hT : 0 < T)
  ...
  · have hsub : Set.Icc (0 : ℝ) t ⊆ Set.Ici 0 := by
    ...
    have hflowIcc' : ∀ s ∈ Set.Icc (0 : ℝ) t,
      ...
      rw [hXeq s hsT' (Φ' s x')]
      exact hd
    exact forward_integral_curves_eqOn_of_jointC1 Xext hXc1 Φ Φ' x x'
