...
theorem iterated_recurrence_bound
    ...
    (hLift : forall p : Nat,
      D (p + 1) <= G p + eps * iteratedRecurrenceConstant B p (s + 1) *
    ...
    forall p r : Nat, 0 < r -> r <= p ->
      D r <= N r + eps * iteratedRecurrenceConstant B p s *
  ...
  | succ p ih =>
      ...
      · have hih := ih r hr0 hr_le_p
        ...
        have hcoef :
            iteratedRecurrenceConstant B p s <= iteratedRecurrenceConstant B (p + 1) s :=
          iterated_recurrence_constant_le_succ hB p s
        have hepsS_nonneg : 0 <= eps * S := mul_nonneg heps0 hS_nonneg
        have hmul :
            eps * iteratedRecurrenceConstant B p s * S <=
              eps * iteratedRecurrenceConstant B (p + 1) s * S := by
          calc
            eps * iteratedRecurrenceConstant B p s * S = (eps * S) * iteratedRecurrenceConstant B p s := by
              ring
            _ <= (eps * S) * iteratedRecurrenceConstant B (p + 1) s :=
              mul_le_mul_of_nonneg_left hcoef hepsS_nonneg
            _ = eps * iteratedRecurrenceConstant B (p + 1) s * S := by
              ring
        have hmono :
            N r + eps * iteratedRecurrenceConstant B p s * S <=
              N r + eps * iteratedRecurrenceConstant B (p + 1) s * S :=
          by
      ...
      · have hr_eq : r = p + 1 := by omega
        subst r
        exact recurrence_step_bound_of_partials
          (eps := eps) (A := D (p + 1)) (B := B) (N := N) (G := G)
          (p := p) (s := s)
          heps0 heps1 hB hN (hLift p) (hOne p)
          (fun k _hk => hOne k)

theorem iterated_recurrence_bound_of_antidiagonal
    ...
    forall p r : Nat, 0 < r -> r <= p ->
      D r <= N r + eps * iteratedRecurrenceConstant B p s *
        Finset.sum (Finset.range r) (fun i => N i) := by
  refine iterated_recurrence_bound
...
theorem iterated_recurrence_bound_of_finite_horizon
  ...
  | succ P ih =>
      ...
      · have hih := ih
        ...
        have hcoef : iteratedRecurrenceConstant B P s <= iteratedRecurrenceConstant B (P + 1) s :=
          iterated_recurrence_constant_le_succ hB P s
        have hepsS : 0 <= eps * Finset.sum (Finset.range r) (fun i => N i) :=
          mul_nonneg heps0 hS_nonneg
        have hmul :
            eps * iteratedRecurrenceConstant B P s *
              Finset.sum (Finset.range r) (fun i => N i) <=
            eps * iteratedRecurrenceConstant B (P + 1) s *
              Finset.sum (Finset.range r) (fun i => N i) := by
          calc
            eps * iteratedRecurrenceConstant B P s * Finset.sum (Finset.range r) (fun i => N i)
                = (eps * Finset.sum (Finset.range r) (fun i => N i)) *
                    iteratedRecurrenceConstant B P s := by ring
            _ <= (eps * Finset.sum (Finset.range r) (fun i => N i)) *
                    iteratedRecurrenceConstant B (P + 1) s :=
                  mul_le_mul_of_nonneg_left hcoef hepsS
            _ = eps * iteratedRecurrenceConstant B (P + 1) s *
...
theorem two_parameter_iterated_recurrence_bound
    ...
    forall p i r : Nat, 0 < r -> r <= p ->
      W (i + r) 0 <= W i r + eps * iteratedRecurrenceConstant B p (s + i) *
        Finset.sum (Finset.range r) (fun j => W i j) := by
  intro p
  induction p using Nat.strong_induction_on with
  | _ p ihp =>
      intro i r hr0 hrp
      refine iterated_recurrence_bound_of_finite_horizon (eps := eps) (B := B)
...
theorem two_parameter_iterated_recurrence_bound_of_finite_horizon
    {eps : Real} {B : Nat -> Real} {s : Nat}
    (heps0 : 0 <= eps)
    (heps1 : eps <= 1)
    (hB : forall i : Nat, 0 <= B i)
