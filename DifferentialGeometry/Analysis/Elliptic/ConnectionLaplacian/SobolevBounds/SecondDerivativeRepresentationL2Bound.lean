...
import DifferentialGeometry.Analysis.Elliptic.ConnectionLaplacian.ChartRepresentationBounds.SecondDerivativeComponentBounds
import DifferentialGeometry.Analysis.Elliptic.ConnectionLaplacian.ChartRepresentationBounds.ConnectionLaplacianRepresentationBound
import DifferentialGeometry.Analysis.Integration.Measure.Chart.TensorPullback
import DifferentialGeometry.Analysis.Integration.L2.SmoothSections.Defs
import DifferentialGeometry.Analysis.Elliptic.ConnectionLaplacian.SobolevBounds.PartitionOfUnityL2Bridge
...
import DifferentialGeometry.Analysis.Elliptic.ConnectionLaplacian.SobolevBounds.RepresentationL2Bound
import DifferentialGeometry.Analysis.Elliptic.ConnectionLaplacian.SobolevBounds.PartitionOfUnityDerivative
import DifferentialGeometry.Analysis.Elliptic.ConnectionLaplacian.SobolevBounds.FirstDerivativeRepresentationL2Bound
...
private lemma aemeasurable_fintype_sum_two
    {α ι κ : Type*} [MeasurableSpace α] [Fintype ι] [Fintype κ]
    (μ : Measure α) (f : ι → κ → α → ℝ≥0∞)
    (hf : ∀ i j, AEMeasurable (f i j) μ) :
    AEMeasurable (fun x => ∑ i, ∑ j, f i j x) μ := by
  have h_sum : AEMeasurable (∑ i, ∑ j, f i j) μ :=
    Finset.aemeasurable_sum Finset.univ fun i _ =>
      Finset.aemeasurable_sum Finset.univ fun j _ => hf i j
  have h_eq : (fun x => ∑ i, ∑ j, f i j x) = ∑ i, ∑ j, f i j := by
    funext x
    simp only [Finset.sum_apply]
  rw [h_eq]
  exact h_sum

private lemma lintegral_fintype_sum_two
    {α ι κ : Type*} [MeasurableSpace α] [Fintype ι] [Fintype κ]
    (μ : Measure α) (f : ι → κ → α → ℝ≥0∞)
    (hf : ∀ i j, AEMeasurable (f i j) μ) :
    (∫⁻ x, ∑ i, ∑ j, f i j x ∂μ) = ∑ i, ∑ j, ∫⁻ x, f i j x ∂μ := by
  have h_inner : ∀ i, AEMeasurable (fun x => ∑ j, f i j x) μ := by
    intro i
    have h_sum : AEMeasurable (∑ j, f i j) μ :=
      Finset.aemeasurable_sum Finset.univ fun j _ => hf i j
    ...
    rw [h_eq]
    exact h_sum
  rw [lintegral_finsetSum' Finset.univ (fun i _ => h_inner i)]
...
theorem chartTargetPouWeightedL2NormSq_iteratedFDeriv_two_repr_le_sum_chartComp_data
  ...
  · let : Nontrivial E := hE
    let : NeZero (Module.finrank ℝ E) :=
      ⟨Nat.ne_of_gt (Module.finrank_pos (R := ℝ) (M := E))⟩
    ...
    have h_pt : ∀ y ∈ chartTargetEuclid (I := I) (M := M) α,
      ...
      have h_real := pou_sq_iteratedFDeriv_two_repr_sq_pointwise
        (I := I) (M := M) g r s T α K_pou2 hK_pou2_bound (e := e) he_target
    ...
    have h_innersum_aeMeas : ∀ (f : (Fin r → Fin (Module.finrank ℝ E)) →
        ...
        AEMeasurable (fun y : EuclN => ∑ Idx, ∑ Jdx, f Idx Jdx y)
          ((volume : Measure EuclN).restrict
            (chartTargetEuclid (I := I) (M := M) α)) := by
      intro f hf
      exact aemeasurable_fintype_sum_two
        ((volume : Measure EuclN).restrict
          (chartTargetEuclid (I := I) (M := M) α)) f hf
    ...
    have h_dist : ∀ (f : (Fin r → Fin (Module.finrank ℝ E)) →
      ...
      intro f hf
      exact lintegral_fintype_sum_two
        ((volume : Measure EuclN).restrict
          (chartTargetEuclid (I := I) (M := M) α)) f hf
  ...
  · let : Subsingleton E := not_nontrivial_iff_subsingleton.mp hE
    refine ⟨0, le_rfl, ?_⟩
    intro T
    have h_deriv (y : EuclN) :
        iteratedFDeriv ℝ 2
            (tensorRSChartESectionRepr (I := I) r s α
              (fun z : M => T.toSection z) ∘ (extChartAt I α).symm)
            ((toEuclidean (E := E)).symm y) = 0 := by
      let F := tensorRSChartESectionRepr (I := I) r s α
        (fun z : M => T.toSection z) ∘ (extChartAt I α).symm
      have hF : F = fun _ => F 0 := by
        funext e
        exact congrArg F (Subsingleton.elim e 0)
      change iteratedFDeriv ℝ 2 F ((toEuclidean (E := E)).symm y) = 0
      rw [hF, iteratedFDeriv_const_of_ne (by norm_num)]
      rfl
    have h_lhs :
        (∫⁻ y in chartTargetEuclid (I := I) (M := M) α,
            ENNReal.ofReal
      ...
      exact Filter.Eventually.of_forall fun y => by
        ...
        rw [h_deriv y, norm_zero, zero_pow (by norm_num), ENNReal.ofReal_zero, mul_zero]
    rw [h_lhs, ENNReal.ofReal_zero, zero_mul]
...
private lemma sum_sq_le_sq_sum_finset
  ...
  have hineq : ∀ i ∈ s, (f i) ^ 2 ≤ f i * S := by
    ...
    have hi_le_S : f i ≤ S := Finset.single_le_sum
      (f := f) (fun _ _ => zero_le) hi
    have hsq_eq : (f i) ^ 2 = f i * f i := sq (f i)
    rw [hsq_eq]
    exact mul_le_mul_of_nonneg_left hi_le_S (zero_le)
  calc ∑ i ∈ s, (f i) ^ 2
...
private lemma tsum_sq_le_sq_tsum_ennreal
  ...
  have h_pointwise : ∀ i, (f i) ^ 2 ≤ f i * S := by
    ...
    rw [hsq]
    exact mul_le_mul_of_nonneg_left (h_le i) (zero_le)
  calc ∑' i, (f i) ^ 2
      ≤ ∑' i, f i * S :=
...
private lemma finset_sum_wkpNorm_zero_sq_le_wtwokTwoNorm_sq
        ...
        ∑ Idx : Fin r → Fin (Module.finrank ℝ E),
          ∑ Jdx : Fin s → Fin (Module.finrank ℝ E),
            (iteratedWeakSobolevNorm (d := Module.finrank ℝ E) 0 2
              (tensorChartComp (I := I) (M := M) g r s T α Idx Jdx)
              (chartTargetEuclid (I := I) (M := M) α)) ^ 2 ≤
      ((chartAtlasPOUFinset (I := I) (M := M)).card : ℝ≥0∞) *
  ...
  have h_bound : ∀ α ∈ chartAtlasPOUFinset (I := I) (M := M),
  ...
  rw [Finset.sum_congr (rfl :
    chartAtlasPOUFinset (I := I) (M := M) = chartAtlasPOUFinset (I := I) (M := M))
    (fun α _ => h_inner_eq α)]
  rw [Finset.sum_const, nsmul_eq_mul]
  rw [show ((chartAtlasPOUFinset (I := I) (M := M)).card : ℝ≥0∞) *
        (((Finset.univ : Finset (Fin r → Fin (Module.finrank ℝ E))).card : ℝ≥0∞) *
          (((Finset.univ : Finset (Fin s → Fin (Module.finrank ℝ E))).card : ℝ≥0∞) *
              W)) =
      ((chartAtlasPOUFinset (I := I) (M := M)).card : ℝ≥0∞) *
