...
theorem cometricDoubleTraceFib_comp_slotExtend_two_eq (g₀ : SmoothRiemannianMetric I M) (b s : ℕ)
  ...
  rw [cometricDoubleTraceFib_toModel, modelDoubleTrace_apply (E := E) s
    (cometricLmodel (I := I) g₀ x)]
  simp only [sum_apply, Tensor0SSpace.toModelL_apply]
  refine Finset.sum_congr rfl (fun k _ => ?_)
...
omit [BoundarylessManifold I M] [SigmaCompactSpace M] in
omit [I.Boundaryless] in
theorem cometricDoubleTrace_operatorFieldApplication_slotExtend_two_comm (g₀ : SmoothRiemannianMetric I M) (b s : ℕ)
  ...
  intro x
  simp only [operatorFieldApplication_toSection]
...
omit [I.Boundaryless] in
theorem cometricTrace_ccOperatorFieldComp (g₀ : SmoothRiemannianMetric I M) (r b s : ℕ)
  ...
  apply ContMDiffSection.ext
  intro x
  simp only [operatorFieldComposition_toSection]
  rw [← ContinuousLinearMap.comp_assoc, ← ContinuousLinearMap.comp_assoc,
    cometricDoubleTraceFib_comp_slotExtend_two_eq (I := I) g₀ b s Φ x]

omit [SigmaCompactSpace M] in
theorem rawTensorConnLap_operatorFieldApplication_comm_of_rank (g₀ : SmoothRiemannianMetric I M) (b s : ℕ)
  ...
  classical
  rw [rawTensorConnLapSmooth_eq_operatorFieldApplication_cometricDoubleTrace_of_rank (I := I) g₀ s
  ...
  rw [cometricDoubleTrace_operatorFieldApplication_slotExtend_two_comm (I := I) g₀ b s Φ
    (covGrad (I := I) (M := M) g₀ 0 (b + 1) (covGrad (I := I) (M := M) g₀ 0 b W))]
  rw [show operatorFieldApply (I := I) (M := M) g₀ (b + 2) b
    (cometricDoubleTraceField (I := I) g₀ b)
        (covGrad (I := I) (M := M) g₀ 0 (b + 1) (covGrad (I := I) (M := M) g₀ 0 b W)) =
      rawTensorConnLapSmooth (I := I) g₀ 0 b W from by
    rw [rawTensorConnLapSmooth_eq_operatorFieldApplication_cometricDoubleTrace_of_rank (I := I) g₀ b W]
    rfl]
  abel
...
omit [SigmaCompactSpace M] in
theorem rawConnLap_operatorFieldApplication_iteratedCovGrad_two_comm (g₀ : SmoothRiemannianMetric I M) (s t : ℕ)
  ...
  rw [rawTensorConnLap_operatorFieldApplication_comm_of_rank (I := I) g₀ (s + 2) t C
    (iteratedCovGrad (I := I) g₀ 0 s 2 S)]
  rw [rawConnLap_iteratedCovGrad_two_comm (I := I) g₀ s S]
  rw [operatorFieldApplication_add_right, operatorFieldApplication_add_right]
