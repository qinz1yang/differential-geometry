...
noncomputable def UnrestrictedSmoothScalar.oneSubLapClassicalRep
    {g : SmoothRiemannianMetric (I_half n) M} (u : UnrestrictedSmoothScalar g) : M → ℝ :=
  fun x => u.toFun x -
    divergenceGWithBoundaryPou (n := n) (M := M) g
      (gradFunSection (M := M) (n := n) g u.smooth) x

lemma UnrestrictedSmoothScalar.oneSubLapClassicalRep_continuous
    {g : SmoothRiemannianMetric (I_half n) M} (u : UnrestrictedSmoothScalar g) :
    Continuous (u.oneSubLapClassicalRep) := by
  unfold UnrestrictedSmoothScalar.oneSubLapClassicalRep
  exact u.smooth.continuous.sub
    (divergence_g_with_boundary_pou_continuous (n := n) (M := M) g
      (gradFunSection (M := M) (n := n) g u.smooth))

lemma UnrestrictedSmoothScalar.oneSubLapClassicalRep_memLp
    {g : SmoothRiemannianMetric (I_half n) M} (u : UnrestrictedSmoothScalar g) :
...
noncomputable def UnrestrictedSmoothScalar.oneSubLapClassicalLp
    {g : SmoothRiemannianMetric (I_half n) M} (u : UnrestrictedSmoothScalar g) :
    Lp ℝ 2 (riemannianVolumeMeasure (I := I_half n) (M := M) g) :=
  u.oneSubLapClassicalRep_memLp.toLp _

lemma UnrestrictedSmoothScalar.coeFn_oneSubLapClassicalLp
    {g : SmoothRiemannianMetric (I_half n) M} (u : UnrestrictedSmoothScalar g) :
...
lemma UnrestrictedSmoothScalar.oneSubLapClassicalLp_ae_oneSubLap
    {g : SmoothRiemannianMetric (I_half n) M} (u : UnrestrictedSmoothScalar g) :
    (u.oneSubLapClassicalLp :
      Lp ℝ 2 (riemannianVolumeMeasure (I := I_half n) (M := M) g)) =ᵐ[
        riemannianVolumeMeasure (I := I_half n) (M := M) g]
...
theorem smoothToUnrestrictedH1Compl_bilin_eq_lpFunctional_neumann
    {g : SmoothRiemannianMetric (I_half n) M}
    ...
    (h_neumann : ∀ x : (I_half n).boundary M,
      g.inner x.val
        (outwardNormal (I := I_half n) (M := M) g x :
          TangentSpace _ x.val)
        (gradFun (I := I_half n) g u.toFun x.val) = 0)
    (h_chart_iden : ∀ (v : UnrestrictedSmoothScalar g),
      ...
      chartFaceIntegralEqualsSurfaceIntegralOnChart (n := n) (M := M)
        ...
        (smoothSmul (I := I_half n) v.toFun v.smooth
          (gradFunSection (M := M) (n := n) g u.smooth))
        ((chartAtlasPOU (I_half n) M) α : M → ℝ))
    (h_int : ∀ (v : UnrestrictedSmoothScalar g),
      Integrable
        (fun b : BoundaryManifold (I_half n) M =>
          g.inner b.val
            (outwardNormal
...
theorem smoothToUnrestrictedH1Compl_eq_unrestrictedNeumannResolvent_oneSubLap_neumann
    {g : SmoothRiemannianMetric (I_half n) M}
    (u : UnrestrictedSmoothScalar g)
    (h_neumann : ∀ x : (I_half n).boundary M,
      g.inner x.val
