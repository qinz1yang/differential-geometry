...
theorem exists_smoothApprox_chartW32
    ...
    (hu_h : u_h ∈ laplacianDomainPow (I := I) (M := M) g 2)
    {ε : ℝ} (hε : 0 < ε) :
    ∃ v : SmoothScalar g, wkpNormChart (I := I) (M := M) 3 2
  ...
  have h_u_mem : MemWkpChart (I := I) (M := M) 3 2 u :=
  ...
  obtain ⟨w, hw_smooth, hw_le⟩ :=
    DifferentialGeometry.Analysis.Sobolev.Chart.contMDiff_dense_in_WkpChart_k
      (I := I) (M := M) 3 (p := 2) (by norm_num : (1 : ℝ≥0∞) ≤ 2)
...
theorem smoothApproxSeqWkpThree_wkpNormChart_diff_le
  ...
  have h_smooth_vm : MemWkpChart (I := I) (M := M) 3 2 vm.toFun :=
    DifferentialGeometry.Analysis.Sobolev.Chart.memWkpChart_of_contMDiff_k
      (I := I) (M := M) hp_one 3 vm.smooth
  have h_smooth_vn : MemWkpChart (I := I) (M := M) 3 2 vn.toFun :=
    DifferentialGeometry.Analysis.Sobolev.Chart.memWkpChart_of_contMDiff_k
      (I := I) (M := M) hp_one 3 vn.smooth
  have h_u_diff_vm : MemWkpChart (I := I) (M := M) 3 2 (fun x => u x - vm.toFun x) :=
    DifferentialGeometry.Analysis.Sobolev.Chart.MemWkpChart_sub
      (I := I) (M := M) hp_one h_u_mem h_smooth_vm
  have h_u_diff_vn : MemWkpChart (I := I) (M := M) 3 2 (fun x => u x - vn.toFun x) :=
    DifferentialGeometry.Analysis.Sobolev.Chart.MemWkpChart_sub
  ...
  have h_neg_smul :
      ...
      ‖(-1 : ℝ)‖ₑ * wkpNormChart (I := I) (M := M) 3 2 (fun x => u x - vm.toFun x) :=
  ...
  have h_u_diff_vm_neg : MemWkpChart (I := I) (M := M) 3 2
  ...
  have h_add :=
    DifferentialGeometry.Analysis.Sobolev.Chart.wkpNormChart_add_le (I := I) (M := M)
      (k := 3) (p := 2) hp_one h_u_diff_vn h_u_diff_vm_neg
  rw [h_neg_smul] at h_add
  have h_bound1 : wkpNormChart (I := I) (M := M) 3 2 (fun x => u x - vn.toFun x) +
        wkpNormChart (I := I) (M := M) 3 2 (fun x => u x - vm.toFun x) ≤
...
theorem smoothApproxSeq_smoothFChartResidual_wkpNorm_cauchy_w22
  ...
  have h_step :
    ...
    exact mul_le_mul_of_nonneg_left h_chartW32_cauchy (zero_le)
