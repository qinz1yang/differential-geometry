...
private lemma memWkp_chartPushedRaw_etaTimesV_three
  ...
  exact DifferentialGeometry.Analysis.Sobolev.Euclidean.MemWkp_of_smooth_compactSupport
    ...
    hCP_smooth hCP_compact hCP_tsupp (by norm_num : (1 : ℝ≥0∞) ≤ 2) 3
...
private lemma wkpNorm_chartPushedRaw_etaTimesV_le_three
    ...
    ∃ C : ℝ, 0 < C ∧ ∀ v : SmoothScalar g,
      DifferentialGeometry.Analysis.Sobolev.Euclidean.iteratedWeakSobolevNorm
        ...
        (chartPushedRaw (I := I) (M := M) α
          (etaTimesV (I := I) (M := M) α v.toFun))
        (chartTargetEuclid (I := I) (M := M) α) ≤
      ENNReal.ofReal C * wkpNormChart (I := I) (M := M) 3 2
  ...
  refine ⟨C, hC_pos, ?_⟩
  intro v
  have h_v_MemWkpChart : MemWkpChart (I := I) (M := M) 3 2 v.toFun :=
    memWkpChart_of_contMDiff_k (I := I) (M := M) (by norm_num) 3 v.smooth
...
private lemma wkpNorm_chartPushedRaw_etaTimesV_le_two
    ...
    ∃ C : ℝ, 0 < C ∧ ∀ v : SmoothScalar g,
      DifferentialGeometry.Analysis.Sobolev.Euclidean.iteratedWeakSobolevNorm
        ...
        (chartPushedRaw (I := I) (M := M) α
          (etaTimesV (I := I) (M := M) α v.toFun))
        (chartTargetEuclid (I := I) (M := M) α) ≤
      ENNReal.ofReal C * wkpNormChart (I := I) (M := M) 2 2
        (fun x : M => v.toFun x) := by
  classical
  obtain ⟨C, hC_pos, hC_bound⟩ :=
    DifferentialGeometry.Analysis.Sobolev.Chart.wkpNorm_chartPushedRaw_strictCutoff_mul_le
  ...
  refine ⟨C, hC_pos, ?_⟩
  intro v
  have h_v_MemWkpChart : MemWkpChart (I := I) (M := M) 2 2 v.toFun :=
    memWkpChart_of_contMDiff_k (I := I) (M := M) (by norm_num) 2 v.smooth
  have h_funext : etaTimesV (I := I) (M := M) α v.toFun =
      fun x : M => chartStrictCutoff (I := I) (M := M) α x * v.toFun x := by
    funext x; rfl
  rw [h_funext]
...
private lemma wkpNorm_chartPushedRaw_lapPiece_le_two
  ...
  obtain ⟨CΛ, hCΛ_nn, hCΛ_bound⟩ :=
    DifferentialGeometry.Analysis.Sobolev.Chart.smoothExtensionScalar_iteratedFDeriv_bound
      (I := I) (M := M) α hbΔρα_smooth hbΔρα_support 2
  ...
  have h_mono : wkpNormChart (I := I) (M := M) 2 2 v.toFun ≤
      wkpNormChart (I := I) (M := M) 3 2 v.toFun := by
    unfold wkpNormChart
    ...
    ∃ C : ℝ, 0 < C ∧ ∀ v : SmoothScalar g,
      DifferentialGeometry.Analysis.Sobolev.Euclidean.iteratedWeakSobolevNorm
        ...
        (smoothFChartResidual
          (I := I) (M := M) g α v)
        (chartTargetEuclid (I := I) (M := M) α)
      ≤ ENNReal.ofReal C * wkpNormChart (I := I) (M := M) 3 2 v.toFun := by
  classical
  ...
  calc DifferentialGeometry.Analysis.Sobolev.Euclidean.iteratedWeakSobolevNorm
      ...
      DifferentialGeometry.Analysis.Sobolev.Euclidean.iteratedWeakSobolevNorm
        ...
        (chartPushedRaw (I := I) (M := M) α
          (lapPiece (I := I) (M := M) g α v.toFun))
        (chartTargetEuclid (I := I) (M := M) α)
      ≤ ENNReal.ofReal C_grad * wkpNormChart (I := I) (M := M) 3 2 v.toFun +
        ENNReal.ofReal C_lap * wkpNormChart (I := I) (M := M) 3 2 v.toFun := by
