...
private theorem edge_riemannianFiberNormSq_neg (g : SmoothRiemannianMetric I M)
...
theorem topOrderDifferenceCoefficient_riemannianFiberNormSq (g₀ : SmoothRiemannianMetric I M) :
    ∃ C : ℝ, 0 ≤ C ∧
  ...
  let Λ := metricComparisonDifferenceEndomorphismField (I := I) (M := M) g₀ g₁
...
theorem topOrderDifference_remainder_bound (g₀ : SmoothRiemannianMetric I M) :
  ...
  classical
  obtain ⟨C₀, hC₀0, hC₀⟩ := topOrderDifferenceCoefficient_riemannianFiberNormSq (I := I) (M := M) g₀
      ...
      ∀ (g₁ : SmoothRiemannianMetric I M) (T : SmoothCcTensor g₀ 0 2)
        ...
        tensorL2Inner (I := I) (M := M) g₀ 0 2 T.toFun
          ...
          -(1 / 2 : ℝ) *
            ‖SmoothCcTensor.toL2
              (iteratedCovGrad (I := I) g₀ 0 2 1 T)‖ ^ 2 := by
  obtain ⟨C, hC0, harm⟩ := topOrderDifference_energy_bound (I := I) (M := M) g₀
  refine ⟨C, hC0, ?_⟩
