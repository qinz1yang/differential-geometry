...
private theorem iteratedCovGrad_pathIntegralCoeffField_comm
  ...
  | zero =>
    ...
    exact pathIntegralCoeffField_congr_of_family_eq (I := I) g₀ r sIdx Φ
  ...
  | succ j ih =>
    ...
    exact pathIntegralCoeffField_congr_of_family_eq (I := I) g₀ r (sIdx + j + 1)
...
theorem termField_pathIntegral_jetL2_perOrder_le
    ...
    ‖iteratedCovGrad (I := I) g₀ r 2 i
        (pathIntegralCoeffField (I := I) (M := M) g₀ r 2 Φ
          (DifferentialGeometry.PDE.DeTurck.RicciLinearization.metricPerturbationPathDomain (δ := δ) (δ' := δ'))
            hSopen hSI hjoint)‖ ^ 2 ≤ B ^ 2 := by
  classical
  set S : Set ℝ :=
    DifferentialGeometry.PDE.DeTurck.RicciLinearization.metricPerturbationPathDomain
      (δ := δ) (δ' := δ') with hS_def
