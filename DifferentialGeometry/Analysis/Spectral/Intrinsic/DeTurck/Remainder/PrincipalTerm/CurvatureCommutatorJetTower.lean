...
private lemma shift_sq_sum_le (u : ℕ → ℝ) (hu : ∀ b, 0 ≤ u b) (j m : ℕ) (hm : m ≤ 2) :
  ...
  refine le_trans (Finset.sum_le_sum_of_subset_of_nonneg ?_ (fun b _ _ => sq_nonneg _))
    (DifferentialGeometry.Analysis.sum_sq_le_sq_sum_of_nonneg u hu)
  intro b hb
  obtain ⟨i, hi, rfl⟩ := Finset.mem_image.mp hb
  have := Finset.mem_range.mp hi
  exact Finset.mem_range.mpr (by omega)

private lemma pointwiseTensorCurvRS_jet_le (g : SmoothRiemannianMetric I M) (r s : ℕ) :
    ∃ K : ℕ → ℝ, (∀ j, 0 ≤ K j) ∧
  ...
  have h₀ : ‖iteratedCovGrad (I := I) g r (s + 1) j
    ...
    have hsum_le : ∑ i ∈ Finset.range (j + 1),
      ...
      have := shift_sq_sum_le (fun b => ‖iteratedCovGrad (I := I) g r s b S‖)
  ...
  have h₁ : ‖iteratedCovGrad (I := I) g r (s + 1) j
    ...
    have hsum_le : ∑ i ∈ Finset.range (1 + j),
      ...
      shift_sq_sum_le (fun b => ‖iteratedCovGrad (I := I) g r s b S‖)
...
private lemma shift_sum_le (u : ℕ → ℝ) (hu : ∀ b, 0 ≤ u b) (k m n : ℕ)
    (hkm : ∀ i < k, i + m < n) :
...
private lemma curvature_commutator_jet_tower (g : SmoothRiemannianMetric I M) (r s : ℕ) (m : ℕ) :
    ∃ K : ℕ → ℝ, (∀ j, 0 ≤ K j) ∧
      ∀ (j : ℕ) (S : SmoothCcTensor g r s),
  ...
  | zero =>
    ...
    have hz : rawTensorConnLapSmooth (I := I) g r (s + 0)
      ...
      exact sub_self _
    rw [hz, DifferentialGeometry.Analysis.Sobolev.iteratedCovGrad_zero_section, norm_zero, zero_mul]
  | succ m ih =>
    obtain ⟨Km, hKm_nn, hKm⟩ := ih
    obtain ⟨Kp, hKp_nn, hKp⟩ := pointwiseTensorCurvRS_jet_le (I := I) (M := M) g r (s + m)
    ...
    have hpiece1 : ‖iteratedCovGrad (I := I) g r ((s + m) + 1) j
      ...
      calc ∑ b ∈ Finset.range (j + 3), ‖iteratedCovGrad (I := I) g r (s + m) b Y‖
        ...
        _ ≤ ∑ b ∈ Finset.range ((m + 1) + j + 2),
            ...
            shift_sum_le (fun b => ‖iteratedCovGrad (I := I) g r s b S‖)
...
private lemma curvature_commutator_second_order_bound (g : SmoothRiemannianMetric I M) (r s : ℕ) :
    ∃ c : ℝ, 0 ≤ c ∧ ∀ S : SmoothCcTensor g r s,
      ‖rawTensorConnLapSmooth (I := I) g r s S‖ ≤
        ‖iteratedCovGrad (I := I) g r s 2 S‖ +
          c * (‖iteratedCovGrad (I := I) g r s 1 S‖ + ‖S‖) := by
  classical
  obtain ⟨Kp, hKp_nn, hKp⟩ := pointwiseTensorCurvRS_jet_le (I := I) (M := M) g r s
  refine ⟨Kp 0 + 1, by linarith [hKp_nn 0], fun S => ?_⟩
  have hW := weitzenbock_integrated_covGrad_l2_normSq_rs (I := I) (M := M) g r s S
  have hiteratedCovGrad2 : covGrad (I := I) (M := M) g r (s + 1) (covGrad (I := I) (M := M) g r s S) =
      iteratedCovGrad (I := I) g r s 2 S := rfl
  have hiteratedCovGrad1 : covGrad (I := I) (M := M) g r s S = iteratedCovGrad (I := I) g r s 1 S := rfl
  ...
  rw [hiteratedCovGrad2, hptc] at hW
  ...
  have hgrad_eq : ‖covGrad (I := I) (M := M) g r s S‖ = a1 := by rw [hiteratedCovGrad1]
...
lemma exists_iteratedCovGrad_rawTensorConnLapSmooth_window_le (g : SmoothRiemannianMetric I M)
  ...
  have hG2fam : ∀ b : ℕ, ∃ c : ℝ, 0 ≤ c ∧ ∀ Y : SmoothCcTensor g r (s + b),
    ...
    fun b => curvature_commutator_second_order_bound (I := I) (M := M) g r (s + b)
  ...
  have hTfam : ∀ m : ℕ, ∃ K : ℕ → ℝ, (∀ j, 0 ≤ K j) ∧
    ...
    fun m => curvature_commutator_jet_tower (I := I) (M := M) g r s m
...
private lemma connLapIterate_one_expansion (g : SmoothRiemannianMetric I M) (r s : ℕ)
...
private lemma connLapIterate_succ_expansion (g : SmoothRiemannianMetric I M) (r s : ℕ) (q : ℕ)
  ...
  rw [oneMinusConnLapSmoothIter_add (I := I) (M := M) g r s q 1 S,
    connLapIterate_one_expansion (I := I) (M := M) g r s S]
  ...
  have hmain : ∀ q : ℕ, ∃ c : ℕ → ℝ, (∀ γ, 0 ≤ c γ) ∧
    ...
    | succ q ih =>
      ...
      refine ⟨fun γ => cq γ * (1 + ∑ b ∈ Finset.range (γ + 2 * q + 1), cL b),
        fun γ => mul_nonneg (hcq_nn γ)
          (by have : 0 ≤ ∑ b ∈ Finset.range (γ + 2 * q + 1), cL b :=
              ...
              linarith),
        fun γ S => ?_⟩
      have hsplit := connLapIterate_succ_expansion (I := I) (M := M) g r s q S
      ...
      have h2 : ‖iteratedCovGrad (I := I) g r s γ
        ...
        have hsum := sum_rawTensorConnLap_jets_bound (I := I) (M := M) g r s cL hcL_nn hcL
          (γ + 2 * q + 1) S
...
private lemma rawTensorConnLap_jets_bound (g : SmoothRiemannianMetric I M) (r s : ℕ) :
    ∃ c : ℕ → ℝ, (∀ b, 0 ≤ c b) ∧
      ∀ (b : ℕ) (S : SmoothCcTensor g r s),
        ‖iteratedCovGrad (I := I) g r s b (rawTensorConnLapSmooth (I := I) g r s S)‖ ≤
          ‖iteratedCovGrad (I := I) g r s (b + 2) S‖ +
...
lemma DeTurckRemainderPrincipalTerm.connLapIterate_jet_bound (g : SmoothRiemannianMetric I M) (r s : ℕ) :
  ...
  obtain ⟨cL, hcL_nn, hcL⟩ := exists_iteratedCovGrad_rawTensorConnLapSmooth_window_le (I := I)
    (M := M) g r s
  obtain ⟨cE, hcE_nn, hcE⟩ := rawTensorConnLap_jets_bound (I := I) (M := M) g r s
  have hmain : ∀ p : ℕ, ∃ c : ℝ, 0 ≤ c ∧
    ...
    | succ p ih =>
      ...
      have hsplit := connLapIterate_succ_expansion (I := I) (M := M) g r s p S
      constructor
      · set Big : ℝ := ∑ b ∈ Finset.range (2 * (p + 1)),
        ...
        have hlow : ∑ b ∈ Finset.range (2 * p),
          ...
          have h := sum_rawTensorConnLap_jets_bound (I := I) (M := M) g r s cL hcL_nn hcL (2 * p) S
      ...
      · set Big : ℝ := ∑ b ∈ Finset.range (2 * (p + 1) + 1),
        ...
        have hlow : ∑ b ∈ Finset.range (2 * p + 1),
            ‖iteratedCovGrad (I := I) g r s b ΔS‖ ≤ CL * Big := by
          have h := sum_rawTensorConnLap_jets_bound (I := I) (M := M) g r s cL hcL_nn hcL (2 * p + 1) S
