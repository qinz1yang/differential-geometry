...
private theorem ric_perm_norm_le
  ...
  have h := ric_l2_of_riemannianFiberNormSq (I := I) (M := M) (c := 1)
    g 0 (s + j) 0 (s + j)
    (iteratedCovGrad (I := I) g 0 s j
      (domDomCongrSection (I := I) g sigma Q))
...
theorem ricciPart_bds (g : SmoothRiemannianMetric I M) :
  ...
  classical
  obtain ⟨K, hK0, hslot1⟩ := slotInsertionCoefficient_covariantDerivative_bound (I := I) (M := M) g
  ...
  let Q : SmoothCcTensor g 0 4 :=
    fourTensorProductCoefficient (I := I) (M := M) g W WR
  ...
  have hW0raw := symmC0_riemannianFiberNormSq_le
  ...
  have hWR0 : riemannianFiberNormSq (I := I) (M := M) g 0 2 x
    ...
    simpa only [WR, F, d, W0] using
      slotInsertionCoefficient_norm_bound (I := I) (M := M) g gm P htie hdelta hdelta0
        hPbound 0 W x
  ...
  have hPhi0 : riemannianFiberNormSq (I := I) (M := M) g 2 4 x
    ...
    simpa only [Phi, d] using
      riemannianFiberNormSq_slotExtendIter_eq (I := I) (M := M) g 0 2 2 WR x
  ...
  have hQ0 : riemannianFiberNormSq (I := I) (M := M) g 0 4 x
    ...
    have hraw : riemannianFiberNormSq (I := I) (M := M) g 0 4 x
      ...
      simpa only [Q, fourTensorProductCoefficient, Phi, W0] using happ
    calc
    ...
    change covGrad (I := I) (M := M) g 0 4
      ...
      operatorFieldApply (I := I) (M := M) g 2 5
        ...
        operatorFieldApply (I := I) (M := M) g 3 5
          (slotExtend (I := I) (M := M) g 2 4 Phi)
          (covGrad (I := I) (M := M) g 0 2 W)
    exact covGrad_operatorFieldComposition_eq (I := I) (M := M) g 0 2 4 Phi W
  ...
  have hFlux0 : riemannianFiberNormSq (I := I) (M := M) g 0 4 x
      ((ricciCovariantDerivativeConnectionDifferenceFlux (I := I) (M := M) g gm W).toSection x) ≤
        C0 * delta ^ 2 * W0 := by
    have hsub := ricciFlux_riemannianFiberNormSq (I := I) (M := M) g gm W 0 x
  ...
  have hFlux1 : riemannianFiberNormSq (I := I) (M := M) g 0 5 x
      ((covGrad (I := I) (M := M) g 0 4
        (ricciCovariantDerivativeConnectionDifferenceFlux (I := I) (M := M) g gm W)).toSection x) ≤
        C1 * delta ^ 2 * W1 := by
    have hsub := ricciFlux_riemannianFiberNormSq (I := I) (M := M) g gm W 1 x
        ...
        riemannianFiberNormSq (I := I) (M := M) g 0 4 x
          ((ricciCovariantDerivativeConnectionDifferenceFlux (I := I) (M := M) g gm W).toSection x) := by
            simpa only [iteratedCovGrad_zero] using
              ricciPart_riemannianFiberNormSq (I := I) (M := M) g gm W 0 x
        ...
        riemannianFiberNormSq (I := I) (M := M) g 0 5 x
          ((covGrad (I := I) (M := M) g 0 4
            ...
            simpa only [iteratedCovGrad_succ, iteratedCovGrad_zero] using
              ricciPart_riemannianFiberNormSq (I := I) (M := M) g gm W 1 x
...
theorem ricciBase_l2 (g : SmoothRiemannianMetric I M) :
  ...
  apply ric_l2_of_riemannianFiberNormSq (I := I) (M := M) g 0 3 0 3
    (ricciConnectionDifferenceLoweredReindex (I := I) (M := M) g gm)
  ...
  have hbase :
      riemannianFiberNormSq (I := I) (M := M) g 0 3 x
          ((ricciConnectionDifferenceLoweredReindex (I := I) (M := M) g gm).toSection x) =
        riemannianFiberNormSq (I := I) (M := M) g 0 3 x
          ((metricLoweredConnectionDifferenceCoefficient (I := I) g gm).toSection x) := by
    simpa only [ricciConnectionDifferenceLoweredReindex, iteratedCovGrad_zero] using
      ric_perm_riemannianFiberNormSq (I := I) (M := M) g
        (metricLoweredConnectionDifferenceCoefficient (I := I) g gm) (finRotate 3) 0 x
  calc
    riemannianFiberNormSq (I := I) (M := M) g 0 3 x
        ((ricciConnectionDifferenceLoweredReindex (I := I) (M := M) g gm).toSection x) =
      riemannianFiberNormSq (I := I) (M := M) g 0 3 x
        ((metricLoweredConnectionDifferenceCoefficient (I := I) g gm).toSection x) := hbase
    _ = riemannianFiberNormSq (I := I) (M := M) g 1 2 x
      ...
      simpa only [iteratedCovGrad_zero, Nat.add_zero] using
        metricLoweredConnectionDifferenceCoefficient_fiber_norm_sq_eq (I := I) (M := M) g gm 0 x
    _ ≤ C ^ 2 *
        ‖((iteratedCovGrad (I := I) g 0 2 1 P).toSection x :
...
theorem ricciCovariantDerivativeConnectionDifference_path_pairing_le
  ...
  have hR0 : ‖R‖ ≤ Real.sqrt C0 * delta * ‖W‖ := by
    apply ric_l2_of_riemannianFiberNormSq (I := I) (M := M) g 0 4 0 2 R W
  ...
  have hR1 : ‖covGrad (I := I) (M := M) g 0 4 R‖ ≤
    ...
    apply ric_l2_of_riemannianFiberNormSq (I := I) (M := M) g 0 5 0 3
