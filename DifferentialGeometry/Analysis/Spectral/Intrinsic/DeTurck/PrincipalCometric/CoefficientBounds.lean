...
private def secondOrderJetProductGrid
...
private theorem exists_secondOrderJetProductGrid_integral_bound
    (hDim : Module.finrank ℝ E = 3)
    (g : SmoothRiemannianMetric I M) :
    ∃ C : ℝ, 0 ≤ C ∧ ∀ T : SmoothCcTensor g 0 2,
      ‖ccTensorToHs (I := I) (M := M) g 2 (2 : ℝ) T‖ ≤ 1 →
      MeasureTheory.Integrable (secondOrderJetProductGrid (I := I) (M := M) g T)
          (riemannianVolumeMeasure (I := I) (M := M) g) ∧
        (∫ x, secondOrderJetProductGrid (I := I) (M := M) g T x
            ∂(riemannianVolumeMeasure (I := I) (M := M) g)) ≤
  ...
  refine ⟨hgrid_int, ?_⟩
  rw [show (∫ x, secondOrderJetProductGrid (I := I) (M := M) g T x
        ∂(riemannianVolumeMeasure (I := I) (M := M) g)) =
      ∑ n ∈ Finset.range 3, ∑ e ∈ Finset.Nat.antidiagonalTuple n 2,
        ∫ x, ∏ m : Fin n,
          riemannianFiberNormSq (I := I) (M := M) g 0 (2 + e m) x
...
theorem exists_inverseMetricDifferenceSlotCoefficient_secondOrder_bound
    (hDim : Module.finrank ℝ E = 3)
    (g₀ : SmoothRiemannianMetric I M) :
    ∃ ρ C : ℝ, 0 < ρ ∧ 0 ≤ C ∧
  ...
  obtain ⟨Cjet, hCjet, hjet⟩ := hs2_low2 (I := I) (M := M) g₀ 2
  obtain ⟨Cgrid, hCgrid, hgrid⟩ :=
    exists_secondOrderJetProductGrid_integral_bound (I := I) (M := M) hDim g₀
  obtain ⟨Cgrad, hCgrad, hgrad⟩ :=
    riemannianFiberNormSq_covGrad_inverseMetricDifferenceSlotCoefficient_le
      (I := I) (M := M) g₀
  obtain ⟨Cinv, hCinv, hinv⟩ :=
    riemannianFiberNormSq_iteratedCovGrad_inverseMetricDifferenceSlotCoefficient_diagonalProductGrid_le
  ...
  have hgrad_sq := normSq_le_integral_of_pointwise_fiberNormSq_le_rs
    (I := I) (M := M) g₀ 2 3
    (iteratedCovGrad (I := I) g₀ 2 2 1
      (inverseMetricDifferenceSlotCoefficient (I := I) g₀ g₁)) _ hgrad_int hgrad_pt
  ...
  have hinv₂_pt : ∀ x : M,
      riemannianFiberNormSq (I := I) (M := M) g₀ 2 4 x
          ((iteratedCovGrad (I := I) g₀ 2 2 2
            (inverseMetricDifferenceSlotCoefficient (I := I) g₀ g₁)).toSection x) ≤
        Cinv 2 * secondOrderJetProductGrid (I := I) (M := M) g₀ T x := by
    intro x
    simpa only [secondOrderJetProductGrid] using
  ...
  have hinv₂ : ‖iteratedCovGrad (I := I) g₀ 2 2 2
    ...
    have hscaled : MeasureTheory.Integrable
        (fun x => Cinv 2 * secondOrderJetProductGrid (I := I) (M := M) g₀ T x)
        (riemannianVolumeMeasure (I := I) (M := M) g₀) :=
      hgridT.1.const_mul (Cinv 2)
    refine (normSq_le_integral_of_pointwise_fiberNormSq_le_rs
      (I := I) (M := M) g₀ 2 4
      (iteratedCovGrad (I := I) g₀ 2 2 2
        (inverseMetricDifferenceSlotCoefficient (I := I) g₀ g₁)) _ hscaled hinv₂_pt).trans ?_
    rw [MeasureTheory.integral_const_mul]
    calc
      Cinv 2 * (∫ x, secondOrderJetProductGrid (I := I) (M := M) g₀ T x
          ∂(riemannianVolumeMeasure (I := I) (M := M) g₀))
          ≤ Cinv 2 * (Cgrid * N) ^ 2 :=
...
theorem exists_deTurckPrincipalCometricCoefficient_secondOrder_bound
    (hDim : Module.finrank ℝ E = 3)
    (g₀ : SmoothRiemannianMetric I M) :
    ∃ ρ C : ℝ, 0 < ρ ∧ 0 ≤ C ∧
      ∀ (T : SmoothCcTensor g₀ 0 2) (g₁ : SmoothRiemannianMetric I M),
...
private theorem convexPerturbation_hsNorm_le
...
theorem exists_deTurckPrincipalCometricCoefficient_path_secondOrder_bound
    (hDim : Module.finrank ℝ E = 3)
    (g₀ : SmoothRiemannianMetric I M) :
    ∃ rho C : ℝ, 0 < rho ∧ 0 ≤ C ∧
          ...
          (∀ x : M,
            riemannianFiberNormSq (I := I) (M := M) g₀ 4 2 x
                ((deTurckPrincipalCometricCoeff (I := I) (M := M) g₀
                  (metricPerturbationPath (I := I) g₀ T T' hdelta hdelta' s)).toSection x) ≤
              (C * R) ^ 2) ∧
            (∑ j ∈ Finset.range 3,
              ‖iteratedCovGrad (I := I) g₀ 4 2 j
                (deTurckPrincipalCometricCoeff (I := I) (M := M) g₀
                  (metricPerturbationPath (I := I) g₀ T T' hdelta hdelta' s))‖ ^ 2) ≤
              (C * R) ^ 2 := by
  obtain ⟨rho, C, hrho, hC, hcoeff⟩ :=
    exists_deTurckPrincipalCometricCoefficient_secondOrder_bound
      (I := I) (M := M) hDim g₀
  ...
  have hP : ‖ccTensorToHs (I := I) (M := M) g₀ 2 (2 : ℝ) P‖ ≤ R := by
    simpa [P] using
      convexPerturbation_hsNorm_le (I := I) (M := M) g₀ T T' hs0 hs1 hT hT'
  have hs_mem : s ∈ metricPerturbationPathDomain (δ := delta) (δ' := delta') :=
    Icc_subset_metricPerturbationPathDomain hdelta_lt hdelta'_lt ⟨hs0, hs1⟩
  have htie : ∀ (y : M) (v w : TangentSpace I y),
      (metricPerturbationPath (I := I) g₀ T T' hdelta hdelta' s).inner y v w =
        g₀.inner y v w + ccTensorBilinSymm (I := I) g₀ P y v w := by
    intro y v w
    simpa [P] using metricPerturbationPath_inner_of_mem
      (I := I) g₀ T T' hdelta hdelta' hs_mem y v w
  obtain ⟨hpt, hjet⟩ := hcoeff P
    (metricPerturbationPath (I := I) g₀ T T' hdelta hdelta' s) (hP.trans hRrho) htie
  have hscale :
