...
def negativeHalfDeTurckRHS (g_bg : SmoothRiemannianMetric I M)
    (g : Real → SmoothRiemannianMetric I M) (t : Real) (x : M) :
    TangentSpace I x →L[Real] TangentSpace I x →L[Real] Real :=
  tensor02Bilin (I := I) (M := M) (negativeHalfDeTurckRHSField (I := I) (M := M) g_bg g t x)
...
omit [NeZero (Module.finrank ℝ E)]
  [CompactSpace M]
  [SigmaCompactSpace M] in
theorem negativeHalfDeTurckRHS_symm
    (g_bg : SmoothRiemannianMetric I M)
    (g : Real → SmoothRiemannianMetric I M) (t : Real) (x : M)
    (v w : TangentSpace I x) :
    negativeHalfDeTurckRHS (I := I) (M := M) g_bg g t x v w =
      negativeHalfDeTurckRHS (I := I) (M := M) g_bg g t x w v := by
...
omit [NeZero (Module.finrank ℝ E)]
  [CompactSpace M]
  [SigmaCompactSpace M] in
theorem deTurckRHSField_continuousOn
...
theorem exists_uniform_negativeHalfDeTurckRHS_bound
    (g_bg : SmoothRiemannianMetric I M) {T : Real}
    (g : Real → SmoothRiemannianMetric I M)
    (hJ : JointChartGramSmooth (I := I) T g) :
    ∃ B : Real, 0 ≤ B ∧ ∀ t ∈ Set.Icc 0 T,
      metricCauchySchwarzBound (I := I) (M := M) (g t)
        (negativeHalfDeTurckRHS (I := I) (M := M) g_bg g t) B := by
  classical
  ...
  obtain ⟨B, hB, hdiag⟩ := compactUnitTimeSlab_absBound
    (I := I) (M := M) g (negativeHalfDeTurckRHSField (I := I) (M := M) g_bg g)
  ...
  intro x u hu
  rw [negativeHalfDeTurckRHS, tensor02Bilin_apply, eval02_self]
...
omit [NeZero (Module.finrank ℝ E)]
  ...
  [T2Space M]
  [SigmaCompactSpace M] in
theorem normSq0S_family_continuousOn
    {s : Nat} {K : Set Real}
    (g : Real → SmoothRiemannianMetric I M)
...
def movingMetricReaction
  ...
  ricciReactionContractInBasis (I := I) (M := M) g x Q W basis
...
omit [NeZero (Module.finrank ℝ E)]
  ...
  [T2Space M]
  [SigmaCompactSpace M] in
theorem ricciReactionContractInBasis_deriv
    ...
    HasDerivAt
      ...
      (ricciReactionContractInBasis (I := I) (M := M) (g t) x Q W basis) t := by
  classical
  ...
  have hinvAll (r : Real) :
    ...
    simpa only [gInv, MetricInverseInBasis, MetricInverseInBasis] using
  ...
  simpa [ricciReactionContractInBasis, gInv, ric, hinnerZero] using hat
...
omit [NeZero (Module.finrank ℝ E)]
  ...
  [T2Space M]
  [SigmaCompactSpace M] in
theorem ricciReactionContractInBasis_eq
    {Idx₁ Idx₂ : Type*}
    [Fintype Idx₁]
    [Fintype Idx₂]
    {x : M} {t : Real}
    ...
    (hg : ∀ X Y : TangentSpace I x,
      HasDerivAt
        (fun r : Real => (g r).inner x X Y)
        ((-2 : Real) * Q (fun a : Fin 2 => if a = 0 then X else Y)) t) :
    ricciReactionContractInBasis (I := I) (M := M) (g t) x Q W basis₁ =
      ricciReactionContractInBasis (I := I) (M := M) (g t) x Q W basis₂ := by
  exact (ricciReactionContractInBasis_deriv (I := I) (M := M) g Q W basis₁ hg).unique
    (ricciReactionContractInBasis_deriv (I := I) (M := M) g Q W basis₂ hg)
...
omit [NeZero (Module.finrank ℝ E)]
  ...
  [T2Space M]
  [SigmaCompactSpace M] in
theorem movingMetricReaction_eq_in_basis
    {Idx : Type*} [Fintype Idx]
    {x : M} {t : Real}
    (g : Real → SmoothRiemannianMetric I M)
    (Q W : Tensor0SSpace 2 I x)
    ...
    movingMetricReaction (I := I) (M := M) (g t) x Q W =
      ricciReactionContractInBasis (I := I) (M := M) (g t) x Q W basis := by
  classical
  unfold movingMetricReaction
  exact ricciReactionContractInBasis_eq (I := I) (M := M) g Q W
...
theorem movingMetricReaction_eq_in_orthonormal_basis
    {Idx : Type*} [Fintype Idx] [DecidableEq Idx]
    {x : M} {t : Real}
    (g : Real → SmoothRiemannianMetric I M)
    (Q W : Tensor0SSpace 2 I x)
  ...
  classical
  rw [movingMetricReaction_eq_in_basis (I := I) (M := M) g Q W basis hg]
  unfold ricciReactionContractInBasis
  have hcanon := basisInvMetric_isInverse (I := I) (g t) x basis
...
theorem ricciReactionCoordinateArray_bound
    {Idx : Type*} [Fintype Idx]
...
theorem movingMetricReaction_bound
  ...
  have harray := ricciReactionCoordinateArray_bound q Wc hB hq
  have harray' :
  ...
  have hreact := movingMetricReaction_eq_in_orthonormal_basis
...
omit [CompactSpace M]
  ...
  [T2Space M]
  [SigmaCompactSpace M] in
theorem metricTrace_operator_norm_bound
...
omit [NeZero (Module.finrank ℝ E)]
  ...
  [T2Space M]
  [SigmaCompactSpace M] in
theorem traceTimeDerivMetric_bound
    {x : M} {t : Real}
    (g : Real → SmoothRiemannianMetric I M)
...
theorem moving_metric_reaction_and_volume_bound
  ...
  have htrace : |traceTimeDerivMetric (I := I) g t x| ≤
    ...
    rw [traceTimeDerivMetric_bound (I := I) (M := M) g Q hg, abs_mul]
    rw [show |(-2 : Real)| = 2 by norm_num]
    exact
      mul_le_mul_of_nonneg_left
        (metricTrace_operator_norm_bound (I := I) (M := M) (g t) Q hQ)
...
def movingMetricDifferenceEnergy
    (g₀ g₁ : Real → SmoothRiemannianMetric I M) (t : Real) : Real :=
  ∫ x, movingMetricDifferenceNormSq (I := I) (M := M) g₀ g₁ t x
    ∂(riemannianMeasureFamily (I := I) (M := M) g₀ t)

omit [NeZero (Module.finrank ℝ E)]
  [I.Boundaryless]
  [BoundarylessManifold I M] in
theorem movingMetricDifferenceEnergy_continuousOn
...
omit [NeZero (Module.finrank ℝ E)]
  ...
  [BoundarylessManifold I M]
  [SigmaCompactSpace M] in
theorem movingMetricDifferenceNormSq_smooth
    ...
    ContMDiffOn (𝓘(Real, Real).prod I) 𝓘(Real, Real) ∞
      (fun p : Real × M ↦
        movingMetricDifferenceNormSq (I := I) (M := M) g₀ g₁ p.1 p.2)
  ...
  have heq :
    ...
    have hinv : MetricInverseInBasis (I := I) (g₀ q.1) q.2
...
omit [NeZero (Module.finrank ℝ E)]
  ...
  [BoundarylessManifold I M]
  [SigmaCompactSpace M] in
theorem movingMetricDifferenceNormSq_time_deriv {x : M} {t : Real}
...
omit [NeZero (Module.finrank ℝ E)]
  [CompactSpace M]
  [SigmaCompactSpace M] in
theorem movingMetricDifferenceNormSq_ricciDeTurck_deriv {x : M} {t : Real}
    (g_bg : SmoothRiemannianMetric I M)
    (g₀ g₁ : Real → SmoothRiemannianMetric I M)
...
omit [NeZero (Module.finrank ℝ E)]
  [I.Boundaryless]
  [BoundarylessManifold I M] in
theorem movingMetricDifferenceEnergy_deriv
    ...
    HasDerivAt
      ...
      (∫ x,
        deriv (fun r : Real ↦
            movingMetricDifferenceNormSq (I := I) (M := M) g₀ g₁ r x) t +
          (1 / 2 : Real) *
            traceTimeDerivMetric (I := I) g₀ t x *
      ...
      (∫ x,
        movingMetricReaction (I := I) (M := M) (g₀ t) x
          ...
          (1 / 2 : Real) * traceTimeDerivMetric (I := I) g₀ t x *
            movingMetricDifferenceNormSq (I := I) (M := M) g₀ g₁ t x
        ∂(riemannianMeasureFamily (I := I) (M := M) g₀ t)) t := by
  have hbase := movingMetricDifferenceEnergy_deriv (I := I) (M := M)
    g₀ g₁ hU ht h₀ h₁
  convert hbase using 1
  apply integral_congr_ae
  filter_upwards with x
  rw [(movingMetricDifferenceNormSq_ricciDeTurck_deriv (I := I) (M := M) g_bg g₀ g₁
    (hPDE₀ x) (hPDE₁ x)).deriv]

def movingMetricDifferenceEnergyRate
    (g_bg : SmoothRiemannianMetric I M)
    (g₀ g₁ : Real → SmoothRiemannianMetric I M) (t : Real) : Real :=
  ∫ x,
    movingMetricReaction (I := I) (M := M) (g₀ t) x
...
omit [NeZero (Module.finrank ℝ E)] in
theorem movingMetricDifferenceEnergy_hasDerivAt
    (g_bg : SmoothRiemannianMetric I M)
    (g₀ g₁ : Real → SmoothRiemannianMetric I M) {U : Set Real} {t : Real}
    (hU : IsOpen U) (ht : t ∈ U)
    (h₀ : ∀ (x₀ : M) (i j : Fin (Module.finrank Real E)),
    ...
    HasDerivAt
      (movingMetricDifferenceEnergy (I := I) (M := M) g₀ g₁)
      (movingMetricDifferenceEnergyRate (I := I) (M := M) g_bg g₀ g₁ t) t := by
  simpa only [movingMetricDifferenceEnergyRate] using
    movingMetricDifferenceEnergy_ricciDeTurck_deriv (I := I) (M := M) g_bg g₀ g₁
      hU ht h₀ h₁ hPDE₀ hPDE₁

omit [NeZero (Module.finrank ℝ E)] in
theorem movingMetricDifferenceEnergy_eq_zero
    (g_bg : SmoothRiemannianMetric I M)
    ...
    (hbound : ∀ t ∈ Ioo (0 : Real) T,
      movingMetricDifferenceEnergyRate (I := I) (M := M) g_bg g₀ g₁ t ≤
        K * movingMetricDifferenceEnergy (I := I) (M := M) g₀ g₁ t) :
    ∀ t ∈ Icc (0 : Real) T,
      movingMetricDifferenceEnergy (I := I) (M := M) g₀ g₁ t = 0 := by
  have hcont : ContinuousOn
      (movingMetricDifferenceEnergy (I := I) (M := M) g₀ g₁) (Icc (0 : Real) T) :=
  ...
  have hnonneg : ∀ t ∈ Icc (0 : Real) T,
      0 ≤ movingMetricDifferenceEnergy (I := I) (M := M) g₀ g₁ t := by
    intro t _
