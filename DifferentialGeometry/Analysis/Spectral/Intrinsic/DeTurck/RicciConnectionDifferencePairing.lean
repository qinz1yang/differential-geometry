    ...
    unitModel (I := I) (M := M) g 4
      ...
      unitModel (I := I) (M := M) g 2 W x ![v 0, v 1] *
          unitModel (I := I) (M := M) g 2 W x
            ![metricComparisonEndomorphismField (I := I) (M := M) g gm x (v 2), v 3] -
        unitModel (I := I) (M := M) g 2 W x ![v 1, v 2] *
          unitModel (I := I) (M := M) g 2 W x
            ![metricComparisonEndomorphismField (I := I) (M := M) g gm x (v 0), v 3] := by
  let WR : SmoothCcTensor g 0 2 :=
    pairSlot2 (I := I) (M := M) g
      (metricComparisonEndomorphismField (I := I) (M := M) g gm) 0 W
  let P : SmoothCcTensor g 0 4 :=
...
private lemma ricLow_eval (g gm : SmoothRiemannianMetric I M)
    ...
    unitModel (I := I) (M := M) g 3
        (metricLoweredConnectionDifferenceCoefficient (I := I) g gm) x m =
...
theorem connRaise_eq (g gm : SmoothRiemannianMetric I M) :
  ...
  rw [show Tensor0SSpace.toModel D
      ...
      unitModel (I := I) (M := M) g 3
        (domDomCongrSection (I := I) g (finRotate 3)
          (metricLoweredConnectionDifferenceCoefficient (I := I) g gm)) x ![u, YZ 0, YZ 1] from by
    rw [hum]
    congr 1
    funext k
    fin_cases k <;> rfl]
...
theorem ricciConnectionDifferenceCovariantDerivativeTensor_pairing (g gm : SmoothRiemannianMetric I M) (x : M)
    ...
    g.inner x
        (rs13ContrVec (I := I) (M := M) x
          ...
          ![p, u, v]) r =
      unitModel (I := I) (M := M) g 4
        (ricciConnectionDifferenceCovariantDerivativeTensor (I := I) (M := M) g gm) x ![r, p, u, v] := by
  classical
...
theorem ricciCovariantDerivativeConnectionDifferenceKernel_operatorFieldApply (g gm : SmoothRiemannianMetric I M)
    ...
    unitModel (I := I) (M := M) g 4
        (operatorFieldApply (I := I) (M := M) g 2 4
          (ricciCovariantDerivativeConnectionDifferenceKernel (I := I) (M := M) g gm) W) x ![a, b, c, d] =
      -unitModel (I := I) (M := M) g 2 W x
          ![connectionDifferenceCovariantDerivativeContraction (I := I) (M := M) g gm x a b c, d] -
        unitModel (I := I) (M := M) g 2 W x
          ![c, connectionDifferenceCovariantDerivativeContraction (I := I) (M := M) g gm x a b d] := by
...
theorem connectionDifferenceCovariantDerivativeContraction_symm (g gm : SmoothRiemannianMetric I M) (x : M)
  ...
  rw [h1, h2]
  exact covDerivConnectionDifference_symm23 (I := I) (M := M) gm g X Y Z x
...
theorem ricciCovariantDerivativeConnectionDifferenceFluxReindex_unitModel (g gm : SmoothRiemannianMetric I M)
    ...
    unitModel (I := I) (M := M) g 4
      ...
      unitModel (I := I) (M := M) g 2 W x ![v 1, v 2] *
          unitModel (I := I) (M := M) g 2 W x
            ![metricComparisonEndomorphismField (I := I) (M := M) g gm x (v 3), v 0] -
        unitModel (I := I) (M := M) g 2 W x ![v 2, v 3] *
          unitModel (I := I) (M := M) g 2 W x
            ![metricComparisonEndomorphismField (I := I) (M := M) g gm x (v 1), v 0] := by
...
theorem ricciCovariantDerivativeConnectionDifference_raw_expansion (g gm : SmoothRiemannianMetric I M)
    ...
    unitModel (I := I) (M := M) g 2
      ...
      (1 / 2 : Real) *
        ∑ i : Fin (Module.finrank Real E),
          (-unitModel (I := I) (M := M) g 2 W x
              ![connectionDifferenceCovariantDerivativeContraction (I := I) (M := M) g gm x (L (e i)) (v 0) (v 1),
                e i] -
            unitModel (I := I) (M := M) g 2 W x
              ![v 1,
                connectionDifferenceCovariantDerivativeContraction (I := I) (M := M) g gm x (L (e i)) (v 0) (e i)] -
            unitModel (I := I) (M := M) g 2 W x
              ![connectionDifferenceCovariantDerivativeContraction (I := I) (M := M) g gm x (L (e i)) (v 1) (v 0),
                e i] -
            unitModel (I := I) (M := M) g 2 W x
              ![v 0,
                connectionDifferenceCovariantDerivativeContraction (I := I) (M := M) g gm x (L (e i)) (v 1) (e i)] +
            unitModel (I := I) (M := M) g 2 W x
              ![connectionDifferenceCovariantDerivativeContraction (I := I) (M := M) g gm x (L (e i)) (e i) (v 0),
                v 1] +
            unitModel (I := I) (M := M) g 2 W x
              ![v 0,
                connectionDifferenceCovariantDerivativeContraction (I := I) (M := M) g gm x (L (e i)) (e i) (v 1)] +
            unitModel (I := I) (M := M) g 2 W x
              ![connectionDifferenceCovariantDerivativeContraction (I := I) (M := M) g gm x (v 0) (v 1) (L (e i)),
                e i] +
            unitModel (I := I) (M := M) g 2 W x
              ![L (e i),
                connectionDifferenceCovariantDerivativeContraction (I := I) (M := M) g gm x (v 0) (v 1) (e i)]) := by
  dsimp only
...
private lemma ricW_D (g gm : SmoothRiemannianMetric I M)
    ...
    unitModel (I := I) (M := M) g 2 W x
        ![connectionDifferenceCovariantDerivativeContraction (I := I) (M := M) g gm x p u v, z] =
    ...
    calc
      ...
      _ = ∑ r : Fin (Module.finrank Real E),
        ...
        rw [ricW_symm (I := I) (M := M) g W hWsymm x
          (e r) (L (e p))]
        ring
  rw [ricciCovariantDerivativeConnectionDifference_reduced_expansion (I := I) (M := M) g gm W hWsymm x v]
  have hsplit :
      (∑ p : Fin (Module.finrank Real E),
          (-2 * unitModel (I := I) (M := M) g 2 W x
              ![connectionDifferenceCovariantDerivativeContraction (I := I) (M := M) g gm x
