...
private def oneTerm2 (g gm : SmoothRiemannianMetric I M) :
    SmoothCcTensor g 3 4 :=
  ccOperatorFieldComp (I := I) (M := M) g 3 4 4
...
private theorem oneKer_split
    ...
    linearizedRicciConnectionDifferenceOrder1KernelField (I := I) g gm =
      -(oneTerm0 (I := I) (M := M) g gm +
        oneTerm1 (I := I) (M := M) g gm +
        oneTerm2 (I := I) (M := M) g gm +
        oneTerm3 (I := I) (M := M) g gm +
        oneTerm4 (I := I) (M := M) g gm) := rfl

omit [SigmaCompactSpace M] in
private theorem oneTerm_riemannianFiberNormSq
    (g gm : SmoothRiemannianMetric I M)
    ...
    riemannianFiberNormSq (I := I) (M := M) g 3 (4 + q) x
        ((iteratedCovGrad (I := I) g 3 4 q
          (ccOperatorFieldComp (I := I) (M := M) g 3 4 4
            ...
            (connectionDifferenceContravariantInsertionField (I := I) g gm))).toSection x) =
      riemannianFiberNormSq (I := I) (M := M) g 3 (4 + q) x
        ((iteratedCovGrad (I := I) g 3 4 q
  ...
  have hy :
      (show Tensor0SBundle.Tensor0SSpace 3 I y →L[Real]
        ...
        (ccOperatorFieldComp (I := I) (M := M) g 3 4 4
          (permCoeff (I := I) (M := M) g sigma)
          (connectionDifferenceContravariantInsertionField (I := I) g gm)).toSection y) d =
...
private theorem oneFull_riemannianFiberNormSq
  ...
  rw [riemannianFiberNormSq_iteratedCovGrad_reindexCoeffGen_eq
    ...
    (ccOperatorFieldComp (I := I) (M := M) g 3 4 4
      (permCoeff (I := I) (M := M) g sigma)
      (connectionDifferenceContravariantInsertionField (I := I) g gm)) rho q x]
  exact oneTerm_riemannianFiberNormSq (I := I) (M := M) g gm sigma q x

omit [NeZero (Module.finrank ℝ E)] [CompactSpace M] [I.Boundaryless] [BoundarylessManifold I M]
  [T2Space M] [SigmaCompactSpace M] in
private theorem one_riemannianFiberNormSq_neg
...
theorem linearizedRicciConnectionDifferenceOrderOneKernel_fiberNormSq_le
    ...
    riemannianFiberNormSq (I := I) (M := M) g 3 4 x
        ((linearizedRicciConnectionDifferenceOrder1KernelField (I := I) g gm).toSection x) ≤
      46 * riemannianFiberNormSq (I := I) (M := M) g 3 4 x
        ((connectionDifferenceContravariantInsertionField (I := I) g gm).toSection x) := by
  let Q : Real := riemannianFiberNormSq (I := I) (M := M) g 3 4 x
    ((connectionDifferenceContravariantInsertionField (I := I) g gm).toSection x)
  let A0 := (oneTerm0 (I := I) (M := M) g gm).toSection x
  let A1 := (oneTerm1 (I := I) (M := M) g gm).toSection x
  let A2 := (oneTerm2 (I := I) (M := M) g gm).toSection x
  let A3 := (oneTerm3 (I := I) (M := M) g gm).toSection x
...
private theorem one_insert_riemannianFiberNormSq
    ...
    riemannianFiberNormSq (I := I) (M := M) g 3 4 x
      ...
      (Module.finrank Real E : Real) ^ 2 *
        riemannianFiberNormSq (I := I) (M := M) g 1 2 x
          ((connectionDifferenceSection (I := I) gm g).toSection x) := by
  let A := connectionDifferenceSection (I := I) gm g
  ...
  have houter := riemannianFiberNormSq_iteratedCovGrad_slotExtend_le
    (I := I) (M := M) g 2 3 B 0 x
  have hinner := riemannianFiberNormSq_iteratedCovGrad_slotExtend_le
    (I := I) (M := M) g 1 2 A 0 x
...
theorem linearizedRicciConnectionDifferenceOrderOneCoefficient_fiberNormSq_le (g : SmoothRiemannianMetric I M) :
    ∃ C : Real, 0 ≤ C ∧
  ...
  have hAi : Ai ≤ d ^ 2 * Ac := by
    simpa only [Ai, Ac, d] using
      one_insert_riemannianFiberNormSq (I := I) (M := M) g gm x
  have hKr : Kr ≤ 46 * Ai := by
    simpa only [Kr, Ai] using linearizedRicciConnectionDifferenceOrderOneKernel_fiberNormSq_le (I := I) (M := M) g gm x
  have hTr : Tr ≤ Ct 0 := by
...
theorem linearizedRicciConnectionDifferenceOrderOne_path_pairing_le (g : SmoothRiemannianMetric I M) :
  ...
  classical
  obtain ⟨C0, hC00, hcoeff⟩ := linearizedRicciConnectionDifferenceOrderOneCoefficient_fiberNormSq_le (I := I) (M := M) g
  let d : Real := Module.finrank Real E
  let C : Real := d * C0
  ...
  let F : SmoothCcTensor g 3 2 :=
    linearizedRicciConnectionDifferenceOrder1CoeffField (I := I) (M := M) g gm
  ...
  have hsSmall : s ∈ metricPerturbationPathDomain (δ := delta) (δ' := 0) :=
    Icc_subset_metricPerturbationPathDomain hdeltaLt (by norm_num) hs
  ...
  have hWpt := symmC0_riemannianFiberNormSq_le
