...
private theorem aa_inner_le
    ...
    riemannianFiberNormSq (I := I) (M := M) g 2 3 x
        ((connectionDifferenceContrInsertionInnerField (I := I) g gm).toSection x) ≤
  ...
  have h0 :
    ...
    rw [connectionDifferenceContrInsertionInnerField_eq_reindex_slotExtend
      (I := I) (M := M) g gm]
    exact aa_in_riemannianFiberNormSq (I := I) (M := M) g
  ...
  have h0 :
    ...
    exact aa_in_riemannianFiberNormSq (I := I) (M := M) g
      (slotExtend (I := I) (M := M) g 2 3 B)
        connectionDifferenceContrInsertionReindexPerm x
  have hout := riemannianFiberNormSq_iteratedCovGrad_slotExtend_le
    (I := I) (M := M) g 2 3 B 0 x
  have hin := riemannianFiberNormSq_iteratedCovGrad_slotExtend_le
    (I := I) (M := M) g 1 2 A 0 x
...
private theorem aa_mid_le
    ...
    riemannianFiberNormSq (I := I) (M := M) g 2 4 x
        ((ccOperatorFieldComp (I := I) (M := M) g 2 3 4
          (connectionDifferenceContravariantInsertionField (I := I) g gm) Q).toSection x) ≤
      (Module.finrank Real E : Real) ^ 3 *
        riemannianFiberNormSq (I := I) (M := M) g 1 2 x
            ((connectionDifferenceSection (I := I) gm g).toSection x) ^ 2 := by
...
private theorem aa_quad_le
  ...
  intro y
  rw [aa_out_riemannianFiberNormSq (I := I) (M := M) g rho
...
theorem ricciConnectionDifferenceQuadraticKernel_fiberNormSq_le
  ...
  let Q : Real := (Module.finrank Real E : Real) ^ 3 *
    riemannianFiberNormSq (I := I) (M := M) g 1 2 x
      ((connectionDifferenceSection (I := I) gm g).toSection x) ^ 2
    ...
    rw [show riemannianFiberNormSq (I := I) (M := M) g 2 4 x A0 =
        riemannianFiberNormSq (I := I) (M := M) g 2 4 x
          ((ccOperatorFieldComp (I := I) (M := M) g 2 3 4
            ...
            (ccOperatorFieldComp (I := I) (M := M) g 2 3 3
              (permCoeff (I := I) (M := M) g aaPerm102)
              (connectionDifferenceContrInsertionInnerField (I := I) g gm))).toSection x) by
          exact aa_out_riemannianFiberNormSq (I := I) (M := M) g aaPerm3201 _ x]
          ...
          ((ccOperatorFieldComp (I := I) (M := M) g 2 4 4
            ...
            (ccOperatorFieldComp (I := I) (M := M) g 2 3 4
              ...
              (ccOperatorFieldComp (I := I) (M := M) g 2 3 3
                (permCoeff (I := I) (M := M) g aaPerm102)
                (connectionDifferenceContrInsertionInnerField (I := I) g gm)))).toSection x) by
          exact aa_in_riemannianFiberNormSq (I := I) (M := M) g _
            innerContractionSwapPerm x]
    rw [aa_out_riemannianFiberNormSq (I := I) (M := M) g aaPerm2301]
    simpa only [Q] using aa_quad_le (I := I) (M := M) g gm aaPerm102 x
        ...
        riemannianFiberNormSq (I := I) (M := M) g 2 4 x
          ((ccOperatorFieldComp (I := I) (M := M) g 2 3 4
            ...
            (ccOperatorFieldComp (I := I) (M := M) g 2 3 3
              (permCoeff (I := I) (M := M) g aaPerm120)
              (connectionDifferenceContrInsertionInnerField (I := I) g gm))).toSection x) by
          exact aa_out_riemannianFiberNormSq (I := I) (M := M) g aaPerm3102 _ x]
  ...
  have h3 : riemannianFiberNormSq (I := I) (M := M) g 2 4 x A3 ≤ Q := by
    rw [show riemannianFiberNormSq (I := I) (M := M) g 2 4 x A3 =
        riemannianFiberNormSq (I := I) (M := M) g 2 4 x
          ...
          exact aa_in_riemannianFiberNormSq (I := I) (M := M) g _
            innerContractionSwapPerm x]
    rw [aa_out_riemannianFiberNormSq (I := I) (M := M) g aaPerm1302]
    ...
    rw [show riemannianFiberNormSq (I := I) (M := M) g 2 4 x A4 =
        riemannianFiberNormSq (I := I) (M := M) g 2 4 x
          ((ccOperatorFieldComp (I := I) (M := M) g 2 3 4
            (connectionDifferenceContravariantInsertionField (I := I) g gm)
            (connectionDifferenceContrInsertionInnerField (I := I) g gm)).toSection x) by
          exact aa_out_riemannianFiberNormSq (I := I) (M := M) g aaPerm1203 _ x]
    simpa only [Q] using hplain
  have h5 : riemannianFiberNormSq (I := I) (M := M) g 2 4 x A5 ≤ Q := by
    rw [show riemannianFiberNormSq (I := I) (M := M) g 2 4 x A5 =
        riemannianFiberNormSq (I := I) (M := M) g 2 4 x
          ...
          exact aa_in_riemannianFiberNormSq (I := I) (M := M) g _
            innerContractionSwapPerm x]
    rw [aa_out_riemannianFiberNormSq (I := I) (M := M) g aaPerm2103]
    simpa only [Q] using aa_quad_le (I := I) (M := M) g gm aaPerm120 x
...
theorem ricciConnectionDifferenceQuadraticCoefficient_fiberNormSq_le (g : SmoothRiemannianMetric I M) :
  ...
  have hKr : Kr ≤ 94 * d ^ 3 * Ac ^ 2 := by
    simpa only [Kr, Ac, d, mul_assoc] using
      ricciConnectionDifferenceQuadraticKernel_fiberNormSq_le (I := I) (M := M) g gm x
  have hTr : Tr ≤ Ct 0 := by
...
theorem ricciConnectionDifferenceQuadratic_path_pairing_le (g : SmoothRiemannianMetric I M)
    {eta : Real} (heta : 0 < eta) :
  ...
  classical
  obtain ⟨C0, hC00, hcoeff⟩ := ricciConnectionDifferenceQuadraticCoefficient_fiberNormSq_le (I := I) (M := M) g
  let d : Real := Module.finrank Real E
  let C : Real := d ^ 2 * C0
