...
def curvatureConnectionActionBlock (ga0 : Fin n → Fin n → Fin n → ℝ)
    (dga0 : Fin n → Fin n → Fin n → Fin n → ℝ) (l i j ρ : Fin n) : ℝ :=
  dga0 i l j ρ - dga0 j l i ρ +
    ∑ c, (ga0 i c ρ * ga0 l j c - ga0 j c ρ * ga0 l i c)


def curvatureContractionBlock (ig : Fin n → Fin n → ℝ) (ga0 : Fin n → Fin n → Fin n → ℝ)
    (dga0 : Fin n → Fin n → Fin n → Fin n → ℝ)
    (f : Fin n → Fin n → ℝ) (i j : Fin n) : ℝ :=
  -(∑ m, ∑ ml, ig m ml * ∑ ρ, curvatureConnectionActionBlock ga0 dga0 ml i j ρ * f ρ m)


theorem curvature_contraction_block_normal_form (ig : Fin n → Fin n → ℝ) (ga0 : Fin n → Fin n → Fin n → ℝ)
    (dga0 : Fin n → Fin n → Fin n → Fin n → ℝ)
    (f : Fin n → Fin n → ℝ)
  ...
  rw [curvatureContractionBlock]
  have hcomb : ∀ m ml : Fin n, ig m ml * ∑ ρ, curvatureConnectionActionBlock ga0 dga0 ml i j ρ * f ρ m =
    ...
    rw [curvatureConnectionActionBlock]
