...
theorem metric_derivative_block_normal_form {n : ℕ} (ig f : Fin n → Fin n → ℝ)
    (dig ga0 ga1 : Fin n → Fin n → Fin n → ℝ)
    (dga0 dga1 : Fin n → Fin n → Fin n → Fin n → ℝ)
    (hfs : ∀ a b, f a b = f b a) (i j : Fin n) :
    metricDerivativeBlock ig dig ga1 ga0 dga1 dga0 f i j =
  ...
  have h1 : metricDerivativeBlock ig dig ga1 ga0 dga1 dga0 f i j =
    ...
    simp only [metricDerivativeBlock]
