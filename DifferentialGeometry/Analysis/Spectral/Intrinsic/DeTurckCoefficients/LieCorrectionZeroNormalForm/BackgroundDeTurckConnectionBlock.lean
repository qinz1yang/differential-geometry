...
theorem background_deTurck_connection_block_normal_form {n : ℕ} (ig f : Fin n → Fin n → ℝ)
    (ga0 ga1 gbg : Fin n → Fin n → Fin n → ℝ)
    (hfs : ∀ a b, f a b = f b a)
    (hga0s : ∀ a b k, ga0 a b k = ga0 b a k)
    (hga1s : ∀ a b k, ga1 a b k = ga1 b a k) (i j : Fin n) :
    backgroundDeTurckConnectionBlock ig ga1 ga0 gbg f i j =
  ...
  have h1 : backgroundDeTurckConnectionBlock ig ga1 ga0 gbg f i j =
      -((∑ b0, ∑ b1, ∑ b2, ∑ b3,
    ...
    simp only [backgroundDeTurckConnectionBlock]
