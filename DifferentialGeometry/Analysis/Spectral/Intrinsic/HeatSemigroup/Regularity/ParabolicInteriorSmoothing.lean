...
omit [NeZero (Module.finrank ℝ E)] in
lemma solutionFieldMass_nonneg (hT : 0 ≤ T)
    (f : timeL2 (TensorHs (I := I) (M := M) g r s a) T) (c : ℝ)
    (i : TensorEigenIdx (I := I) (M := M) g r s) :
    0 ≤ solutionFieldMass (I := I) (M := M) hT f c i :=
  mul_nonneg (tensorSobolevWeight_nonneg (I := I) (M := M) i c) (sq_nonneg _)

omit [NeZero (Module.finrank ℝ E)] in
theorem solutionFieldMass_le_forcingMass (hT : 0 ≤ T)
    (f : timeL2 (TensorHs (I := I) (M := M) g r s a) T) (c : ℝ)
    (i : TensorEigenIdx (I := I) (M := M) g r s) :
    solutionFieldMass (I := I) (M := M) hT f (c + 2) i ≤
  ...
  have hbound :
    ...
    have h := weighted_solutionModeCoeff_le (I := I) (M := M) (a := a) hT f i
    ...
    have hsol_nn : 0 ≤ ‖solutionModeCoeff (I := I) (M := M) (a := a) hT f i‖ :=
      norm_nonneg _
    have hweight_split :
        tensorSobolevWeight (I := I) (M := M) i (c + 2) =
  ...
  simpa [solutionFieldMass, forcingMass] using hbound

omit [NeZero (Module.finrank ℝ E)] in
theorem solutionFieldMass_summable_of_forcingMass_summable (hT : 0 ≤ T)
    (f : timeL2 (TensorHs (I := I) (M := M) g r s a) T) (c : ℝ)
    (hf : Summable (forcingMass (I := I) (M := M) f c)) :
    Summable (solutionFieldMass (I := I) (M := M) hT f (c + 2)) := by
  refine Summable.of_nonneg_of_le
    (fun i => solutionFieldMass_nonneg (I := I) (M := M) hT f (c + 2) i)
    (fun i => solutionFieldMass_le_forcingMass (I := I) (M := M) hT f c i)
    (hf.mul_left ((1 + T) ^ 2))

omit [NeZero (Module.finrank ℝ E)] in
theorem solutionFieldMass_summable_succ (hT : 0 ≤ T)
    (f : timeL2 (TensorHs (I := I) (M := M) g r s a) T) (c : ℝ)
    (hcouple : ∀ d : ℝ,
      Summable (solutionFieldMass (I := I) (M := M) hT f (d + 1)) →
        Summable (forcingMass (I := I) (M := M) f d))
    (hc : Summable (solutionFieldMass (I := I) (M := M) hT f (c + 1))) :
    Summable (solutionFieldMass (I := I) (M := M) hT f (c + 2)) :=
  solutionFieldMass_summable_of_forcingMass_summable (I := I) (M := M) hT f c
    (hcouple c hc)

omit [NeZero (Module.finrank ℝ E)] in
theorem solutionFieldMass_summable_bootstrap (hT : 0 ≤ T)
    (f : timeL2 (TensorHs (I := I) (M := M) g r s a) T) {b : ℝ}
    (hcouple : ∀ d : ℝ,
      Summable (solutionFieldMass (I := I) (M := M) hT f (d + 1)) →
        Summable (forcingMass (I := I) (M := M) f d))
    (hbase : Summable (solutionFieldMass (I := I) (M := M) hT f b)) :
    ∀ n : ℕ, Summable (solutionFieldMass (I := I) (M := M) hT f (b + n)) := by
  intro n
  induction n with
  | zero => simpa using hbase
  | succ k ih =>
    have hstep := solutionFieldMass_summable_succ (I := I) (M := M) hT f (b + k - 1)
      hcouple
...
omit [NeZero (Module.finrank ℝ E)] in
theorem solutionFieldMass_summable_all (hT : 0 ≤ T)
    (f : timeL2 (TensorHs (I := I) (M := M) g r s a) T) {b : ℝ}
    (hcouple : ∀ d : ℝ,
      Summable (solutionFieldMass (I := I) (M := M) hT f (d + 1)) →
        Summable (forcingMass (I := I) (M := M) f d))
    (hbase : Summable (solutionFieldMass (I := I) (M := M) hT f b)) :
    ∀ σ : ℝ, Summable (solutionFieldMass (I := I) (M := M) hT f σ) := by
  intro σ
  obtain ⟨n, hn⟩ := exists_nat_ge (σ - b)
  have hσ_le : σ ≤ b + n := by linarith
  have hgain := solutionFieldMass_summable_bootstrap (I := I) (M := M) hT f
    hcouple hbase n
  refine Summable.of_nonneg_of_le
    (fun i => solutionFieldMass_nonneg (I := I) (M := M) hT f σ i)
    (fun i => ?_) hgain
  ...
  simpa only [solutionFieldMass] using
    mul_le_mul_of_nonneg_right hwle (sq_nonneg _)

def solutionFieldAtOrder (hT : 0 ≤ T)
...
omit [NeZero (Module.finrank ℝ E)] in
theorem solutionFieldAtOrder_timeModeCoeff (hT : 0 ≤ T)
    (f : timeL2 (TensorHs (I := I) (M := M) g r s a) T) (σ : ℝ)
    (hσ : Summable (solutionFieldMass (I := I) (M := M) hT f σ))
    (i : TensorEigenIdx (I := I) (M := M) g r s) :
    timeModeCoeff (I := I) (M := M)
        (solutionFieldAtOrder (I := I) (M := M) hT f σ) i =
      solutionModeCoeff (I := I) (M := M) (a := a) hT f i :=
  timeL2OfModes_timeModeCoeff (I := I) (M := M) (σ := σ)
    (fun i => solutionModeCoeff (I := I) (M := M) (a := a) hT f i) hσ i

omit [NeZero (Module.finrank ℝ E)] in
theorem solutionField_into_all_tensorHs_interior (hT : 0 ≤ T)
    (f : timeL2 (TensorHs (I := I) (M := M) g r s a) T) {b : ℝ}
    (hcouple : ∀ d : ℝ,
      Summable (solutionFieldMass (I := I) (M := M) hT f (d + 1)) →
        Summable (forcingMass (I := I) (M := M) f d))
    ...
    ∀ σ : ℝ,
      ∃ v : timeL2 (TensorHs (I := I) (M := M) g r s σ) T,
        ∀ i, timeModeCoeff (I := I) (M := M) v i =
          solutionModeCoeff (I := I) (M := M) (a := a) hT f i :=
  fun σ =>
    let hσ := solutionFieldMass_summable_all (I := I) (M := M) hT f hcouple hbase σ
    ⟨solutionFieldAtOrder (I := I) (M := M) hT f σ,
      solutionFieldAtOrder_timeModeCoeff (I := I) (M := M) hT f σ hσ⟩
