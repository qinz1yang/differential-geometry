...
private theorem metricPerturbationPath_chartDeTurckVFComp_jointContMDiffOn
  ...
  have hentryM : ContMDiffAt (𝓘(ℝ, ℝ × E)) 𝓘(ℝ) ∞
      (fun r : ℝ × E => PDE.DeTurck.DeTurckLinearization.chartDeTurckVFComp (I := I)
        (metricPerturbationPath (I := I) g₀ T T' hδ hδ' r.1) g_bg α k r.2) (p.2, extChartAt I α p.1) :=
    hentry.contMDiffAt
...
private theorem deTurckVFChartLocal_metricPerturbationPath_jointContMDiffOn
  ...
  have hcoeff : ∀ k : Fin (Module.finrank ℝ E),
    ...
    fun k => metricPerturbationPath_chartDeTurckVFComp_jointContMDiffOn (I := I) g₀ T T' hδ hδ' g_bg α k
  set e := trivializationAt E (TangentSpace I) α with he
  have hcoord_eq : ∀ q ∈ (chartAt H α).source ×ˢ metricPerturbationPathDomain (δ := δ) (δ' := δ'),
...
theorem deTurckVF_metricPerturbationPath_jointContMDiffOn
  ...
  intro p hp
  obtain ⟨_, hps⟩ := hp
  have hlocal := deTurckVFChartLocal_metricPerturbationPath_jointContMDiffOn (I := I) g₀ T T' hδ hδ' g_bg p.1
  have heqOn : ∀ q ∈ (chartAt H p.1).source ×ˢ metricPerturbationPathDomain (δ := δ) (δ' := δ'),
    ...
    rw [PDE.DeTurck.deTurckVF_apply_eq_chartDeTurckVFComp_sum (I := I)
      (metricPerturbationPath (I := I) g₀ T T' hδ hδ' q.2) g_bg p.1 hqgood]
  have hpmem : p ∈ (chartAt H p.1).source ×ˢ metricPerturbationPathDomain (δ := δ) (δ' := δ') :=
    ⟨mem_chart_source H p.1, hps⟩
  have hnhd : (chartAt H p.1).source ×ˢ metricPerturbationPathDomain (δ := δ) (δ' := δ') ∈
      nhdsWithin p ((Set.univ : Set M) ×ˢ metricPerturbationPathDomain (δ := δ) (δ' := δ')) := by
    refine mem_nhdsWithin.mpr ⟨(chartAt H p.1).source ×ˢ metricPerturbationPathDomain (δ := δ) (δ' := δ'),
      (chartAt H p.1).open_source.prod metricPerturbationPathDomain_isOpen, hpmem, fun q hq => hq.1⟩
...
private theorem connectionDifference_split_middle (gA gC gB : SmoothRiemannianMetric I M) (x : M)
    (u v : TangentSpace I x) :
    PDE.DeTurck.connectionDifference (I := I) gA gB x u v =
      PDE.DeTurck.connectionDifference (I := I) gA gC x u v +
        PDE.DeTurck.connectionDifference (I := I) gC gB x u v := by
  classical
  ...
  have h1 := PDE.DeTurck.connectionDifference_apply (I := I) gA gB (σ := fun y => σ y) hmd v
  have h2 := PDE.DeTurck.connectionDifference_apply (I := I) gA gC (σ := fun y => σ y) hmd v
  have h3 := PDE.DeTurck.connectionDifference_apply (I := I) gC gB (σ := fun y => σ y) hmd v
  rw [hσx] at h1 h2 h3
  rw [h1, h2, h3]
...
omit [SigmaCompactSpace M] in
private theorem metricConnectionDifferenceLoweredFib_split (gm gA gC gB : SmoothRiemannianMetric I M)
    (x : M) :
    metricConnectionDifferenceLoweredFib (I := I) gm gA gB x =
      metricConnectionDifferenceLoweredFib (I := I) gm gA gC x +
        metricConnectionDifferenceLoweredFib (I := I) gm gC gB x := by
...
private theorem metricConnectionDifferenceLowered_fixedPair_affine (g₀ : SmoothRiemannianMetric I M)
    ...
    (hs : s ∈ metricPerturbationPathDomain (δ := δ) (δ' := δ')) (x : M) :
    metricConnectionDifferenceLoweredFib (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ gB x =
      metricConnectionDifferenceLoweredFib (I := I) g₀ g₀ gB x
        + (1 - s) • ccBilinConnectionDifferenceLoweredFib (I := I) g₀ T' g₀ gB x
        + s • ccBilinConnectionDifferenceLoweredFib (I := I) g₀ T g₀ gB x := by
  apply Tensor0SBundle.Tensor0SSpace.toModel_injective
  beta_reduce
...
theorem metricConnectionDifferenceLowered_selfFam_jointContMDiffOn
    ...
    ContMDiffOn (I.prod 𝓘(ℝ, ℝ)) (I.prod 𝓘(ℝ, Tensor0SBundle.Tensor0SModel 3 ℝ E)) ∞
      (fun p : M × ℝ => TotalSpace.mk' (Tensor0SBundle.Tensor0SModel 3 ℝ E)
        (E := fun z : M => Tensor0SBundle.Tensor0SSpace 3 I z) p.1
        (metricConnectionDifferenceLoweredFib (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' p.2)
...
theorem metricConnectionDifferenceLowered_bgFam_jointContMDiffOn
    ...
    ContMDiffOn (I.prod 𝓘(ℝ, ℝ)) (I.prod 𝓘(ℝ, Tensor0SBundle.Tensor0SModel 3 ℝ E)) ∞
      ...
      ((Set.univ : Set M) ×ˢ metricPerturbationPathDomain (δ := δ) (δ' := δ')) := by
  classical
  have hself := metricConnectionDifferenceLowered_selfFam_jointContMDiffOn (I := I) g₀ T T' hδ hδ'
  have hfix0 : ContMDiffOn (I.prod 𝓘(ℝ, ℝ)) (I.prod 𝓘(ℝ, Tensor0SBundle.Tensor0SModel 3 ℝ E)) ∞
      (fun p : M × ℝ => TotalSpace.mk' (Tensor0SBundle.Tensor0SModel 3 ℝ E)
        (E := fun z : M => Tensor0SBundle.Tensor0SSpace 3 I z) p.1
        (metricConnectionDifferenceLoweredFib (I := I) g₀ g₀ g_bg p.1))
      ((Set.univ : Set M) ×ˢ metricPerturbationPathDomain (δ := δ) (δ' := δ')) :=
    (metricConnectionDifferenceLoweredFib_contMDiff (I := I) g₀ g₀ g_bg).comp_contMDiffOn contMDiffOn_fst
  have hfixT' : ContMDiffOn (I.prod 𝓘(ℝ, ℝ)) (I.prod 𝓘(ℝ, Tensor0SBundle.Tensor0SModel 3 ℝ E)) ∞
      (fun p : M × ℝ => TotalSpace.mk' (Tensor0SBundle.Tensor0SModel 3 ℝ E)
        (E := fun z : M => Tensor0SBundle.Tensor0SSpace 3 I z) p.1
        (ccBilinConnectionDifferenceLoweredFib (I := I) g₀ T' g₀ g_bg p.1))
      ((Set.univ : Set M) ×ˢ metricPerturbationPathDomain (δ := δ) (δ' := δ')) :=
    (ccBilinConnectionDifferenceLoweredFib_contMDiff (I := I) g₀ T' g₀ g_bg).comp_contMDiffOn contMDiffOn_fst
  have hfixT : ContMDiffOn (I.prod 𝓘(ℝ, ℝ)) (I.prod 𝓘(ℝ, Tensor0SBundle.Tensor0SModel 3 ℝ E)) ∞
      (fun p : M × ℝ => TotalSpace.mk' (Tensor0SBundle.Tensor0SModel 3 ℝ E)
        (E := fun z : M => Tensor0SBundle.Tensor0SSpace 3 I z) p.1
        (ccBilinConnectionDifferenceLoweredFib (I := I) g₀ T g₀ g_bg p.1))
  ...
  have h1 := jointTotalSpace0S_smulFun_local (I := I) (d := 3)
    (S := metricPerturbationPathDomain (δ := δ) (δ' := δ')) hone
    (fun p : M × ℝ => ccBilinConnectionDifferenceLoweredFib (I := I) g₀ T' g₀ g_bg p.1) hfixT'
  have h2 := jointTotalSpace0S_smulFun_local (I := I) (d := 3)
    (S := metricPerturbationPathDomain (δ := δ) (δ' := δ')) hids
    (fun p : M × ℝ => ccBilinConnectionDifferenceLoweredFib (I := I) g₀ T g₀ g_bg p.1) hfixT
  have hsum1 := jointTotalSpace0S_add_local (I := I) (d := 3)
    (S := metricPerturbationPathDomain (δ := δ) (δ' := δ'))
    (fun p : M × ℝ => metricConnectionDifferenceLoweredFib (I := I) g₀ g₀ g_bg p.1)
    (fun p : M × ℝ => (1 - p.2) • ccBilinConnectionDifferenceLoweredFib (I := I) g₀ T' g₀ g_bg p.1)
    hfix0 h1
  have hsum2 := jointTotalSpace0S_add_local (I := I) (d := 3)
    (S := metricPerturbationPathDomain (δ := δ) (δ' := δ'))
    (fun p : M × ℝ => metricConnectionDifferenceLoweredFib (I := I) g₀ g₀ g_bg p.1 +
      (1 - p.2) • ccBilinConnectionDifferenceLoweredFib (I := I) g₀ T' g₀ g_bg p.1)
    (fun p : M × ℝ => p.2 • ccBilinConnectionDifferenceLoweredFib (I := I) g₀ T g₀ g_bg p.1)
    hsum1 h2
  have hsum3 := jointTotalSpace0S_add_local (I := I) (d := 3)
    ...
    (fun p : M × ℝ => metricConnectionDifferenceLoweredFib (I := I) g₀ g₀ g_bg p.1 +
        (1 - p.2) • ccBilinConnectionDifferenceLoweredFib (I := I) g₀ T' g₀ g_bg p.1 +
      p.2 • ccBilinConnectionDifferenceLoweredFib (I := I) g₀ T g₀ g_bg p.1)
    hself hsum2
  refine hsum3.congr (fun p hp => ?_)
  refine congrArg (fun t => TotalSpace.mk' (Tensor0SBundle.Tensor0SModel 3 ℝ E)
    (E := fun z : M => Tensor0SBundle.Tensor0SSpace 3 I z) p.1 t) ?_
  rw [metricConnectionDifferenceLoweredFib_split (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' p.2)
    (metricPerturbationPath (I := I) g₀ T T' hδ hδ' p.2) g₀ g_bg p.1]
  congr 1
  exact metricConnectionDifferenceLowered_fixedPair_affine (I := I) g₀ T T' hδ hδ' g_bg hp.2 p.1
