...
def metricComparisonEndomorphismField (g₀ g₁ : SmoothRiemannianMetric I M) :
    ContMDiffSection I (E →L[ℝ] E) ∞
      (fun x : M => TangentSpace I x →L[ℝ] TangentSpace I x) where
  toFun := fun x : M => metricComparisonEndomorphism (I := I) g₀ g₁ x
  contMDiff_toFun := by
    apply contMDiff_clm_section_of_pointwise (I := I) (M := M)
      (F₁ := E) (V₁ := fun z : M => TangentSpace I z)
      (F₂ := E) (V₂ := fun z : M => TangentSpace I z)
      (φ := fun x => metricComparisonEndomorphism (I := I) g₀ g₁ x)
    intro Y
    ...
    rw [metricComparisonEndomorphism_apply, inverseMetricSharpFib_g0FlatCLM_eq_metricSharp]
...
private lemma riemannianFiberNormSq_neg_fib (g : SmoothRiemannianMetric I M) (r s : ℕ) (x : M)
...
omit [NeZero (Module.finrank ℝ E)] [CompactSpace M] [I.Boundaryless] [BoundarylessManifold I M]
    [T2Space M] [SigmaCompactSpace M] in
private lemma riemannianFiberNormSq_smul_fib (g : SmoothRiemannianMetric I M) (r s : ℕ) (x : M) (c : ℝ)
    (v : TensorRSSpace r s I x) :
...
private lemma operatorFieldApplicationLeibnizPsi_diag_eq (g : SmoothRiemannianMetric I M) (b c : ℕ)
    (Φ : SmoothCcTensor g b c) (i : ℕ) :
    operatorFieldApplicationLeibnizPsi (I := I) (M := M) g b c Φ i i =
      slotExtendIter (I := I) (M := M) g b c i Φ := by
  induction i with
  | zero => rfl
  | succ i ih =>
      have hss : operatorFieldApplicationLeibnizPsi (I := I) (M := M) g b c Φ (i + 1) (i + 1) =
          (if i + 1 < i + 1 then
              covGrad (I := I) (M := M) g (b + (i + 1)) (c + i)
                (operatorFieldApplicationLeibnizPsi (I := I) (M := M) g b c Φ i (i + 1))
            else 0) +
      ...
      rw [show castCcTensorSourceRank g (c + (i + 1)) (by omega : (b + i) + 1 = b + (i + 1))
            (castCcTensorRank g ((b + i) + 1) (by omega : (c + i) + 1 = c + (i + 1))
              (slotExtend (I := I) (M := M) g (b + i) (c + i)
                (operatorFieldApplicationLeibnizPsi (I := I) (M := M) g b c Φ i i))) =
          slotExtend (I := I) (M := M) g (b + i) (c + i)
            (operatorFieldApplicationLeibnizPsi (I := I) (M := M) g b c Φ i i) from by
...
private lemma riemannianFiberNormSq_iteratedCovGrad_exp_congr (g : SmoothRiemannianMetric I M) (r s : ℕ)
...
private lemma riemannianFiberNormSq_iteratedCovGrad_operatorFieldApplicationLeibnizPsi_le
  ...
  | zero =>
    ...
    have happ : operatorFieldApplicationLeibnizPsi (I := I) (M := M) g b c Φ 0 0 = Φ := rfl
    rw [happ]
  ...
  | succ i ih =>
    ...
    | 0, hkd =>
      ...
      have h0 : operatorFieldApplicationLeibnizPsi (I := I) (M := M) g b c Φ (i + 1) 0 =
          covGrad (I := I) (M := M) g (b + 0) (c + i)
            (operatorFieldApplicationLeibnizPsi (I := I) (M := M) g b c Φ i 0) := rfl
      rw [h0]
      have hcomm : riemannianFiberNormSq (I := I) (M := M) g (b + 0) ((c + (i + 1)) + p) x
            ((iteratedCovGrad (I := I) g (b + 0) (c + (i + 1)) p
              (covGrad (I := I) (M := M) g (b + 0) (c + i)
                (operatorFieldApplicationLeibnizPsi (I := I) (M := M) g b c Φ i 0))).toSection x) =
          riemannianFiberNormSq (I := I) (M := M) g (b + 0) ((c + i) + (p + 1)) x
            ((iteratedCovGrad (I := I) g (b + 0) (c + i) (p + 1)
              (operatorFieldApplicationLeibnizPsi (I := I) (M := M) g b c Φ i 0)).toSection x) :=
        riemannianFiberNormSq_iteratedCovGrad_covGrad_comm_rs (I := I) (M := M) g (b + 0) (c + i) p
          (operatorFieldApplicationLeibnizPsi (I := I) (M := M) g b c Φ i 0) x
      rw [hcomm]
      have hih := ih 0 (p + 1) i (by omega)
      rw [riemannianFiberNormSq_iteratedCovGrad_exp_congr (I := I) (M := M) g b c Φ x
    ...
    | (j + 1), hkd =>
      ...
      have hrec : operatorFieldApplicationLeibnizPsi (I := I) (M := M) g b c Φ (i + 1) (j + 1) =
          (if j + 1 < i + 1 then
        ...
        rw [show castCcTensorSourceRank g (c + (i + 1)) (by omega : (b + j) + 1 = b + (j + 1))
            ...
            slotExtend (I := I) (M := M) g (b + j) (c + i)
              (operatorFieldApplicationLeibnizPsi (I := I) (M := M) g b c Φ i j) from by
          rw [castCcTensorRank, castCcTensorSourceRank]]
      rw [hrec]
      by_cases hji : j < i
      · rw [if_pos (by omega : j + 1 < i + 1)]
        ...
        rw [show ((iteratedCovGrad (I := I) g (b + (j + 1)) (c + (i + 1)) p
            ...
            iteratedCovGrad (I := I) g (b + (j + 1)) (c + (i + 1)) p
              (slotExtend (I := I) (M := M) g (b + j) (c + i)
                (operatorFieldApplicationLeibnizPsi (I := I) (M := M) g b c Φ i j))).toSection x) =
            (iteratedCovGrad (I := I) g (b + (j + 1)) (c + (i + 1)) p
              (covGrad (I := I) (M := M) g (b + (j + 1)) (c + i)
                (operatorFieldApplicationLeibnizPsi (I := I) (M := M) g b c Φ i (j + 1)))).toSection x +
              (iteratedCovGrad (I := I) g (b + (j + 1)) (c + (i + 1)) p
                (slotExtend (I := I) (M := M) g (b + j) (c + i)
                  (operatorFieldApplicationLeibnizPsi (I := I) (M := M) g b c Φ i j))).toSection x from by
        ...
        have hcommA : riemannianFiberNormSq (I := I) (M := M) g (b + (j + 1)) ((c + (i + 1)) + p) x
            ...
            riemannianFiberNormSq (I := I) (M := M) g (b + (j + 1)) ((c + i) + (p + 1)) x
              ((iteratedCovGrad (I := I) g (b + (j + 1)) (c + i) (p + 1)
                (operatorFieldApplicationLeibnizPsi (I := I) (M := M) g b c Φ i (j + 1))).toSection x) :=
          riemannianFiberNormSq_iteratedCovGrad_covGrad_comm_rs (I := I) (M := M) g (b + (j + 1)) (c + i) p
        ...
        have hsl : riemannianFiberNormSq (I := I) (M := M) g (b + (j + 1)) ((c + (i + 1)) + p) x
          ...
          riemannianFiberNormSq_iteratedCovGrad_slotExtend_le (I := I) (M := M) g (b + j) (c + i)
            (operatorFieldApplicationLeibnizPsi (I := I) (M := M) g b c Φ i j) p x
...
private lemma fullRaisedEndo_comp_recovery (g₀ g₁ : SmoothRiemannianMetric I M) (x : M) :
    (metricComparisonEndomorphism (I := I) g₀ g₁ x).comp (metricComparisonEndomorphism (I := I) g₁ g₀ x) =
      ContinuousLinearMap.id ℝ (TangentSpace I x) := by
  apply ContinuousLinearMap.ext
  intro v
...
private lemma operatorFieldComposition_recovery_eq (g₀ g₁ : SmoothRiemannianMetric I M) :
  ...
  simp only [slotInsertEndoCc_toSection, metricComparisonEndomorphismField_apply]
  rw [slotInsertEndoFib_comp_eq, fullRaisedEndo_comp_recovery, metricComparisonEndomorphism_self]
...
private lemma slotInsertEndoCc_raisedEndoField_comp_swap_eq_id (g₀ g₁ : SmoothRiemannianMetric I M)
    ...
    (show Tensor0SSpace 1 I x →L[ℝ] Tensor0SSpace 1 I x from
        (endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
          (metricComparisonEndomorphismField (I := I) (M := M) g₀ g₁)).toSection x).comp
      (show Tensor0SSpace 1 I x →L[ℝ] Tensor0SSpace 1 I x from
        (endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
...
private lemma master_isolation' (g₀ g₁ : SmoothRiemannianMetric I M) (w : ℕ)
    ...
    ccOperatorFieldComp (I := I) (M := M) g₀ 1 (1 + w) (1 + w)
        ...
        (ccOperatorFieldComp (I := I) (M := M) g₀ 1 (1 + w) (1 + w)
          (slotExtendIter (I := I) (M := M) g₀ 1 1 w
            (endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
              (metricComparisonEndomorphismField (I := I) (M := M) g₁ g₀))) X) = X := by
  apply SmoothCcTensor.ext
  apply ContMDiffSection.ext
  intro x
...
private lemma unitModel_eq_ccTensorBilin_loc (g₀ : SmoothRiemannianMetric I M)
    ...
    unitModel (I := I) (M := M) g₀ 2 S b
        ![tangentSpaceModelContinuousLinearEquiv (I := I) b u,
          tangentSpaceModelContinuousLinearEquiv (I := I) b w] =
      smoothCcTensorBilinForm (I := I) g₀ S b u w := by
...
private lemma cotangentToDual_cometricRaiseSlot0_eq (g₀ : SmoothRiemannianMetric I M)
  ...
  rw [show (Tensor0SBundle.interiorProduct (𝕜 := ℝ) (I := I) (0 + 1) x
      ...
      Tensor0SSpace.toModel
        ...
        (fun _ : Fin 1 => tangentSpaceModelContinuousLinearEquiv (I := I) x w) from
...
private lemma slotInsertEndoCc_gInvDiffRaised_eq_cometricRaise
  ...
  rw [cotangentToDual_slotInsertEndoFib (I := I) (M := M) x
    (metricComparisonDifferenceEndomorphism (I := I) g₁ g₀ x) om w]
  rw [← cotangentToDualLinear_apply, ← inverseMetricSharpFib_inner]
  rw [g₀.symm x (inverseMetricSharpFib (I := I) g₀ x om)
    (metricComparisonDifferenceEndomorphism (I := I) g₁ g₀ x w)]
  rw [inner_g1_metricComparisonDifferenceEndomorphism (I := I) g₁ g₀ x w
    (inverseMetricSharpFib (I := I) g₀ x om)]
...
private lemma slotInsert_recovery_decomp
    ...
    endoSlotZeroCcTensor (I := I) (M := M) g₀ 0 (metricComparisonEndomorphismField (I := I) (M := M) g₁ g₀) =
      endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
          (metricComparisonEndomorphismField (I := I) (M := M) g₀ g₀) +
        cometricRaiseSlot0Field (I := I) (M := M) g₀ 0 (ccTensor02Symm (I := I) (M := M) g₀ T) := by
  rw [metricComparisonEndomorphismField_recovery_decomp (I := I) (M := M) g₀ g₁]
...
private lemma riemannianFiberNormSq_iteratedCovGrad_symmS_le_pointwise
  ...
  calc riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + k) x
    ...
    _ = (1 / 2 : ℝ) * riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + k) x (A.toSection x) +
        ...
        rw [riemannianFiberNormSq_smul_fib, riemannianFiberNormSq_smul_fib]; ring
    _ = (1 / 2 : ℝ) * riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + k) x (A.toSection x) +
...
omit [NeZero (Module.finrank ℝ E)] [SigmaCompactSpace M] in
private lemma riemannianFiberNormSq_iteratedCovGrad_SI_M_succ_le
    (g₀ g₁ : SmoothRiemannianMetric I M) (T : SmoothCcTensor g₀ 0 2)
    (htie : ∀ (y : M) (v w : TangentSpace I y),
      g₁.inner y v w = g₀.inner y v w + ccTensorBilinSymm (I := I) g₀ T y v w)
    (j : ℕ) (x : M) :
...
omit [SigmaCompactSpace M] in
private lemma riemannianFiberNormSq_operatorFieldApplicationLeibnizPsi_recovery_le
    (g₀ g₁ : SmoothRiemannianMetric I M) (T : SmoothCcTensor g₀ 0 2)
    (htie : ∀ (y : M) (v w : TangentSpace I y),
      g₁.inner y v w = g₀.inner y v w + ccTensorBilinSymm (I := I) g₀ T y v w)
    (m : ℕ) (k : ℕ) (hk : k ≤ m) (x : M) :
    riemannianFiberNormSq (I := I) (M := M) g₀ (1 + k) (1 + (m + 1)) x
        ((operatorFieldApplicationLeibnizPsi (I := I) (M := M) g₀ 1 1
          (endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
            (metricComparisonEndomorphismField (I := I) (M := M) g₁ g₀)) (m + 1) k).toSection x) ≤
      4 ^ (m + 1) * (Module.finrank ℝ E : ℝ) ^ k *
        riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + ((m - k) + 1)) x
          ((iteratedCovGrad (I := I) g₀ 0 2 ((m - k) + 1) T).toSection x) := by
  have hkey := riemannianFiberNormSq_iteratedCovGrad_operatorFieldApplicationLeibnizPsi_le (I := I) (M := M) g₀ 1 1
    (endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
      (metricComparisonEndomorphismField (I := I) (M := M) g₁ g₀)) x (m + 1) k 0 ((m + 1) - k) (by omega)
  refine le_trans (le_of_eq ?_) (le_trans hkey ?_)
  · rfl
  · refine mul_le_mul_of_nonneg_left ?_ (by positivity)
    rw [riemannianFiberNormSq_iteratedCovGrad_exp_congr (I := I) (M := M) g₀ 1 1
      (endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
        (metricComparisonEndomorphismField (I := I) (M := M) g₁ g₀)) x
      (show 0 + ((m + 1) - k) = (m - k) + 1 by omega)]
    exact riemannianFiberNormSq_iteratedCovGrad_SI_M_succ_le (I := I) (M := M) g₀ g₁ T htie (m - k) x

omit [SigmaCompactSpace M] in
private lemma riemannianFiberNormSq_iteratedCovGrad_SI_F_leibniz_grid_bound
    (g₀ g₁ : SmoothRiemannianMetric I M) (T : SmoothCcTensor g₀ 0 2)
    (htie : ∀ (y : M) (v w : TangentSpace I y),
      g₁.inner y v w = g₀.inner y v w + ccTensorBilinSymm (I := I) g₀ T y v w)
    (m : ℕ) (x : M) :
  ...
  have hleib := iteratedCovGrad_operatorFieldComposition_eq (I := I) (M := M) g₀ 1 1 1
    (endoSlotZeroCcTensor (I := I) (M := M) g₀ 0 (metricComparisonEndomorphismField (I := I) (M := M) g₁ g₀))
    (endoSlotZeroCcTensor (I := I) (M := M) g₀ 0 (metricComparisonEndomorphismField (I := I) (M := M) g₀ g₁))
      (m + 1)
  rw [operatorFieldComposition_recovery_eq (I := I) (M := M) g₀ g₁,
    iteratedCovGrad_slotInsert_self_eq_zero (I := I) (M := M) g₀ m,
    Finset.sum_range_succ,
    operatorFieldApplicationLeibnizPsi_diag_eq (I := I) (M := M) g₀ 1 1
      (endoSlotZeroCcTensor (I := I) (M := M) g₀ 0 (metricComparisonEndomorphismField (I := I) (M := M) g₁ g₀))
      (m + 1)] at hleib
  ...
  have hdiag : ccOperatorFieldComp (I := I) (M := M) g₀ 1 (1 + (m + 1)) (1 + (m + 1))
        ...
        (iteratedCovGrad (I := I) g₀ 1 1 (m + 1)
          (endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
            (metricComparisonEndomorphismField (I := I) (M := M) g₀ g₁))) = -sigSum :=
  ...
  have hsigToSection : (sigSum.toSection x : TensorRSSpace 1 (1 + (m + 1)) I x) =
      ∑ k ∈ Finset.range (m + 1),
        ((ccOperatorFieldComp (I := I) (M := M) g₀ 1 (1 + k) (1 + (m + 1))
          (operatorFieldApplicationLeibnizPsi (I := I) (M := M) g₀ 1 1
            (endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
              (metricComparisonEndomorphismField (I := I) (M := M) g₁ g₀)) (m + 1) k)
          (iteratedCovGrad (I := I) g₀ 1 1 k
            (endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
              (metricComparisonEndomorphismField (I := I) (M := M) g₀ g₁)))).toSection x) := by
  ...
  have hSigmaBound : riemannianFiberNormSq (I := I) (M := M) g₀ 1 (1 + (m + 1)) x
    ...
    refine le_trans (riemannianFiberNormSq_sum_le_card_mul (I := I) (M := M) g₀ 1 (1 + (m + 1)) x
      (Finset.range (m + 1)) (fun k =>
        ...
        (iteratedCovGrad (I := I) g₀ 1 1 k
          (endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
            (metricComparisonEndomorphismField (I := I) (M := M) g₀ g₁)))).toSection x)) ?_
    ...
    apply mul_le_mul_of_nonneg_right
      (riemannianFiberNormSq_operatorFieldApplicationLeibnizPsi_recovery_le (I := I) (M := M) g₀ g₁ T htie m k hkm x)
      (riemannianFiberNormSq_nonneg (I := I) (M := M) g₀ 1 (1 + k) x _)
  rw [← hmi, operatorFieldComposition_toSection]
  refine le_trans (riemannianFiberNormSq_compRS_le_mul (I := I) (M := M) g₀ 1 (1 + (m + 1))
    (1 + (m + 1)) x _ _) ?_
  rw [riemannianFiberNormSq_slotExtendIter_eq (I := I) (M := M) g₀ 1 1 (m + 1)
    (endoSlotZeroCcTensor (I := I) (M := M) g₀ 0 (metricComparisonEndomorphismField (I := I) (M := M) g₀ g₁)) x]
  rw [show ((-sigSum).toSection x : TensorRSSpace 1 (1 + (m + 1)) I x) =
      -(sigSum.toSection x) from by rw [SmoothCcTensor.toSection_neg]; rfl]
  rw [riemannianFiberNormSq_neg_fib]
...
private lemma fiberNormSqComponent_slotInsertFULL
    ...
    fiberNormSqComponent (I := I) (M := M) g₀ x 1 1
      ...
      g₀.inner x (e (K 0)) (metricComparisonEndomorphism (I := I) g₀ g₁ x (e (J 0))) := by
...
private lemma riemannianFiberNormSq_slotInsertFULL_zero_le
  ...
  rw [riemannianFiberNormSq_eq_sum_componentSq_of_basis (I := I) (M := M) g₀ 1 1 x
    ((endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
      (metricComparisonEndomorphismField (I := I) (M := M) g₀ g₁)).toSection x) e bse hnE hbse horth]
  ...
  have heach : ∀ (K J : Fin 1 → Fin n),
      (fiberNormSqComponent (I := I) (M := M) g₀ x 1 1
        ((endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
          (metricComparisonEndomorphismField (I := I) (M := M) g₀ g₁)).toSection x) n e K J) ^ 2 ≤
        (1 / (1 - δ₀)) ^ 2 := by
    intro K J
    rw [fiberNormSqComponent_slotInsertFULL (I := I) (M := M) g₀ g₁ x e K J]
    set u : TangentSpace I x := metricComparisonEndomorphism (I := I) g₀ g₁ x (e (J 0)) with hu_def
    ...
    have hsfib := sqrt_inner_metricComparisonEndomorphism_le (I := I) g₀ g₁
...
private lemma fiberNormSqComponent_slotInsertE
    ...
    fiberNormSqComponent (I := I) (M := M) g₀ x 1 1
      ...
      g₀.inner x (e (K 0)) (metricComparisonDifferenceEndomorphism (I := I) g₀ g₁ x (e (J 0))) := by
  have hcomp : fiberNormSqComponent (I := I) (M := M) g₀ x 1 1
        ((endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
          (metricComparisonDifferenceEndomorphismField (I := I) g₀ g₁)).toSection x) n e K J =
      Tensor0SSpace.toModel
...
private lemma riemannianFiberNormSq_slotInsertE_zero_le
  ...
  rw [riemannianFiberNormSq_eq_sum_componentSq_of_basis (I := I) (M := M) g₀ 1 1 x
    ((endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
      (metricComparisonDifferenceEndomorphismField (I := I) g₀ g₁)).toSection x) e bse hnE hbse horth]
  have hinv₀_nn : 0 ≤ 1 / (1 - δ₀) := by positivity
  ...
  have heach : ∀ (K J : Fin 1 → Fin n),
      (fiberNormSqComponent (I := I) (M := M) g₀ x 1 1
        ((endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
          (metricComparisonDifferenceEndomorphismField (I := I) g₀ g₁)).toSection x) n e K J) ^ 2 ≤
        (1 / (1 - δ₀)) ^ 2 := by
    intro K J
    rw [fiberNormSqComponent_slotInsertE (I := I) (M := M) g₀ g₁ x e K J]
    set u : TangentSpace I x := metricComparisonDifferenceEndomorphism (I := I) g₀ g₁ x (e (J 0)) with hu_def
    have hcs := metric_inner_cauchy_schwarz_sq (I := I) (M := M) g₀ x (e (K 0)) u
    have hKK : g₀.inner x (e (K 0)) (e (K 0)) = 1 := by rw [horth (K 0) (K 0)]; simp
    rw [hKK, one_mul] at hcs
    have hsfib := sqrt_inner_metricComparisonDifferenceEndomorphism_le (I := I) g₀ g₁
      (ccTensorBilinSymm (I := I) g₀ T) hg₁ (by linarith : δ < 1) hδ0 hbound x (e (J 0))
    have hJJ : g₀.inner x (e (J 0)) (e (J 0)) = 1 := by rw [horth (J 0) (J 0)]; simp
    rw [hJJ, Real.sqrt_one, mul_one] at hsfib
    rw [← hu_def] at hsfib
...
private theorem
    ...
    ∃ (A : ℝ) (B : ℕ → ℝ), 0 ≤ A ∧ (∀ m, 0 ≤ B m) ∧
      ∀ (g₀ g₁ : SmoothRiemannianMetric I M) (T : SmoothCcTensor g₀ 0 2)
        ...
        (riemannianFiberNormSq (I := I) (M := M) g₀ 1 (1 + 0) x
            ((iteratedCovGrad (I := I) g₀ 1 1 0
              (endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
                (metricComparisonEndomorphismField (I := I) (M := M) g₀ g₁))).toSection x) ≤ A) ∧
        (∀ m : ℕ,
          riemannianFiberNormSq (I := I) (M := M) g₀ 1 (1 + (m + 1)) x
              ((iteratedCovGrad (I := I) g₀ 1 1 (m + 1)
                (endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
  ...
  · rw [iteratedCovGrad_zero]
    exact riemannianFiberNormSq_slotInsertFULL_zero_le (I := I) (M := M) g₀ hδ₀0 hδ₀ g₁ T htie hδ_le hδ0 hbound x
  · intro m
    have hbase : riemannianFiberNormSq (I := I) (M := M) g₀ 1 (1 + 0) x
      ...
      rw [iteratedCovGrad_zero]
      exact riemannianFiberNormSq_slotInsertFULL_zero_le (I := I) (M := M) g₀ hδ₀0 hδ₀ g₁ T htie hδ_le hδ0 hbound x
    have hgrid := riemannianFiberNormSq_iteratedCovGrad_SI_F_leibniz_grid_bound (I := I) (M := M) g₀ g₁ T htie m x
    refine le_trans hgrid ?_
    set SF0 : ℝ := riemannianFiberNormSq (I := I) (M := M) g₀ 1 (1 + 0) x
      ((iteratedCovGrad (I := I) g₀ 1 1 0
        (endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
          (metricComparisonEndomorphismField (I := I) (M := M) g₀ g₁))).toSection x) with hSF0
    ...
    set S : ℝ := ∑ k ∈ Finset.range (m + 1),
      riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + ((m - k) + 1)) x
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 1 (1 + k) x
          ((iteratedCovGrad (I := I) g₀ 1 1 k
            (endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
              (metricComparisonEndomorphismField (I := I) (M := M) g₀ g₁))).toSection x) with hSdef
...
theorem invDiff_zero_unif
  ...
  have hfg : metricComparisonEndomorphismField (I := I) (M := M) g₁ g₁ =
      metricComparisonEndomorphismField (I := I) (M := M) g₀ g₀ := by
    apply ContMDiffSection.ext
    intro y
    apply ContinuousLinearMap.ext
    intro v
    rw [metricComparisonEndomorphismField_apply, metricComparisonEndomorphismField_apply, metricComparisonEndomorphism_self,
      metricComparisonEndomorphism_self]
  ...
  have hFULLeq : endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
    ...
    rw [← slotInsertEndoCc_add_local]
    congr 1
    exact metricComparisonEndomorphismField_recovery_decomp (I := I) (M := M) g₁ g₀
  have hE0 : riemannianFiberNormSq (I := I) (M := M) g₀ 1 1 x
        ((endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
          (metricComparisonDifferenceEndomorphismField (I := I) g₀ g₁)).toSection x) ≤
      (Module.finrank ℝ E : ℝ) ^ 2 * (1 / (1 - δ₀)) ^ 2 :=
    riemannianFiberNormSq_slotInsertE_zero_le (I := I) (M := M) g₀ hδ₀0 hδ₀ g₁ T
...
theorem invDiff_slot_unif
  ...
  intro g₀ g₁ T htie δ hδ_le hδ0 hbound i x
  have h2862 := riemannianFiberNormSq_iteratedCovGrad_slotInsertEndoCc_le_endo (E := E) (I := I) (M := M) g₀ 1
...
omit [SigmaCompactSpace M] in
theorem riemannianFiberNormSq_iteratedCovGrad_inverseMetricDifferenceSlotCoefficient_diagonalProductGrid_le
    (g₀ : SmoothRiemannianMetric I M) {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ C : ℕ → ℝ, (∀ i, 0 ≤ C i) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (T : SmoothCcTensor g₀ 0 2)
        (_htie : ∀ y v w, g₁.inner y v w =
