...
private lemma fiberNormSqComponent_raisedKoszul
  ...
  rw [show (g0FlatCLM (I := I) g₀ x (e (K 0)))
        (fun _ : Fin 1 => inverseMetricSharpFib (I := I) g₀ x
          (g0FlatCLM (I := I) g₁ x (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x (e (J 0)) (e (J 1))))) =
...
theorem riemannianFiberNormSq_raisedKoszul_le_of_lt_one
  ...
  have heach : ∀ (K : Fin 1 → Fin n) (J : Fin 2 → Fin n),
    ...
    set d : TangentSpace I x :=
      PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x (e (J 0)) (e (J 1)) with hd_def
      ...
      Tensor0SSpace.toModel
        ...
        (Fin.cons (tangentSpaceModelContinuousLinearEquiv (I := I) x (X x))
          (Fin.cons (tangentSpaceModelContinuousLinearEquiv (I := I) x (Y x))
            ![tangentSpaceModelContinuousLinearEquiv (I := I) x (Z x)])) := by
    ...
    rw [show ((show Tensor0SSpace 1 I x →L[ℝ] Tensor0SSpace 3 I x from
        ...
        Tensor0SSpace.toModel
          ((show Tensor0SSpace 1 I x →L[ℝ] Tensor0SSpace 3 I x from
            (covGrad (I := I) (M := M) g₀ 1 2 (raisedKoszul (I := I) g₀ g₁)).toSection x)
            (om x))
          (fun k => tangentSpaceModelContinuousLinearEquiv (I := I) x (e (J k))) from by
      rw [Tensor0SSpace.toModel_apply_tangent, Tensor0SSpace.eval_eq]]
    congr 1
    funext k
    refine Fin.cases ?_ (fun i => ?_) k
    · rw [Fin.cons_zero, hXx]
    · refine Fin.cases ?_ (fun j => ?_) i
      · rw [Fin.cons_succ, Fin.cons_zero, hYx,
          show (Fin.succ 0 : Fin 3) = 1 from rfl]
      · refine Fin.cases ?_ (fun j' => ?_) j
        · rw [Fin.cons_succ, Fin.cons_succ]
          rw [show (![tangentSpaceModelContinuousLinearEquiv (I := I) x (Z x)] : Fin 1 → E) 0 =
            tangentSpaceModelContinuousLinearEquiv (I := I) x (Z x) from rfl, hZx,
...
private lemma covDerivRaisedKoszulVec_eq_endoCov_add_sharpFlat
  ...
  have hDsm : ContMDiff I (I.prod 𝓘(ℝ, E)) ∞
      (fun b : M => TotalSpace.mk' E (E := fun z : M => TangentSpace I z) b
        (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ b (Y b) (Z b))) :=
    PDE.DeTurck.connectionDifference_contMDiff (I := I) g₁ g₀ Y.contMDiff Z.contMDiff
  set Dsec : Cₛ^∞⟮I; E, (TangentSpace I : M → Type _)⟯ :=
    ⟨fun b : M => PDE.DeTurck.connectionDifference (I := I) g₁ g₀ b (Y b) (Z b), hDsm⟩ with hDsec_def
  have hDsecx : Dsec x = PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x (Y x) (Z x) := rfl
  ...
  have hΛapp : (fun y : M => (sharpFlatRaiseEndoField (I := I) (M := M) g₀ g₁ y) (Dsec y)) =
    ...
    change sharpFlatRaiseEndo (I := I) g₀ g₁ y
        (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ y (Y y) (Z y)) = _
    rw [raisedKoszulVec]
  rw [hΛapp] at hLeib
  ...
  set ET : TangentSpace I x :=
    ((endoCovariantDerivative (I := I) (M := M) g₀)
        (sharpFlatRaiseEndoField (I := I) (M := M) g₀ g₁) x (X x))
      (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x (Y x) (Z x)) with hET_def
  ...
  have hrkY : raisedKoszulVec (I := I) g₀ g₁ x gradY (Z x) =
      sharpFlatRaiseEndo (I := I) g₀ g₁ x
        (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x gradY (Z x)) := by
    rw [raisedKoszulVec]
  have hrkZ : raisedKoszulVec (I := I) g₀ g₁ x (Y x) gradZ =
      sharpFlatRaiseEndo (I := I) g₀ g₁ x
        (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x (Y x) gradZ) := by
    rw [raisedKoszulVec]
  rw [hrkY, hrkZ]
  have hcollapse :
      ET + sharpFlatRaiseEndo (I := I) g₀ g₁ x gradD
        - sharpFlatRaiseEndo (I := I) g₀ g₁ x
            (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x gradY (Z x))
        - sharpFlatRaiseEndo (I := I) g₀ g₁ x
            (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x (Y x) gradZ) =
      ET + sharpFlatRaiseEndo (I := I) g₀ g₁ x
          (gradD - PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x gradY (Z x)
            - PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x (Y x) gradZ) := by
    rw [map_sub, map_sub]; abel
  rw [hcollapse]
  congr 1
  rw [covDerivConnectionDifference, covDerivDiff]
  ...
  rw [show CovariantDerivative.difference (LeviCivita (I := I) g₁) (LeviCivita (I := I) g₀) x
        (Y x) gradZ = PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x (Y x) gradZ from rfl]
  rw [show CovariantDerivative.difference (LeviCivita (I := I) g₁) (LeviCivita (I := I) g₀) x
        gradY (Z x) = PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x gradY (Z x) from rfl]
  abel_nf
...
private theorem g0_inner_endoCov_sharpFlatRaise_eq
    ...
    g₀.inner x
      ...
      g₁.inner x (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x (Y x) v) z
        + g₁.inner x (Y x) (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x z v) := by
  classical
  ...
  rw [map_sub, sub_apply, hA, hBb]
  have hθmd : MDiffAtCotangent (I := I) θ x := cotangent_g0FlatY_mdiffAtCotangent_g1 (I := I) g₁ Y x
  have hsplit := cotangentCov_leviCivita_connectionDifference (I := I) g₀ g₁ hθmd v z
  ...
  have hθval : θ x (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x z v) =
      g₁.inner x (Y x) (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x z v) := by
    rw [hθdef]
    change cotangentToDual (I := I) (g0FlatCLM (I := I) g₁ x (Y x))
        (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x z v) = _
    rw [cotangentToDual_g0FlatCLM (I := I) g₁ x (Y x) (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x z v)]
  have hcg0 : ((cotangentCov (LeviCivita (I := I) g₀)).toFun θ x v) z =
      g₁.inner x gradY₁ z + g₁.inner x (Y x) (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x z v) := by
    have hstep : ((cotangentCov (LeviCivita (I := I) g₀)).toFun θ x v) z =
        ((cotangentCov (LeviCivita (I := I) g₁)).toFun θ x v) z
          + θ x (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x z v) := by linarith [hsplit]
    rw [hstep, hdual, hθval]
  rw [hcg0]
  have hconnY : PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x (Y x) v = gradY₁ - gradY₀ := by
    have hca := PDE.DeTurck.connectionDifference_apply (I := I) g₁ g₀ (σ := fun b => Y b)
      (Y.contMDiff.mdifferentiableAt (by norm_num)) v
    rw [hgY1, hgY0]
    exact hca
  rw [hconnY, map_sub, sub_apply]
  ring
...
omit [NeZero (Module.finrank ℝ E)] [CompactSpace M] [I.Boundaryless] [BoundarylessManifold I M]
    [T2Space M] [SigmaCompactSpace M] in
private lemma connectionDifference_quadratic_inner_bounds
    (g₀ g₁ : SmoothRiemannianMetric I M) (T : SmoothCcTensor g₀ 0 2)
    (h : ∀ y v w, g₁.inner y v w =
      g₀.inner y v w + ccTensorBilinSymm (I := I) g₀ T y v w)
    {δ : ℝ} (hδ0 : 0 ≤ δ)
    ...
    |g₁.inner x
        (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x Y Z)
        (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x ζ X)| ≤
      (1 + δ) * C ^ 2 * G ^ 2 *
    ...
    |g₁.inner x
        (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x
          (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x Y Z) X) ζ| ≤
      (1 + δ) * C ^ 2 * G ^ 2 *
        Real.sqrt (g₀.inner x X X) * Real.sqrt (g₀.inner x Y Y) *
  ...
  set cYZ : TangentSpace I x := PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x Y Z
  set cζX : TangentSpace I x := PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x ζ X
  set cWX : TangentSpace I x := PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x cYZ X
...
private lemma sqrt_inner_endoCov_sharpFlatRaiseEndo_apply_le
  ...
  have hbd : ∀ z : TangentSpace I x,
    ...
    rw [← hu_def] at hform
    rw [hform]
    set cw : TangentSpace I x := PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x w v with hcw_def
    set cz : TangentSpace I x := PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x z v with hcz_def
...
private lemma half_iteratedCovGrad_ccTensor02Symm_combination_le
  ...
  let inst4 : Bundle.RiemannianBundle
      (fun y : M => Tensor0SBundle.TensorRSSpace 0 4 I y) :=
    Tensor0SBundle.tensorRSRiemannianBundle (I := I) (M := M) g₀ 0 4
...
attribute [-instance] Tensor0SBundle.tensorRSSpaceNormedAddCommGroup
  Tensor0SBundle.tensorRSSpaceNormedSpace in
omit [NeZero (Module.finrank ℝ E)] [SigmaCompactSpace M] in
private lemma g1_inner_covDerivConnectionDifference_le_pointwise
    (g₀ : SmoothRiemannianMetric I M) {δ₀ : ℝ} (hδ₀ : δ₀ < 1)
    (C₀ : ℝ) (hC₀0 : 0 ≤ C₀)
  ...
  have hNZ_nn : 0 ≤ NZ := Real.sqrt_nonneg _
  set D : TangentSpace I x :=
    covDerivConnectionDifference (I := I) g₀ g₁ (fun b => X b) (fun b => Z b) (fun b => Y b) x with hD_def
  ...
  have hident := covDerivConnectionDifference_g1inner_eq_half_secondCovGrad_sub_connectionDifferenceSq
  ...
  set R43 : ℝ := unitModel (I := I) (M := M) g₀ 4
    (iteratedCovGrad (I := I) g₀ 0 2 2 (ccTensor02Symm (I := I) g₀ T)) x ![X x, ζ, Z x, Y x] with
      hR43_def
  set cYZ : TangentSpace I x := PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x (Y x) (Z x) with hcYZ_def
  have hT2eq : inverseMetricSharpFib (I := I) g₁ x
        (koszulCovGradCovec (I := I) (M := M) g₀ g₁ X ζf x) =
      PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x ζ (X x) := by
    have := connectionDifference_eq_operatorFieldApplication_invGram_covGrad (I := I) (M := M) g₀ g₁ X ζf x
    rw [hζfx] at this
    exact this.symm
  have hT5eq : inverseMetricSharpFib (I := I) g₁ x
        (koszulCovGradCovec (I := I) (M := M) g₀ g₁ Z Y x) = cYZ := by
    have := connectionDifference_eq_operatorFieldApplication_invGram_covGrad (I := I) (M := M) g₀ g₁ Z Y x
  ...
  set cζX : TangentSpace I x := PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x ζ (X x) with hcζX_def
  set cWX : TangentSpace I x := PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x cYZ (X x) with hcWX_def
  ...
  have hpw : ∀ v w : TangentSpace I x,
    ...
    rw [← hG1_def] at hraw
    exact hraw
  have hquad := connectionDifference_quadratic_inner_bounds
...
attribute [-instance] Tensor0SBundle.tensorRSSpaceNormedAddCommGroup
  Tensor0SBundle.tensorRSSpaceNormedSpace in
omit [NeZero (Module.finrank ℝ E)] [SigmaCompactSpace M] in
private lemma sqrt_inner_covDerivConnectionDifference_le
    (g₀ : SmoothRiemannianMetric I M) {δ₀ : ℝ} (hδ₀0 : 0 ≤ δ₀) (hδ₀ : δ₀ < 1) :
    ∃ C : ℝ, 0 ≤ C ∧ ∀ (g₁ : SmoothRiemannianMetric I M)
      (T : SmoothCcTensor g₀ 0 2)
      (_h : ∀ y v w, g₁.inner y v w =
  ...
  have hbd : ∀ ζ : TangentSpace I x, g₁.inner x D ζ ≤ Kc * Real.sqrt (g₀.inner x ζ ζ) := by
    intro ζ
    have hpw := g1_inner_covDerivConnectionDifference_le_pointwise (I := I) (M := M) g₀ hδ₀ C₀ hC₀0 hconn
...
private lemma sqrt_inner_covDerivRaisedKoszul_endoTerm_le
  ...
  set D : TangentSpace I x :=
    PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x (Y x) (Z x) with hD_def
...
private lemma sqrt_inner_covDerivRaisedKoszulVec_le
  ...
  set A : TangentSpace I x :=
    ((endoCovariantDerivative (I := I) (M := M) g₀)
        (sharpFlatRaiseEndoField (I := I) (M := M) g₀ g₁) x (X x))
      (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x (Y x) (Z x)) with hA_def
  ...
  have hbound_target : Real.sqrt (g₀.inner x
    ...
    | true =>
        ...
        rw [show g₀.inner x (- PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x s v0)
            ...
            g₀.inner x (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x s v0)
              (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x s v0) from by
    ...
    | false =>
        ...
        have hDval : ∀ z : TangentSpace I x, Dfun z =
            - g₀.inner x w (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x z v0) := by
          intro z
          rw [hDfun_def, neg_apply, ContinuousLinearMap.comp_apply,
            ContinuousLinearMap.flip_apply]
          change - cotangentToDual (I := I) (g0FlatCLM (I := I) g₀ x w)
              (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x z v0) = _
        ...
        have hpp_val : g₀.inner x p p =
            - g₀.inner x w (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v0) := by
          rw [hpz p, hDval p]
        ...
        have hcs := abs_metric_inner_le_sqrt_metric_quadratic (I := I) (M := M) g₀ x w
          (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v0)
        ...
        have hNp_le : Np ≤ C₀ * G * (1 / (1 - δ₀)) * Nv0 := by
          have hpp_le : g₀.inner x p p ≤
              Real.sqrt (g₀.inner x (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v0)
                (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v0)) := by
            rw [hpp_val]
            calc - g₀.inner x w (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v0)
                ≤ |g₀.inner x w (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v0)| := neg_le_abs _
              _ ≤ Real.sqrt (g₀.inner x (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v0)
                  (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v0)) := hcs
          have hconnG : Real.sqrt (g₀.inner x (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v0)
                (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v0)) ≤ C₀ * G * Np * Nv0 := hconn_p
