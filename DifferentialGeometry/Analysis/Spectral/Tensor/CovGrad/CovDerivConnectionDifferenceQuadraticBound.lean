...
private theorem exists_norm_covGrad_connectionDifferenceSection_le_of_jetEnvelope
  ...
  have hjet := riemannianFiberNormSq_iteratedCovGrad_connectionDifferenceSection_le (I := I) (M := M) g₀ g₁ 1 x
    Bf Sf
    hKos hSharp hSf_nn
  have hiter1 : (iteratedCovGrad (I := I) g₀ 1 2 1 (connectionDifferenceSection (I := I) g₁ g₀)) =
      covGrad (I := I) (M := M) g₀ 1 2 (connectionDifferenceSection (I := I) g₁ g₀) := by
...
theorem exists_covDerivConnectionDifference_gQuadratic_le_of_jetEnvelope
  ...
  obtain ⟨Cw, hCw_nn, hCw⟩ :=
    exists_norm_covGrad_connectionDifferenceSection_le_of_jetEnvelope (I := I) (M := M) g₀ hm0 hm1 B hB
  refine ⟨Cw, hCw_nn, ?_⟩
