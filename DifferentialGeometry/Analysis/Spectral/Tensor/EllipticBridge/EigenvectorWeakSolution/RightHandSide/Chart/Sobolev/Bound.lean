...
omit [CompleteSpace E] in
private lemma eigenvectorChartComponentFun_wkpNorm_le :
    ∃ C : ℝ, 0 ≤ C ∧
...
omit [CompleteSpace E] in
private lemma eigenvectorChartCrossLeftContraction_wkpNorm_le
  ...
  have h_data : ∀ x : TensorCompIdx (E := E) r (s + 1) ×
    ...
    obtain ⟨h_summand_memWkp, C, hC_nn, hC_bd⟩ :=
      wkpNorm_smooth_coef_mul_ae_zero_factor_le
        (I := I) (M := M) α K
  ...
  obtain ⟨C, hC_nn, hC_bd⟩ := wkpNorm_sum_le_of_le_ofReal_mul
...
omit [CompleteSpace E] in
private lemma eigenvectorChartCrossRightContraction_wkpNorm_le
    (h_pou : eigenvectorResolventChartWkpRegularity (I := I) (M := M) g r s i K) :
    ∃ C : ℝ, 0 ≤ C ∧
      iteratedWeakSobolevNorm (d := Module.finrank ℝ E) K 2
...
omit [CompleteSpace E] in
private lemma covPrincipalRotationCoeffLimit_wkpNorm_le
    (h_pou : eigenvectorResolventChartWkpRegularity (I := I) (M := M) g r s i K) :
    ∃ C : ℝ, 0 ≤ C ∧
      iteratedWeakSobolevNorm (d := Module.finrank ℝ E) K 2
...
omit [CompleteSpace E] in
private lemma covLowerOrderRotationValueCoeffLimit_wkpNorm_le
    (h_pou : eigenvectorResolventChartWkpRegularity (I := I) (M := M) g r s i K) :
    ∃ C : ℝ, 0 ≤ C ∧
      iteratedWeakSobolevNorm (d := Module.finrank ℝ E) K 2
    ...
    calc
      ...
      _ = ENNReal.ofReal (2 * C) *
          ...
          rw [← ENNReal.ofReal_ofNat 2, ← mul_assoc, ← ENNReal.ofReal_mul hC_nn,
            mul_comm C 2]
  exact wkpNorm_sum_le_of_le_ofReal_mul (Ω := Ω) (by norm_num) hΩ_open
...
omit [CompleteSpace E] in
private lemma eigenvectorChartWeightedDivergence_wkpNorm_le
    (h_pou : eigenvectorResolventChartWkpRegularity (I := I) (M := M) g r s i K) :
    ∃ C : ℝ, 0 ≤ C ∧
      iteratedWeakSobolevNorm (d := Module.finrank ℝ E) K 2
  ...
  obtain ⟨_, C₁, hC₁_nn, hC₁_bd⟩ := wkpNorm_smooth_coef_mul_ae_zero_factor_le
    (I := I) (M := M) α K
...
omit [CompleteSpace E] in
private lemma eigenvectorChartCrossRightDivergence_wkpNorm_le
    (h_pou : eigenvectorResolventChartWkpRegularity (I := I) (M := M) g r s i K) :
    ∃ C : ℝ, 0 ≤ C ∧
      iteratedWeakSobolevNorm (d := Module.finrank ℝ E) K 2
...
omit [CompleteSpace E] in
private lemma eigenvectorChartRHSNumerator_wkpNorm_le
    (h_pou : eigenvectorResolventChartWkpRegularity (I := I) (M := M) g r s i K) :
    ∃ C : ℝ, 0 ≤ C ∧
      iteratedWeakSobolevNorm (d := Module.finrank ℝ E) K 2
  ...
  obtain ⟨D1, hD1_nn, hD1⟩ := eigenvectorChartComponentFun_wkpNorm_le
    (I := I) (M := M) g r s i α P₀ K
  obtain ⟨D2, hD2_nn, hD2⟩ := eigenvectorChartCrossLeftContraction_wkpNorm_le
    (I := I) (M := M) g r s i α P₀ K h_pou
  obtain ⟨D3, hD3_nn, hD3⟩ := eigenvectorChartCrossRightContraction_wkpNorm_le
    (I := I) (M := M) g r s i α P₀ K h_pou
  obtain ⟨D4, hD4_nn, hD4⟩ := covPrincipalRotationCoeffLimit_wkpNorm_le
    (I := I) (M := M) g r s i α P₀ K h_pou
  obtain ⟨D5, hD5_nn, hD5⟩ := covLowerOrderRotationValueCoeffLimit_wkpNorm_le
    (I := I) (M := M) g r s i α P₀ K h_pou
  obtain ⟨D6, hD6_nn, hD6⟩ := eigenvectorChartWeightedDivergence_wkpNorm_le
    (I := I) (M := M) g r s i α P₀ K h_pou
  obtain ⟨D7, hD7_nn, hD7⟩ := eigenvectorChartCrossRightDivergence_wkpNorm_le
    (I := I) (M := M) g r s i α P₀ K h_pou
...
theorem eigenvectorChartRHS_wkpNorm_le
  ...
  have h_bracket_memWkp : MemWkp (d := Module.finrank ℝ E) K 2
      (eigenvectorChartRHSNumerator (I := I) (M := M) g r s i α P₀) Ω := by
    have hM1 := eigenvectorChartComponentFun_memWkp (I := I) (M := M)
      g r s i α P₀ K h_pou
    have hM2 := eigenvectorChartCrossLeftContraction_memWkp (I := I) (M := M)
      g r s i α P₀ K h_pou
      ...
      (MemWkp.add (d := Module.finrank ℝ E) hp2 hΩ_open
        (MemWkp.sub (d := Module.finrank ℝ E) hp2 hΩ_open
          (MemWkp.sub (d := Module.finrank ℝ E) hp2 hΩ_open
            ...
            hM4) hM5) hM6) hM7
  obtain ⟨C, hC_nn, hC_bd⟩ := eigenvectorChartRHSNumerator_wkpNorm_le
    (I := I) (M := M) g r s i α P₀ K h_pou
  rw [← hΩ_def] at hC_bd
  refine ⟨C, hC_nn, ?_⟩
  have h_smul_eq :
    ...
    change iteratedWeakSobolevNorm (d := Module.finrank ℝ E) K 2
        (fun x => (i.fst.val)⁻¹ *
          eigenvectorChartRHSNumerator (I := I) (M := M) g r s i α P₀ x) Ω = _
  ...
  simp only [hΩ_def, eigenvectorChartRHSWkpControl, eigenvectorChartComponentWkpNorm,
    eigenvectorCrossLeftWkpControl, eigenvectorCrossRightWkpControl,
    eigenvectorPartialLimitWkpNormSum, eigenvectorComponentLimitWkpNormSum,
              ...
              + (∑ P : TensorCompIdx (E := E) r s,
                  ∑ l : Fin (Module.finrank ℝ E),
                    iteratedWeakSobolevNorm (d := Module.finrank ℝ E) K 2
                      ...
                      (chartTargetEuclid (I := I) (M := M) α))) := by
  classical
  obtain ⟨C, hC_nn, hC_bd⟩ := eigenvectorChartRHSNumerator_wkpNorm_le_uniform
