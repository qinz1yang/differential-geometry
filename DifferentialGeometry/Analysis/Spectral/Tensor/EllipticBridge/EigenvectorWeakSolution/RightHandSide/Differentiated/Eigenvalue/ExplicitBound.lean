...
private lemma rhsZeroAggregate_le_at_target
    ...
    (hEig_le : H.eigenvectorExponent K ≤ target)
    (hResH_le_K : H.highResolventExponent K ≤ target)
    (hResL_le : H.lowResolventExponent K ≤ target)
    (hPar_le : H.partialExponent K ≤ target)
    (hCom_le : H.componentExponent K ≤ target)
    (hCcR_le : H.crossRightExponent K ≤ target)
    (hCcut_le : H.cutoffExponent K ≤ target) :
    ∃ C : ℝ, 0 ≤ C ∧
  ...
  have hCqtot_nn : 0 ≤ Cqtot := by
    have h := H.highResolventConstant_nonneg K; positivity
  ...
  have hClow_α_nn : 0 ≤ Clow_α := by
    have h := H.lowResolventConstant_nonneg K; positivity
  have hCpar'_nn : 0 ≤ partialConstant' := by
    have h := H.partialConstant_nonneg K; positivity
  have hCcom'_nn : 0 ≤ componentConstant' := by
    have h := H.componentConstant_nonneg K; positivity
  have hCcR'_nn : 0 ≤ crossRightConstant' := by
    have h := H.crossRightConstant_nonneg K; positivity
  have hCcut'_nn : 0 ≤ cutoffConstant' := by
    have h := H.cutoffConstant_nonneg K; positivity
  ...
  have hS2_inner : ∀ β ∈ transportChartCenters (I := I) (M := M) α,
    ...
    have h_inner_β :
      ...
      have h_each : ∀ Q ∈ (Finset.univ : Finset (TensorCompIdx (E := E) r s)),
        ...
        h_bridge _ (H.highResolventConstant K) (H.highResolventExponent K) (H.highResolventConstant_nonneg K) hResH_le_K
  ...
  have hS4 :
    ...
    have h_perP : ∀ P ∈ (Finset.univ : Finset (TensorCompIdx (E := E) r s)),
      ...
      have h_each : ∀ k ∈ (Finset.univ : Finset (Fin (Module.finrank ℝ E))),
        ...
        h_bridge _ (H.partialConstant K) (H.partialExponent K) (H.partialConstant_nonneg K) hPar_le (H.partial_bound i P k K hKN)
      have h_sum := sum_le_of_le_ofReal_mul
  ...
  have hS5 :
    ...
    have h_each : ∀ p ∈ (Finset.univ : Finset (TensorCompIdx (E := E) r s)),
      ...
      h_bridge _ (H.componentConstant K) (H.componentExponent K) (H.componentConstant_nonneg K) hCom_le (H.component_bound i p K hK_le_N)
    have h_sum := sum_le_of_le_ofReal_mul
      (Finset.univ : Finset (TensorCompIdx (E := E) r s))
  ...
  have hS6 :
    ...
    have h_each : ∀ P ∈ (Finset.univ : Finset (TensorCompIdx (E := E) r s)),
      ...
      h_bridge _ (H.crossRightConstant K) (H.crossRightExponent K) (H.crossRightConstant_nonneg K) hCcR_le (H.crossRight_bound i P K hK_le_N)
    have h_sum := sum_le_of_le_ofReal_mul
      (Finset.univ : Finset (TensorCompIdx (E := E) r s))
      (fun P => iteratedWeakSobolevNorm (d := Module.finrank ℝ E) K 2
          (fun y => ((crossRightLimitComponent (I := I) (M := M)
  ...
  have hS7 :
    ...
    have h_perP : ∀ P ∈ (Finset.univ : Finset (TensorCompIdx (E := E) r s)),
      ...
      have h_each : ∀ l ∈ (Finset.univ : Finset (Fin (Module.finrank ℝ E))),
        ...
        h_bridge _ (H.cutoffConstant K) (H.cutoffExponent K) (H.cutoffConstant_nonneg K) hCcut_le (H.cutoff_bound i P l K hKN)
      have h_sum := sum_le_of_le_ofReal_mul
        (Finset.univ : Finset (Fin (Module.finrank ℝ E)))
...
private lemma sharpDiffBdd_recursion_at_target
    ...
    (H : EigenvectorChartRightHandSideDifferentiatedSharpWkpBoundsUpToOrder (I := I) (M := M) g r s α P₀ N)
    (h_atom_bd : ∀ K', K' ≤ N →
      H.eigenvectorExponent K' ≤ eAtomMax ∧ H.highResolventExponent K' ≤ eAtomMax ∧
        H.lowResolventExponent K' ≤ eAtomMax ∧ H.partialExponent K' ≤ eAtomMax ∧
        H.componentExponent K' ≤ eAtomMax ∧ H.crossRightExponent K' ≤ eAtomMax ∧
        H.cutoffExponent K' ≤ eAtomMax) :
    ∀ (m : ℕ) (K : ℕ) (_ : K + m + 1 ≤ N)
  ...
  | succ m ih =>
      ...
      have hAtomA_bd : ∀ (i : TensorEigenIdx (I := I) (M := M) g r s)
        ...
        exact sharpDiff_ofReal_const_pow_eigen_inv_le
          (I := I) (M := M) g r s i (H.eigenvectorConstant_nonneg _)
      ...
      have hAtomC_bd : ∀ (i : TensorEigenIdx (I := I) (M := M) g r s),
        ...
        have h_bridge :=
          (eigenvectorChartIteratedPartial_wkpNorm_le_of_memWkp
            ...
            (Fin.init l)).2
        refine le_trans h_bridge ?_
        refine le_trans (H.eigenvector_bound i (K + m) (by omega)) ?_
        refine mul_le_mul_of_nonneg_right ?_ (zero_le)
        exact sharpDiff_ofReal_const_pow_eigen_inv_le
          (I := I) (M := M) g r s i (H.eigenvectorConstant_nonneg _)
      ...
      obtain ⟨Cnum, hCnum_nn, hCnum_bd⟩ :=
        eigenvectorChartRHSDiffNumerator_wkpNorm_le_chartcpt_at_target
          ...
          (fun i => eigenvectorChartRHSDiff (I := I) (M := M)
            g r s i α P₀ m (Fin.init l))
          (H.eigenvectorConstant (K + m + 1)) (H.eigenvectorConstant_nonneg _) hAtomA_bd
          (H.eigenvectorConstant (K + m + 2)) (H.eigenvectorConstant_nonneg _) hAtomB_bd
          (H.eigenvectorConstant (K + m)) (H.eigenvectorConstant_nonneg _) hAtomC_bd
...
theorem eigenvectorChartRHSDiff_wkpNorm_le_chartcpt_sharp_bdd_explicit
    ...
    (l : Fin m → Fin (Module.finrank ℝ E))
    (H : EigenvectorChartRightHandSideDifferentiatedSharpWkpBoundsUpToOrder (I := I) (M := M) g r s α P₀ (K + m + 1))
    (eAtomMax : ℕ)
