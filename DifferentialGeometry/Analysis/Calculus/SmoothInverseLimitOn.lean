...
def iteratedFDerivBoundsOnCompactsWithin (U : Set E) (Φ : ℕ → E → F) : Prop :=
  ∀ r : ℕ, ∀ K : Set E, IsCompact K → K ⊆ U →
    ∃ M : ℝ, ∀ k : ℕ, ∀ x ∈ K, ‖iteratedFDeriv ℝ r (Φ k) x‖ ≤ M

omit [FiniteDimensional ℝ E] [FiniteDimensional ℝ F] in
theorem iteratedFDerivBoundsOnCompactsWithin.comp_subseq {U : Set E} {Φ : ℕ → E → F}
    (h : iteratedFDerivBoundsOnCompactsWithin U Φ) (φ : ℕ → ℕ) :
    iteratedFDerivBoundsOnCompactsWithin U (fun k => Φ (φ k)) := by
  intro r K hK hKU
  obtain ⟨M, hM⟩ := h r K hK hKU
  exact ⟨M, fun k x hx => hM (φ k) x hx⟩

omit [FiniteDimensional ℝ E] [FiniteDimensional ℝ F] in
theorem iteratedFDerivBoundsOnCompactsWithin.pi
    {ι : Type*} [Fintype ι] {U : Set E} {Φ : ℕ → E → (ι → F)}
    (hU : IsOpen U)
    (hsmooth : ∀ k i, ContDiffOn ℝ (⊤ : ℕ∞) (fun x => Φ k x i) U)
    (hbdd : ∀ i, iteratedFDerivBoundsOnCompactsWithin U (fun k x => Φ k x i)) :
    iteratedFDerivBoundsOnCompactsWithin U Φ := by
  classical
  intro r K hK hKU
...
omit [FiniteDimensional ℝ E] [FiniteDimensional ℝ F] in
theorem iteratedFDerivBoundsOnCompacts.to_within {Φ : ℕ → E → F} (h : iteratedFDerivBoundsOnCompacts Φ)
    (U : Set E) : iteratedFDerivBoundsOnCompactsWithin U Φ :=
...
theorem exists_c_inf_convergent_subsequence_on
...
theorem exists_smooth_inverse_limit_subsequence_on
    ...
    (hbΦ : iteratedFDerivBoundsOnCompactsWithin U Φ) (hbΨ : iteratedFDerivBoundsOnCompactsWithin V Ψ)
