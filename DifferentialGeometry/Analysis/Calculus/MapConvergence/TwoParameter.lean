...
theorem mapCInf_comp_pair_dist_tail
    ...
    (hB : MapCInfConvergenceOnCompacts U B Binf) (hA : MapCInfConvergenceOnCompacts V A Ainf)
  ...
  have huc : UniformContinuousOn Ainf K' :=
    hK'compact.uniformContinuousOn_of_continuous (hAcont.mono hK'V)
  rw [Metric.uniformContinuousOn_iff] at huc
  obtain ⟨δ, hδpos, hδ⟩ := huc (ε / 2) (by positivity)
  have hA₀ : TendstoUniformlyOn A Ainf Filter.atTop K' :=
    tendstoUniformlyOn_of_cPConvergence (hA K' hK'compact hK'V 0)
  ...
  have hB₀ : TendstoUniformlyOn B Binf Filter.atTop K :=
    tendstoUniformlyOn_of_cPConvergence (hB K hKcpt hKU 0)
...
theorem mapCInfConvergenceOnCompacts_comp_prodMk_id {U : Set E'} {V : Set (P × Q)}
    ...
    MapCInfConvergenceOnCompacts U (fun k y => Φ (u k y, v k y)) (fun y => y) := by
  ...
  have hcomp := MapCInfConvergenceOnCompacts.comp hU hV hpair
    (mapCInfConvergence_const (U := V) Φ) hpairc hpairinfc
    (fun _ => hΦc) hΦc hmap hmapk
  exact hcomp.congr hU (fun _ => Set.eqOn_refl _ _) (fun y hy => (hdiag y hy).symm)

theorem mapCInfConvergenceOnCompacts_comp_tendsto_atTop_id {F' : Type*}
    ...
    MapCInfConvergenceOnCompacts U (fun n y => A (ln n) (B (kn n) y)) (fun y => y) := by
  have hB' : MapCInfConvergenceOnCompacts U (fun n => B (kn n)) Binf :=
    hB.comp_tendsto_atTop hkn
  have hA' : MapCInfConvergenceOnCompacts V (fun n => A (ln n)) Ainf :=
    hA.comp_tendsto_atTop hln
  have hcomp := MapCInfConvergenceOnCompacts.comp hU hV hB' hA'
...
theorem mapCInf_comp_pair_tail
  ...
  apply mapCInf_pair_tail
    (U := U) (Φ := fun k l y => A l (B k y)) (Φinf := fun y : E => y)
      ?_ hKcpt hKU p
  intro kn ln hkn hln
  exact mapCInfConvergenceOnCompacts_comp_tendsto_atTop_id
    (E' := E) hU hV hB hA hBc hBinfc hAc hAinfc hmap hmapk hid
    kn ln hkn hln

theorem mapCInfConvergenceOnCompacts_pi_comp_tendsto_atTop_id
    {ι : Type*} [Fintype ι] {F' : Type*}
    [NormedAddCommGroup F'] [NormedSpace Real F'] [ProperSpace F']
    {U : Set E'} {V : ι → Set F'} (hU : IsOpen U) (hV : ∀ i, IsOpen (V i))
    {B : ι → Nat → E' → F'} {Binf : ι → E' → F'}
    {A : ι → Nat → F' → E'} {Ainf : ι → F' → E'}
    (hB : ∀ i, MapCInfConvergenceOnCompacts U (B i) (Binf i))
    (hA : ∀ i, MapCInfConvergenceOnCompacts (V i) (A i) (Ainf i))
    ...
    (kn ln : Nat → Nat) (hkn : Filter.Tendsto kn Filter.atTop Filter.atTop)
    (hln : Filter.Tendsto ln Filter.atTop Filter.atTop) :
    MapCInfConvergenceOnCompacts U (fun n y (i : ι) => A i (ln n) (B i (kn n) y))
      (fun y (_ : ι) => y) := by
  have hslot : ∀ i, MapCInfConvergenceOnCompacts U
      (fun n y => A i (ln n) (B i (kn n) y)) (fun y => y) := fun i =>
    mapCInfConvergenceOnCompacts_comp_tendsto_atTop_id
  ...
  exact mapCInfConvergence_pi hU hslot hslotc (fun _ => contDiffOn_id)
...
theorem mapCInf_comp_prodMk_pair_tail {U : Set E'} {V : Set (P × Q)}
    ...
    (hu : ∀ kn ln : Nat → Nat, Filter.Tendsto kn Filter.atTop Filter.atTop →
      ...
      MapCInfConvergenceOnCompacts U (fun n => u (kn n) (ln n)) uinf)
    (hv : ∀ kn ln : Nat → Nat, Filter.Tendsto kn Filter.atTop Filter.atTop →
      Filter.Tendsto ln Filter.atTop Filter.atTop →
      MapCInfConvergenceOnCompacts U (fun n => v (kn n) (ln n)) vinf)
  ...
  intro kn ln hkn hln
  exact mapCInfConvergenceOnCompacts_comp_prodMk_id hU hV
...
theorem mapCInf_comp_prodMk_pi_pair_tail
    ...
    (hw : ∀ kn ln : Nat → Nat, Filter.Tendsto kn Filter.atTop Filter.atTop →
      Filter.Tendsto ln Filter.atTop Filter.atTop →
      MapCInfConvergenceOnCompacts U (fun n => w (kn n) (ln n)) winf)
  ...
  mapCInf_comp_prodMk_pair_tail (v := fun k l y (i : ι) => A i l (B i k y))
    ...
    (fun kn ln hkn hln => mapCInfConvergenceOnCompacts_pi_comp_tendsto_atTop_id
