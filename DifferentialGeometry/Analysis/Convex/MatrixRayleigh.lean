...
theorem convex_neg_part_minimumRayleighQuotient3 :
...
private theorem matrixQuadraticForm3_eq_dotProduct
  ...
  change inner Real (Matrix.toLpLin 2 2 A (WithLp.toLp 2 x)) (WithLp.toLp 2 x) = _
  rw [Matrix.toLpLin_toLp]
  calc
    inner Real (WithLp.toLp 2 (Matrix.toLin' A x)) (WithLp.toLp 2 x) =
        dotProduct x (star (Matrix.toLin' A x)) :=
      EuclideanSpace.inner_toLp_toLp (Matrix.toLin' A x) x
    _ = dotProduct (star x) (A.mulVec x) := by
...
theorem neg_three_mul_neg_part_minimumRayleighQuotient3_le_trace
    {A : Matrix (Fin 3) (Fin 3) Real} (hA : A.IsHermitian) :
