...
private theorem exists_uniformBound_riemannianFiberNormSq_linearizedRicciConnectionDifferenceFib_of_jetEnvelope
      ...
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 2 2 x
            (show Tensor0SBundle.TensorRSSpace 2 2 I x from
              linearizedRicciConnectionDifferenceOrder0CometricTracedCLM (I := I) g₀ g₁ x) ≤ C ∧
        riemannianFiberNormSq (I := I) (M := M) g₀ 3 2 x
            (show Tensor0SBundle.TensorRSSpace 3 2 I x from
              linearizedRicciConnectionDifferenceOrder1CometricTracedCLM (I := I) g₀ g₁ x) ≤ C := by
  classical
  ...
  have hpwA' : ∀ (a : Fin (Module.finrank ℝ E)) (v : Fin 2 → TangentSpace I x),
      |(Tensor0SBundle.TensorRSSpace.toModel (connectionDifferenceFib (I := I) g₁ g₀ x)
          (Tensor0SBundle.Tensor0SSpace.toModel (g0FlatCLM (I := I) g₀ x (e a))))
          (fun j => (v j : E))| ≤
        (C₀ * G) * (Real.sqrt (g₀.inner x (v 0) (v 0)) *
          Real.sqrt (g₀.inner x (v 1) (v 1))) := by
    refine connectionDifference_flat_factor_bound (I := I) g₀ g₁ x e horth ?_
    intro v w'
    have h := hpwA v w'
    calc Real.sqrt (g₀.inner x (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x v w')
          (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x v w'))
  ...
  have hDAf : ∀ (a : Fin (Module.finrank ℝ E)) (v : Fin 3 → TangentSpace I x),
    ...
    have hveceq : covDerivConnectionDifference (I := I) g₀ g₁
        (smoothExtensionTangent (I := I) x (v 0))
        (smoothExtensionTangent (I := I) x (v 2))
  ...
  set A : Tensor0SBundle.TensorRSSpace 1 2 I x :=
    (connectionDifferenceSection (I := I) g₁ g₀).toSection x with hAset
  set DAv : Tensor0SBundle.TensorRSSpace 1 3 I x :=
    (covGrad (I := I) (M := M) g₀ 1 2 (connectionDifferenceSection (I := I) g₁ g₀)).toSection x with hDAset
  ...
  have hO1 : riemannianFiberNormSq (I := I) (M := M) g₀ 3 2 x
      (show Tensor0SBundle.TensorRSSpace 3 2 I x from
        linearizedRicciConnectionDifferenceOrder1CometricTracedCLM (I := I) g₀ g₁ x) ≤ nn ^ 3 * nn ^ 2 * Mc1 ^
          2 := by
    rw [riemannianFiberNormSq_eq_sum_componentSq_of_basis (I := I) (M := M) g₀ 3 2 x
      (show Tensor0SBundle.TensorRSSpace 3 2 I x from
        linearizedRicciConnectionDifferenceOrder1CometricTracedCLM (I := I) g₀ g₁ x) e bse rfl hbse horth]
    have hcompb : ∀ (K : Fin 3 → Fin (Module.finrank ℝ E))
        (J : Fin 2 → Fin (Module.finrank ℝ E)),
        (fiberNormSqComponent (I := I) (M := M) g₀ x 3 2
          (show Tensor0SBundle.TensorRSSpace 3 2 I x from
            linearizedRicciConnectionDifferenceOrder1CometricTracedCLM (I := I) g₀ g₁ x)
  ...
  have hO0 : riemannianFiberNormSq (I := I) (M := M) g₀ 2 2 x
      (show Tensor0SBundle.TensorRSSpace 2 2 I x from
        linearizedRicciConnectionDifferenceOrder0CometricTracedCLM (I := I) g₀ g₁ x) ≤ nn ^ 2 * nn ^ 2 * Mc0 ^
          2 := by
    rw [riemannianFiberNormSq_eq_sum_componentSq_of_basis (I := I) (M := M) g₀ 2 2 x
      (show Tensor0SBundle.TensorRSSpace 2 2 I x from
        linearizedRicciConnectionDifferenceOrder0CometricTracedCLM (I := I) g₀ g₁ x) e bse rfl hbse horth]
    have hcompb : ∀ (K : Fin 2 → Fin (Module.finrank ℝ E))
        (J : Fin 2 → Fin (Module.finrank ℝ E)),
        (fiberNormSqComponent (I := I) (M := M) g₀ x 2 2
          (show Tensor0SBundle.TensorRSSpace 2 2 I x from
            linearizedRicciConnectionDifferenceOrder0CometricTracedCLM (I := I) g₀ g₁ x)
      ...
      have hcomp : fiberNormSqComponent (I := I) (M := M) g₀ x 2 2
          ...
          Tensor0SBundle.Tensor0SSpace.toModel
            (ricciCometricFourTraceCLM (I := I) g₁ x
              (linearizedRicciConnectionDifferenceOrder0CLM (I := I) x A DAv
...
theorem exists_uniformBound_sqrt_riemannianFiberNormSq_linRicciConnectionDifferenceCoeff_of_jetEnvelope
  ...
  obtain ⟨C, hC0, hcore⟩ :=
    exists_uniformBound_riemannianFiberNormSq_linearizedRicciConnectionDifferenceFib_of_jetEnvelope (I := I)
      (M := M) g₀ hδ₀ (Csob * R)
      (by positivity)
  ...
  have hmain := hcore (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s)
