...
theorem linearizedRicciConnectionDifferenceOrder1CLM_field_contMDiff
...
noncomputable def linearizedRicciConnectionDifferenceOrder1CometricTracedCLM
    (g₀ g₁ : SmoothRiemannianMetric I M)
    (x : M) :
...
noncomputable def linearizedRicciConnectionDifferenceOrder0CometricTracedCLM
    (g₀ g₁ : SmoothRiemannianMetric I M)
    (x : M) :
...
theorem linearizedRicciConnectionDifferenceOrder1CometricTracedCLM_contMDiff
    ...
    ContMDiff I (I.prod 𝓘(ℝ, Tensor0SBundle.TensorRSModel 3 2 ℝ E)) ∞
      (fun x : M => TotalSpace.mk' (Tensor0SBundle.TensorRSModel 3 2 ℝ E)
        ...
        (linearizedRicciConnectionDifferenceOrder1CometricTracedCLM (I := I) g₀ g₁ x)) := by
  apply contMDiff_clm_section_of_pointwise (I := I) (M := M)
    ...
    (φ := fun x : M => linearizedRicciConnectionDifferenceOrder1CometricTracedCLM (I := I) g₀ g₁ x)
  intro Y
  ...
  have hCK := ricciCometricFourTraceCLM_field_contMDiff (I := I) g₁
    (fun x => linearizedRicciConnectionDifferenceOrder1CLM (I := I) x
...
theorem linearizedRicciConnectionDifferenceOrder0CometricTracedCLM_contMDiff
    ...
    ContMDiff I (I.prod 𝓘(ℝ, Tensor0SBundle.TensorRSModel 2 2 ℝ E)) ∞
      (fun x : M => TotalSpace.mk' (Tensor0SBundle.TensorRSModel 2 2 ℝ E)
        ...
        (linearizedRicciConnectionDifferenceOrder0CometricTracedCLM (I := I) g₀ g₁ x)) := by
  apply contMDiff_clm_section_of_pointwise (I := I) (M := M)
    (F₁ := Tensor0SBundle.Tensor0SModel 2 ℝ E)
    (V₁ := fun z : M => Tensor0SBundle.Tensor0SSpace 2 I z)
    (F₂ := Tensor0SBundle.Tensor0SModel 2 ℝ E)
    (V₂ := fun z : M => Tensor0SBundle.Tensor0SSpace 2 I z)
    (φ := fun x : M => linearizedRicciConnectionDifferenceOrder0CometricTracedCLM (I := I) g₀ g₁ x)
  intro Y
  have hE0 := linearizedRicciConnectionDifferenceOrder0CLM_field_contMDiff (I := I) g₀ g₁
    (fun x => Y x) Y.contMDiff
  have hCK := ricciCometricFourTraceCLM_field_contMDiff (I := I) g₁
    (fun x => linearizedRicciConnectionDifferenceOrder0CLM (I := I) x
      ((connectionDifferenceSection (I := I) g₁ g₀).toSection x)
      ((covGrad (I := I) (M := M) g₀ 1 2
          (connectionDifferenceSection (I := I) g₁ g₀)).toSection x) (Y x)) hE0
  refine hCK.congr (fun x => ?_)
  apply TotalSpace.mk_inj.mpr
  rw [linearizedRicciConnectionDifferenceOrder0CometricTracedCLM]
  rw [ContinuousLinearMap.comp_apply]

noncomputable def linearizedRicciConnectionDifferenceOrder1CoeffField
  ...
  toSection :=
    { toFun := fun x : M =>
      ...
      contMDiff_toFun := linearizedRicciConnectionDifferenceOrder1CometricTracedCLM_contMDiff (I := I) g₀ g₁ }
  hasCompactSupport := HasCompactSupport.of_compactSpace _

noncomputable def linearizedRicciConnectionDifferenceOrder0CoeffField
...
@[simp] theorem linearizedRicciConnectionDifferenceOrder0CoeffField_toSection
    ...
    (linearizedRicciConnectionDifferenceOrder0CoeffField (I := I) (M := M) g₀ g₁).toSection x =
      (show Tensor0SBundle.TensorRSSpace 2 2 I x from
        linearizedRicciConnectionDifferenceOrder0CometricTracedCLM (I := I) g₀ g₁ x) := rfl

def linearizedRicciConnectionDifferenceOrder1Coeff (g₀ : SmoothRiemannianMetric I M)
...
def linearizedRicciConnectionDifferenceOrder0Coeff (g₀ : SmoothRiemannianMetric I M)
    (T T' : SmoothCcTensor g₀ 0 2)
    {δ : ℝ} (hδ : metricCauchySchwarzBound (I := I) (M := M) g₀ (ccTensorBilinSymm (I := I) g₀ T) δ)
    {δ' : ℝ} (hδ' : metricCauchySchwarzBound (I := I) (M := M) g₀ (ccTensorBilinSymm (I := I) g₀ T')
      δ')
