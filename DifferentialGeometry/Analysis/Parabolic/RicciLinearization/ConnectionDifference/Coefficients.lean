...
lemma deriv0_eq_extDeriv (g : SmoothRiemannianMetric I M)
    ...
    Tensor0SNabla.tensor0Iso I M x
      ...
      mvfderiv (I := I) (Tensor0SNabla.scalarFn I M sc) x v := by
...
lemma curried2_toModel_eval
  ...
  rw [TensorMultilinear.tensor0S_curry_toModel_apply_tangent (I := I) (M := M)
    (T := W y) (v0 := Yf y)
    (vs := Fin.cons (tangentSpaceModelContinuousLinearEquiv (I := I) y (Zf y))
      (fun i : Fin 0 => i.elim0))]
...
lemma curried3_toModel_eval
  ...
  rw [TensorMultilinear.tensor0S_curry_toModel_apply_tangent (I := I) (M := M)
    ...
    (vs := Fin.cons (tangentSpaceModelContinuousLinearEquiv (I := I) y (Df y))
      (fun i : Fin 0 => i.elim0))]
  rw [Tensor0SNabla.curriedSection_apply]
  rw [TensorMultilinear.tensor0S_curry_toModel_apply_tangent (I := I) (M := M)
    (T := W y) (v0 := Bf y)]
...
omit [CompactSpace M] [I.Boundaryless] in
omit [NeZero (Module.finrank ℝ E)] [SigmaCompactSpace M] in
lemma tensor0SCovariantDerivative_two_toModel (g : SmoothRiemannianMetric I M)
  ...
  have hcons1 : (Fin.cons (Yf x : E) ![(Zf x : E)] : Fin 2 → E) =
      ![(Yf x : E), (Zf x : E)] := by
    funext i; fin_cases i <;> rfl
  have hcons2 : (Fin.cons
      (((LeviCivita (I := I) g).toFun (fun y => Yf y) x v : TangentSpace I x) : E)
      ![(Zf x : E)] : Fin 2 → E) =
      ![(((LeviCivita (I := I) g).toFun (fun y => Yf y) x v : TangentSpace I x) : E),
        (Zf x : E)] := by
    funext i; fin_cases i <;> rfl
  simp only [tangentSpaceModelContinuousLinearEquiv_apply] at hpeel1
  with_unfolding_all
    rw [hcons1, hcons2] at hpeel1
  have hcons3 : (Fin.cons (Zf x : E) (fun i : Fin 0 => i.elim0) : Fin 1 → E) =
      ![(Zf x : E)] := by
    funext i
    fin_cases i
    rfl
  simp only [tangentSpaceModelContinuousLinearEquiv_apply] at hpeel2
  rw [hcons3] at hpeel2
  have hcorr2 : Tensor0SBundle.Tensor0SSpace.toModel
      ...
      Tensor0SBundle.Tensor0SSpace.toModel (W x)
        ![Yf x, (LeviCivita (I := I) g).toFun (fun y => Zf y) x v] := by
    rw [Tensor0SNabla.curriedSection_apply]
    let z := (((LeviCivita (I := I) g).toFun (fun y => Zf y) x v :
      TangentSpace I x) : E)
    have h := TensorMultilinear.tensor0S_curry_toModel_apply_tangent (I := I) (M := M)
      (T := W x) (v0 := Yf x) (vs := Fin.cons z (fun i : Fin 0 => i.elim0))
    simp only [tangentSpaceModelContinuousLinearEquiv_apply] at h
    have htuple : (Fin.cons (Yf x : E) (Fin.cons z (fun i : Fin 0 => i.elim0)) :
        Fin 2 → E) = ![(Yf x : E), z] := by
      funext i
      fin_cases i <;> rfl
    rw [htuple] at h
  ...
  have hs := congrArg (fun z : ℝ => z - Tensor0SBundle.Tensor0SSpace.toModel (W x)
    ![(LeviCivita (I := I) g).toFun (fun y => Yf y) x v, Zf x]) hpeel2
  have hdegree : Tensor0SBundle.Tensor0SSpace.toModel
      (Tensor0SNabla.tensor0SCovariantDerivative I M (0 + 1) (LeviCivita (I := I) g)
        (fun y : M => Tensor0SNabla.curriedSection I M W y (Yf y)) x v) ![Zf x] =
      Tensor0SBundle.Tensor0SSpace.toModel
        (Tensor0SNabla.tensor0SCovariantDerivative I M 1 (LeviCivita (I := I) g)
          (fun y : M => Tensor0SNabla.curriedSection I M W y (Yf y)) x v) ![Zf x] := by
    rfl
  have hs' := (congrArg (fun z : ℝ => z - Tensor0SBundle.Tensor0SSpace.toModel (W x)
    ![(LeviCivita (I := I) g).toFun (fun y => Yf y) x v, Zf x]) hdegree.symm).trans hs
  rw [hcorr2, hd0] at hs'
  let A : ℝ := Tensor0SBundle.Tensor0SSpace.toModel
    (Tensor0SNabla.tensor0SCovariantDerivative I M 1 (LeviCivita (I := I) g)
      (fun y : M => Tensor0SNabla.curriedSection I M W y (Yf y)) x v) ![Zf x]
  let B : ℝ := Tensor0SBundle.Tensor0SSpace.toModel (W x)
    ![(LeviCivita (I := I) g).toFun (fun y => Yf y) x v, Zf x]
  let C : ℝ := Tensor0SBundle.Tensor0SSpace.toModel (W x)
    ![Yf x, (LeviCivita (I := I) g).toFun (fun y => Zf y) x v]
  let D : ℝ := mvfderiv (I := I) (Tensor0SNabla.scalarFn I M
    (fun y : M => Tensor0SNabla.curriedSection I M
      (fun z : M => Tensor0SNabla.curriedSection I M W z (Yf z)) y (Zf y))) x v
  change A - B = D - B - C
  have hsNamed : A - B = (D - C) - B := by
    dsimp only [A, B, C, D]
    exact hs'
  linarith
...
omit [CompactSpace M] [I.Boundaryless] in
omit [NeZero (Module.finrank ℝ E)] [SigmaCompactSpace M] in
lemma tensor0SCovariantDerivative_three_toModel (g : SmoothRiemannianMetric I M)
  ...
  have hcons1 : (Fin.cons (Bf x : E) ![(Cf x : E), (Df x : E)] : Fin 3 → E) =
      ![(Bf x : E), (Cf x : E), (Df x : E)] := by
    funext i; fin_cases i <;> rfl
  have hcons2 : (Fin.cons
      (((LeviCivita (I := I) g).toFun (fun y => Bf y) x v : TangentSpace I x) : E)
      ![(Cf x : E), (Df x : E)] : Fin 3 → E) =
      ![(((LeviCivita (I := I) g).toFun (fun y => Bf y) x v : TangentSpace I x) : E),
        (Cf x : E), (Df x : E)] := by
    funext i; fin_cases i <;> rfl
  simp only [tangentSpaceModelContinuousLinearEquiv_apply] at hpeel1
  with_unfolding_all
    rw [hcons1, hcons2] at hpeel1
  have hpeelrest := tensor0SCovariantDerivative_two_toModel (I := I) (M := M) g
  ...
  have hcorrC : Tensor0SBundle.Tensor0SSpace.toModel
    ...
    rw [Tensor0SNabla.curriedSection_apply]
    let c := (((LeviCivita (I := I) g).toFun (fun y => Cf y) x v :
      TangentSpace I x) : E)
    have h := TensorMultilinear.tensor0S_curry_toModel_apply_tangent (I := I) (M := M)
      (T := W x) (v0 := Bf x) (vs := ![c, (Df x : E)])
    simp only [tangentSpaceModelContinuousLinearEquiv_apply] at h
    have htuple : (Fin.cons (Bf x : E) ![c, (Df x : E)] : Fin 3 → E) =
        ![(Bf x : E), c, (Df x : E)] := by
      funext i
      fin_cases i <;> rfl
    rw [htuple] at h
  ...
  have hcorrD : Tensor0SBundle.Tensor0SSpace.toModel
      ...
      Tensor0SBundle.Tensor0SSpace.toModel (W x)
        ![Bf x, Cf x, (LeviCivita (I := I) g).toFun (fun y => Df y) x v] := by
    rw [Tensor0SNabla.curriedSection_apply]
    let d := (((LeviCivita (I := I) g).toFun (fun y => Df y) x v :
      TangentSpace I x) : E)
    have h := TensorMultilinear.tensor0S_curry_toModel_apply_tangent (I := I) (M := M)
      (T := W x) (v0 := Bf x) (vs := ![(Cf x : E), d])
    simp only [tangentSpaceModelContinuousLinearEquiv_apply] at h
    have htuple : (Fin.cons (Bf x : E) ![(Cf x : E), d] : Fin 3 → E) =
        ![(Bf x : E), (Cf x : E), d] := by
      funext i
      fin_cases i <;> rfl
    rw [htuple] at h
...
lemma bridge02_eval (gA gB : SmoothRiemannianMetric I M)
    ...
    Tensor0SBundle.Tensor0SSpace.toModel
      ...
      Tensor0SBundle.Tensor0SSpace.toModel
          ...
          ![p, q]
        - Tensor0SBundle.Tensor0SSpace.toModel (W x)
            ![PDE.DeTurck.connectionDifference (I := I) gA gB x p v, q]
        - Tensor0SBundle.Tensor0SSpace.toModel (W x)
            ![p, PDE.DeTurck.connectionDifference (I := I) gA gB x q v] := by
  classical
  ...
  have hA := tensor0SCovariantDerivative_two_toModel (I := I) (M := M) gA W hW Yf Zf v
  have hB := tensor0SCovariantDerivative_two_toModel (I := I) (M := M) gB W hW Yf Zf v
  have hcdY := PDE.DeTurck.connectionDifference_apply (I := I) gA gB (σ := fun y => Yf y)
    Yf.mdifferentiableAt v
  have hcdZ := PDE.DeTurck.connectionDifference_apply (I := I) gA gB (σ := fun y => Zf y)
  ...
  have hsplitY : Tensor0SBundle.Tensor0SSpace.toModel (W x)
      ...
      + Tensor0SBundle.Tensor0SSpace.toModel (W x)
        ![PDE.DeTurck.connectionDifference (I := I) gA gB x (Yf x) v, Zf x] := by
    with_unfolding_all
      change (W x) ![(LeviCivita (I := I) gA).toFun (fun y => Yf y) x v, Zf x] =
        (W x) ![(LeviCivita (I := I) gB).toFun (fun y => Yf y) x v, Zf x] +
          (W x) ![PDE.DeTurck.connectionDifference (I := I) gA gB x (Yf x) v, Zf x]
    have hAB : (LeviCivita (I := I) gA).toFun (fun y => Yf y) x v =
        (LeviCivita (I := I) gB).toFun (fun y => Yf y) x v
          + PDE.DeTurck.connectionDifference (I := I) gA gB x (Yf x) v := by
      rw [hcdY]; abel
    rw [hAB]
    ...
    with_unfolding_all
      rw [hupd, hupd ((LeviCivita (I := I) gB).toFun (fun y => Yf y) x v),
        hupd (PDE.DeTurck.connectionDifference (I := I) gA gB x (Yf x) v)]
  ...
  have hsplitZ : Tensor0SBundle.Tensor0SSpace.toModel (W x)
      ...
      + Tensor0SBundle.Tensor0SSpace.toModel (W x)
        ![Yf x, PDE.DeTurck.connectionDifference (I := I) gA gB x (Zf x) v] := by
    with_unfolding_all
      change (W x) ![Yf x, (LeviCivita (I := I) gA).toFun (fun y => Zf y) x v] =
        (W x) ![Yf x, (LeviCivita (I := I) gB).toFun (fun y => Zf y) x v] +
          (W x) ![Yf x, PDE.DeTurck.connectionDifference (I := I) gA gB x (Zf x) v]
    have hAB : (LeviCivita (I := I) gA).toFun (fun y => Zf y) x v =
        (LeviCivita (I := I) gB).toFun (fun y => Zf y) x v
          + PDE.DeTurck.connectionDifference (I := I) gA gB x (Zf x) v := by
      rw [hcdZ]; abel
    rw [hAB]
    have hupd : ∀ z : TangentSpace I x,
    ...
    with_unfolding_all
      rw [hupd, hupd ((LeviCivita (I := I) gB).toFun (fun y => Zf y) x v),
        hupd (PDE.DeTurck.connectionDifference (I := I) gA gB x (Zf x) v)]
...
lemma covDerivConnectionDifference_expand (g₁ g₀ : SmoothRiemannianMetric I M)
    (Xf Yf Zf : Cₛ^(⊤ : ℕ∞)⟮I; E, (TangentSpace I : M → Type _)⟯) (x : M) :
    (LeviCivita (I := I) g₀).toFun
        (fun y : M => PDE.DeTurck.connectionDifference (I := I) g₁ g₀ y (Yf y) (Zf y)) x (Xf x) =
      covDerivConnectionDifference (I := I) g₀ g₁ (fun y => Xf y) (fun y => Zf y) (fun y => Yf y) x
        + PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x (Yf x)
...
lemma covDerivConnectionDifference_symm23 (g₁ g₀ : SmoothRiemannianMetric I M)
    (Xf Yf Zf : Cₛ^(⊤ : ℕ∞)⟮I; E, (TangentSpace I : M → Type _)⟯) (x : M) :
    covDerivConnectionDifference (I := I) g₀ g₁ (fun y => Xf y) (fun y => Zf y) (fun y => Yf y) x =
      covDerivConnectionDifference (I := I) g₀ g₁ (fun y => Xf y) (fun y => Yf y) (fun y => Zf y) x := by
  have h1 : covDerivConnectionDifference (I := I) g₀ g₁
      (fun y => Xf y) (fun y => Zf y) (fun y => Yf y) x =
      (LeviCivita (I := I) g₀).toFun
          (fun y : M => PDE.DeTurck.connectionDifference (I := I) g₁ g₀ y (Yf y) (Zf y)) x (Xf x)
        - PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x (Yf x)
        ...
        - PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x
            ((LeviCivita (I := I) g₀).toFun (fun y => Yf y) x (Xf x)) (Zf x) := rfl
  have h2 : covDerivConnectionDifference (I := I) g₀ g₁
      (fun y => Xf y) (fun y => Yf y) (fun y => Zf y) x =
      (LeviCivita (I := I) g₀).toFun
          (fun y : M => PDE.DeTurck.connectionDifference (I := I) g₁ g₀ y (Zf y) (Yf y)) x (Xf x)
        - PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x (Zf x)
            ((LeviCivita (I := I) g₀).toFun (fun y => Yf y) x (Xf x))
        - PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x
            ((LeviCivita (I := I) g₀).toFun (fun y => Zf y) x (Xf x)) (Yf x) := rfl
  rw [h1, h2]
  have hsec : (fun y : M => PDE.DeTurck.connectionDifference (I := I) g₁ g₀ y (Zf y) (Yf y)) =
      (fun y : M => PDE.DeTurck.connectionDifference (I := I) g₁ g₀ y (Yf y) (Zf y)) := by
    funext y
    exact PDE.DeTurck.connectionDifference_symm (I := I) g₁ g₀ y (Zf y) (Yf y)
  rw [hsec]
  rw [PDE.DeTurck.connectionDifference_symm (I := I) g₁ g₀ x (Zf x)
    ((LeviCivita (I := I) g₀).toFun (fun y => Yf y) x (Xf x))]
  rw [PDE.DeTurck.connectionDifference_symm (I := I) g₁ g₀ x
    ((LeviCivita (I := I) g₀).toFun (fun y => Zf y) x (Xf x)) (Yf x)]
  abel

def connectionDifferenceVecField (g₁ g₀ : SmoothRiemannianMetric I M)
    (Yf Zf : Cₛ^(⊤ : ℕ∞)⟮I; E, (TangentSpace I : M → Type _)⟯) :
    Cₛ^(⊤ : ℕ∞)⟮I; E, (TangentSpace I : M → Type _)⟯ :=
  ⟨fun y : M => PDE.DeTurck.connectionDifference (I := I) g₁ g₀ y (Yf y) (Zf y),
    PDE.DeTurck.connectionDifference_contMDiff (I := I) g₁ g₀ Yf.contMDiff Zf.contMDiff⟩

omit [CompactSpace M] [I.Boundaryless] in
omit [NeZero (Module.finrank ℝ E)] in
omit [SigmaCompactSpace M] in
@[simp] lemma connectionDifferenceVecField_apply (g₁ g₀ : SmoothRiemannianMetric I M)
    (Yf Zf : Cₛ^(⊤ : ℕ∞)⟮I; E, (TangentSpace I : M → Type _)⟯) (y : M) :
    connectionDifferenceVecField (I := I) (M := M) g₁ g₀ Yf Zf y =
      PDE.DeTurck.connectionDifference (I := I) g₁ g₀ y (Yf y) (Zf y) := rfl
...
lemma connContrCLM_toModel_apply (m k : ℕ) (x : M)
  ...
  rw [hA, Tensor0SBundle.model_contract_trace_apply, sum_apply,
    sum_apply]
...
omit [NeZero (Module.finrank ℝ E)] in
omit [SigmaCompactSpace M] in
lemma connectionDifference_model_coeff (g₁ g₀ : SmoothRiemannianMetric I M) (x : M)
  ...
  have h1 : Tensor0SBundle.TensorRSSpace.toModel
      (show Tensor0SBundle.TensorRSSpace 1 2 I x from
        (connectionDifferenceSection (I := I) g₁ g₀).toSection x) β =
      Tensor0SBundle.Tensor0SSpace.toModel
  ...
  rw [show (Tensor0SBundle.Tensor0SSpace.ofModel (I := I) (x := x) β)
      (fun _ : Fin 1 => PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x ((w 0 : E)) ((w 1 : E))) =
      Tensor0SBundle.Tensor0SSpace.toModel
        (Tensor0SBundle.Tensor0SSpace.ofModel (I := I) (x := x) β)
        (fun _ : Fin 1 => ((PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x
          ((w 0 : E)) ((w 1 : E)) : TangentSpace I x) : E)) from rfl]
  rw [Tensor0SBundle.Tensor0SSpace.toModel_ofModel]
  let z : TangentSpace I x := PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x
...
lemma connContr21_insert (g₁ g₀ : SmoothRiemannianMetric I M) (x : M)
    ...
    Tensor0SBundle.Tensor0SSpace.toModel
        (connContrCLM (I := I) 2 1 x
          ((connectionDifferenceSection (I := I) g₁ g₀).toSection x) D) u =
      Tensor0SBundle.Tensor0SSpace.toModel D
        ![((PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x ((u 2 : E)) ((u 3 : E)) :
  ...
  have hterm : ∀ i : Fin (Module.finrank ℝ E),
    ...
    intro i
    rw [consCast21, consNat21, connectionDifference_model_coeff (I := I) (M := M) g₁ g₀ x i (![u 2, u 3])]
    rfl
  rw [Finset.sum_congr rfl (fun i _ => hterm i)]
  rw [sum_cons_cDual_collapse (I := I) (M := M) D ![u 0, u 1]
    ((PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x ((u 2 : E)) ((u 3 : E)) : TangentSpace I x) : E)]
  rfl
      ...
      Tensor0SBundle.Tensor0SSpace.toModel D
        ...
        ((Module.finBasis ℝ E).cDualBasis i)
          ((PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x ((u 1 : E)) ((u 2 : E)) :
            TangentSpace I x) : E) := by
    intro i
    rw [consCast11, consNat11, connectionDifference_model_coeff (I := I) (M := M) g₁ g₀ x i (![u 1, u 2])]
    rfl
  rw [Finset.sum_congr rfl (fun i _ => hterm i)]
  rw [sum_cons_cDual_collapse (I := I) (M := M) D ![u 0]
    ((PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x ((u 1 : E)) ((u 2 : E)) : TangentSpace I x) : E)]
  rfl
...
lemma rs13ContrVec_covGrad_eq (g₁ g₀ : SmoothRiemannianMetric I M)
  ...
  have hcoeff : ∀ i : Fin (Module.finrank ℝ E),
    ...
    have h1 : Tensor0SBundle.TensorRSSpace.toModel
        (show Tensor0SBundle.TensorRSSpace 1 3 I x from
          (covGrad (I := I) (M := M) g₀ 1 2 (connectionDifferenceSection (I := I) g₁ g₀)).toSection x) β =
        Tensor0SBundle.Tensor0SSpace.toModel
      ...
      rw [toModel_tensorRS_apply (I := I) 1 3 x _
        ...
        Tensor0SBundle.Tensor0SSpace.toModel_ofModel]
    rw [h1]
    have h2 := connectionDifferenceSection_covGrad_eq_covDerivConnectionDifference (I := I) (M := M) g₁ g₀ om Xf Yf Zf x
    simp only [tangentSpaceModelContinuousLinearEquiv_apply] at h2
    with_unfolding_all
      rw [show (Fin.cons (Xf x : E) (Fin.cons (Yf x : E) ![(Zf x : E)]) : Fin 3 → E) =
          ![(Xf x : E), (Yf x : E), (Zf x : E)] from by
        funext j
    ...
    rw [h2, homx]
    rw [show (Tensor0SBundle.Tensor0SSpace.ofModel (I := I) (x := x) β)
        (fun _ : Fin 1 => covDerivConnectionDifference (I := I) g₀ g₁
          (fun y => Xf y) (fun y => Zf y) (fun y => Yf y) x) =
        Tensor0SBundle.Tensor0SSpace.toModel
          (Tensor0SBundle.Tensor0SSpace.ofModel (I := I) (x := x) β)
          (fun _ : Fin 1 => ((covDerivConnectionDifference (I := I) g₀ g₁
...
omit [NeZero (Module.finrank ℝ E)] in
omit [SigmaCompactSpace M] in
lemma connContr21_insert_apply (g₁ g₀ : SmoothRiemannianMetric I M) (x : M)
    (D : Tensor0SBundle.Tensor0SSpace 3 I x) (p q r s : TangentSpace I x) :
    Tensor0SBundle.Tensor0SSpace.toModel
        (connContrCLM (I := I) 2 1 x
          ((connectionDifferenceSection (I := I) g₁ g₀).toSection x) D) ![p, q, r, s] =
      Tensor0SBundle.Tensor0SSpace.toModel D
        ![PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x r s, p, q] :=
  connContr21_insert (I := I) (M := M) g₁ g₀ x D ![p, q, r, s]

omit [I.Boundaryless] in
omit [NeZero (Module.finrank ℝ E)] in
omit [SigmaCompactSpace M] in
lemma connContr11_insert_apply (g₁ g₀ : SmoothRiemannianMetric I M) (x : M)
    (D : Tensor0SBundle.Tensor0SSpace 2 I x) (p q r : TangentSpace I x) :
    Tensor0SBundle.Tensor0SSpace.toModel
        (connContrCLM (I := I) 1 1 x
          ((connectionDifferenceSection (I := I) g₁ g₀).toSection x) D) ![p, q, r] =
      Tensor0SBundle.Tensor0SSpace.toModel D
        ![PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x q r, p] :=
  connContr11_insert (I := I) (M := M) g₁ g₀ x D ![p, q, r]

omit [NeZero (Module.finrank ℝ E)] [CompactSpace M] [I.Boundaryless] [BoundarylessManifold I M]
    [T2Space M] [SigmaCompactSpace M] in
lemma connContr12_insert_apply (x : M) (B : Tensor0SBundle.TensorRSSpace 1 3 I x)
...
private lemma order1CLM_toModel_eval (g₁ g₀ : SmoothRiemannianMetric I M) (x : M)
    ...
    Tensor0SBundle.Tensor0SSpace.toModel
        (linearizedRicciConnectionDifferenceOrder1CLM (I := I) x
          ((connectionDifferenceSection (I := I) g₁ g₀).toSection x) Z) ![a, b, c, d] =
      -(Tensor0SBundle.Tensor0SSpace.toModel Z
          ![a, PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x b c, d]
        + Tensor0SBundle.Tensor0SSpace.toModel Z
            ![a, c, PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x b d]
        + Tensor0SBundle.Tensor0SSpace.toModel Z
            ![PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x a b, c, d]
        + Tensor0SBundle.Tensor0SSpace.toModel Z
            ![b, PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x a c, d]
        + Tensor0SBundle.Tensor0SSpace.toModel Z
            ![b, c, PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x a d]) := by
  rw [linearizedRicciConnectionDifferenceOrder1CLM]
  rw [neg_apply, add_apply,
    add_apply, add_apply,
    add_apply]
  ...
  rw [neg_apply, add_apply, add_apply, add_apply, add_apply]
  rw [ContinuousLinearMap.comp_apply, ContinuousLinearMap.comp_apply,
    ContinuousLinearMap.comp_apply, ContinuousLinearMap.comp_apply]
  rw [slotPerm4_0312_toModel, slotPerm4_0213_toModel, slotPerm4_2301_toModel,
    slotPerm4_1302_toModel, slotPerm4_1203_toModel]
  rw [connContr21_insert_apply, connContr21_insert_apply, connContr21_insert_apply,
    connContr21_insert_apply, connContr21_insert_apply]
...
private lemma order0CLM_toModel_eval (g₁ g₀ : SmoothRiemannianMetric I M) (x : M)
    ...
    Tensor0SBundle.Tensor0SSpace.toModel
      ...
      (Tensor0SBundle.Tensor0SSpace.toModel hT
          ![PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x
              (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x a b) c, d]
        + Tensor0SBundle.Tensor0SSpace.toModel hT
            ![c, PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x
              (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x a b) d]
        + Tensor0SBundle.Tensor0SSpace.toModel hT
            ![PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x b
              (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x a c), d]
        + Tensor0SBundle.Tensor0SSpace.toModel hT
            ![PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x a c,
              PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x b d]
        + Tensor0SBundle.Tensor0SSpace.toModel hT
            ![PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x b c,
              PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x a d]
        + Tensor0SBundle.Tensor0SSpace.toModel hT
            ![c, PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x b
              (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x a d)])
      - Tensor0SBundle.Tensor0SSpace.toModel hT
          ![rs13ContrVec (I := I) (M := M) x
            ((covGrad (I := I) (M := M) g₀ 1 2 (connectionDifferenceSection (I := I) g₁ g₀)).toSection x)
  ...
  rw [connContr21_insert_apply, connContr21_insert_apply, connContr21_insert_apply,
    connContr21_insert_apply, connContr21_insert_apply, connContr21_insert_apply]
  rw [connContr12_insert_apply, connContr12_insert_apply]
  rw [slotPerm3_102_toModel, slotPerm3_102_toModel, slotPerm3_120_toModel,
    slotPerm3_120_toModel]
  rw [connContr11_insert_apply, connContr11_insert_apply, connContr11_insert_apply,
    connContr11_insert_apply, connContr11_insert_apply, connContr11_insert_apply]
  rw [slotPerm2_10_toModel, slotPerm2_10_toModel, slotPerm2_10_toModel]
  congr 1
  exact slotPerm2_10_toModel (I := I) (M := M) x hT _ _
...
lemma covGradUnit_toModel_eval (g : SmoothRiemannianMetric I M) (n : ℕ)
  ...
  change Tensor0SBundle.Tensor0SSpace.eval
      ((show Tensor0SBundle.Tensor0SSpace 0 I y →L[ℝ]
          Tensor0SBundle.Tensor0SSpace (n + 1) I y from
        (covGrad (I := I) (M := M) g 0 n W).toSection y)
        (unitZeroSec (I := I) (M := M) y)) (Fin.cons p w) =
    Tensor0SBundle.Tensor0SSpace.eval
      (Tensor0SNabla.tensor0SCovariantDerivative I M n (LeviCivita (I := I) g)
        (fun z : M =>
          (show Tensor0SBundle.Tensor0SSpace 0 I z →L[ℝ]
              Tensor0SBundle.Tensor0SSpace n I z from
            W.toSection z) (unitZeroSec (I := I) (M := M) z)) y p) w
  rw [covGrad_toSection_apply_natural (I := I) (M := M) g 0 n W y
    (unitZeroSec (I := I) (M := M) y) (Fin.cons p w)]
  congr 1
...
private lemma kOneSec_tsmdiffAt (g₀ : SmoothRiemannianMetric I M)
  ...
  unitEval_tensorSectionMDiffAt (I := I) (M := M) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) 3
    (covGrad (I := I) (M := M) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) 0 2
    ...
    Tensor0SBundle.Tensor0SSpace.toModel
      ...
      Tensor0SBundle.Tensor0SSpace.toModel
        ...
        - Tensor0SBundle.Tensor0SSpace.toModel (symmVelocityDiffSec (I := I) (M := M) g₀ T T' y)
            ![PDE.DeTurck.connectionDifference (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ y q p, r]
        - Tensor0SBundle.Tensor0SSpace.toModel (symmVelocityDiffSec (I := I) (M := M) g₀ T T' y)
            ![q, PDE.DeTurck.connectionDifference (I := I)
              (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ y r p] := by
  rw [kOneSec_eval (I := I) (M := M) g₀ T T' hδ hδ' s y p q r,
    kZeroSec_eval (I := I) (M := M) g₀ T T' y p q r]
  exact bridge02_eval (I := I) (M := M) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀
    (symmVelocityDiffSec (I := I) (M := M) g₀ T T')
    (hUnitSec_tsmdiffAt (I := I) (M := M) g₀ T T' y) p q r

omit [NeZero (Module.finrank ℝ E)] in
omit [SigmaCompactSpace M] in
private lemma velocityFibre_toModel_eval (g₀ : SmoothRiemannianMetric I M)
    (T T' : SmoothCcTensor g₀ 0 2)
    {δ : ℝ} (hδ : metricCauchySchwarzBound (I := I) (M := M) g₀ (ccTensorBilinSymm (I := I) g₀ T) δ)
    {δ' : ℝ} (hδ' : metricCauchySchwarzBound (I := I) (M := M) g₀ (ccTensorBilinSymm (I := I) g₀ T')
      δ')
  ...
  have hm : m = Fin.cons (m 0) ![m 1, m 2, m 3] := by
    ...
    fin_cases j <;> rfl
  have h := covGradUnit_toModel_eval (I := I) (M := M)
    (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) 3
    (covGrad (I := I) (M := M) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) 0 2
...
lemma toModel3_add_slot0 {x : M} (T : Tensor0SBundle.Tensor0SSpace 3 I x)
  ...
  with_unfolding_all
    change T ![p + p', q, r] = T ![p, q, r] + T ![p', q, r]
  have hupd : ∀ z : TangentSpace I x,
...
lemma toModel3_add_slot1 {x : M} (T : Tensor0SBundle.Tensor0SSpace 3 I x)
    ...
    Tensor0SBundle.Tensor0SSpace.toModel T ![p, q + q', r] =
      Tensor0SBundle.Tensor0SSpace.toModel T ![p, q, r]
        + Tensor0SBundle.Tensor0SSpace.toModel T ![p, q', r] := by
  with_unfolding_all
    change T ![p, q + q', r] = T ![p, q, r] + T ![p, q', r]
  have hupd : ∀ z : TangentSpace I x,
...
lemma toModel3_add_slot2 {x : M} (T : Tensor0SBundle.Tensor0SSpace 3 I x)
    ...
    Tensor0SBundle.Tensor0SSpace.toModel T ![p, q, r + r'] =
      Tensor0SBundle.Tensor0SSpace.toModel T ![p, q, r]
        + Tensor0SBundle.Tensor0SSpace.toModel T ![p, q, r'] := by
  with_unfolding_all
    change T ![p, q, r + r'] = T ![p, q, r] + T ![p, q, r']
  have hupd : ∀ z : TangentSpace I x,
...
private theorem kOneSec_deriv_eq_covariantJet_kernel (g₀ : SmoothRiemannianMetric I M)
    ...
    Tensor0SBundle.Tensor0SSpace.toModel
      ...
      Tensor0SBundle.Tensor0SSpace.toModel
        ...
        + Tensor0SBundle.Tensor0SSpace.toModel
            (linearizedRicciConnectionDifferenceOrder1CLM (I := I) x
              ((connectionDifferenceSection (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀).toSection x)
              (symmVelocityDiffCovGradBaseSec (I := I) (M := M) g₀ T T' x)) ![a, b, c, d]
        + Tensor0SBundle.Tensor0SSpace.toModel
            (linearizedRicciConnectionDifferenceOrder0CLM (I := I) x
              ((connectionDifferenceSection (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀).toSection x)
              ((covGrad (I := I) (M := M) g₀ 1 2
                (connectionDifferenceSection (I := I)
                  (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀)).toSection x)
  ...
  rw [← hAx, ← hBx, ← hCx, ← hDx]
  have hL := tensor0SCovariantDerivative_three_toModel (I := I) (M := M) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s)
    (symmVelocityDiffCovGradRealizedSec (I := I) (M := M) g₀ T T' hδ hδ' s)
    (kOneSec_tsmdiffAt (I := I) (M := M) g₀ T T' hδ hδ' s x) Bf Cf Df (Af x)
  have hR := tensor0SCovariantDerivative_three_toModel (I := I) (M := M) g₀
    (symmVelocityDiffCovGradBaseSec (I := I) (M := M) g₀ T T')
    (kZeroSec_tsmdiffAt (I := I) (M := M) g₀ T T' x) Bf Cf Df (Af x)
  have hDB : (LeviCivita (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s)).toFun
      (fun y => Bf y) x (Af x) =
      (LeviCivita (I := I) g₀).toFun (fun y => Bf y) x (Af x)
        + PDE.DeTurck.connectionDifference (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ x
            (Bf x) (Af x) := by
    rw [PDE.DeTurck.connectionDifference_apply (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀
      (σ := fun y => Bf y) Bf.mdifferentiableAt (Af x)]
    abel
  have hDC : (LeviCivita (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s)).toFun
      (fun y => Cf y) x (Af x) =
    ...
    abel
  have hDD : (LeviCivita (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s)).toFun
      (fun y => Df y) x (Af x) =
  ...
  rw [kSec_bridge (I := I) (M := M) g₀ T T' hδ hδ' s x
    ...
    kSec_bridge (I := I) (M := M) g₀ T T' hδ hδ' s x
      (Bf x) (PDE.DeTurck.connectionDifference (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ x
        (Cf x) (Af x)) (Df x),
    kSec_bridge (I := I) (M := M) g₀ T T' hδ hδ' s x
      (Bf x) (Cf x) ((LeviCivita (I := I) g₀).toFun (fun y => Df y) x (Af x)),
    kSec_bridge (I := I) (M := M) g₀ T T' hδ hδ' s x
      (Bf x) (Cf x) (PDE.DeTurck.connectionDifference (I := I)
        (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ x (Df x) (Af x))] at hL
  have hexpC1 := tensor0SCovariantDerivative_two_toModel (I := I) (M := M) g₀
    (symmVelocityDiffSec (I := I) (M := M) g₀ T T')
    (hUnitSec_tsmdiffAt (I := I) (M := M) g₀ T T' x)
    (connectionDifferenceVecField (I := I) (M := M) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Cf Bf)
    Df (Af x)
  have hexpC2 := tensor0SCovariantDerivative_two_toModel (I := I) (M := M) g₀
    (symmVelocityDiffSec (I := I) (M := M) g₀ T T')
    (hUnitSec_tsmdiffAt (I := I) (M := M) g₀ T T' x) Cf
    (connectionDifferenceVecField (I := I) (M := M) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Df Bf)
    (Af x)
  rw [show (fun y : M =>
      (connectionDifferenceVecField (I := I) (M := M)
        (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Cf Bf) y) =
      (fun y : M => PDE.DeTurck.connectionDifference (I := I)
        (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ y (Cf y) (Bf y)) from rfl] at hexpC1
  rw [covDerivConnectionDifference_expand (I := I) (M := M)
    (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Af Cf Bf x] at hexpC1
  rw [show ((connectionDifferenceVecField (I := I) (M := M)
      (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Cf Bf) x : TangentSpace I x) =
      PDE.DeTurck.connectionDifference (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ x
  ...
  rw [show (fun y : M =>
      (connectionDifferenceVecField (I := I) (M := M)
        (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Df Bf) y) =
      (fun y : M => PDE.DeTurck.connectionDifference (I := I)
        (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ y (Df y) (Bf y)) from rfl] at hexpC2
  rw [covDerivConnectionDifference_expand (I := I) (M := M)
    (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Af Df Bf x] at hexpC2
  rw [show ((connectionDifferenceVecField (I := I) (M := M)
      (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Df Bf) x : TangentSpace I x) =
      PDE.DeTurck.connectionDifference (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ x
  ...
  rw [← kZeroSec_eval (I := I) (M := M) g₀ T T' x (Af x) (Cf x)
    (PDE.DeTurck.connectionDifference (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ x
      (Df x) (Bf x))] at hexpC2
  rw [PDE.DeTurck.connectionDifference_symm (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ x (Bf x) (Af x)]
    at hL
  rw [PDE.DeTurck.connectionDifference_symm (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ x (Cf x) (Af x)]
    at hL
  rw [PDE.DeTurck.connectionDifference_symm (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ x (Df x) (Af x)]
    at hL
  rw [PDE.DeTurck.connectionDifference_symm (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀
    x (Cf x) (Bf x)] at hL
  rw [PDE.DeTurck.connectionDifference_symm (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀
  ...
  rw [PDE.DeTurck.connectionDifference_symm (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ x (Cf x)
    (PDE.DeTurck.connectionDifference (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ x
      (Af x) (Bf x))] at hL
  rw [PDE.DeTurck.connectionDifference_symm (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ x (Df x)
    (PDE.DeTurck.connectionDifference (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ x
      (Af x) (Bf x))] at hL
  rw [PDE.DeTurck.connectionDifference_symm (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ x
    (PDE.DeTurck.connectionDifference (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ x
      (Af x) (Cf x)) (Bf x)] at hL
  rw [PDE.DeTurck.connectionDifference_symm (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ x
    (PDE.DeTurck.connectionDifference (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ x
      (Af x) (Df x)) (Bf x)] at hL
  rw [covDerivConnectionDifference_symm23 (I := I) (M := M)
    (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Af Cf Bf x] at hexpC1
  rw [covDerivConnectionDifference_symm23 (I := I) (M := M)
    (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Af Df Bf x] at hexpC2
  have hscal : ∀ y : M,
        ...
        - Tensor0SNabla.scalarFn I M
            (fun y' : M => Tensor0SNabla.curriedSection I M
              (fun z : M => Tensor0SNabla.curriedSection I M
                ...
                ((connectionDifferenceVecField (I := I) (M := M)
                  (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Cf Bf) z)) y' (Df y')) y
        - Tensor0SNabla.scalarFn I M
            (fun y' : M => Tensor0SNabla.curriedSection I M
              (fun z : M => Tensor0SNabla.curriedSection I M
                (symmVelocityDiffSec (I := I) (M := M) g₀ T T') z (Cf z)) y'
              ((connectionDifferenceVecField (I := I) (M := M)
                (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Df Bf) y')) y := by
    intro y
    ...
    rw [curried2_toModel_eval (I := I) (M := M)
      (symmVelocityDiffSec (I := I) (M := M) g₀ T T')
      (connectionDifferenceVecField (I := I) (M := M)
        (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Cf Bf) Df y]
    rw [curried2_toModel_eval (I := I) (M := M)
      (symmVelocityDiffSec (I := I) (M := M) g₀ T T') Cf
      (connectionDifferenceVecField (I := I) (M := M)
        (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Df Bf) y]
    rw [show ((connectionDifferenceVecField (I := I) (M := M)
        (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Cf Bf) y : TangentSpace I y) =
        PDE.DeTurck.connectionDifference (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ y
          (Cf y) (Bf y) from rfl]
    rw [show ((connectionDifferenceVecField (I := I) (M := M)
        (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Df Bf) y : TangentSpace I y) =
        PDE.DeTurck.connectionDifference (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ y
          (Df y) (Bf y) from rfl]
      ...
      (Tensor0SNabla.scalarFn I M
        (fun y' : M => Tensor0SNabla.curriedSection I M
          (fun z : M => Tensor0SNabla.curriedSection I M
            (symmVelocityDiffSec (I := I) (M := M) g₀ T T') z
            ((connectionDifferenceVecField (I := I) (M := M)
              (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Cf Bf) z)) y' (Df y'))) x :=
    (Tensor0SNabla.mdifferentiableAt_scalarFn_iff_section I M _).mpr
      (curried_tsmdiffAt (I := I) (M := M) 0 _
        (curried_tsmdiffAt (I := I) (M := M) 1 _
          (hUnitSec_tsmdiffAt (I := I) (M := M) g₀ T T' x)
          (connectionDifferenceVecField (I := I) (M := M)
            (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Cf Bf)) Df)
      ...
      (Tensor0SNabla.scalarFn I M
        (fun y' : M => Tensor0SNabla.curriedSection I M
          (fun z : M => Tensor0SNabla.curriedSection I M
            (symmVelocityDiffSec (I := I) (M := M) g₀ T T') z (Cf z)) y'
          ((connectionDifferenceVecField (I := I) (M := M)
            (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Df Bf) y'))) x :=
    (Tensor0SNabla.mdifferentiableAt_scalarFn_iff_section I M _).mpr
      (curried_tsmdiffAt (I := I) (M := M) 0 _
        (curried_tsmdiffAt (I := I) (M := M) 1 _
          (hUnitSec_tsmdiffAt (I := I) (M := M) g₀ T T' x) Cf)
        (connectionDifferenceVecField (I := I) (M := M)
          (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Df Bf))
  have hExt : mvfderiv (I := I)
      (Tensor0SNabla.scalarFn I M
        (fun y' : M => Tensor0SNabla.curriedSection I M
          (fun z : M => Tensor0SNabla.curriedSection I M
            (fun u : M => Tensor0SNabla.curriedSection I M
          ...
          (Tensor0SNabla.scalarFn I M
            (fun y' : M => Tensor0SNabla.curriedSection I M
              (fun z : M => Tensor0SNabla.curriedSection I M
                (symmVelocityDiffSec (I := I) (M := M) g₀ T T') z
                ((connectionDifferenceVecField (I := I) (M := M)
                  (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Cf Bf) z)) y' (Df y'))) x (Af x)
      - mvfderiv (I := I)
          (Tensor0SNabla.scalarFn I M
            (fun y' : M => Tensor0SNabla.curriedSection I M
              (fun z : M => Tensor0SNabla.curriedSection I M
                (symmVelocityDiffSec (I := I) (M := M) g₀ T T') z (Cf z)) y'
              ((connectionDifferenceVecField (I := I) (M := M)
                (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Df Bf) y'))) x (Af x) := by
          ...
          - Tensor0SNabla.scalarFn I M
              (fun y' : M => Tensor0SNabla.curriedSection I M
                (fun z : M => Tensor0SNabla.curriedSection I M
                  (symmVelocityDiffSec (I := I) (M := M) g₀ T T') z
                  ((connectionDifferenceVecField (I := I) (M := M)
                    (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Cf Bf) z)) y' (Df y')))
          - Tensor0SNabla.scalarFn I M
              (fun y' : M => Tensor0SNabla.curriedSection I M
                (fun z : M => Tensor0SNabla.curriedSection I M
                  (symmVelocityDiffSec (I := I) (M := M) g₀ T T') z (Cf z)) y'
                ((connectionDifferenceVecField (I := I) (M := M)
                  (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Df Bf) y')) := by
      funext y
      have h := hscal y
      rw [h]
      rfl
    rw [hfx]
    rw [_root_.mvfderiv_sub (I := I) (hMD0.sub hMDC1) hMDC2,
      _root_.mvfderiv_sub (I := I) hMD0 hMDC1]
  ...
  rw [rs13ContrVec_covGrad_eq (I := I) (M := M)
      (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Af Bf Cf x,
    rs13ContrVec_covGrad_eq (I := I) (M := M)
      (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀ Af Bf Df x] at hE0
...
private lemma lichnerowiczFib_toModel_eq_fourTrace (g₀ : SmoothRiemannianMetric I M)
  ...
  have hsplit : (show Tensor0SBundle.Tensor0SSpace 4 I x →L[ℝ]
    ...
    have hts : ((ricciDeTurckPrincipalCoefficient (I := I) (M := M) g₀
          (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s)
        - (1 / 2 : ℝ) • traceHessianCoeff (I := I) (M := M) g₀
  ...
  with_unfolding_all
    rw [sub_apply, smul_apply, smul_eq_mul]
  rw [show (show Tensor0SBundle.Tensor0SSpace 4 I x →L[ℝ]
        Tensor0SBundle.Tensor0SSpace 2 I x from
      (ricciDeTurckPrincipalCoefficient (I := I) (M := M) g₀
        (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s)).toSection x) P =
      ricciDeTurckPrincipalCoefficientAtPoint (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) x P from
        rfl]
  rw [ricciDeTurckPrincipalCoefficientFiber_toModel,
    ricciPrincipalCoefficientDoubleTraceModel_apply (E := E)
      (cometricLmodel (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) x)]
  rw [show (show Tensor0SBundle.Tensor0SSpace 4 I x →L[ℝ]
        Tensor0SBundle.Tensor0SSpace 2 I x from
      (traceHessianCoeff (I := I) (M := M) g₀
        (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s)).toSection x) P =
      traceHessianFib (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) x P from rfl]
  rw [traceHessianFib_toModel,
    modelDoubleTrace_apply (E := E) 2
      (cometricLmodel (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) x)]
  rw [ricciCometricFourTraceCLM]
  rw [smul_apply, Tensor0SBundle.Tensor0SSpace.toModel_smul,
  ...
  rw [Tensor0SBundle.Tensor0SSpace.toModel_sub, Tensor0SBundle.Tensor0SSpace.toModel_sub,
    Tensor0SBundle.Tensor0SSpace.toModel_add]
  rw [sub_apply, sub_apply, add_apply]
...
private theorem lichnerowicz_velocitySecondCovGrad_eq_covariantJet_symm
        ...
        + unitModel (I := I) (M := M) g₀ 2
            (operatorFieldApply (I := I) (M := M) g₀ 4 2
              ...
              (iteratedCovGrad (I := I) g₀ 0 2 2 (ccTensor02Symm (I := I) (M := M) g₀ (T - T')))) x
                v := by
  classical
  have hVelocity : (show Tensor0SBundle.Tensor0SSpace 0 I x →L[ℝ]
        Tensor0SBundle.Tensor0SSpace 4 I x from
    ...
    intro m
    have h2 := velocityFibre_toModel_eval (I := I) (M := M) g₀ T T' hδ hδ' s x m
    ...
    rw [← h4, add_apply, add_apply]
    congr 1
    · congr 1
      change (linearizedRicciConnectionDifferenceOrder1CLM (I := I) x
          ((connectionDifferenceSection (I := I)
            (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀).toSection x)
          (symmVelocityDiffCovGradBaseSec (I := I) (M := M) g₀ T T' x))
          ![m 0, m 1, m 2, m 3] = _
      exact congrArg _ hm4.symm
    · change (linearizedRicciConnectionDifferenceOrder0CLM (I := I) x
          ((connectionDifferenceSection (I := I)
            (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀).toSection x)
          ((covGrad (I := I) (M := M) g₀ 1 2
            (connectionDifferenceSection (I := I)
              (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀)).toSection x)
          (symmVelocityDiffSec (I := I) (M := M) g₀ T T' x))
          ![m 0, m 1, m 2, m 3] = _
      exact congrArg _ hm4.symm
  ...
  with_unfolding_all
    rw [hVelocity, map_add, map_add, Tensor0SBundle.Tensor0SSpace.toModel_add,
      Tensor0SBundle.Tensor0SSpace.toModel_add, add_apply, add_apply]
  ...
  have horder1 :
    ...
    rw [linearizedRicciConnectionDifferenceOrder1Coeff,
      linearizedRicciConnectionDifferenceOrder1CoeffField_toSection]
  ...
  have horder0 :
    ...
    rw [unitModel, operatorFieldApplication_toSection, ContinuousLinearMap.comp_apply]
    rw [linearizedRicciConnectionDifferenceOrder0Coeff,
      linearizedRicciConnectionDifferenceOrder0CoeffField_toSection]
...
theorem linearizedRicciAt_eq_covariantJet_connectionDifferenceCoeff (g₀ : SmoothRiemannianMetric I M)
  ...
  let A := operatorFieldApply (I := I) (M := M) g₀ 2 2
    (linearizedRicciConnectionDifferenceOrder0Coeff (I := I) g₀ T T' hδ hδ' s)
    (iteratedCovGrad (I := I) g₀ 0 2 0 (T - T'))
  let B := operatorFieldApply (I := I) (M := M) g₀ 3 2
    (linearizedRicciConnectionDifferenceOrder1Coeff (I := I) g₀ T T' hδ hδ' s)
    (iteratedCovGrad (I := I) g₀ 0 2 1 (T - T'))
  ...
  change linearizedRicciAt (I := I) g₀ T T' hδ_lt hδ hδ'_lt hδ' x (v 0) (v 1) s =
    unitModel (I := I) (M := M) g₀ 2 (A + B + C) x vE
  have hAB : unitModel (I := I) (M := M) g₀ 2 (A + B) x vE =
      unitModel (I := I) (M := M) g₀ 2 A x vE +
        unitModel (I := I) (M := M) g₀ 2 B x vE := by
    simp only [unitModel]
    rw [SmoothCcTensor.toSection_add, ContMDiffSection.coe_add, Pi.add_apply,
      add_apply, Tensor0SBundle.Tensor0SSpace.toModel_add, add_apply]
  have hABC : unitModel (I := I) (M := M) g₀ 2 (A + B + C) x vE =
      unitModel (I := I) (M := M) g₀ 2 (A + B) x vE +
        unitModel (I := I) (M := M) g₀ 2 C x vE := by
    simp only [unitModel]
    rw [SmoothCcTensor.toSection_add, ContMDiffSection.coe_add, Pi.add_apply,
      add_apply, Tensor0SBundle.Tensor0SSpace.toModel_add, add_apply]
  rw [hABC, hAB]
  dsimp only [A, B, C]
  rw [← hcollapse]
  have hlin := linearizedRicciAt_eq_lichnerowicz_velocitySecondCovGrad (I := I) g₀ T T'
