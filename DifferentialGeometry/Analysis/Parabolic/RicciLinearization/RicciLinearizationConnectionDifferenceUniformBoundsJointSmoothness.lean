...
private theorem tensorProdField_jointContMDiffOn (m k : ℕ) {S : Set ℝ}
    ...
    ContMDiffOn (I.prod 𝓘(ℝ, ℝ)) (I.prod 𝓘(ℝ, Tensor0SBundle.Tensor0SModel (m + k) ℝ E)) ∞
      ...
      ((Set.univ : Set M) ×ˢ S) := by
  let _ := Tensor0SBundle.tensor0SBundleTopology (𝕜 := ℝ) (E := E) (H := H) (I := I) (M := M) m
  let _ := Tensor0SBundle.tensor0SBundleTopology (𝕜 := ℝ) (E := E) (H := H) (I := I) (M := M) k
...
private theorem connectionDifferenceSection_metricPerturbationPath_jointContMDiffOn
  ...
  let φ : ∀ p : M × ℝ,
    ...
    fun p =>
      (show Tensor0SBundle.Tensor0SSpace 1 I p.1 →L[ℝ]
        ...
        raisedKoszulFib (I := I) g₀ (metricPerturbationPath (I := I) g₀ T T' hδ hδ' p.2) p.1).comp
  ...
  have happly : ∀ om : Cₛ^∞⟮I; Tensor0SBundle.Tensor0SModel 1 ℝ E,
    ...
    have hsharpom := ContMDiffOn.clm_bundle_apply (b := Prod.fst)
      (inverseMetricSharpField_metricPerturbationPath_jointContMDiffOn (I := I) g₀ T T' hδ hδ') homj
  ...
  dsimp only [φ]
  rw [connectionDifferenceSection_eq_operatorFieldComposition_raisedKoszul_sharpFlatEndoCc (I := I) (M := M) g₀
...
theorem connIns_joint
  ...
  have h := connContrField_jointContMDiffOn (I := I) 2 1
    (S := metricPerturbationPathDomain (δ := δ) (δ' := δ'))
    (fun p => (connectionDifferenceSection (I := I)
      (metricPerturbationPath (I := I) g₀ T T' hδ hδ' p.2) g₀).toSection p.1)
...
private theorem covGradConnectionDifference_metricPerturbationPath_jointContMDiffOn
  ...
  have h := covGrad_step_jointContMDiffOn (I := I) (M := M) g₀ 1 2
    (fun s => connectionDifferenceSection (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g₀)
...
private theorem order1CLM_metricPerturbationPath_jointContMDiffOn
  ...
  set Af : ∀ p : M × ℝ, Tensor0SBundle.TensorRSSpace 1 2 I p.1 :=
    fun p => (connectionDifferenceSection (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' p.2) g₀).toSection p.1
    with hAf
  have hA : ContMDiffOn (I.prod 𝓘(ℝ, ℝ)) (I.prod 𝓘(ℝ, Tensor0SBundle.TensorRSModel 1 2 ℝ E)) ∞
      ...
      ((Set.univ : Set M) ×ˢ S) :=
    connectionDifferenceSection_metricPerturbationPath_jointContMDiffOn (I := I) g₀ T T' hδ hδ'
  have hpre102 := slotPermField_jointContMDiffOn (I := I) cdPerm3102 (S := S) Z hZ
  have hpre120 := slotPermField_jointContMDiffOn (I := I) cdPerm3120 (S := S) Z hZ
  have h₁ := slotPermField_jointContMDiffOn (I := I) cdPerm40312 (S := S) _
    (connContrField_jointContMDiffOn (I := I) 2 1 (S := S) Af hA
      (fun p => slotPermCLM (I := I) cdPerm3102 p.1 (Z p)) hpre102)
  have h₂ := slotPermField_jointContMDiffOn (I := I) cdPerm40213 (S := S) _
    (connContrField_jointContMDiffOn (I := I) 2 1 (S := S) Af hA
      (fun p => slotPermCLM (I := I) cdPerm3120 p.1 (Z p)) hpre120)
  have h₃ := slotPermField_jointContMDiffOn (I := I) cdPerm42301 (S := S) _
    (connContrField_jointContMDiffOn (I := I) 2 1 (S := S) Af hA (fun p => Z p) hZ)
  have h₄ := slotPermField_jointContMDiffOn (I := I) cdPerm41302 (S := S) _
    (connContrField_jointContMDiffOn (I := I) 2 1 (S := S) Af hA
      (fun p => slotPermCLM (I := I) cdPerm3102 p.1 (Z p)) hpre102)
  have h₅ := slotPermField_jointContMDiffOn (I := I) cdPerm41203 (S := S) _
    (connContrField_jointContMDiffOn (I := I) 2 1 (S := S) Af hA
      (fun p => slotPermCLM (I := I) cdPerm3120 p.1 (Z p)) hpre120)
...
private theorem order0CLM_metricPerturbationPath_jointContMDiffOn
    ...
    ContMDiffOn (I.prod 𝓘(ℝ, ℝ)) (I.prod 𝓘(ℝ, Tensor0SBundle.Tensor0SModel 4 ℝ E)) ∞
      (fun p : M × ℝ => TotalSpace.mk' (Tensor0SBundle.Tensor0SModel 4 ℝ E)
        (E := fun z : M => Tensor0SBundle.Tensor0SSpace 4 I z) p.1
        (linearizedRicciConnectionDifferenceOrder0CLM (I := I) p.1
          ((connectionDifferenceSection (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' p.2) g₀).toSection p.1)
          ((covGrad (I := I) (M := M) g₀ 1 2
              (connectionDifferenceSection (I := I)
                (metricPerturbationPath (I := I) g₀ T T' hδ hδ' p.2) g₀)).toSection p.1)
  ...
  have hDA : ContMDiffOn (I.prod 𝓘(ℝ, ℝ)) (I.prod 𝓘(ℝ, Tensor0SBundle.TensorRSModel 1 3 ℝ E)) ∞
      ...
      ((Set.univ : Set M) ×ˢ S) :=
    covGradConnectionDifference_metricPerturbationPath_jointContMDiffOn (I := I) g₀ T T' hδ hδ'
  have hZswap := slotPermField_jointContMDiffOn (I := I) cdPerm210 (S := S) Z hZ
  ...
  have hinnJ' := connContrField_jointContMDiffOn (I := I) 1 1 (S := S) Af hA
    (fun p => slotPermCLM (I := I) cdPerm210 p.1 (Z p)) hZswap
  have hu₃ := slotPermField_jointContMDiffOn (I := I) cdPerm43201 (S := S) _
    (connContrField_jointContMDiffOn (I := I) 2 1 (S := S) Af hA
      (fun p => slotPermCLM (I := I) cdPerm3102 p.1
        (connContrCLM (I := I) 1 1 p.1 (Af p) (Z p)))
      (slotPermField_jointContMDiffOn (I := I) cdPerm3102 (S := S) _ hinnJ))
  have hu₄ := slotPermField_jointContMDiffOn (I := I) cdPerm42301 (S := S) _
    (connContrField_jointContMDiffOn (I := I) 2 1 (S := S) Af hA
      (fun p => slotPermCLM (I := I) cdPerm3102 p.1
        (connContrCLM (I := I) 1 1 p.1 (Af p) (slotPermCLM (I := I) cdPerm210 p.1 (Z p))))
      (slotPermField_jointContMDiffOn (I := I) cdPerm3102 (S := S) _ hinnJ'))
  have hu₅ := slotPermField_jointContMDiffOn (I := I) cdPerm43102 (S := S) _
    (connContrField_jointContMDiffOn (I := I) 2 1 (S := S) Af hA
      (fun p => slotPermCLM (I := I) cdPerm3120 p.1
        (connContrCLM (I := I) 1 1 p.1 (Af p) (Z p)))
      (slotPermField_jointContMDiffOn (I := I) cdPerm3120 (S := S) _ hinnJ))
  have hu₆ := slotPermField_jointContMDiffOn (I := I) cdPerm41302 (S := S) _
    (connContrField_jointContMDiffOn (I := I) 2 1 (S := S) Af hA
      (fun p => connContrCLM (I := I) 1 1 p.1 (Af p) (slotPermCLM (I := I) cdPerm210 p.1 (Z p)))
      hinnJ')
  have hu₇ := slotPermField_jointContMDiffOn (I := I) cdPerm41203 (S := S) _
    (connContrField_jointContMDiffOn (I := I) 2 1 (S := S) Af hA
      (fun p => connContrCLM (I := I) 1 1 p.1 (Af p) (Z p))
      hinnJ)
  have hu₈ := slotPermField_jointContMDiffOn (I := I) cdPerm42103 (S := S) _
    (connContrField_jointContMDiffOn (I := I) 2 1 (S := S) Af hA
      (fun p => slotPermCLM (I := I) cdPerm3120 p.1
        (connContrCLM (I := I) 1 1 p.1 (Af p) (slotPermCLM (I := I) cdPerm210 p.1 (Z p))))
      (slotPermField_jointContMDiffOn (I := I) cdPerm3120 (S := S) _ hinnJ'))
  have hu₁ := slotPermField_jointContMDiffOn (I := I) cdPerm43012 (S := S) _
    (connContrField_jointContMDiffOn (I := I) 1 2 (S := S) DAf hDA (fun p => Z p) hZ)
  have hu₂ := slotPermField_jointContMDiffOn (I := I) cdPerm42013 (S := S) _
...
theorem fourTrace_joint
    ...
    ContMDiffOn (I.prod 𝓘(ℝ, ℝ)) (I.prod 𝓘(ℝ, Tensor0SBundle.Tensor0SModel 2 ℝ E)) ∞
      (fun p : M × ℝ => TotalSpace.mk' (Tensor0SBundle.Tensor0SModel 2 ℝ E)
        ...
        (ricciCometricFourTraceCLM (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' p.2) p.1 (Z p)))
      ((Set.univ : Set M) ×ˢ metricPerturbationPathDomain (δ := δ) (δ' := δ')) := by
  set S := metricPerturbationPathDomain (δ := δ) (δ' := δ') with hS
  have ha := cometricDoubleTraceFib_metricPerturbationPath_jointContMDiffOn (I := I) (p := 2) g₀ T T' hδ hδ'
    (fun p => slotPermCLM (I := I) cdPerm40231 p.1 (Z p))
    (slotPermField_jointContMDiffOn (I := I) cdPerm40231 (S := S) Z hZ)
  have hb := cometricDoubleTraceFib_metricPerturbationPath_jointContMDiffOn (I := I) (p := 2) g₀ T T' hδ hδ'
    (fun p => slotPermCLM (I := I) cdPerm40321 p.1 (Z p))
    (slotPermField_jointContMDiffOn (I := I) cdPerm40321 (S := S) Z hZ)
  have hc := cometricDoubleTraceFib_metricPerturbationPath_jointContMDiffOn (I := I) (p := 2) g₀ T T' hδ hδ'
    (fun p => Z p) hZ
  have hd := cometricDoubleTraceFib_metricPerturbationPath_jointContMDiffOn (I := I) (p := 2) g₀ T T' hδ hδ'
    (fun p => slotPermCLM (I := I) cdPerm42301 p.1 (Z p))
    (slotPermField_jointContMDiffOn (I := I) cdPerm42301 (S := S) Z hZ)
...
theorem linearizedRicciConnectionDifferenceOrder1Fib_metricPerturbationPath_jointContMDiffOn
    ...
    ContMDiffOn (I.prod 𝓘(ℝ, ℝ)) (I.prod 𝓘(ℝ, Tensor0SBundle.TensorRSModel 3 2 ℝ E)) ∞
      (fun p : M × ℝ => TotalSpace.mk' (Tensor0SBundle.TensorRSModel 3 2 ℝ E)
        ...
        (linearizedRicciConnectionDifferenceOrder1CometricTracedCLM (I := I) g₀
          (metricPerturbationPath (I := I) g₀ T T' hδ hδ' p.2) p.1))
      ((Set.univ : Set M) ×ˢ metricPerturbationPathDomain (δ := δ) (δ' := δ')) := by
  apply contMDiffOn_clm_section_of_apply (I := I) (M := M)
    (F₁ := Tensor0SBundle.Tensor0SModel 3 ℝ E)
    ...
    (F₂ := Tensor0SBundle.Tensor0SModel 2 ℝ E)
    (V₂ := fun z : M => Tensor0SBundle.Tensor0SSpace 2 I z)
    (φ := fun p : M × ℝ => linearizedRicciConnectionDifferenceOrder1CometricTracedCLM (I := I) g₀
      (metricPerturbationPath (I := I) g₀ T T' hδ hδ' p.2) p.1)
    (S := metricPerturbationPathDomain (δ := δ) (δ' := δ'))
  intro Y
...
theorem linearizedRicciConnectionDifferenceOrder0Fib_metricPerturbationPath_jointContMDiffOn
  ...
  apply contMDiffOn_clm_section_of_apply (I := I) (M := M)
    ...
    (F₂ := Tensor0SBundle.Tensor0SModel 2 ℝ E)
    (V₂ := fun z : M => Tensor0SBundle.Tensor0SSpace 2 I z)
    (φ := fun p : M × ℝ => linearizedRicciConnectionDifferenceOrder0CometricTracedCLM (I := I) g₀
      (metricPerturbationPath (I := I) g₀ T T' hδ hδ' p.2) p.1)
    (S := metricPerturbationPathDomain (δ := δ) (δ' := δ'))
  intro Y
  have hYj : ContMDiffOn (I.prod 𝓘(ℝ, ℝ)) (I.prod 𝓘(ℝ, Tensor0SBundle.Tensor0SModel 2 ℝ E)) ∞
      ...
      ((Set.univ : Set M) ×ˢ metricPerturbationPathDomain (δ := δ) (δ' := δ')) :=
    Y.contMDiff.comp_contMDiffOn contMDiffOn_fst
  have hE0 := order0CLM_metricPerturbationPath_jointContMDiffOn (I := I) g₀ T T' hδ hδ'
    (fun p => Y p.1) hYj
  have hCK := fourTrace_joint (I := I) g₀ T T' hδ hδ'
    (fun p => linearizedRicciConnectionDifferenceOrder0CLM (I := I) p.1
      ((connectionDifferenceSection (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' p.2) g₀).toSection p.1)
      ((covGrad (I := I) (M := M) g₀ 1 2
          (connectionDifferenceSection (I := I)
            (metricPerturbationPath (I := I) g₀ T T' hδ hδ' p.2) g₀)).toSection p.1)
