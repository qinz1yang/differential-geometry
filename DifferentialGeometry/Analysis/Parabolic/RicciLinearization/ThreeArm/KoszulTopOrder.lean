...
private lemma riemannianFiberNormSq_iteratedCovGrad_cometricCastG0_gridWindow_le
  ...
  · intro l
    ...
    have h4 : 0 ≤ diagonalGridGrowthFactor (E := E) l := operatorFieldApplicationGdiag_nonneg (E := E) l
    positivity
  · intro g₁ P δ hδ_le hδ0 hδ htie l x
    ...
    have hid : cometricDoubleTraceCastG0 (I := I) g₀ g₁ = Φ + ccOperatorFieldComp (I := I) (M := M)
      g₀ 3 3 1 Φ W := by
      rw [hΦ_def, hW_def]
      exact cometricCastG0_eq_doubleTrace_add_ccOperatorFieldComp (I := I) g₀ g₁
...
private lemma riemannianFiberNormSq_operatorFieldComposition_coeffLower_general_le (g : SmoothRiemannianMetric I M)
    ...
    riemannianFiberNormSq (I := I) (M := M) g p (b + i) x
        ((∑ k ∈ Finset.range i,
          ccOperatorFieldComp (I := I) (M := M) g p (a + (k + 1)) (b + i)
            (operatorFieldApplicationLeibnizPsi (I := I) (M := M) g a b Φ i (k + 1))
  ...
  rw [operatorFieldComposition_toSection (I := I) (M := M) g p (a + (k + 1)) (b + i)
    (operatorFieldApplicationLeibnizPsi (I := I) (M := M) g a b Φ i (k + 1))
...
theorem ricciArmOrder1KoszulCoeff_topOrderSeparatedResidual_jetL2_flat_leak_allOrders
  ...
  · intro i
    ...
    have h2 : 0 ≤ diagonalGridGrowthFactor (E := E) i := operatorFieldApplicationGdiag_nonneg (E := E) i
        ...
        rw [ricciArmOrder1KoszulCoeff_eq_raisedKoszul_contract_cometricDoubleTraceCastG0 (I := I) g₀
          ...
          Finset.sum_range_succ' (fun k =>
            ccOperatorFieldComp (I := I) (M := M) g₀ 3 (1 + k) (2 + i)
              (operatorFieldApplicationLeibnizPsi (I := I) (M := M) g₀ 1 2 (raisedKoszul (I := I) g₀ g₁) i k)
              (iteratedCovGrad (I := I) g₀ 3 1 k (cometricDoubleTraceCastG0 (I := I) g₀ g₁))) i]
        congr 1
        rw [operatorFieldApplicationLeibnizPsi_zero_right_eq (I := I) (M := M) g₀ 1 2 (raisedKoszul (I := I) g₀ g₁) i,
    ...
    · obtain ⟨x₀⟩ := hMne
      ...
      have hpt : ∀ x : M,
        ...
        have hsumcell : (∑ k ∈ Finset.range i,
          ...
          have hRK := riemannianFiberNormSq_iteratedCovGrad_raisedKoszul_pointwise_le (I := I) (M := M) g₀ g₁ P htie
            (i - (k + 1)) x
      ...
      · simp only [Finset.range_zero, Finset.sum_empty, norm_zero, ne_eq, OfNat.ofNat_ne_zero,
        ...
        have h1 : 0 ≤ ∑ k ∈ Finset.range 0, 10 * Kw (k + 1) :=
          Finset.sum_nonneg (fun k _ => by have := hKw_nn (k + 1); linarith)
        have h2 : 0 ≤ diagonalGridGrowthFactor (E := E) 0 := operatorFieldApplicationGdiag_nonneg (E := E) 0
        ...
        have hkey := normSq_le_integral_of_pointwise_fiberNormSq_le_rs (I := I) (M := M)
          ...
          (∑ k ∈ Finset.range (j + 1),
            ccOperatorFieldComp (I := I) (M := M) g₀ 3 (1 + (k + 1)) (2 + (j + 1))
              (operatorFieldApplicationLeibnizPsi (I := I) (M := M) g₀ 1 2 (raisedKoszul (I := I) g₀ g₁) (j + 1)
                (k + 1))
              (iteratedCovGrad (I := I) g₀ 3 1 (k + 1) (cometricDoubleTraceCastG0 (I := I) g₀ g₁)))
          _ hint hpt
  ...
  have hheadpt : ∀ x : M,
    ...
    have h1 : riemannianFiberNormSq (I := I) (M := M) g₀ 1 (2 + i) x
        ...
        10 * riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + (i + 1)) x
          ((iteratedCovGrad (I := I) g₀ 0 2 (i + 1) P).toSection x) :=
      riemannianFiberNormSq_iteratedCovGrad_raisedKoszul_pointwise_le (I := I) (M := M) g₀ g₁ P htie i x
