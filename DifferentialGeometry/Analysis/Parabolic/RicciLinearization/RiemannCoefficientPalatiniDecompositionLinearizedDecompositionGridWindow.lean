import DifferentialGeometry.Analysis.Parabolic.RicciLinearization.RiemannCoefficientPalatiniDecompositionLinearizedDecompositionCoreIdentity
...
private theorem riemannCurvatureCoefficientFieldGridWindow (g₀ : SmoothRiemannianMetric I M) (Λ0 : ℝ)
  ...
  have hpart : ∀ (F : SmoothCcTensor g₀ 2 4) (cF : ℕ → ℝ) (hcF_nn : ∀ j, 0 ≤ cF j)
    ...
    refine le_trans (mul_le_mul_of_nonneg_left (Finset.sum_le_sum hcell2)
      (operatorFieldApplicationGdiag_nonneg (E := E) w)) ?_
...
private theorem lrOmegaHat_gridWindow (g₀ : SmoothRiemannianMetric I M)
      ...
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 0 (3 + l) x
            ((iteratedCovGrad (I := I) g₀ 0 3 l
              (connectionDifferenceMetricLoweredTensor (I := I) (M := M) g₀ g₁)).toSection x) ≤
  ...
  rw [connectionDifferenceMetricLoweredTensor]
  refine le_trans
    (riemannianFiberNormSq_iteratedCovGrad_ccTensorCompose_diagonalProductGrid_leftFactor_le
    (I := I) (M := M) g₀ l 0 3 3
    (endoSlotZeroCcTensor (I := I) (M := M) g₀ 2 (metricComparisonEndomorphismField (I := I) (M := M) g₁ g₀))
    (domDomCongrSection (I := I) g₀ (finRotate 3) (metricLoweredConnectionDifferenceCoefficient (I := I) g₀ g₁)) x) ?_
  have hop : ∀ i' : ℕ,
...
private theorem connectionDifferenceQuadraticCurvatureTermGridWindow (g₀ : SmoothRiemannianMetric I M)
    ...
    ∃ C : ℕ → ℝ, (∀ w, 0 ≤ C w) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + w) x
            ((iteratedCovGrad (I := I) g₀ 0 4 w
              (connectionDifferenceQuadraticCurvatureTerm (I := I) (M := M) g₀ g₁)).toSection x) ≤
          C w * Combinatorics.boundedFactorGridWindow
            (fun l' => riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + l') x
  ...
  have hCQ_nn : ∀ w, 0 ≤ CQ w := by
    intro w
    rw [hCQ_def]
    exact mul_nonneg (operatorFieldApplicationGdiag_nonneg (E := E) w)
  ...
  have hbase : ∀ (S : SmoothCcTensor g₀ 0 3)
    ...
    refine le_trans
      (riemannianFiberNormSq_iteratedCovGrad_ccTensorCompose_diagonalProductGrid_leftFactor_le
      (I := I) (M := M) g₀ w 0 3 4
      (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 2 (connectionDifferenceEndomorphism (I := I) (M := M) g₀ g₁)) S x) ?_
  ...
  have hΩ : ∀ (w' : ℕ) (y : M),
      riemannianFiberNormSq (I := I) (M := M) g₀ 0 (3 + w') y
          ((iteratedCovGrad (I := I) g₀ 0 3 w'
            (connectionDifferenceMetricLoweredTensor (I := I) (M := M) g₀ g₁)).toSection y) ≤
        CΩ w' * Combinatorics.antidiagonalTupleGridWindow
          (fun l' => riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + l') y
            ((iteratedCovGrad (I := I) g₀ 0 2 l' P).toSection y)) (w' + 2) :=
  ...
  have hΩswap : ∀ (w' : ℕ) (y : M),
    ...
    intro w' y
    rw [riemannianFiberNormSq_iteratedCovGrad_domDomCongrSection (I := I) (M := M) g₀
      (Equiv.swap (0 : Fin 3) 1) (connectionDifferenceMetricLoweredTensor (I := I) (M := M) g₀ g₁) w' y]
    exact hΩ w' y
  have hQB := hbase (connectionDifferenceMetricLoweredTensor (I := I) (M := M) g₀ g₁) hΩ
  have hQA := hbase (domDomCongrSection (I := I) g₀ (Equiv.swap (0 : Fin 3) 1)
    (connectionDifferenceMetricLoweredTensor (I := I) (M := M) g₀ g₁)) hΩswap
  have hQB' : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + w) x
      ((iteratedCovGrad (I := I) g₀ 0 4 w
        (connectionDifferenceQuadraticPairedTensor (I := I) (M := M) g₀ g₁)).toSection x) ≤
      CQ w * W := by
    rw [connectionDifferenceQuadraticPairedTensor]
    exact hQB
  have hQA' : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + w) x
      ((iteratedCovGrad (I := I) g₀ 0 4 w
        (connectionDifferenceQuadraticComposedTensor (I := I) (M := M) g₀ g₁)).toSection x) ≤
      CQ w * W := by
  ...
  have h6 := hsum6
    (domDomCongrSection (I := I) g₀ (Equiv.swap (0 : Fin 4) 1)
      (connectionDifferenceQuadraticPairedTensor (I := I) (M := M) g₀ g₁))
    (connectionDifferenceQuadraticPairedTensor (I := I) (M := M) g₀ g₁)
    (domDomCongrSection (I := I) g₀ lrPermA
      (connectionDifferenceQuadraticComposedTensor (I := I) (M := M) g₀ g₁))
...
private theorem riemannCurvatureRemainderGridWindow (g₀ : SmoothRiemannianMetric I M) (Λ0 : ℝ)
  ...
  classical
  obtain ⟨CF, hCF_nn, hCF⟩ := riemannCurvatureCoefficientFieldGridWindow (I := I) (M := M) g₀ Λ0 hΛ0
  obtain ⟨CQ, hCQ_nn, hCQ⟩ := connectionDifferenceQuadraticCurvatureTermGridWindow (I := I) (M := M) g₀ hδ₀
  ...
  have hB : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + w) x
      ...
      CQ w * W := by
    have hbase := hCQ (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s)
...
theorem deTurckLieCovariantDerivativeArmDifferenceGridWindow (g₀ : SmoothRiemannianMetric I M) (Λ0 : ℝ)
  ...
  have hWtower : ∀ w, w ≤ i →
    ...
    have h1 := riemannianFiberNormSq_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 1 5
      (slotExtendIter (I := I) (M := M) g₀ 0 4 1
        (deTurckLieCovariantDerivativeRemainderTensor (I := I) (M := M) g₀ T hδ hδZ s)) w x
    ...
    calc riemannianFiberNormSq (I := I) (M := M) g₀ 2 (6 + w) x
        ...
        ≤ fr * riemannianFiberNormSq (I := I) (M := M) g₀ 1 (5 + w) x
            ((iteratedCovGrad (I := I) g₀ 1 5 w
              (slotExtendIter (I := I) (M := M) g₀ 0 4 1
                (deTurckLieCovariantDerivativeRemainderTensor (I := I) (M := M) g₀ T hδ hδZ s))).toSection x) := h1
  ...
  have hcell : ∀ u ∈ Finset.range (i + 1),
    ...
    have hA1 : riemannianFiberNormSq (I := I) (M := M) g₀ 6 (2 + u) x
      ...
      have h0 := hCP (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s)
