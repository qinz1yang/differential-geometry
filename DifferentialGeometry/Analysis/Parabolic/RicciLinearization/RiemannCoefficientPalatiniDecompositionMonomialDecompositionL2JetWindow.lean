...
private theorem curvatureDecompositionMonomialCoeffField_eq_pairTrace (g₀ g₁ : SmoothRiemannianMetric I M)
  ...
  have hYval : ∀ w : Fin 6 → TangentSpace I x,
    ...
    · refine congrArg _ ?_
      funext j
      fin_cases j
      · exact congrArg w (curvatureDecompositionSlotPerm_castAdd σ 0)
      · exact congrArg w (curvatureDecompositionSlotPerm_castAdd σ 1)
      · exact congrArg w (curvatureDecompositionSlotPerm_castAdd σ 2)
      · exact congrArg w (curvatureDecompositionSlotPerm_castAdd σ 3)
    · refine congrArg _ ?_
      funext k
      fin_cases k
      · exact congrArg w (curvatureDecompositionSlotPerm_natAdd σ 0)
      · exact congrArg w (curvatureDecompositionSlotPerm_natAdd σ 1)
  have hLHS : Tensor0SSpace.toModel
    ...
    exact curvatureDecompositionMonomialFibFixedFrame_toModel (I := I) (M := M)
      (ccTensorUnitValueSection (I := I) (M := M) g₀ S) σ
...
private theorem curvatureDecompositionMonomialCoeffField_pointwise_gridWindow
        ...
        fr * riemannianFiberNormSq (I := I) (M := M) g₀ 3 (5 + l) x
          ((iteratedCovGrad (I := I) g₀ 3 5 l
            (slotExtendIter (I := I) (M := M) g₀ 0 2 3
              (ccTensor02Symm (I := I) (M := M) g₀ S))).toSection x) :=
      riemannianFiberNormSq_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 3 5
    ...
    have h1 : riemannianFiberNormSq (I := I) (M := M) g₀ 1 (3 + l) x
        ...
        fr * riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + l) x
          ((iteratedCovGrad (I := I) g₀ 0 2 l
            (ccTensor02Symm (I := I) (M := M) g₀ S)).toSection x) :=
      riemannianFiberNormSq_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 0 2
...
theorem exists_curvatureDecompositionMonomialCoeffField_symmS_metricPerturbationPath_l2JetWindow
  ...
  obtain ⟨C, hC_nn, hpt⟩ := curvatureDecompositionMonomialCoeffField_pointwise_gridWindow (I := I) (M := M)
    g₀ hδ₁_lt σ
  obtain ⟨Kg, hKg_nn, hKg⟩ :=
