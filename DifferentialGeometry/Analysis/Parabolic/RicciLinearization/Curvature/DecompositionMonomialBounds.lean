...
theorem riemannianFiberNormSq_curvatureActionMonomialTrace_le
  ...
  have hcomp : ∀ u : TangentSpace I x,
    ...
    have hti := htie x u u
    nlinarith [hlb, hti]
  have hBackground1 : ∀ a b : Fin (Module.finrank ℝ E),
  ...
  have hVsum : ∑ k, ∑ l, (V k l) ^ 2 ≤
    ...
    have hcol : ∀ l : Fin (Module.finrank ℝ E),
      ...
      exact sum_sq_component_le_of_orthonormal (I := I) (M := M) g₀ g₁ x h1mδ hcomp
        e hpars (fun a => smoothOrthoFrame (I := I) g₁ x a x) hBackground1
    ...
    have hrow_col : ∀ a : Fin (Module.finrank ℝ E),
      ...
      exact weight_row_g0norm_le (I := I) (M := M) g₀ g₁ x h1mδ hδW0 hcomp
        (fun a => smoothOrthoFrame (I := I) g₁ x a x) hBackground1 hB0 (W x) (hW x) a
  ...
  have hriemannianFiberNormSq_eq := hriemannianFiberNormSq
    (show TensorRSSpace 4 2 I x from
      TensorRSSpace.ofCLM
        (curvatureActionMonomialTrace (I := I) (M := M) g₁ W σ x))
  rw [hriemannianFiberNormSq_eq]
