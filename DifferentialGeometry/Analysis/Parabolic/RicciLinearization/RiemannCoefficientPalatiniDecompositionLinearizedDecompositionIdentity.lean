...
import DifferentialGeometry.Analysis.Parabolic.RicciLinearization.RiemannCoefficientPalatiniDecompositionLinearizedDecompositionGridWindow
...
private theorem lrFamily_threeArmHjoint (g₀ : SmoothRiemannianMetric I M)
  ...
  have hperY : ∀ (Y : Cₛ^∞⟮I; Tensor0SModel 2 ℝ E, fun x : M => Tensor0SSpace 2 I x⟯),
    ...
    have hbase : ∀ (X : SmoothCcTensor g₀ 2 6),
      ...
      have h4 := cometricDoubleTraceFib_metricPerturbationPath_jointContMDiffOn (I := I) (p := 4)
        g₀ T 0 hδ hδZ
        (fun q : M × ℝ => (show Tensor0SSpace 2 I q.1 →L[ℝ] Tensor0SSpace 6 I q.1 from
          X.toSection q.1) (Y q.1)) hXjoint
      exact cometricDoubleTraceFib_metricPerturbationPath_jointContMDiffOn (I := I) (p := 2)
        g₀ T 0 hδ hδZ
    ...
    have hA0 := lrJoint0S_add_local (I := I) (M := M) (d := 2)
      (S := metricPerturbationPathDomain (δ := δ) (δ' := δ)) _ _ hP00 hP01
    ...
    have hA1 := lrJoint0S_add_local (I := I) (M := M) (d := 2)
      (S := metricPerturbationPathDomain (δ := δ) (δ' := δ)) _ _ hP10 hP11
    ...
    have hA2 := lrJoint0S_add_local (I := I) (M := M) (d := 2)
      (S := metricPerturbationPathDomain (δ := δ) (δ' := δ)) _ _ hP20 hP21
    ...
    have hZ01 := lrJoint0S_add_local (I := I) (M := M) (d := 2)
      (S := metricPerturbationPathDomain (δ := δ) (δ' := δ)) _ _ hE0 hE1
    have hZ := lrJoint0S_add_local (I := I) (M := M) (d := 2)
      (S := metricPerturbationPathDomain (δ := δ) (δ' := δ)) _ _ hZ01 hE2
...
theorem exists_deTurckLieCovariantDerivativeArm_decomposition_identity_data
  ...
  · intro s hs
    ...
    conv_rhs => rw [operatorFieldApplication_add_left]
    abel
  · intro s hs x
