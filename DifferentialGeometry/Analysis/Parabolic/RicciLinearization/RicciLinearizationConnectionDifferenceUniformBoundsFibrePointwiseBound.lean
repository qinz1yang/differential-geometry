...
lemma connectionDifference_flat_factor_bound (g₀ g₁ : SmoothRiemannianMetric I M) (x : M)
    {n : ℕ} (e : Fin n → TangentSpace I x)
    (horth : ∀ a b : Fin n, g₀.inner x (e a) (e b) = if a = b then (1 : ℝ) else 0)
    {CA : ℝ}
  ...
  have h1 : Tensor0SBundle.TensorRSSpace.toModel (connectionDifferenceFib (I := I) g₁ g₀ x)
      (Tensor0SBundle.Tensor0SSpace.toModel (g0FlatCLM (I := I) g₀ x (e a))) =
      Tensor0SBundle.Tensor0SSpace.toModel
        ((show Tensor0SBundle.Tensor0SSpace 1 I x →L[ℝ] Tensor0SBundle.Tensor0SSpace 2 I x from
          connectionDifferenceFib (I := I) g₁ g₀ x) (g0FlatCLM (I := I) g₀ x (e a))) :=
    (toModel_tensorRS_apply (I := I) 1 2 x (connectionDifferenceFib (I := I) g₁ g₀ x)
  ...
  have h2 : Tensor0SBundle.Tensor0SSpace.toModel
      ...
      (fun j => (v j : E)) =
      ((show Tensor0SBundle.Tensor0SSpace 1 I x →L[ℝ] Tensor0SBundle.Tensor0SSpace 2 I x from
        connectionDifferenceFib (I := I) g₁ g₀ x) (g0FlatCLM (I := I) g₀ x (e a))) (fun j => v j) := rfl
  rw [h2, connectionDifferenceFib_apply_eval]
  rw [show (g0FlatCLM (I := I) g₀ x (e a))
      (fun _ : Fin 1 => PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x (v 0) (v 1)) =
      cotangentToDual (I := I) (x := x) (g0FlatCLM (I := I) g₀ x (e a))
        (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x (v 0) (v 1)) from
    (cotangentToDual_apply (I := I) (x := x) _ _).symm]
  rw [cotangentToDual_g0FlatCLM]
  have hcs := abs_metric_inner_le_sqrt_metric_quadratic (I := I) (M := M) g₀ x (e a)
    (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x (v 0) (v 1))
