...
omit [NeZero (Module.finrank ℝ E)] in
omit [SigmaCompactSpace M] in
private theorem connectionDifference_app_joint
    {D : RealTimeInterval}
    (g_fam : ℝ → SmoothRiemannianMetric I M)
    (hG : MetricFamilySmoothOn (I := I) (M := M) D g_fam)
    (q : SmoothRiemannianMetric I M)
  ...
  have hcoordEach : ∀ σ : Fin 2 → Fin (Module.finrank ℝ E),
      ContMDiffWithinAt (I.prod 𝓘(ℝ, ℝ)) 𝓘(ℝ) ∞
        (fun p : M × ℝ => Bcmm.repr
          (e ⟨p.1, (show Tensor0SSpace 1 I p.1 →L[ℝ] Tensor0SSpace 2 I p.1 from
            (connectionDifferenceSection (I := I) (g_fam p.2) q).toSection p.1) (om p.1)⟩).2 σ)
    ...
    have hreadout : ∀ {z : M × ℝ}, z.1 ∈ e.baseSet →
      ...
      have hcoe :
          (e ⟨z.1, (show Tensor0SSpace 1 I z.1 →L[ℝ] Tensor0SSpace 2 I z.1 from
            (connectionDifferenceSection (I := I) (g_fam z.2) q).toSection z.1) (om z.1)⟩).2 =
            (e.linearMapAt ℝ z.1)
              ((show Tensor0SSpace 1 I z.1 →L[ℝ] Tensor0SSpace 2 I z.1 from
  ...
  have hfinal : ContMDiffWithinAt (I.prod 𝓘(ℝ, ℝ)) 𝓘(ℝ, Tensor0SModel 2 ℝ E) ∞
      (fun p : M × ℝ =>
        (e ⟨p.1, (show Tensor0SSpace 1 I p.1 →L[ℝ] Tensor0SSpace 2 I p.1 from
          (connectionDifferenceSection (I := I) (g_fam p.2) q).toSection p.1) (om p.1)⟩).2)
      ((Set.univ : Set M) ×ˢ D.regular) p₀ := by
    have hequiv :=
...
omit [NeZero (Module.finrank ℝ E)] in
omit [SigmaCompactSpace M] in
theorem connectionDifference_joint
    {D : RealTimeInterval}
    (g_fam : ℝ → SmoothRiemannianMetric I M)
    (hG : MetricFamilySmoothOn (I := I) (M := M) D g_fam)
    (q : SmoothRiemannianMetric I M) :
  ...
  apply contMDiffOn_clm_section_of_apply (I := I) (M := M)
    ...
    (S := D.regular)
  intro om
  exact connectionDifference_app_joint (I := I) g_fam hG q om
