...
lemma metricCauchySchwarzBound_nonneg (g₀ : SmoothRiemannianMetric I M) (x₀ : M)
...
lemma smoothCcTensor_norm_eq_zero_of_isEmpty [IsEmpty M] (g₀ : SmoothRiemannianMetric I M)
    (r s : ℕ) (V : SmoothCcTensor g₀ r s) : ‖V‖ = 0 := by
  rw [SmoothCcTensor.norm_def, tensorL2Norm_def, tensorL2Inner,
    MeasureTheory.integral_of_isEmpty, Real.sqrt_zero]

def termPairTraceSlotPerm6 : Equiv.Perm (Fin 6) :=
  ⟨fun i => (![1, 3, 4, 5, 0, 2] : Fin 6 → Fin 6) i,
   fun i => (![4, 0, 5, 1, 2, 3] : Fin 6 → Fin 6) i,
   by decide, by decide⟩

abbrev deTurckLieCovariantDerivativePairTracePermutation : Equiv.Perm (Fin 6) :=
  termPairTraceSlotPerm6

omit [FiniteDimensional ℝ E] [NeZero (Module.finrank ℝ E)] [CompactSpace M] [I.Boundaryless]
    [BoundarylessManifold I M] [T2Space M] [SigmaCompactSpace M] in
lemma tensor0S_zero_rank_eq_smul_unitTensor (x : M) (t : Tensor0SSpace 0 I x) :
...
private lemma palatiniToModel_eq_eval {s : ℕ} {x : M} (T : Tensor0SSpace s I x)
    (m : Fin s → E) :
    Tensor0SSpace.toModel T m =
...
omit [NeZero (Module.finrank ℝ E)] [CompactSpace M] [I.Boundaryless]
    [BoundarylessManifold I M] [T2Space M] [SigmaCompactSpace M] in
private lemma palatiniTermSlotFib_apply_toModel {s : ℕ} {x : M}
    (Term : TangentSpace I x →L[ℝ] TangentSpace I x →L[ℝ] TangentSpace I x)
    (D : Tensor0SSpace (s + 1) I x) (m : Fin (s + 1 + 1) → E) :
...
omit [NeZero (Module.finrank ℝ E)] [I.Boundaryless] [BoundarylessManifold I M]
    [SigmaCompactSpace M] in
lemma palatiniSlotExtendIter_two_toModel (g₀ : SmoothRiemannianMetric I M)
  ...
  have hkey1 := TensorMultilinear.tensor0S_curry_toModel_apply (I := I) (M := M) (n := 5)
  ...
  rw [show (Fin.cons (u 0) (Matrix.vecTail u) : Fin 6 → E) = u from by
    funext k
  ...
  rw [show (Matrix.vecTail u : Fin 5 → E) =
  ...
  have hkey2 := TensorMultilinear.tensor0S_curry_toModel_apply (I := I) (M := M) (n := 4)
  ...
  rw [show ((show Tensor0SSpace 0 I x →L[ℝ] Tensor0SSpace 4 I x from X.toSection x).comp
      ...
      (show Tensor0SSpace 0 I x →L[ℝ] Tensor0SSpace 4 I x from X.toSection x)
        (tensor0SCurry (I := I) (M := M) (𝕜 := ℝ) 0 x
          (tensor0SCurry (I := I) (M := M) (𝕜 := ℝ) 1 x D
            ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (u 0)))
            ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (u 1))) from rfl]
  ...
  have htval : Tensor0SSpace.toModel t (fun i : Fin 0 => i.elim0) =
    ...
    have h1 := TensorMultilinear.tensor0S_curry_toModel_apply (I := I) (M := M) (n := 0)
      (T := tensor0SCurry (I := I) (M := M) (𝕜 := ℝ) 1 x D
        ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (u 0))) (v0 := u 1)
...
omit [NeZero (Module.finrank ℝ E)] [TopologicalSpace M] [CompactSpace M] [T2Space M]
    [SigmaCompactSpace M] in
lemma palatiniToModel_cons_sum_smul {n : ℕ}
...
omit [NeZero (Module.finrank ℝ E)] [TopologicalSpace M] [CompactSpace M] [T2Space M]
    [SigmaCompactSpace M] in
private lemma palatiniToModel_cons_cons_sum_smul (_x : M) {n : ℕ}
...
omit [CompactSpace M] [I.Boundaryless] [BoundarylessManifold I M] [T2Space M]
    [SigmaCompactSpace M] in
lemma palatiniOrthoFrame_center_repr (g : SmoothRiemannianMetric I M) (x : M)
    (v : TangentSpace I x) :
...
private lemma palatiniG1_inner_metricComparisonEndomorphism_left (g₀ g₁ : SmoothRiemannianMetric I M) (x : M)
...
private lemma palatiniPureDT_eq_trace_fullRaised (g₀ g₁ : SmoothRiemannianMetric I M)
    (s : ℕ) :
  ...
  have hGrep : ∀ a : Fin (Module.finrank ℝ E),
    ...
    intro a
    have h1 := palatiniOrthoFrame_center_repr (I := I) (M := M) g₁ x
    ...
    rw [g₁.symm x (smoothOrthoFrame (I := I) g₁ x c x)
      (metricComparisonEndomorphism (I := I) g₀ g₁ x (smoothOrthoFrame (I := I) g₀ x a x))]
    rw [palatiniG1_inner_metricComparisonEndomorphism_left (I := I) (M := M) g₀ g₁ x
  ...
  calc (∑ a : Fin (Module.finrank ℝ E),
      ...
      = ∑ a : Fin (Module.finrank ℝ E), ∑ c : Fin (Module.finrank ℝ E),
        ...
        refine Finset.sum_congr rfl fun a _ => ?_
        rw [hGrep a]
        exact palatiniToModel_cons_sum_smul (E := E) (Tensor0SSpace.toModel Z)
          (Module.finrank ℝ E)
    ...
    _ = ∑ c : Fin (Module.finrank ℝ E),
          Tensor0SSpace.toModel Z
            (Fin.cons (tangentSpaceModelContinuousLinearEquiv (I := I) x
              ...
              (Fin.cons (tangentSpaceModelContinuousLinearEquiv (I := I) x
                (smoothOrthoFrame (I := I) g₁ x c x)) mm)) := by
        refine Finset.sum_congr rfl fun c _ => ?_
        have hsum := palatiniToModel_cons_cons_sum_smul (E := E) x (Tensor0SSpace.toModel Z)
          (tangentSpaceModelContinuousLinearEquiv (I := I) x
            (smoothOrthoFrame (I := I) g₁ x c x))
          (Module.finrank ℝ E)
        ...
        rw [← hsum]
        congr 2
        have hrep0 := palatiniOrthoFrame_center_repr (I := I) (M := M) g₀ x
          (smoothOrthoFrame (I := I) g₁ x c x)
        congr 1
        calc
          (∑ a : Fin (Module.finrank ℝ E),
              ...
              tangentSpaceModelContinuousLinearEquiv (I := I) x
                ...
                rw [map_sum]
                simp only [map_smul]
          _ = tangentSpaceModelContinuousLinearEquiv (I := I) x
                (smoothOrthoFrame (I := I) g₁ x c x) :=
              congrArg (tangentSpaceModelContinuousLinearEquiv (I := I) x) hrep0.symm

def cometricDoublePairTraceCoefficient
    (g₀ g₁ : SmoothRiemannianMetric I M) : SmoothCcTensor g₀ 6 2 :=
...
omit [BoundarylessManifold I M] [SigmaCompactSpace M] in
omit [I.Boundaryless] in
lemma palatiniPairTraceOp_apply_toModel (g₀ gm : SmoothRiemannianMetric I M)
    (X : SmoothCcTensor g₀ 0 4) (x : M) (D : Tensor0SSpace 2 I x) (v : Fin 2 → E) :
    Tensor0SSpace.toModel
        ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 2 I x from
          (ccOperatorFieldComp (I := I) (M := M) g₀ 2 6 2 (cometricDoublePairTraceCoefficient (I := I) (M := M) g₀ gm)
  ...
  have hYval : ∀ w : Fin 6 → E,
    ...
    rw [palatiniSlotExtendIter_two_toModel (I := I) (M := M) g₀ X x D
...
omit [NeZero (Module.finrank ℝ E)] [I.Boundaryless] [BoundarylessManifold I M]
    [SigmaCompactSpace M] in
private lemma palatiniSlotInsertEndoCc_add (g₀ : SmoothRiemannianMetric I M) (s : ℕ)
...
omit [NeZero (Module.finrank ℝ E)] [CompactSpace M] [BoundarylessManifold I M]
    [SigmaCompactSpace M] in
omit [I.Boundaryless] in
private lemma palatiniRaised_diff_split (g₀ g₁ : SmoothRiemannianMetric I M) :
  ...
  rw [show (metricComparisonDifferenceEndomorphismField (I := I) g₀ g₁ x) =
      DifferentialGeometry.Analysis.Sobolev.TensorHilbert.metricComparisonDifferenceEndomorphism (I := I) g₀ g₁ x
    from rfl]
...
omit [NeZero (Module.finrank ℝ E)] [CompactSpace M] [BoundarylessManifold I M]
    [SigmaCompactSpace M] in
omit [I.Boundaryless] in
private lemma palatiniEndoCovDeriv_fullRaised_id_zero (g₀ : SmoothRiemannianMetric I M)
    (Y : Cₛ^∞⟮I; E, (TangentSpace I : M → Type _)⟯) (x : M) (v : TangentSpace I x) :
    ((endoCovariantDerivative (I := I) (M := M) g₀)
        (metricComparisonEndomorphismField (I := I) (M := M) g₀ g₀) x v) (Y x) = 0 := by
  have hLeib := endoCovariantDerivative_apply (I := I) (M := M) g₀
    (metricComparisonEndomorphismField (I := I) (M := M) g₀ g₀) Y x v
  have hΛapp : (fun y : M => (metricComparisonEndomorphismField (I := I) (M := M) g₀ g₀ y) (Y y)) =
      (fun y : M => Y y) := by
    funext y
    rw [metricComparisonEndomorphismField_apply]
    rw [show metricComparisonEndomorphism (I := I) g₀ g₀ y (Y y) = Y y from by
      rw [metricComparisonEndomorphism_apply, inverseMetricSharpFib_g0FlatCLM]]
  rw [hLeib, hΛapp]
  rw [metricComparisonEndomorphismField_apply]
  rw [show metricComparisonEndomorphism (I := I) g₀ g₀ x
...
private lemma palatiniCovGrad_slotInsert_fullRaised_id_zero (g₀ : SmoothRiemannianMetric I M) :
  ...
  rw [covGrad_toSection_apply_natural (I := I) (M := M) g₀ 1 1
    (endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
...
private lemma palatiniICG_slotInsert_fullRaised_id_succ_zero
  ...
  | zero =>
      ...
      exact palatiniCovGrad_slotInsert_fullRaised_id_zero (I := I) (M := M) g₀
  | succ m' ih =>
      rw [iteratedCovGrad_succ, ih, covGrad_zero]

private theorem metricComparisonEndomorphismField_iteratedCovGrad_gridWindow (g₀ : SmoothRiemannianMetric I M)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ S : ℕ → ℝ, (∀ l, 0 ≤ S l) ∧
  ...
  have hsplit : endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
      ...
      endoSlotZeroCcTensor (I := I) (M := M) g₀ 0 (metricComparisonDifferenceEndomorphismField (I := I) g₀ g₁) +
        endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
          (metricComparisonEndomorphismField (I := I) (M := M) g₀ g₀) := by
    rw [palatiniRaised_diff_split (I := I) (M := M) g₀ g₁,
      palatiniSlotInsertEndoCc_add (I := I) (M := M) g₀ 0]
  rw [hsplit]
  refine le_trans (palatiniRiemannianFiberNormSq_iteratedCovGrad_add_le (I := I) (M := M) g₀ 1 1 l _ _ x) ?_
  ...
  have hB : riemannianFiberNormSq (I := I) (M := M) g₀ 1 (1 + l) x
    ...
    | 0 =>
        ...
        exact hcid x
    | (m + 1) =>
        rw [palatiniICG_slotInsert_fullRaised_id_succ_zero (I := I) (M := M) g₀ m]
        rw [palatiniRiemannianFiberNormSq_zero_toSection]
...
omit [I.Boundaryless] in
lemma palatiniSlotInsertZero_fullRaisedRev_eq_omRecover
  ...
  rw [cotangentToDual_g0FlatCLM]
  rw [show (metricComparisonEndomorphismField (I := I) (M := M) g₁ g₀ x) w =
...
theorem fullRev0_eq (g₀ g₁ : SmoothRiemannianMetric I M) :
  ...
  palatiniSlotInsertZero_fullRaisedRev_eq_omRecover (I := I) (M := M) g₀ g₁
...
omit [BoundarylessManifold I M] [SigmaCompactSpace M] in
omit [I.Boundaryless] in
private lemma palatiniOmRecover_eq_idEndo_add_raise
  ...
  rw [show cotangentToDual (I := I)
      ...
      cotangentToDual (I := I)
          ((show Tensor0SSpace 1 I x →L[ℝ] Tensor0SSpace 1 I x from
            (endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
              (metricComparisonEndomorphismField (I := I) (M := M) g₀ g₀)).toSection x) om) w +
        cotangentToDual (I := I)
          ((show Tensor0SSpace 1 I x →L[ℝ] Tensor0SSpace 1 I x from
            (cometricRaiseSlot0Field (I := I) (M := M) g₀ 0
  ...
  rw [palatiniCotangentToDual_slotInsertEndoFib (I := I) (M := M) x
    (metricComparisonEndomorphismField (I := I) (M := M) g₀ g₀ x) om w]
  rw [show (metricComparisonEndomorphismField (I := I) (M := M) g₀ g₀ x) w = w from by
  ...
  rw [show cotangentToDual (I := I)
    ...
    rw [show (Tensor0SBundle.interiorProduct (𝕜 := ℝ) (I := I) (0 + 1) x
        ...
        Tensor0SSpace.toModel
          ...
          (fun _ : Fin 1 => tangentSpaceModelContinuousLinearEquiv (I := I) x w) from rfl]
    ...
    rw [show Tensor0SSpace.toModel
          ...
          (Fin.cons (tangentSpaceModelContinuousLinearEquiv (I := I) x
              (inverseMetricSharpFib (I := I) g₀ x om))
            (fun _ : Fin 1 => tangentSpaceModelContinuousLinearEquiv (I := I) x w)) =
        unitModel (I := I) (M := M) g₀ 2 (ccTensor02Symm (I := I) (M := M) g₀ T) x
          ![tangentSpaceModelContinuousLinearEquiv (I := I) x
              (inverseMetricSharpFib (I := I) g₀ x om),
            tangentSpaceModelContinuousLinearEquiv (I := I) x w] from by
      rw [unitModel]
      congr 1
      funext k
...
theorem omRecover_add
  ...
  exact palatiniOmRecover_eq_idEndo_add_raise (I := I) (M := M) g₀ g₁ T htie

omit [SigmaCompactSpace M] in
omit [NeZero (Module.finrank ℝ E)] in
lemma palatiniRiemannianFiberNormSq_iteratedCovGrad_ccTensor02Symm_le (g₀ : SmoothRiemannianMetric I M)
...
omit [NeZero (Module.finrank ℝ E)] in
theorem palatiniOmRecover_gridWindow (g₀ : SmoothRiemannianMetric I M)
    {δ₀ : ℝ} :
    ∃ C : ℕ → ℝ, (∀ l, 0 ≤ C l) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (T : SmoothCcTensor g₀ 0 2)
        (_htie : ∀ (y : M) (v w : TangentSpace I y),
  ...
  rw [palatiniOmRecover_eq_idEndo_add_raise (I := I) (M := M) g₀ g₁ T htie]
  refine le_trans (palatiniRiemannianFiberNormSq_iteratedCovGrad_add_le (I := I) (M := M) g₀ 1 1 l _ _ x) ?_
  have hA : riemannianFiberNormSq (I := I) (M := M) g₀ 1 (1 + l) x
      ((iteratedCovGrad (I := I) g₀ 1 1 l
        (endoSlotZeroCcTensor (I := I) (M := M) g₀ 0
  ...
  have hB : riemannianFiberNormSq (I := I) (M := M) g₀ 1 (1 + l) x
    ...
    | 0 =>
        ...
        linarith [h3, hW_nn]
    | (m + 1) =>
        have h1 := palatiniRiemannianFiberNormSq_iteratedCovGrad_ccTensor02Symm_le (I := I) (M := M) g₀ T (m + 1) x
...
private theorem palatiniLambdaSlotInsert3_gridWindow (g₀ : SmoothRiemannianMetric I M)
    {δ₀ : ℝ} :
    ∃ C : ℕ → ℝ, (∀ l, 0 ≤ C l) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (T : SmoothCcTensor g₀ 0 2)
        (_htie : ∀ (y : M) (v w : TangentSpace I y),
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 4 (4 + l) x
          ...
          C l * Combinatorics.antidiagonalTupleGridWindow
            (fun j => riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + j) x
              ((iteratedCovGrad (I := I) g₀ 0 2 j T).toSection x)) (l + 1) := by
  classical
  obtain ⟨C, hC_nn, hC⟩ := palatiniOmRecover_gridWindow (I := I) (M := M) g₀
  ...
  refine le_trans (riemannianFiberNormSq_iteratedCovGrad_slotInsertEndoCc_le_endo (I := I) (M := M) g₀ 3
    (metricComparisonEndomorphismField (I := I) (M := M) g₁ g₀) l x) ?_
...
def connectionDifferenceEndomorphism (g₀ gc : SmoothRiemannianMetric I M) :
...
omit [NeZero (Module.finrank ℝ E)] in
omit [SigmaCompactSpace M] in
lemma connectionDifferenceEndomorphism_apply (g₀ gc : SmoothRiemannianMetric I M) (x : M) :
    connectionDifferenceEndomorphism (I := I) (M := M) g₀ gc x = PDE.DeTurck.connectionDifference (I := I) gc g₀ x := rfl

omit [NeZero (Module.finrank ℝ E)] [I.Boundaryless] in
omit [SigmaCompactSpace M] in
lemma connectionDifferenceSection_eq_bilinearSlotInsertionCoefficient_zero (g₀ g₁ : SmoothRiemannianMetric I M) :
    connectionDifferenceSection (I := I) g₁ g₀ =
      bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 0 (connectionDifferenceEndomorphism (I := I) (M := M) g₀ g₁) := by
  apply SmoothCcTensor.ext
  apply ContMDiffSection.ext
  intro x
  apply ContinuousLinearMap.ext
  ...
  rw [show ((show Tensor0SSpace 1 I x →L[ℝ] Tensor0SSpace 2 I x from
        (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 0
          (connectionDifferenceEndomorphism (I := I) (M := M) g₀ g₁)).toSection x) om) =
      bilinearSlotInsertCLM (I := I) (M := M) 0 x (connectionDifferenceEndomorphism (I := I) (M := M) g₀ g₁ x) om
      from rfl]
...
omit [NeZero (Module.finrank ℝ E)] [I.Boundaryless] [BoundarylessManifold I M]
    [SigmaCompactSpace M] in
private lemma palatiniTermSlotEndoCc_one_eq_reindex_slotExtend (g₀ : SmoothRiemannianMetric I M)
  ...
  have e3 : Tensor0SSpace.eval
      ...
      Tensor0SSpace.eval
        (bilinearSlotInsertCLM (I := I) (M := M) 0 x (Term x)
...
private def bilinearSlotInsertionCoefficientReindexPermFour : Equiv.Perm (Fin 4) :=
...
omit [NeZero (Module.finrank ℝ E)] [I.Boundaryless] [BoundarylessManifold I M]
    [SigmaCompactSpace M] in
private lemma palatiniTermSlotEndoCc_two_eq_reindex_slotExtend (g₀ : SmoothRiemannianMetric I M)
    (Term : ContMDiffSection I (E →L[ℝ] (E →L[ℝ] E)) ∞
      (fun x : M => TangentSpace I x →L[ℝ] (TangentSpace I x →L[ℝ] TangentSpace I x))) :
  ...
  intro w
  have hτ0 : bilinearSlotInsertionCoefficientReindexPermFour (0 : Fin 4) = (2 : Fin 4) := by decide
  have hτ1 : bilinearSlotInsertionCoefficientReindexPermFour (1 : Fin 4) = (0 : Fin 4) := by decide
  have hτ2 : bilinearSlotInsertionCoefficientReindexPermFour (2 : Fin 4) = (1 : Fin 4) := by decide
  have hτ3 : bilinearSlotInsertionCoefficientReindexPermFour (3 : Fin 4) = (3 : Fin 4) := by decide
  ...
  have hLHS : Tensor0SSpace.eval
      ((show Tensor0SSpace 3 I x →L[ℝ] Tensor0SSpace 4 I x from
        (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 2 Term).toSection x) D) w =
      Tensor0SSpace.eval D
        (Function.update (Matrix.vecTail w) 0 (Term x (w 0) (Matrix.vecTail w 0))) := by
  ...
  have e1 : Tensor0SSpace.eval
    ...
    have h1 : ((show Tensor0SSpace 3 I x →L[ℝ] Tensor0SSpace 4 I x from
      ...
      rw [← hD'_def]
      rw [rsDomDomCongrSection_toSection]
    exact congrArg (fun t : Tensor0SSpace 4 I x => Tensor0SSpace.eval t w) h1
  have e2 : Tensor0SSpace.eval
      ((show Tensor0SSpace 3 I x →L[ℝ] Tensor0SSpace 4 I x from
        rsDomDomCongr bilinearSlotInsertionCoefficientReindexPermFour
          ((slotExtend (I := I) (M := M) g₀ 2 3
      ...
      Tensor0SSpace.eval
        ((show Tensor0SSpace 3 I x →L[ℝ] Tensor0SSpace 4 I x from
          (slotExtend (I := I) (M := M) g₀ 2 3
            (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 1 Term)).toSection x) D')
        (fun i => w (bilinearSlotInsertionCoefficientReindexPermFour i)) := by
    rw [rsDomDomCongr_apply_eval (I := I) (M := M)
      bilinearSlotInsertionCoefficientReindexPermFour
      ((slotExtend (I := I) (M := M) g₀ 2 3
        (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 1 Term)).toSection x) D' w]
  have e3 : Tensor0SSpace.eval
      ((show Tensor0SSpace 3 I x →L[ℝ] Tensor0SSpace 4 I x from
        (slotExtend (I := I) (M := M) g₀ 2 3
          (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 1 Term)).toSection x) D')
      (fun i => w (bilinearSlotInsertionCoefficientReindexPermFour i)) =
      Tensor0SSpace.eval
        ...
        (Matrix.vecTail (fun i => w (bilinearSlotInsertionCoefficientReindexPermFour i))) := by
    rw [show ((show Tensor0SSpace 3 I x →L[ℝ] Tensor0SSpace 4 I x from
          (slotExtend (I := I) (M := M) g₀ 2 3
            (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 1 Term)).toSection x) D') =
        (tensor0SCurry (I := I) (M := M) (𝕜 := ℝ) 3 x).symm
          ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 3 I x from
              (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 1 Term).toSection x).comp
            (tensor0SCurry (I := I) (M := M) (𝕜 := ℝ) 2 x D')) from rfl]
    rw [show (fun i => w (bilinearSlotInsertionCoefficientReindexPermFour i)) =
        Fin.cons (w (bilinearSlotInsertionCoefficientReindexPermFour 0))
          (Matrix.vecTail (fun i => w (bilinearSlotInsertionCoefficientReindexPermFour i))) from by
      funext k
      refine Fin.cases rfl (fun j => rfl) k]
    have hkey := TensorMultilinear.tensor0S_curry_apply_eval (I := I) (M := M) (n := 3)
      (T := (tensor0SCurry (I := I) (M := M) (𝕜 := ℝ) 3 x).symm
        ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 3 I x from
            (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 1 Term).toSection x).comp
          (tensor0SCurry (I := I) (M := M) (𝕜 := ℝ) 2 x D')))
      (v0 := w (bilinearSlotInsertionCoefficientReindexPermFour 0))
      (vs := Matrix.vecTail (fun i => w (bilinearSlotInsertionCoefficientReindexPermFour i)))
    rw [ContinuousLinearEquiv.apply_symm_apply] at hkey
    rw [← hkey]
    rfl
  have e4 : Tensor0SSpace.eval
      ...
      Tensor0SSpace.eval
        ...
        (Function.update (Matrix.vecTail
          (Matrix.vecTail (fun i => w (bilinearSlotInsertionCoefficientReindexPermFour i)))) 0
          (Term x (w (bilinearSlotInsertionCoefficientReindexPermFour 1)) (w (bilinearSlotInsertionCoefficientReindexPermFour 2)))) := by
  ...
  have e5 : Tensor0SSpace.eval
      (tensor0SCurry (I := I) (M := M) (𝕜 := ℝ) 2 x D' (w (bilinearSlotInsertionCoefficientReindexPermFour 0)))
      (Function.update (Matrix.vecTail (Matrix.vecTail (fun i => w (bilinearSlotInsertionCoefficientReindexPermFour i))))
        0
        (Term x (w (bilinearSlotInsertionCoefficientReindexPermFour 1)) (w (bilinearSlotInsertionCoefficientReindexPermFour 2)))) =
      Tensor0SSpace.eval D'
        (Fin.cons (w (bilinearSlotInsertionCoefficientReindexPermFour 0))
          (Function.update (Matrix.vecTail
            (Matrix.vecTail (fun i => w (bilinearSlotInsertionCoefficientReindexPermFour i)))) 0
    ...
    TensorMultilinear.tensor0S_curry_apply_eval (I := I) (M := M) (n := 2)
      (T := D') (v0 := w (bilinearSlotInsertionCoefficientReindexPermFour 0))
  ...
  have e6 : Tensor0SSpace.eval D'
    ...
    funext k
    fin_cases k
    · change ((Term x) (w (bilinearSlotInsertionCoefficientReindexPermFour 1))) (w (bilinearSlotInsertionCoefficientReindexPermFour 2)) =
        Function.update (Matrix.vecTail w) 0
          (((Term x) (w 0)) (Matrix.vecTail w 0)) (0 : Fin 3)
      rw [Function.update_self, hτ1, hτ2]
      rfl
    · change w (bilinearSlotInsertionCoefficientReindexPermFour 0) =
        Function.update (Matrix.vecTail w) 0
          (((Term x) (w 0)) (Matrix.vecTail w 0)) (1 : Fin 3)
      rw [hτ0, Function.update_of_ne (by decide : (1 : Fin 3) ≠ 0)]
      rfl
    · change w (bilinearSlotInsertionCoefficientReindexPermFour 3) =
        Function.update (Matrix.vecTail w) 0
          (((Term x) (w 0)) (Matrix.vecTail w 0)) (2 : Fin 3)
...
omit [SigmaCompactSpace M] in
lemma palatiniTermSlot2_riemannianFiberNormSq_le (g₀ g₁ : SmoothRiemannianMetric I M) (j : ℕ) (x : M) :
  ...
  rw [palatiniTermSlotEndoCc_two_eq_reindex_slotExtend (I := I) (M := M) g₀
    (connectionDifferenceEndomorphism (I := I) (M := M) g₀ g₁)]
  rw [riemannianFiberNormSq_iteratedCovGrad_rsDomDomCongr_both_eq (I := I) (M := M) g₀ 3 4
    (Equiv.swap (0 : Fin 3) 1) bilinearSlotInsertionCoefficientReindexPermFour
    (slotExtend (I := I) (M := M) g₀ 2 3
      (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 1 (connectionDifferenceEndomorphism (I := I) (M := M) g₀ g₁))) j x]
  refine le_trans (riemannianFiberNormSq_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 2 3
    (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 1 (connectionDifferenceEndomorphism (I := I) (M := M) g₀ g₁)) j x) ?_
  rw [pow_two, mul_assoc]
  refine mul_le_mul_of_nonneg_left ?_ (Nat.cast_nonneg _)
  rw [palatiniTermSlotEndoCc_one_eq_reindex_slotExtend (I := I) (M := M) g₀
    (connectionDifferenceEndomorphism (I := I) (M := M) g₀ g₁)]
  rw [riemannianFiberNormSq_iteratedCovGrad_rsDomDomCongr_both_eq (I := I) (M := M) g₀ 2 3
    (Equiv.swap (0 : Fin 2) 1) (finRotate 3).symm
    (slotExtend (I := I) (M := M) g₀ 1 2
      (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 0 (connectionDifferenceEndomorphism (I := I) (M := M) g₀ g₁))) j x]
  refine le_trans (riemannianFiberNormSq_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 1 2
    (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 0 (connectionDifferenceEndomorphism (I := I) (M := M) g₀ g₁)) j x) ?_
  refine mul_le_mul_of_nonneg_left (le_of_eq ?_) (Nat.cast_nonneg _)
  rw [← connectionDifferenceSection_eq_bilinearSlotInsertionCoefficient_zero (I := I) (M := M) g₀ g₁]

omit [NeZero (Module.finrank ℝ E)] [I.Boundaryless] [BoundarylessManifold I M]
    [SigmaCompactSpace M] in
private def palatiniBilinearSectionSub
    ...
    ContMDiffSection I (E →L[ℝ] (E →L[ℝ] E)) ∞
      (fun x : M =>
        TangentSpace I x →L[ℝ] (TangentSpace I x →L[ℝ] TangentSpace I x)) :=
  ⟨fun x => A x - B x, A.contMDiff.sub_section B.contMDiff⟩

omit [NeZero (Module.finrank ℝ E)] [I.Boundaryless] [BoundarylessManifold I M]
    [SigmaCompactSpace M] in
private lemma palatiniTermSlot_sub (g₀ : SmoothRiemannianMetric I M) (s : ℕ)
    (A B : ContMDiffSection I (E →L[ℝ] (E →L[ℝ] E)) ∞
      (fun x : M =>
        TangentSpace I x →L[ℝ] (TangentSpace I x →L[ℝ] TangentSpace I x))) :
    bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ s (palatiniBilinearSectionSub A B) =
  ...
  apply ContinuousLinearMap.ext
  intro D
  change termSlotFib (I := I) (M := M) s x (A x - B x) D =
    termSlotFib (I := I) (M := M) s x (A x) D -
      termSlotFib (I := I) (M := M) s x (B x) D
  apply Tensor0SSpace.toModel_injective
  apply ContinuousMultilinearMap.ext
  intro v
  change Tensor0SSpace.toModel (termSlotFib (I := I) (M := M) s x (A x - B x) D) v =
    Tensor0SSpace.toModel (termSlotFib (I := I) (M := M) s x (A x) D) v -
      Tensor0SSpace.toModel (termSlotFib (I := I) (M := M) s x (B x) D) v
  rw [palatiniTermSlotFib_apply_toModel, palatiniTermSlotFib_apply_toModel,
    palatiniTermSlotFib_apply_toModel]
  rw [sub_apply, slotInsertEndoFib_sub_left, sub_apply,
    Tensor0SSpace.toModel_sub, sub_apply]

omit [SigmaCompactSpace M] in
private lemma palatiniTermSlot2_sub_le
    (g₀ : SmoothRiemannianMetric I M)
    (A : ContMDiffSection I (E →L[ℝ] (E →L[ℝ] E)) ∞
      (fun x : M =>
        TangentSpace I x →L[ℝ] (TangentSpace I x →L[ℝ] TangentSpace I x)))
    (S : SmoothCcTensor g₀ 1 2)
    (hAS : bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 0 A = S)
    (j : ℕ) (x : M) :
    riemannianFiberNormSq (I := I) (M := M) g₀ 3 (4 + j) x
        ((iteratedCovGrad (I := I) g₀ 3 4 j
          (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 2 A)).toSection x) ≤
      (Module.finrank ℝ E : ℝ) ^ 2 *
        riemannianFiberNormSq (I := I) (M := M) g₀ 1 (2 + j) x
          ((iteratedCovGrad (I := I) g₀ 1 2 j S).toSection x) := by
  rw [palatiniTermSlotEndoCc_two_eq_reindex_slotExtend (I := I) (M := M) g₀ A]
  rw [riemannianFiberNormSq_iteratedCovGrad_rsDomDomCongr_both_eq
    (I := I) (M := M) g₀ 3 4
    (Equiv.swap (0 : Fin 3) 1) bilinearSlotInsertionCoefficientReindexPermFour
    (slotExtend (I := I) (M := M) g₀ 2 3
      (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 1 A)) j x]
  refine le_trans (riemannianFiberNormSq_iteratedCovGrad_slotExtend_le
    (I := I) (M := M) g₀ 2 3
    (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 1 A) j x) ?_
  rw [pow_two, mul_assoc]
  refine mul_le_mul_of_nonneg_left ?_ (Nat.cast_nonneg _)
  rw [palatiniTermSlotEndoCc_one_eq_reindex_slotExtend (I := I) (M := M) g₀ A]
  rw [riemannianFiberNormSq_iteratedCovGrad_rsDomDomCongr_both_eq
    (I := I) (M := M) g₀ 2 3
    (Equiv.swap (0 : Fin 2) 1) (finRotate 3).symm
    (slotExtend (I := I) (M := M) g₀ 1 2
      (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 0 A)) j x]
  refine le_trans (riemannianFiberNormSq_iteratedCovGrad_slotExtend_le
    (I := I) (M := M) g₀ 1 2
    (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 0 A) j x) ?_
  refine mul_le_mul_of_nonneg_left (le_of_eq ?_) (Nat.cast_nonneg _)
  rw [hAS]

def deTurckLieCovariantDerivativeSecondOrderCoefficient (g₀ g₁ : SmoothRiemannianMetric I M) : SmoothCcTensor g₀ 3 4 :=
  bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 2
    (connectionDifferenceEndomorphism (I := I) (M := M) g₀ g₁)

omit [SigmaCompactSpace M] in
theorem deTurckLieCovariantDerivativeSecondOrderCoefficient_l2 (g₀ g₁ : SmoothRiemannianMetric I M) (j : ℕ) (x : M) :
    riemannianFiberNormSq (I := I) (M := M) g₀ 3 (4 + j) x
        ((iteratedCovGrad (I := I) g₀ 3 4 j
          (deTurckLieCovariantDerivativeSecondOrderCoefficient (I := I) (M := M) g₀ g₁)).toSection x) ≤
      (Module.finrank ℝ E : ℝ) ^ 2 *
        riemannianFiberNormSq (I := I) (M := M) g₀ 1 (2 + j) x
          ((iteratedCovGrad (I := I) g₀ 1 2 j
            (connectionDifferenceSection (I := I) g₁ g₀)).toSection x) := by
  simpa only [deTurckLieCovariantDerivativeSecondOrderCoefficient] using
    palatiniTermSlot2_riemannianFiberNormSq_le (I := I) (M := M) g₀ g₁ j x

omit [SigmaCompactSpace M] in
theorem deTurckLieCovariantDerivativeSecondOrderCoefficient_sub_l2
      ...
      (Module.finrank ℝ E : ℝ) ^ 2 *
        riemannianFiberNormSq (I := I) (M := M) g₀ 1 (2 + j) x
          ((iteratedCovGrad (I := I) g₀ 1 2 j
            (connectionDifferenceSection (I := I) g₁ g₀ -
              connectionDifferenceSection (I := I) g₂ g₀)).toSection x) := by
  let A : ContMDiffSection I (E →L[ℝ] (E →L[ℝ] E)) ∞
      (fun x : M =>
        TangentSpace I x →L[ℝ] (TangentSpace I x →L[ℝ] TangentSpace I x)) :=
    palatiniBilinearSectionSub (connectionDifferenceEndomorphism (I := I) (M := M) g₀ g₁)
      (connectionDifferenceEndomorphism (I := I) (M := M) g₀ g₂)
  have hA :
      bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 0 A =
        connectionDifferenceSection (I := I) g₁ g₀ - connectionDifferenceSection (I := I) g₂ g₀ := by
    dsimp only [A]
    rw [palatiniTermSlot_sub]
    rw [← connectionDifferenceSection_eq_bilinearSlotInsertionCoefficient_zero (I := I) (M := M) g₀ g₁,
      ← connectionDifferenceSection_eq_bilinearSlotInsertionCoefficient_zero (I := I) (M := M) g₀ g₂]
  rw [show deTurckLieCovariantDerivativeSecondOrderCoefficient (I := I) (M := M) g₀ g₁ -
      deTurckLieCovariantDerivativeSecondOrderCoefficient (I := I) (M := M) g₀ g₂ =
        bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 2 A by
    dsimp only [A, deTurckLieCovariantDerivativeSecondOrderCoefficient]
    rw [palatiniTermSlot_sub]]
  exact palatiniTermSlot2_sub_le (I := I) (M := M) g₀ A _ hA j x

private noncomputable def palatiniKernelCLM (gA gB : SmoothRiemannianMetric I M) (x : M) :
    TangentSpace I x →L[ℝ] TangentSpace I x →L[ℝ] TangentSpace I x →L[ℝ] TangentSpace I x :=
  haveI : FiniteDimensional ℝ (TangentSpace I x) := inferInstanceAs (FiniteDimensional ℝ E)
  LinearMap.toContinuousLinearMap
    { toFun := fun v0 => connectionDifferenceCovDerivOp (I := I) gA gB x v0
      map_add' := fun v0 v0' => by
        ...
        intro q
        simp only [add_apply]
        exact deTurckLieConnectionDifferenceDerivativeCovKernel_add_left (I := I) gA gB x v0 v0' p q
      map_smul' := fun c v0 => by
        ...
        exact deTurckLieConnectionDifferenceDerivativeCovKernel_smul_left (I := I) gA gB x c v0 p q }

omit [CompactSpace M] [I.Boundaryless] in
omit [NeZero (Module.finrank ℝ E)] in
omit [SigmaCompactSpace M] in
private lemma palatiniKernelCLM_apply (gA gB : SmoothRiemannianMetric I M) (x : M)
    (v0 p q : TangentSpace I x) :
    palatiniKernelCLM (I := I) (M := M) gA gB x v0 p q =
      connectionDifferenceCovDerivOp (I := I) gA gB x v0 p q := by
  rw [palatiniKernelCLM, LinearMap.coe_toContinuousLinearMap']
  rfl

private noncomputable def palatiniFixLoweredCovec (g₀ gA gB : SmoothRiemannianMetric I M) (x : M) :
    Tensor0SSpace 4 I x :=
  ...
  (show ContinuousMultilinearMap ℝ (fun _ : Fin 4 => TangentSpace I x) ℝ from
    { toFun := fun m =>
        g₀.inner x (palatiniKernelCLM (I := I) (M := M) gA gB x (m 1) (m 2) (m 3)) (m 0)
      map_update_add' := by
        ...
        fin_cases i <;>
          simp only [Fin.reduceFinMk, Fin.isValue, Function.update_self, ne_eq,
            ...
            not_false_eq_true, ContinuousLinearMap.map_add, add_apply]
      map_update_smul' := by
        have h01 : (0 : Fin 4) ≠ 1 := by decide
        have h02 : (0 : Fin 4) ≠ 2 := by decide
        have h03 : (0 : Fin 4) ≠ 3 := by decide
        ...
        fin_cases i <;>
          simp only [Fin.reduceFinMk, Fin.isValue, Function.update_self, ne_eq,
            Function.update_of_ne, h01, h02, h03, h10, h12, h13, h20, h21, h23, h30, h31, h32,
            not_false_eq_true, ContinuousLinearMap.map_smul, smul_apply]
      cont := by
        have hK : Continuous (fun m : Fin 4 → TangentSpace I x =>
            palatiniKernelCLM (I := I) (M := M) gA gB x (m 1) (m 2) (m 3)) :=
          (((palatiniKernelCLM (I := I) (M := M) gA gB x).continuous.comp
...
omit [NeZero (Module.finrank ℝ E)] in
omit [SigmaCompactSpace M] in
private lemma palatiniFixLoweredCovec_apply (g₀ gA gB : SmoothRiemannianMetric I M) (x : M)
    (m : Fin 4 → TangentSpace I x) :
    palatiniFixLoweredCovec (I := I) g₀ gA gB x m =
      g₀.inner x (connectionDifferenceCovDerivOp (I := I) gA gB x (m 1) (m 2) (m 3)) (m 0) := by
  change g₀.inner x (palatiniKernelCLM (I := I) (M := M) gA gB x (m 1) (m 2) (m 3)) (m 0) = _
  rw [palatiniKernelCLM_apply]

omit [CompactSpace M] [I.Boundaryless] in
omit [NeZero (Module.finrank ℝ E)] in
omit [SigmaCompactSpace M] in
private lemma palatiniFixLoweredScalar_global (g₀ gA gB : SmoothRiemannianMetric I M)
    ...
    ContMDiff I 𝓘(ℝ, ℝ) ∞
      (fun x : M => g₀.inner x
        (connectionDifferenceCovDerivOp (I := I) gA gB x (V0 x) (p x) (q x)) (W x)) := by
  classical
  have hAsec := deTurckLieCovariantDerivativeA_section_contMDiff (I := I) gA gB V0 p q hV0 hp hq
  have hcongr : (fun x : M => g₀.inner x
        (connectionDifferenceCovDerivOp (I := I) gA gB x (V0 x) (p x) (q x)) (W x)) =
      (fun x : M => g₀.inner x (deTurckConnectionDifferenceCovDeriv (I := I) gA gB V0 p q x) (W x)) := by
    funext x
    rw [deTurckLieConnectionDifferenceDerivativeCovKernel_apply_field3 (I := I) gA gB x V0 p q
  ...
  exact contMDiff_g_inner_of_smooth_sections (I := I) g₀
    ⟨fun b => deTurckConnectionDifferenceCovDeriv (I := I) gA gB V0 p q b, hAsec⟩ ⟨fun b => W b, hW⟩

omit [CompactSpace M] [I.Boundaryless] in
omit [NeZero (Module.finrank ℝ E)] in
omit [SigmaCompactSpace M] in
private lemma palatiniFixLoweredScalar_contMDiffAt (g₀ gA gB : SmoothRiemannianMetric I M)
    (V0 V1 V2 V3 : Cₛ^∞⟮I; E, (TangentSpace I : M → Type _)⟯) (x₀ : M) :
    ContMDiffAt I 𝓘(ℝ, ℝ) ∞
      (fun x : M =>
        g₀.inner x (connectionDifferenceCovDerivOp (I := I) gA gB x (V1 x) (V2 x) (V3 x)) (V0 x)) x₀ := by
  have hglob := palatiniFixLoweredScalar_global (I := I) (M := M) g₀ gA gB
...
private theorem palatiniFixLoweredCovec_section_contMDiff (g₀ gA gB : SmoothRiemannianMetric I M) :
  ...
  have hscalar : ContMDiffAt I 𝓘(ℝ, ℝ) ∞
      (fun x : M => g₀.inner x
        (connectionDifferenceCovDerivOp (I := I) gA gB x (Y (σ 1) x) (Y (σ 2) x) (Y (σ 3) x))
        (Y (σ 0) x)) x₀ :=
    palatiniFixLoweredScalar_contMDiffAt (I := I) (M := M) g₀ gA gB
  ...
  have hframeEq : ∀ k : Fin 4, e₁.symmL ℝ x (b (σ k)) = (Y (σ k)) x := by
    ...
    exact Trivialization.symmL_apply e₁ hx₁ (b (σ k))
  change palatiniFixLoweredCovec (I := I) g₀ gA gB x (fun k => e₁.symmL ℝ x (b (σ k))) = _
  rw [palatiniFixLoweredCovec_apply]
  rw [hframeEq 0, hframeEq 1, hframeEq 2, hframeEq 3]

private def palatiniFixLoweredField (g₀ gA gB : SmoothRiemannianMetric I M) :
    Tensor0SBundle.Tensor0SField (𝕜 := ℝ) (E := E) (H := H) (I := I) (M := M) ∞ 4 :=
  letI := Tensor0SBundle.tensor0SBundleTopology (𝕜 := ℝ) (E := E) (H := H) (I := I) (M := M) 4
  ⟨fun x => palatiniFixLoweredCovec (I := I) g₀ gA gB x,
    palatiniFixLoweredCovec_section_contMDiff (I := I) (M := M) g₀ gA gB⟩

private def palatiniFixLoweredCc (g₀ gA gB : SmoothRiemannianMetric I M) :
    SmoothCcTensor g₀ 0 4 where
  toSection :=
    MixedSection.fromMultilinearSection (𝕜 := ℝ) (F := E) (IB := I)
      (E := (TangentSpace I : M → Type _)) ∞ (palatiniFixLoweredField (I := I) (M := M) g₀ gA gB)
  hasCompactSupport := HasCompactSupport.of_compactSpace _

omit [I.Boundaryless] in
omit [NeZero (Module.finrank ℝ E)] in
omit [SigmaCompactSpace M] in
private lemma palatiniFixLoweredCc_unitModel (g₀ gA gB : SmoothRiemannianMetric I M) (x : M) :
    unitModel (I := I) (M := M) g₀ 4 (palatiniFixLoweredCc (I := I) (M := M) g₀ gA gB) x =
      Tensor0SSpace.toModel (palatiniFixLoweredCovec (I := I) g₀ gA gB x) := by
  rw [unitModel]
  rw [show (palatiniFixLoweredCc (I := I) (M := M) g₀ gA gB).toSection x
        (unitTensor (I := I) (M := M) x) =
      (MixedSection.eval₀ (F := E) (E := (TangentSpace I : M → Type _)) x).smulRight
          (palatiniFixLoweredField (I := I) (M := M) g₀ gA gB x)
...
omit [NeZero (Module.finrank ℝ E)] in
omit [SigmaCompactSpace M] in
private lemma palatiniFixLoweredCc_unitModel_apply (g₀ gA gB : SmoothRiemannianMetric I M) (x : M)
    (m : Fin 4 → E) :
    unitModel (I := I) (M := M) g₀ 4 (palatiniFixLoweredCc (I := I) (M := M) g₀ gA gB) x m =
      g₀.inner x
        (connectionDifferenceCovDerivOp (I := I) gA gB x
  ...
  rw [palatiniFixLoweredCc_unitModel]
  rw [palatiniToModel_eq_eval]
  exact palatiniFixLoweredCovec_apply (I := I) (M := M) g₀ gA gB x
    (fun i => (tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m i))

private def connectionDifferenceQuadraticLoweredCc (g₀ gTerm gOut : SmoothRiemannianMetric I M) :
    SmoothCcTensor g₀ 0 4 :=
  ccOperatorFieldComp (I := I) (M := M) g₀ 0 3 4
    (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 2 (connectionDifferenceEndomorphism (I := I) (M := M) g₀ gTerm))
    (metricLoweredConnectionDifferenceCoefficient (I := I) g₀ gOut)

omit [I.Boundaryless] in
omit [NeZero (Module.finrank ℝ E)] in
omit [SigmaCompactSpace M] in
private lemma palatiniQuadLowCc_unitModel_apply (g₀ gTerm gOut : SmoothRiemannianMetric I M)
    (x : M) (m : Fin 4 → E) :
    unitModel (I := I) (M := M) g₀ 4 (connectionDifferenceQuadraticLoweredCc (I := I) (M := M) g₀ gTerm gOut) x m
      =
      g₀.inner x
        (PDE.DeTurck.connectionDifference (I := I) gOut g₀ x
          (PDE.DeTurck.connectionDifference (I := I) gTerm g₀ x
            ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 0))
            ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 1)))
          ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 2)))
        ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 3)) := by
  rw [unitModel]
  rw [show (connectionDifferenceQuadraticLoweredCc (I := I) (M := M) g₀ gTerm gOut).toSection x
        (unitTensor (I := I) (M := M) x) =
      bilinearSlotInsertCLM (I := I) (M := M) 2 x (connectionDifferenceEndomorphism (I := I) (M := M) g₀ gTerm x)
        ((show Tensor0SSpace 0 I x →L[ℝ] Tensor0SSpace 3 I x from
          (metricLoweredConnectionDifferenceCoefficient (I := I) g₀ gOut).toSection x)
          (unitTensor (I := I) (M := M) x)) from by
    rw [connectionDifferenceQuadraticLoweredCc, operatorFieldComposition_toSection]
    rfl]
  rw [palatiniTermSlotFib_apply_toModel (I := I) (M := M)
    (connectionDifferenceEndomorphism (I := I) (M := M) g₀ gTerm x)
    ((show Tensor0SSpace 0 I x →L[ℝ] Tensor0SSpace 3 I x from
      (metricLoweredConnectionDifferenceCoefficient (I := I) g₀ gOut).toSection x)
  ...
  rw [show Tensor0SSpace.toModel
        ((show Tensor0SSpace 0 I x →L[ℝ] Tensor0SSpace 3 I x from
          (metricLoweredConnectionDifferenceCoefficient (I := I) g₀ gOut).toSection x)
          (unitTensor (I := I) (M := M) x)) =
      unitModel (I := I) (M := M) g₀ 3 (metricLoweredConnectionDifferenceCoefficient (I := I) g₀ gOut) x from rfl]
  rw [show (Function.update (Matrix.vecTail m) 0
    ...
    · change Function.update (Matrix.vecTail m) 0
          (tangentLinearMapToModel
            (connectionDifferenceEndomorphism (I := I) (M := M) g₀ gTerm x
              ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 0)))
            (Matrix.vecTail m 0)) (2 : Fin 3) =
        m 3
      rw [Function.update_of_ne (by decide : (2 : Fin 3) ≠ 0)]
      rfl]
  rw [palatiniConnectionDifferenceLoweredCc_unitModel_apply (I := I) (M := M) g₀ gOut x]
  simp only [Matrix.cons_val_zero, Matrix.cons_val_one, Matrix.head_cons,
    Matrix.cons_val_two, Matrix.tail_cons, ContinuousLinearEquiv.symm_apply_apply]

omit [CompactSpace M] [I.Boundaryless] in
omit [NeZero (Module.finrank ℝ E)] [BoundarylessManifold I M] [SigmaCompactSpace M] in
lemma palatiniConnectionDifference_self_apply (g₀ : SmoothRiemannianMetric I M) (x : M)
    (u v : TangentSpace I x) :
    PDE.DeTurck.connectionDifference (I := I) g₀ g₀ x u v = 0 := by
  have h := DifferentialGeometry.Analysis.Sobolev.connectionDifference_cocycle (I := I) (M := M) g₀ g₀ g₀ x u v
  have h2 : PDE.DeTurck.connectionDifference (I := I) g₀ g₀ x u v +
      PDE.DeTurck.connectionDifference (I := I) g₀ g₀ x u v =
      PDE.DeTurck.connectionDifference (I := I) g₀ g₀ x u v := h
  have h3 := add_right_cancel (a := PDE.DeTurck.connectionDifference (I := I) g₀ g₀ x u v)
    (b := PDE.DeTurck.connectionDifference (I := I) g₀ g₀ x u v) (c := 0)
  apply h3
  rw [zero_add]
  exact h2
...
private lemma deTurckLieConnectionDifferenceDerivativeCovKernel_diff_eq_deTurckLieConnectionDifferenceDerivativeCovKernel_connectionDifference_expansion
    ...
    connectionDifferenceCovDerivOp (I := I) g₁ g_bg x v0 p q - connectionDifferenceCovDerivOp (I := I) g₁ g₀ x v0 p q =
      -connectionDifferenceCovDerivOp (I := I) g_bg g₀ x v0 p q
        + PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x
            (PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x p q) v0
        - PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x
            (PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x p v0) q
        - PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x p
            (PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x q v0)
        - PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x
            (PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x p q) v0
        + PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x
  ...
  have hcc : ∀ u v : TangentSpace I x,
      PDE.DeTurck.connectionDifference (I := I) g₁ g_bg x u v =
        PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x u v -
          PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x u v := by
    intro u v
    have h := DifferentialGeometry.Analysis.Sobolev.connectionDifference_cocycle (I := I)
      (M := M) g₁ g_bg g₀ x u v
    exact eq_sub_of_add_eq h
  have hS1 := DifferentialGeometry.Analysis.Sobolev.deTurckLieConnectionDifferenceDerivativeCovKernel_backgroundSplit (I := I) (M := M)
    g₀ g₁ g_bg x v0 p q
  have hS2 := DifferentialGeometry.Analysis.Sobolev.deTurckLieConnectionDifferenceDerivativeCovKernel_backgroundSplit (I := I) (M := M)
    g₀ g₁ g₀ x v0 p q
  have hS3 := DifferentialGeometry.Analysis.Sobolev.deTurckLieConnectionDifferenceDerivativeCovKernel_backgroundSplit (I := I) (M := M)
    g₀ g_bg g₀ x v0 p q
  rw [hS1, hS2, hS3]
  have hT1 : PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x
      (PDE.DeTurck.connectionDifference (I := I) g₁ g_bg x p q) v0 =
      PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x
          (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p q) v0 -
        PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x
          (PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x p q) v0 := by
    rw [hcc p q, map_sub, sub_apply]
  have hT2 : PDE.DeTurck.connectionDifference (I := I) g₁ g_bg x
      (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v0) q =
      PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x
          (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v0) q -
        PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x
          (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v0) q :=
    hcc (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p v0) q
  have hT3 : PDE.DeTurck.connectionDifference (I := I) g₁ g_bg x p
      (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x q v0) =
      PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p
          (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x q v0) -
        PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x p
          (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x q v0) :=
    hcc p (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x q v0)
  have hT4 : PDE.DeTurck.connectionDifference (I := I) g₁ g_bg x
      (PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x p q) v0 =
      PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x
          (PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x p q) v0 -
        PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x
          (PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x p q) v0 :=
    hcc (PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x p q) v0
  have hT1' : PDE.DeTurck.connectionDifference (I := I) g₁ g_bg x p q =
      PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p q -
        PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x p q := hcc p q
  rw [hT1', hT2, hT3]
  rw [show PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x
      ...
      PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x
          (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x p q) v0 -
        PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x
          (PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x p q) v0 from by
    rw [map_sub, sub_apply]]
  abel

private def palatiniSigma2 : Equiv.Perm (Fin 4) :=
  ⟨fun i => (![2, 3, 1, 0] : Fin 4 → Fin 4) i,
   fun i => (![3, 2, 0, 1] : Fin 4 → Fin 4) i,
   by decide, by decide⟩

private def palatiniSigma3 : Equiv.Perm (Fin 4) :=
  ⟨fun i => (![2, 1, 3, 0] : Fin 4 → Fin 4) i,
   fun i => (![3, 1, 0, 2] : Fin 4 → Fin 4) i,
   by decide, by decide⟩

private def palatiniSigma4 : Equiv.Perm (Fin 4) :=
  ⟨fun i => (![3, 1, 2, 0] : Fin 4 → Fin 4) i,
   fun i => (![3, 1, 2, 0] : Fin 4 → Fin 4) i,
   by decide, by decide⟩

private def deTurckLieConnectionDifferenceDerivativeCovKernelDiffLoweredCc (g₀ g₁ g_bg : SmoothRiemannianMetric I M) : SmoothCcTensor g₀
    0 4 :=
  (-1 : ℝ) • palatiniFixLoweredCc (I := I) (M := M) g₀ g_bg g₀
    + domDomCongrSection (I := I) g₀ palatiniSigma2
      (connectionDifferenceQuadraticLoweredCc (I := I) (M := M) g₀ g_bg g_bg)
    - domDomCongrSection (I := I) g₀ palatiniSigma3
      (connectionDifferenceQuadraticLoweredCc (I := I) (M := M) g₀ g_bg g_bg)
    - domDomCongrSection (I := I) g₀ palatiniSigma4
      (connectionDifferenceQuadraticLoweredCc (I := I) (M := M) g₀ g_bg g_bg)
    - domDomCongrSection (I := I) g₀ palatiniSigma2
      (connectionDifferenceQuadraticLoweredCc (I := I) (M := M) g₀ g_bg g₁)
    + domDomCongrSection (I := I) g₀ palatiniSigma3
      (connectionDifferenceQuadraticLoweredCc (I := I) (M := M) g₀ g₁ g_bg)
    + domDomCongrSection (I := I) g₀ palatiniSigma4
      (connectionDifferenceQuadraticLoweredCc (I := I) (M := M) g₀ g₁ g_bg)

omit [NeZero (Module.finrank ℝ E)] [CompactSpace M] [I.Boundaryless] [BoundarylessManifold I M]
    [T2Space M] [SigmaCompactSpace M] in
lemma palatiniUnitModel_smul (g₀ : SmoothRiemannianMetric I M) (s : ℕ) (c : ℝ)
...
omit [NeZero (Module.finrank ℝ E)] in
omit [SigmaCompactSpace M] in
private lemma palatiniLow0_unitModel_apply (g₀ g₁ g_bg : SmoothRiemannianMetric I M)
    (x : M) (m : Fin 4 → E) :
    unitModel (I := I) (M := M) g₀ 4 (deTurckLieConnectionDifferenceDerivativeCovKernelDiffLoweredCc (I := I) (M := M) g₀ g₁ g_bg) x m =
      g₀.inner x
        (connectionDifferenceCovDerivOp (I := I) g₁ g_bg x
            ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 1))
            ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 2))
            ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 3)) -
          connectionDifferenceCovDerivOp (I := I) g₁ g₀ x
            ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 1))
            ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 2))
            ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 3)))
        ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 0)) := by
  classical
  rw [deTurckLieConnectionDifferenceDerivativeCovKernelDiffLoweredCc]
  rw [palatiniUnitModel_add, palatiniUnitModel_add, palatiniUnitModel_sub, palatiniUnitModel_sub, palatiniUnitModel_sub,
    palatiniUnitModel_add, palatiniUnitModel_smul]
  ...
  rw [palatiniFixLoweredCc_unitModel_apply (I := I) (M := M) g₀ g_bg g₀ x m]
  rw [palatiniQuadLowCc_unitModel_apply (I := I) (M := M) g₀ g_bg g_bg x
      (fun i => m (palatiniSigma2 i)),
    palatiniQuadLowCc_unitModel_apply (I := I) (M := M) g₀ g_bg g_bg x
      (fun i => m (palatiniSigma3 i)),
    palatiniQuadLowCc_unitModel_apply (I := I) (M := M) g₀ g_bg g_bg x
      (fun i => m (palatiniSigma4 i)),
    palatiniQuadLowCc_unitModel_apply (I := I) (M := M) g₀ g_bg g₁ x
      (fun i => m (palatiniSigma2 i)),
    palatiniQuadLowCc_unitModel_apply (I := I) (M := M) g₀ g₁ g_bg x
      (fun i => m (palatiniSigma3 i)),
    palatiniQuadLowCc_unitModel_apply (I := I) (M := M) g₀ g₁ g_bg x
      (fun i => m (palatiniSigma4 i))]
  have hs2_0 : palatiniSigma2 (0 : Fin 4) = 2 := by decide
  have hs2_1 : palatiniSigma2 (1 : Fin 4) = 3 := by decide
  have hs2_2 : palatiniSigma2 (2 : Fin 4) = 1 := by decide
  have hs2_3 : palatiniSigma2 (3 : Fin 4) = 0 := by decide
  have hs3_0 : palatiniSigma3 (0 : Fin 4) = 2 := by decide
  have hs3_1 : palatiniSigma3 (1 : Fin 4) = 1 := by decide
  have hs3_2 : palatiniSigma3 (2 : Fin 4) = 3 := by decide
  have hs3_3 : palatiniSigma3 (3 : Fin 4) = 0 := by decide
  have hs4_0 : palatiniSigma4 (0 : Fin 4) = 3 := by decide
  have hs4_1 : palatiniSigma4 (1 : Fin 4) = 1 := by decide
  ...
  rw [deTurckLieConnectionDifferenceDerivativeCovKernel_diff_eq_deTurckLieConnectionDifferenceDerivativeCovKernel_connectionDifference_expansion
    ...
    ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 2))
    ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 3))]
  have hsymm4 : PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x
      ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 2))
      (PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x
        ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 3))
        ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 1))) =
      PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x
        (PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x
          ...
          ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 1)))
        ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 2)) :=
    PDE.DeTurck.connectionDifference_symm (I := I) g_bg g₀ x
      ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 2))
      (PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x
        ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 3))
        ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 1)))
  have hsymm7 : PDE.DeTurck.connectionDifference (I := I) g_bg g₀ x
      ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 2))
      (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x
        ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 3))
        ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 1))) =
    ...
    PDE.DeTurck.connectionDifference_symm (I := I) g_bg g₀ x
      ...
      (PDE.DeTurck.connectionDifference (I := I) g₁ g₀ x
        ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 3))
        ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 1)))
  simp only [map_add, map_sub, map_neg, add_apply,
    sub_apply, neg_apply]
  rw [← hsymm4, ← hsymm7]
  ring

private def deTurckTermCoeffDiffHalfCc (g₀ g₁ g_bg : SmoothRiemannianMetric I M) : SmoothCcTensor g₀
    0 4 :=
  ccOperatorFieldComp (I := I) (M := M) g₀ 0 4 4
    (endoSlotZeroCcTensor (I := I) (M := M) g₀ 3 (metricComparisonEndomorphismField (I := I) (M := M) g₁ g₀))
    (deTurckLieConnectionDifferenceDerivativeCovKernelDiffLoweredCc (I := I) (M := M) g₀ g₁ g_bg)

private def deTurckTermCoeffDiffCc (g₀ g₁ g_bg : SmoothRiemannianMetric I M) : SmoothCcTensor g₀ 0
    4 :=
  domDomCongrSection (I := I) g₀ (Equiv.swap (0 : Fin 4) 1)
      (deTurckTermCoeffDiffHalfCc (I := I) (M := M) g₀ g₁ g_bg)
    + deTurckTermCoeffDiffHalfCc (I := I) (M := M) g₀ g₁ g_bg
...
omit [NeZero (Module.finrank ℝ E)] [CompactSpace M] [I.Boundaryless] [BoundarylessManifold I M]
    [T2Space M] [SigmaCompactSpace M] in
lemma palatiniG0_inner_lambda (g₀ g₁ : SmoothRiemannianMetric I M) (x : M)
    (u v : TangentSpace I x) :
    g₀.inner x u (metricComparisonEndomorphism (I := I) g₁ g₀ x v) = g₁.inner x u v := by
  rw [metricComparisonEndomorphism_apply]
...
omit [I.Boundaryless] in
omit [SigmaCompactSpace M] in
private lemma palatiniXdHalf_unitModel_apply (g₀ g₁ g_bg : SmoothRiemannianMetric I M)
    (x : M) (m : Fin 4 → E) :
    unitModel (I := I) (M := M) g₀ 4 (deTurckTermCoeffDiffHalfCc (I := I) (M := M) g₀ g₁ g_bg) x m =
      g₁.inner x
        (connectionDifferenceCovDerivOp (I := I) g₁ g_bg x
            ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 1))
            ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 2))
  ...
  rw [show Tensor0SSpace.toModel
        ((show Tensor0SSpace 0 I x →L[ℝ] Tensor0SSpace 4 I x from
          (deTurckLieConnectionDifferenceDerivativeCovKernelDiffLoweredCc (I := I) (M := M) g₀ g₁ g_bg).toSection x)
          (unitTensor (I := I) (M := M) x)) =
      unitModel (I := I) (M := M) g₀ 4 (deTurckLieConnectionDifferenceDerivativeCovKernelDiffLoweredCc (I := I) (M := M) g₀ g₁ g_bg) x
        from rfl]
  rw [palatiniLow0_unitModel_apply (I := I) (M := M) g₀ g₁ g_bg x
    (Function.update m 0
      (tangentLinearMapToModel
  ...
  simp only [Function.update_of_ne (by decide : (1 : Fin 4) ≠ 0),
    ...
    Function.update_of_ne (by decide : (3 : Fin 4) ≠ 0), Function.update_self,
    tangentLinearMapToModel_apply, ContinuousLinearEquiv.symm_apply_apply]
  rw [metricComparisonEndomorphismField_apply]
  exact palatiniG0_inner_lambda (I := I) (M := M) g₀ g₁ x
    (connectionDifferenceCovDerivOp (I := I) g₁ g_bg x
        ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 1))
        ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 2))
        ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 3)) -
...
omit [I.Boundaryless] in
omit [SigmaCompactSpace M] in
private lemma palatiniXd_unitModel_apply (g₀ g₁ g_bg : SmoothRiemannianMetric I M)
    (x : M) (m : Fin 4 → E) :
    unitModel (I := I) (M := M) g₀ 4 (deTurckTermCoeffDiffCc (I := I) (M := M) g₀ g₁ g_bg) x m =
      g₁.inner x
          (connectionDifferenceCovDerivOp (I := I) g₁ g_bg x
              ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 0))
              ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 2))
        ...
        g₁.inner x
          (connectionDifferenceCovDerivOp (I := I) g₁ g_bg x
            ...
            connectionDifferenceCovDerivOp (I := I) g₁ g₀ x
              ...
              ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 3)))
          ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (m 0)) := by
  rw [deTurckTermCoeffDiffCc, palatiniUnitModel_add, add_apply]
  rw [domDomCongrSection_unitModel, ContinuousMultilinearMap.domDomCongr_apply]
  rw [palatiniXdHalf_unitModel_apply (I := I) (M := M) g₀ g₁ g_bg x
    (fun i => m ((Equiv.swap (0 : Fin 4) 1) i))]
  rw [palatiniXdHalf_unitModel_apply (I := I) (M := M) g₀ g₁ g_bg x m]
...
private theorem palatiniCovDerivTermDiff_eq_pairTrace
      ...
      (-1 : ℝ) • ccOperatorFieldComp (I := I) (M := M) g₀ 2 6 2
        ...
        (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 termPairTraceSlotPerm6
          (slotExtendIter (I := I) (M := M) g₀ 0 4 2
            (deTurckTermCoeffDiffCc (I := I) (M := M) g₀ g₁ g_bg))) := by
  classical
  apply SmoothCcTensor.ext
  apply ContMDiffSection.ext
  intro x
  ...
  have hsmul : ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 2 I x from
      (((-1 : ℝ) • ccOperatorFieldComp (I := I) (M := M) g₀ 2 6 2
        ...
        (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 termPairTraceSlotPerm6
          (slotExtendIter (I := I) (M := M) g₀ 0 4 2
            (deTurckTermCoeffDiffCc (I := I) (M := M) g₀ g₁ g_bg)))).toSection x)) D) =
      (-1 : ℝ) • ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 2 I x from
        ((ccOperatorFieldComp (I := I) (M := M) g₀ 2 6 2 (cometricDoublePairTraceCoefficient (I := I) (M := M) g₀ g₁)
          (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 termPairTraceSlotPerm6
            (slotExtendIter (I := I) (M := M) g₀ 0 4 2
              (deTurckTermCoeffDiffCc (I := I) (M := M) g₀ g₁ g_bg)))).toSection x)) D) := by
    rw [show ((((-1 : ℝ) • ccOperatorFieldComp (I := I) (M := M) g₀ 2 6 2
        (cometricDoublePairTraceCoefficient (I := I) (M := M) g₀ g₁)
        (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 termPairTraceSlotPerm6
          (slotExtendIter (I := I) (M := M) g₀ 0 4 2
            (deTurckTermCoeffDiffCc (I := I) (M := M) g₀ g₁ g_bg)))).toSection x)) =
        (-1 : ℝ) • ((ccOperatorFieldComp (I := I) (M := M) g₀ 2 6 2
          (cometricDoublePairTraceCoefficient (I := I) (M := M) g₀ g₁)
          (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 termPairTraceSlotPerm6
            (slotExtendIter (I := I) (M := M) g₀ 0 4 2
  ...
  rw [palatiniPairTraceOp_apply_toModel (I := I) (M := M) g₀ g₁
    (deTurckTermCoeffDiffCc (I := I) (M := M) g₀ g₁ g_bg) x D v]
  rw [show ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 2 I x from
      ((deTurckLieConnectionDifferenceDerivCoeffField (I := I) (M := M) g₀ g₁ g_bg -
        deTurckLieConnectionDifferenceDerivCoeffField (I := I) (M := M) g₀ g₁ g₀).toSection x)) D) =
      (show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 2 I x from
          (deTurckLieConnectionDifferenceDerivCoeffField (I := I) (M := M) g₀ g₁ g_bg).toSection x) D -
        (show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 2 I x from
          (deTurckLieConnectionDifferenceDerivCoeffField (I := I) (M := M) g₀ g₁ g₀).toSection x) D from by
    rw [show ((deTurckLieConnectionDifferenceDerivCoeffField (I := I) (M := M) g₀ g₁ g_bg -
        deTurckLieConnectionDifferenceDerivCoeffField (I := I) (M := M) g₀ g₁ g₀).toSection x) =
        (deTurckLieConnectionDifferenceDerivCoeffField (I := I) (M := M) g₀ g₁ g_bg).toSection x -
          (deTurckLieConnectionDifferenceDerivCoeffField (I := I) (M := M) g₀ g₁ g₀).toSection x from by
      rw [SmoothCcTensor.toSection_sub]; rfl]
    rfl]
  rw [Tensor0SSpace.toModel_sub, sub_apply]
  rw [show (show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 2 I x from
      (deTurckLieConnectionDifferenceDerivCoeffField (I := I) (M := M) g₀ g₁ g_bg).toSection x) D =
      connectionDifferenceCovDerivBiContrFib (I := I) g₁ g_bg x D from rfl]
  rw [show (show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 2 I x from
      (deTurckLieConnectionDifferenceDerivCoeffField (I := I) (M := M) g₀ g₁ g₀).toSection x) D =
      connectionDifferenceCovDerivBiContrFib (I := I) g₁ g₀ x D from rfl]
  rw [show (connectionDifferenceCovDerivBiContrFib (I := I) g₁ g_bg x : Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace
    2 I x) =
      connectionDifferenceCovDerivBiContrFibFixedFrame (I := I) g₁ g_bg (smoothOrthoFrame (I := I) g₁ x) x from
        rfl]
  rw [show (connectionDifferenceCovDerivBiContrFib (I := I) g₁ g₀ x : Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 2
    I x) =
      connectionDifferenceCovDerivBiContrFibFixedFrame (I := I) g₁ g₀ (smoothOrthoFrame (I := I) g₁ x) x from
        rfl]
  rw [deTurckLieConnectionDifferenceDerivativeBiContrFibFixedFrame_toModel (I := I) g₁ g_bg (smoothOrthoFrame (I := I) g₁ x) x D v]
  rw [deTurckLieConnectionDifferenceDerivativeBiContrFibFixedFrame_toModel (I := I) g₁ g₀ (smoothOrthoFrame (I := I) g₁ x) x D v]
  have hXval : ∀ a b : Fin (Module.finrank ℝ E),
      unitModel (I := I) (M := M) g₀ 4 (deTurckTermCoeffDiffCc (I := I) (M := M) g₀ g₁ g_bg) x
        ![v 0, v 1,
      ...
      (g₁.inner x (connectionDifferenceCovDerivOp (I := I) g₁ g_bg x
        ...
        g₁.inner x (connectionDifferenceCovDerivOp (I := I) g₁ g_bg x
          ...
          (smoothOrthoFrame (I := I) g₁ x a x) (smoothOrthoFrame (I := I) g₁ x b x))
          ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (v 0))) -
      (g₁.inner x (connectionDifferenceCovDerivOp (I := I) g₁ g₀ x
          ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (v 0))
          (smoothOrthoFrame (I := I) g₁ x a x) (smoothOrthoFrame (I := I) g₁ x b x))
          ((tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (v 1)) +
        g₁.inner x (connectionDifferenceCovDerivOp (I := I) g₁ g₀ x
    ...
    intro a b
    rw [palatiniXd_unitModel_apply (I := I) (M := M) g₀ g₁ g_bg x
      (![v 0, v 1,
        tangentSpaceModelContinuousLinearEquiv (I := I) x (smoothOrthoFrame (I := I) g₁ x a x),
        tangentSpaceModelContinuousLinearEquiv (I := I) x
          (smoothOrthoFrame (I := I) g₁ x b x)] : Fin 4 → E)]
    simp only [Matrix.cons_val_zero, Matrix.cons_val_one, Matrix.head_cons,
        ...
        Tensor0SSpace.toModel D
          ...
          unitModel (I := I) (M := M) g₀ 4 (deTurckTermCoeffDiffCc (I := I) (M := M) g₀ g₁ g_bg) x
            ![v 0, v 1,
              tangentSpaceModelContinuousLinearEquiv (I := I) x (smoothOrthoFrame (I := I) g₁ x a x),
              tangentSpaceModelContinuousLinearEquiv (I := I) x
                (smoothOrthoFrame (I := I) g₁ x b x)] from Finset.sum_comm]
...
omit [NeZero (Module.finrank ℝ E)] [CompactSpace M] [I.Boundaryless] [BoundarylessManifold I M]
    [T2Space M] [SigmaCompactSpace M] in
lemma palatiniRiemannianFiberNormSq_neg (g : SmoothRiemannianMetric I M) (r s : ℕ) (x : M)
...
omit [NeZero (Module.finrank ℝ E)] in
lemma palatiniRiemannianFiberNormSq_iteratedCovGrad_sub_le (g : SmoothRiemannianMetric I M) (r s : ℕ) (j : ℕ)
  ...
  rw [palatiniRiemannianFiberNormSq_neg (I := I) (M := M) g r (s + j) x]

theorem palatiniPureDT_tgrid (g₀ : SmoothRiemannianMetric I M) (s : ℕ)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ C : ℕ → ℝ, (∀ j, 0 ≤ C j) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (T : SmoothCcTensor g₀ 0 2)
        (_htie : ∀ (y : M) (v w : TangentSpace I y),
  ...
  classical
  obtain ⟨S, hS_nn, hS⟩ := metricComparisonEndomorphismField_iteratedCovGrad_gridWindow (I := I) (M := M) g₀ hδ₀
  obtain ⟨c0, hc0_nn, hc0⟩ := exists_iteratedCovGrad_riemannianFiberNormSq_bound (I := I) (M := M) g₀ (s + 2) s
    (cometricDoubleTraceField (I := I) g₀ s)
  ...
  have hsFlat : ∀ l : ℕ, l < j + 1 →
    ...
    intro l hl
    refine le_trans (riemannianFiberNormSq_iteratedCovGrad_slotInsertEndoCc_le_endo (I := I) (M := M) g₀ (s + 1)
      (metricComparisonEndomorphismField (I := I) (M := M) g₀ g₁) l x) ?_
    rw [← hfr_def]
  ...
  have hzero : ∀ i' ∈ Finset.range (j + 1), i' ≠ 0 →
    ...
    rw [iteratedCovGrad_succ_cometricDoubleTraceField_eq_zero (I := I) (M := M) g₀ s m]
    rw [palatiniRiemannianFiberNormSq_zero_toSection, zero_mul]
      ...
      riemannianFiberNormSq (I := I) (M := M) g₀ (s + 2) (s + 0) x
        ...
        ∑ l ∈ Finset.range (j + 1 - 0),
          riemannianFiberNormSq (I := I) (M := M) g₀ (s + 2) ((s + 2) + l) x
            ((iteratedCovGrad (I := I) g₀ (s + 2) (s + 2) l
              (endoSlotZeroCcTensor (I := I) (M := M) g₀ (s + 1)
                (metricComparisonEndomorphismField (I := I) (M := M) g₀ g₁))).toSection x) := by
  ...
  have hriemannianFiberNormSq_nn : 0 ≤ riemannianFiberNormSq (I := I) (M := M) g₀ (s + 2) (s + 0) x
      ((iteratedCovGrad (I := I) g₀ (s + 2) s 0
        (cometricDoubleTraceField (I := I) g₀ s)).toSection x) :=
  ...
  refine le_trans (mul_le_mul_of_nonneg_left
    (mul_le_mul hc0' hsumS hsum_nn (hc0_nn 0)) (operatorFieldApplicationGdiag_nonneg (E := E) j)) ?_
  rw [← mul_assoc, ← mul_assoc]
  rw [mul_assoc (diagonalGridGrowthFactor (E := E) j) (c0 0)]

theorem palatiniPairTraceOp_tgrid (g₀ : SmoothRiemannianMetric I M)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ C : ℕ → ℝ, (∀ j, 0 ≤ C j) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (T : SmoothCcTensor g₀ 0 2)
        (_htie : ∀ (y : M) (v w : TangentSpace I y),
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 6 (2 + j) x
            ((iteratedCovGrad (I := I) g₀ 6 2 j
              (cometricDoublePairTraceCoefficient (I := I) (M := M) g₀ g₁)).toSection x) ≤
          C j * Combinatorics.antidiagonalTupleGridWindow
            (fun l => riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + l) x
              ((iteratedCovGrad (I := I) g₀ 0 2 l T).toSection x)) (j + 1) := by
  classical
  obtain ⟨C2, hC2_nn, hC2⟩ := palatiniPureDT_tgrid (I := I) (M := M) g₀ 2 hδ₀
  ...
  rw [cometricDoublePairTraceCoefficient]
  refine le_trans
    (riemannianFiberNormSq_iteratedCovGrad_ccTensorCompose_diagonalProductGrid_leftFactor_le
    (I := I) (M := M) g₀ j 6 4 2
    (cometricDoubleTraceCc (I := I) (M := M) g₀ g₁ 2)
...
private theorem palatiniQuadLow_movingOuter_gridWindow (g₀ g_bg : SmoothRiemannianMetric I M)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ C : ℕ → ℝ, (∀ l, 0 ≤ C l) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + l) x
            ((iteratedCovGrad (I := I) g₀ 0 4 l
              (connectionDifferenceQuadraticLoweredCc (I := I) (M := M) g₀ g_bg g₁)).toSection x) ≤
          C l * Combinatorics.antidiagonalTupleGridWindow
            (fun l' => riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + l') x
  ...
  obtain ⟨cAS, hcAS_nn, hcAS⟩ := exists_iteratedCovGrad_riemannianFiberNormSq_bound (I := I) (M := M) g₀ 3 4
    (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 2 (connectionDifferenceEndomorphism (I := I) (M := M) g₀ g_bg))
  ...
  rw [connectionDifferenceQuadraticLoweredCc]
  refine le_trans
    (riemannianFiberNormSq_iteratedCovGrad_ccTensorCompose_diagonalProductGrid_leftFactor_le
    (I := I) (M := M) g₀ l 0 3 4
    (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 2 (connectionDifferenceEndomorphism (I := I) (M := M) g₀ g_bg))
  ...
  have hcell : ∀ i' ∈ Finset.range (l + 1),
      riemannianFiberNormSq (I := I) (M := M) g₀ 3 (4 + i') x
          ((iteratedCovGrad (I := I) g₀ 3 4 i'
            (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 2
              (connectionDifferenceEndomorphism (I := I) (M := M) g₀ g_bg))).toSection x) *
    ...
    have hA1 : riemannianFiberNormSq (I := I) (M := M) g₀ 3 (4 + i') x
        ((iteratedCovGrad (I := I) g₀ 3 4 i'
          (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 2
            (connectionDifferenceEndomorphism (I := I) (M := M) g₀ g_bg))).toSection x) ≤ cAS i' := hcAS i' x
...
private theorem palatiniQuadLow_movingTerm_gridWindow (g₀ g_bg : SmoothRiemannianMetric I M)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ C : ℕ → ℝ, (∀ l, 0 ≤ C l) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        (_htie : ∀ (y : M) (v w : TangentSpace I y),
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + l) x
            ((iteratedCovGrad (I := I) g₀ 0 4 l
              (connectionDifferenceQuadraticLoweredCc (I := I) (M := M) g₀ g₁ g_bg)).toSection x) ≤
          C l * Combinatorics.antidiagonalTupleGridWindow
            (fun l' => riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + l') x
              ((iteratedCovGrad (I := I) g₀ 0 2 l' P).toSection x)) (l + 2) := by
  classical
  obtain ⟨CA, hCA_nn, hCA⟩ :=
    exists_riemannianFiberNormSq_iteratedCovGrad_connectionDifferenceSection_tgrid (I := I) (M := M) g₀ hδ₀
  obtain ⟨cL, hcL_nn, hcL⟩ := exists_iteratedCovGrad_riemannianFiberNormSq_bound (I := I) (M := M) g₀ 0 3
  ...
  refine le_trans
    (riemannianFiberNormSq_iteratedCovGrad_ccTensorCompose_diagonalProductGrid_leftFactor_le
    (I := I) (M := M) g₀ l 0 3 4
    (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 2 (connectionDifferenceEndomorphism (I := I) (M := M) g₀ g₁))
    (metricLoweredConnectionDifferenceCoefficient (I := I) g₀ g_bg) x) ?_
  have hcell : ∀ i' ∈ Finset.range (l + 1),
      riemannianFiberNormSq (I := I) (M := M) g₀ 3 (4 + i') x
          ((iteratedCovGrad (I := I) g₀ 3 4 i'
            (bilinearSlotInsertionCoefficient (I := I) (M := M) g₀ 2
        ...
        ∑ l' ∈ Finset.range (l + 1 - i'),
          riemannianFiberNormSq (I := I) (M := M) g₀ 0 (3 + l') x
            ((iteratedCovGrad (I := I) g₀ 0 3 l'
              (metricLoweredConnectionDifferenceCoefficient (I := I) g₀ g_bg)).toSection x) ≤
    ...
    have hA1 : riemannianFiberNormSq (I := I) (M := M) g₀ 3 (4 + i') x
      ...
      refine le_trans (palatiniTermSlot2_riemannianFiberNormSq_le (I := I) (M := M) g₀ g₁ i' x) ?_
      rw [← hfr_def]
...
private lemma palatini_seven_term_binary_bound
...
private theorem palatiniLow0_gridWindow (g₀ g_bg : SmoothRiemannianMetric I M)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ C : ℕ → ℝ, (∀ l, 0 ≤ C l) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        (_htie : ∀ (y : M) (v w : TangentSpace I y),
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + l) x
          ...
          C l * Combinatorics.antidiagonalTupleGridWindow
            (fun l' => riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + l') x
              ((iteratedCovGrad (I := I) g₀ 0 2 l' P).toSection x)) (l + 2) := by
  classical
  obtain ⟨cF, hcF_nn, hcF⟩ := exists_iteratedCovGrad_riemannianFiberNormSq_bound (I := I) (M := M) g₀ 0 4
    (palatiniFixLoweredCc (I := I) (M := M) g₀ g_bg g₀)
  obtain ⟨cQ, hcQ_nn, hcQ⟩ := exists_iteratedCovGrad_riemannianFiberNormSq_bound (I := I) (M := M) g₀ 0 4
    (connectionDifferenceQuadraticLoweredCc (I := I) (M := M) g₀ g_bg g_bg)
  obtain ⟨CE, hCE_nn, hCE⟩ := palatiniQuadLow_movingOuter_gridWindow (I := I) (M := M) g₀ g_bg hδ₀
  obtain ⟨CF, hCF_nn, hCF⟩ := palatiniQuadLow_movingTerm_gridWindow (I := I) (M := M) g₀ g_bg hδ₀
  ...
  set A := (-1 : ℝ) • palatiniFixLoweredCc (I := I) (M := M) g₀ g_bg g₀ with hA_def
  set B := domDomCongrSection (I := I) g₀ palatiniSigma2
    (connectionDifferenceQuadraticLoweredCc (I := I) (M := M) g₀ g_bg g_bg) with hB_def
  set Cc := domDomCongrSection (I := I) g₀ palatiniSigma3
    (connectionDifferenceQuadraticLoweredCc (I := I) (M := M) g₀ g_bg g_bg) with hC_def
  set Dd := domDomCongrSection (I := I) g₀ palatiniSigma4
    (connectionDifferenceQuadraticLoweredCc (I := I) (M := M) g₀ g_bg g_bg) with hD_def
  set Ee := domDomCongrSection (I := I) g₀ palatiniSigma2
    (connectionDifferenceQuadraticLoweredCc (I := I) (M := M) g₀ g_bg g₁) with hE_def
  set Ff := domDomCongrSection (I := I) g₀ palatiniSigma3
    (connectionDifferenceQuadraticLoweredCc (I := I) (M := M) g₀ g₁ g_bg) with hF_def
  set Gg := domDomCongrSection (I := I) g₀ palatiniSigma4
    (connectionDifferenceQuadraticLoweredCc (I := I) (M := M) g₀ g₁ g_bg) with hG_def
  have hlow : deTurckLieConnectionDifferenceDerivativeCovKernelDiffLoweredCc (I := I) (M := M) g₀ g₁ g_bg = ((((A + B) - Cc) - Dd) - Ee)
    + Ff + Gg := by
    rw [deTurckLieConnectionDifferenceDerivativeCovKernelDiffLoweredCc, hA_def, hB_def, hC_def, hD_def, hE_def, hF_def, hG_def]
  have hA' : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + l) x
    ...
    rw [show (((-1 : ℝ) • iteratedCovGrad (I := I) g₀ 0 4 l
        (palatiniFixLoweredCc (I := I) (M := M) g₀ g_bg g₀)).toSection x) =
  ...
  have hQfix : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + l) x
      ((iteratedCovGrad (I := I) g₀ 0 4 l
        (connectionDifferenceQuadraticLoweredCc (I := I) (M := M) g₀ g_bg g_bg)).toSection x) ≤ cQ l * W := by
  ...
  have hG' : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + l) x
    ...
    exact hCF g₁ P htie hδ_le hδ0 hbound l x
  rw [hlow]
  have h1 := palatiniRiemannianFiberNormSq_iteratedCovGrad_add_le (I := I) (M := M) g₀ 0 4 l
    (((((A + B) - Cc) - Dd) - Ee) + Ff) Gg x
  have h2 := palatiniRiemannianFiberNormSq_iteratedCovGrad_add_le (I := I) (M := M) g₀ 0 4 l ((((A + B) - Cc) - Dd) - Ee) Ff x
  have h3 := palatiniRiemannianFiberNormSq_iteratedCovGrad_sub_le (I := I) (M := M) g₀ 0 4 l (((A + B) - Cc) - Dd) Ee x
  have h4 := palatiniRiemannianFiberNormSq_iteratedCovGrad_sub_le (I := I) (M := M) g₀ 0 4 l ((A + B) - Cc) Dd x
  have h5 := palatiniRiemannianFiberNormSq_iteratedCovGrad_sub_le (I := I) (M := M) g₀ 0 4 l (A + B) Cc x
  have h6 := palatiniRiemannianFiberNormSq_iteratedCovGrad_add_le (I := I) (M := M) g₀ 0 4 l A B x
  exact palatini_seven_term_binary_bound h1 h2 h3 h4 h5 h6 hA' hB' hC' hD' hE' hF' hG'

private theorem palatiniXd_gridWindow (g₀ g_bg : SmoothRiemannianMetric I M)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ C : ℕ → ℝ, (∀ l, 0 ≤ C l) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        (_htie : ∀ (y : M) (v w : TangentSpace I y),
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + l) x
          ...
          C l * Combinatorics.antidiagonalTupleGridWindow
            (fun l' => riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + l') x
              ((iteratedCovGrad (I := I) g₀ 0 2 l' P).toSection x)) (l + 2) := by
  classical
  obtain ⟨CΛ, hCΛ_nn, hCΛ⟩ := palatiniLambdaSlotInsert3_gridWindow (I := I) (M := M) g₀
  obtain ⟨CL, hCL_nn, hCL⟩ := palatiniLow0_gridWindow (I := I) (M := M) g₀ g_bg hδ₀
  refine ⟨fun l => 4 * (diagonalGridGrowthFactor (E := E) l *
    ...
    fun l => by
      have h : (0 : ℝ) ≤ diagonalGridGrowthFactor (E := E) l *
          ∑ i' ∈ Finset.range (l + 1), CΛ i' * ∑ l' ∈ Finset.range (l + 1 - i'),
            CL l' * Combinatorics.antidiagonalTupleGridWindowMulConst i' (l' + 1) :=
        mul_nonneg (operatorFieldApplicationGdiag_nonneg (E := E) l)
  ...
  have hhalf : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + l) x
      ((iteratedCovGrad (I := I) g₀ 0 4 l
        (deTurckTermCoeffDiffHalfCc (I := I) (M := M) g₀ g₁ g_bg)).toSection x) ≤
      (diagonalGridGrowthFactor (E := E) l *
        ∑ i' ∈ Finset.range (l + 1), CΛ i' * ∑ l' ∈ Finset.range (l + 1 - i'),
          CL l' * Combinatorics.antidiagonalTupleGridWindowMulConst i' (l' + 1)) * W := by
    rw [deTurckTermCoeffDiffHalfCc]
    refine le_trans
      (riemannianFiberNormSq_iteratedCovGrad_ccTensorCompose_diagonalProductGrid_leftFactor_le
      (I := I) (M := M) g₀ l 0 4 4
      (endoSlotZeroCcTensor (I := I) (M := M) g₀ 3
        (metricComparisonEndomorphismField (I := I) (M := M) g₁ g₀))
      (deTurckLieConnectionDifferenceDerivativeCovKernelDiffLoweredCc (I := I) (M := M) g₀ g₁ g_bg) x) ?_
    have hcell : ∀ i' ∈ Finset.range (l + 1),
    ...
    refine le_trans (mul_le_mul_of_nonneg_left (Finset.sum_le_sum hcell)
      (operatorFieldApplicationGdiag_nonneg (E := E) l)) ?_
    rw [← Finset.sum_mul, ← mul_assoc]
  rw [deTurckTermCoeffDiffCc]
  refine le_trans (palatiniRiemannianFiberNormSq_iteratedCovGrad_add_le (I := I) (M := M) g₀ 0 4 l
    (domDomCongrSection (I := I) g₀ (Equiv.swap (0 : Fin 4) 1)
      (deTurckTermCoeffDiffHalfCc (I := I) (M := M) g₀ g₁ g_bg))
    (deTurckTermCoeffDiffHalfCc (I := I) (M := M) g₀ g₁ g_bg) x) ?_
  have hswap : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + l) x
    ...
    riemannianFiberNormSq_iteratedCovGrad_domDomCongrSection (I := I) (M := M) g₀
      (Equiv.swap (0 : Fin 4) 1) (deTurckTermCoeffDiffHalfCc (I := I) (M := M) g₀ g₁ g_bg) l x
  rw [hswap]
  linarith [hhalf]

theorem palatiniCovDerivTermDiff_pointwise_gridWindow (g₀ g_bg : SmoothRiemannianMetric I M)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ C : ℕ → ℝ, (∀ i, 0 ≤ C i) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        (_htie : ∀ (y : M) (v w : TangentSpace I y),
  ...
  obtain ⟨CX, hCX_nn, hCX⟩ := palatiniXd_gridWindow (I := I) (M := M) g₀ g_bg hδ₀
  set fr : ℝ := (Module.finrank ℝ E : ℝ) with hfr_def
  have hfr_nn : 0 ≤ fr := Nat.cast_nonneg _
  ...
  have hWtower : ∀ l, l ≤ i →
    ...
    have hperm : riemannianFiberNormSq (I := I) (M := M) g₀ 2 (6 + l) x
      ...
      riemannianFiberNormSq_iteratedCovGrad_rs_eq_of_section_domDomCongr (I := I) (M := M) g₀ 2 6
        termPairTraceSlotPerm6
        (slotExtendIter (I := I) (M := M) g₀ 0 4 2
          (deTurckTermCoeffDiffCc (I := I) (M := M) g₀ g₁ g_bg))
        (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 termPairTraceSlotPerm6
          (slotExtendIter (I := I) (M := M) g₀ 0 4 2
            (deTurckTermCoeffDiffCc (I := I) (M := M) g₀ g₁ g_bg)))
        (fun y d => by
    ...
    have h1 : riemannianFiberNormSq (I := I) (M := M) g₀ 2 (6 + l) x
      ...
      riemannianFiberNormSq_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 1 5
        (slotExtendIter (I := I) (M := M) g₀ 0 4 1
          (deTurckTermCoeffDiffCc (I := I) (M := M) g₀ g₁ g_bg)) l x
    ...
    calc riemannianFiberNormSq (I := I) (M := M) g₀ 2 (6 + l) x
        ...
        ≤ fr * riemannianFiberNormSq (I := I) (M := M) g₀ 1 (5 + l) x
            ((iteratedCovGrad (I := I) g₀ 1 5 l
              (slotExtendIter (I := I) (M := M) g₀ 0 4 1
                (deTurckTermCoeffDiffCc (I := I) (M := M) g₀ g₁ g_bg))).toSection x) := h1
      ...
      riemannianFiberNormSq (I := I) (M := M) g₀ 2 (2 + i) x
        ((iteratedCovGrad (I := I) g₀ 2 2 i
          (ccOperatorFieldComp (I := I) (M := M) g₀ 2 6 2 (cometricDoublePairTraceCoefficient (I := I) (M := M) g₀ g₁)
            (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 termPairTraceSlotPerm6
              (slotExtendIter (I := I) (M := M) g₀ 0 4 2
                (deTurckTermCoeffDiffCc (I := I) (M := M) g₀ g₁ g_bg))))).toSection x) := by
    rw [palatiniCovDerivTermDiff_eq_pairTrace (I := I) (M := M) g₀ g_bg g₁]
  ...
  refine le_trans
    ...
    (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 termPairTraceSlotPerm6
      (slotExtendIter (I := I) (M := M) g₀ 0 4 2
        (deTurckTermCoeffDiffCc (I := I) (M := M) g₀ g₁ g_bg))) x) ?_
...
theorem exists_deTurckLieCovariantDerivativeTerm_backgroundDifference_perOrder_l2_tameEnvelope
        ...
        ∀ (i : ℕ),
          ‖iteratedCovGrad (I := I) g₀ 2 2 i
              (deTurckLieCovariantDerivativeTermField (I := I) (M := M) g₀ g₁ g_bg -
                deTurckLieCovariantDerivativeTermField (I := I) (M := M) g₀ g₁ g₀)‖ ^ 2 ≤
            K i * (1 + ∑ j ∈ Finset.range (i + 2),
              ‖iteratedCovGrad (I := I) g₀ 0 2 j P‖ ^ 2) := by
  classical
  set δ₁ : ℝ := max δ₀ 0 with hδ₁_def
  have hδ₁_lt : δ₁ < 1 := max_lt hδ₀ one_pos
  obtain ⟨C, hC_nn, hpt⟩ := palatiniCovDerivTermDiff_pointwise_gridWindow (I := I) (M := M)
    g₀ g_bg hδ₁_lt
  ...
  have hsubj : deTurckLieCovariantDerivativeTermField (I := I) (M := M) g₀ g₁ g_bg -
      deTurckLieCovariantDerivativeTermField (I := I) (M := M) g₀ g₁ g₀ =
      deTurckLieConnectionDifferenceDerivCoeffField (I := I) (M := M) g₀ g₁ g_bg -
        deTurckLieConnectionDifferenceDerivCoeffField (I := I) (M := M) g₀ g₁ g₀ := by
    rw [covariantDerivativeTermField_eq_deTurckLieConnectionDifferenceDerivativeCoeffField, covariantDerivativeTermField_eq_deTurckLieConnectionDifferenceDerivativeCoeffField]
  ...
  · have hM' : IsEmpty M := not_nonempty_iff.mp hM
    have hz : ‖iteratedCovGrad (I := I) g₀ 2 2 i
        (deTurckLieCovariantDerivativeTermField (I := I) (M := M) g₀ g₁ g_bg -
          deTurckLieCovariantDerivativeTermField (I := I) (M := M) g₀ g₁ g₀)‖ = 0 :=
...
theorem exists_deTurckLieCovariantDerivativeTerm_backgroundDifference_l2JetWindow
    (g₀ g_bg : SmoothRiemannianMetric I M) (a : ℕ)
    (ha_super : 2 * Module.finrank ℝ E + 10 ≤ a) {R : ℝ} (hR : 0 ≤ R)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ K : ℕ → ℝ, (∀ i, 0 ≤ K i) ∧
  ...
  have htie : ∀ (y : M) (v w : TangentSpace I y),
    ...
    fun y v w => metricPerturbationPath_inner_of_mem (I := I) g₀ T 0 hδ hδZ hs_mem y v w
  obtain ⟨hs0, hs1⟩ := hs
  have habs : |s| ≤ 1 := by
  ...
  have hPball : ∀ j : ℕ, j ≤ a + 2 →
    ...
    calc |s| * ‖iteratedCovGrad (I := I) g₀ 0 2 j T‖ ≤
      ...
      _ ≤ R := hball j hj
  refine le_trans (hK (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s)
...
def deTurckLieCovariantDerivativeDecompositionPairTraceFamily (g₀ : SmoothRiemannianMetric I M)
...
def deTurckLieBackgroundDifferencePermutations : Fin 3 → Equiv.Perm (Fin 4) :=
  ![palatiniSigma2, palatiniSigma3, palatiniSigma4]

def deTurckLieBackgroundDifferenceLoweredCoefficient (g₀ g₁ g_bg : SmoothRiemannianMetric I M) : SmoothCcTensor g₀ 0 4 :=
  (-1 : ℝ) • palatiniFixLoweredCc (I := I) (M := M) g₀ g_bg g₀
...
private theorem lieBackground_dom_sub
...
theorem deTurckLieBackgroundDifferenceLoweredCoefficient_sub
    ...
    deTurckLieBackgroundDifferenceLoweredCoefficient (I := I) (M := M) g₀ gT g_bg -
      ...
      (-1 : ℝ) • domDomCongrSection (I := I) g₀ (deTurckLieBackgroundDifferencePermutations 0)
        (ccOperatorFieldComp (I := I) (M := M) g₀ 0 3 4
          (deTurckLieCovariantDerivativeSecondOrderCoefficient (I := I) (M := M) g₀ g_bg)
  ...
  rw [deTurckLieBackgroundDifferenceLoweredCoefficient, deTurckLieBackgroundDifferenceLoweredCoefficient]
  simp only [operatorFieldComposition_sub_right, operatorFieldComposition_sub_left, lieBackground_dom_sub]
  module

def deTurckLieBackgroundDifferenceCoefficient (g₀ g₁ g_bg : SmoothRiemannianMetric I M) : SmoothCcTensor g₀ 0 4 :=
  let half := ccOperatorFieldComp (I := I) (M := M) g₀ 0 4 4
...
omit [I.Boundaryless] in
omit [SigmaCompactSpace M] in
private theorem deTurckLieBackgroundDifferenceCoefficient_eq_canonical
...
omit [I.Boundaryless] [SigmaCompactSpace M] in
theorem deTurckLieConnectionDifferenceDerivative_backgroundDifference_eq
    (g₀ g_bg g₁ : SmoothRiemannianMetric I M) :
    deTurckLieConnectionDifferenceDerivCoeffField (I := I) (M := M) g₀ g₁ g_bg -
        deTurckLieConnectionDifferenceDerivCoeffField (I := I) (M := M) g₀ g₁ g₀ =
      (-1 : ℝ) • ccOperatorFieldComp (I := I) (M := M) g₀ 2 6 2
  ...
  exact palatiniCovDerivTermDiff_eq_pairTrace (I := I) (M := M) g₀ g_bg g₁
