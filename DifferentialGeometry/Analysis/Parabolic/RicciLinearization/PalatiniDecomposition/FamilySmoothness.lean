...
def riemannPalatiniDecompositionC2Family (g₀ : SmoothRiemannianMetric I M)
...
@[simp] lemma riemannPalatiniDecompositionC2Family_zero (g₀ : SmoothRiemannianMetric I M)
    ...
    riemannPalatiniDecompositionC2Family (I := I) (M := M) g₀ T hδ hδZ qA qB 0 = 0 := by
  rw [riemannPalatiniDecompositionC2Family, zero_smul]
...
theorem exists_ricciArmOrder0RiemannCoeff_metricPerturbationPath_riemannianFiberNormSq_ballUniform_sq
  ...
  have hδP : metricCauchySchwarzBound (I := I) (M := M) g₀
    ...
    nlinarith [hle', hprod]
  have hCD0 := hCD (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s)
...
private lemma pairFeedScalarCLM_apply (s : ℕ) (x : M) (G : Tensor0SSpace (s + 2) I x)
  ...
  rw [
    ...
    TensorMultilinear.tensor0S_curry_apply_eval (I := I) (M := M)
      (T := G) (v0 := p)
      (vs := Fin.cons q (fun i => (tangentSpaceModelContinuousLinearEquiv (I := I) x).symm (v i)))]
  rw [← Tensor0SSpace.toModel_apply_tangent]
  congr 1

omit [CompactSpace M] [I.Boundaryless] [BoundarylessManifold I M] [T2Space M]
    [SigmaCompactSpace M] in
private lemma curvatureDecompositionMonomialBiContrFib_toModel_chartα
          ...
          (Tensor0SSpace.toModel (𝕜 := ℝ) (W x)
            ...
            Tensor0SSpace.toModel (𝕜 := ℝ) G
              (fun i => (Fin.cons
                ...
                (Fin.cons
                  (tangentSpaceModelContinuousLinearEquiv (I := I) x
                    (DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α l x)) v) : Fin 4 → E)
                (σp i)))) := by
  classical
...
private lemma curvatureDecompositionMonomialBiContrFibAppY_chartCoord_jointContMDiffOn
      ...
      (fun p : M × ℝ => Tensor0SSpace.toModel
        ...
        (fun i => tangentSpaceModelContinuousLinearEquiv (I := I) p.1
          ((![DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α (σc 0) p.1,
            DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α (σc 1) p.1] : Fin 2 → TangentSpace I p.1) i)))
      ((chartAt H α).source ×ˢ metricPerturbationPathDomain (δ := δ) (δ' := δ)) := by
  classical
  have htuple : ∀ (y : M) (n l : Fin (Module.finrank ℝ E)) (j : Fin 4),
      (Fin.cons (tangentSpaceModelContinuousLinearEquiv (I := I) y
          (DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α n y))
        (Fin.cons (tangentSpaceModelContinuousLinearEquiv (I := I) y
            (DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α l y))
          ![tangentSpaceModelContinuousLinearEquiv (I := I) y
              (DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α (σc 0) y),
            tangentSpaceModelContinuousLinearEquiv (I := I) y
              (DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α (σc 1) y)]) : Fin 4 → E) j =
      tangentSpaceModelContinuousLinearEquiv (I := I) y
        (DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α
          ((Fin.cons n (Fin.cons l ![σc 0, σc 1]) : Fin 4 → Fin (Module.finrank ℝ E)) j) y) := by
    intro y n l j
    fin_cases j <;> rfl
  have hYtuple : ∀ w : Fin 4 → Fin (Module.finrank ℝ E),
      ContMDiffOn (I.prod 𝓘(ℝ, ℝ)) 𝓘(ℝ) ∞
        (fun p : M × ℝ => Tensor0SSpace.toModel (𝕜 := ℝ) (Y p.1)
          (fun i => tangentSpaceModelContinuousLinearEquiv (I := I) p.1
            (DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α (w i) p.1)))
        ((chartAt H α).source ×ˢ metricPerturbationPathDomain (δ := δ) (δ' := δ)) := by
    intro w p₀ hp₀
    ...
    exact TensorMultilinear.contMDiffWithinAt_section_apply_prod (I := I) 4
      ...
      (fun i => fun b : M => DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α (w i) b) hv
      ...
      ContMDiffOn (I.prod 𝓘(ℝ, ℝ)) 𝓘(ℝ) ∞
        (fun p : M × ℝ => Tensor0SSpace.toModel (𝕜 := ℝ) (W p.1)
          ![tangentSpaceModelContinuousLinearEquiv (I := I) p.1
              (DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α m p.1),
            tangentSpaceModelContinuousLinearEquiv (I := I) p.1
  ...
  have hcomb : ContMDiffOn (I.prod 𝓘(ℝ, ℝ)) 𝓘(ℝ) ∞
      (fun p : M × ℝ => ∑ m, ∑ n, chartInvGramMatrix (I := I)
          (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ p.2) α p.1 m n *
        (∑ k, ∑ l, chartInvGramMatrix (I := I)
            (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ p.2) α p.1 k l *
              ...
              (fun i => (Fin.cons
                ...
                (Fin.cons
                  ...
                  ![tangentSpaceModelContinuousLinearEquiv (I := I) p.1
                      (DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α (σc 0) p.1),
                    tangentSpaceModelContinuousLinearEquiv (I := I) p.1
                      (DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α (σc 1) p.1)]) : Fin 4 → E)
                (σp i)))))
      ((chartAt H α).source ×ˢ metricPerturbationPathDomain (δ := δ) (δ' := δ)) := by
    refine contMDiffOn_finsetSum (fun m _ => contMDiffOn_finsetSum (fun n _ => ?_))
    refine (metricPerturbationPath_chartInvGramMatrix_jointContMDiffOn_free (I := I) g₀ T 0
      hδ hδZ α m n).mul ?_
  ...
  have hxbase : p.1 ∈ (trivializationAt E (TangentSpace I) α).baseSet := by
    ...
    exact hp.1
  rw [curvatureDecompositionMonomialBiContrFib_toModel_chartα (I := I) (M := M)
    (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ p.2) W σp α hxbase]
  refine Finset.sum_congr rfl (fun m _ => ?_)
  refine Finset.sum_congr rfl (fun n _ => ?_)
  refine congrArg (fun z => chartInvGramMatrix (I := I)
    (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ p.2) α p.1 m n * z) ?_
  refine Finset.sum_congr rfl (fun k _ => ?_)
  refine Finset.sum_congr rfl (fun l _ => ?_)
  refine congrArg (fun z => chartInvGramMatrix (I := I)
    (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ p.2) α p.1 k l * z) ?_
  congr 2
    ...
    have hreadout : ∀ {q : M × ℝ}, q.1 ∈ e.baseSet →
        Bcmm.repr (e ⟨q.1, curvatureActionMonomialTrace (I := I) (M := M)
          ...
          Tensor0SSpace.toModel (curvatureActionMonomialTrace (I := I) (M := M)
            ...
            (fun i => tangentSpaceModelContinuousLinearEquiv (I := I) q.1
              ((![DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α (σc 0) q.1,
                DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α (σc 1) q.1] :
                  Fin 2 → TangentSpace I q.1) i)) := by
      intro q hqbase
...
theorem curvatureDecompositionMonomialCoeffField_metricPerturbationPath_jointContMDiffOn
  ...
  have hCLM := contMDiffOn_clm_section_of_apply (I := I) (M := M)
    ...
    (φ := fun p : M × ℝ => curvatureActionMonomialTrace (I := I) (M := M)
      (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ p.2) W σp p.1)
    (S := metricPerturbationPathDomain (δ := δ) (δ' := δ))
    (fun Y => curvatureDecompositionMonomialBiContrFibAppY_metricPerturbationPath_jointContMDiffOn
...
theorem riemannPalatiniDecompositionC2Family_threeArmHjoint
  ...
  have hker : ∀ q : Fin 4 → Equiv.Perm (Fin 4),
    ...
    have hadd := jointTotalSpaceRS_add_local (I := I) (M := M) (r := 4) (s := 2)
      (S := metricPerturbationPathDomain (δ := δ) (δ' := δ)) _ _ (hmono (q 0)) (hmono (q 1))
  ...
  refine congrArg (fun t => TotalSpace.mk' (Tensor0SBundle.TensorRSModel 4 2 ℝ E)
    (E := fun z : M => Tensor0SBundle.TensorRSSpace 4 2 I z) p.1 t) ?_
  rw [riemannPalatiniDecompositionC2Family, SmoothCcTensor.toSection_smul,
