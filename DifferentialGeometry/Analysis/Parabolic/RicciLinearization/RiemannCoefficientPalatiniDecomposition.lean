...
import DifferentialGeometry.Analysis.Parabolic.RicciLinearization.RiemannCoefficientPalatiniDecompositionLinearizedDecompositionIdentity
import DifferentialGeometry.Analysis.Parabolic.RicciLinearization.RiemannCoefficientPalatiniDecompositionMonomialDecompositionL2JetWindow
import DifferentialGeometry.Analysis.Parabolic.RicciLinearization.RiemannCoefficientPalatiniDecompositionRicciFoldWeightKernel
import DifferentialGeometry.Analysis.Parabolic.RicciLinearization.RiemannCoefficientPalatiniDecompositionSharpGradKoszulResidualSmoothness
import DifferentialGeometry.Analysis.Parabolic.RicciLinearization.RiemannCoefficientPalatiniDecompositionResidualFieldBallUniform
import DifferentialGeometry.Analysis.Parabolic.RicciLinearization.RiemannCoefficientPalatiniDecompositionResidualFieldL2JetWindow
...
theorem exists_riemannPalatini_decomposition_identity_data
            ...
            operatorFieldApply (I := I) (M := M) g₀ 2 2
              ...
              operatorFieldApply (I := I) (M := M) g₀ 2 2 (C0ra s)
                ...
                operatorFieldApply (I := I) (M := M) g₀ 4 2
                  ((2 : ℝ) • riemannPalatiniDecompositionC2Family (I := I) (M := M) g₀ T hδ hδZ
                    qA qB s)
  ...
  obtain ⟨ΛQ, hΛQ_nn, hcapQ⟩ :=
    exists_ricciArmOrder0AACommCoeffField_metricPerturbationPath_fiberNormSq_ballUniform (I := I) (M := M)
      g₀ a ha_super hR hδ₀
  ...
  obtain ⟨ΛF, hΛF_nn, hcapF⟩ :=
    exists_ricciArmRicciFoldRemainderField_metricPerturbationPath_riemannianFiberNormSq_uniformBound (I := I)
      (M := M)
      g₀ a ha_super hR hδ₀
  ...
  obtain ⟨KQw, hKQw_nn, hwinQ⟩ :=
    exists_ricciArmOrder0AACommCoeffField_metricPerturbationPath_l2JetWindow (I := I) (M := M)
      g₀ a ha_super hR hδ₀
  obtain ⟨KBw, hKBw_nn, hwinB⟩ :=
    exists_ricciArmOrder0BackgroundRCommCoeffField_metricPerturbationPath_backgroundDifference_l2JetWindow
      (I := I) (M := M) g₀ a ha_super hR hδ₀
  obtain ⟨KSw, hKSw_nn, hwinS⟩ :=
    exists_ricciArmSharpGradKoszulResidualField_metricPerturbationPath_l2JetWindow (I := I) (M := M)
      g₀ a ha_super hR hδ₀
  obtain ⟨KFw, hKFw_nn, hwinF⟩ :=
    exists_ricciArmRicciFoldRemainderField_metricPerturbationPath_l2JetWindow (I := I) (M := M)
      g₀ a ha_super hR hδ₀
  have hΛSQ_nn : (0 : ℝ) ≤
  ...
  · exact threeArmHjoint_add_local (I := I) (M := M) g₀ _ _
      ...
      (threeArmHjoint_const_smul_local (I := I) (M := M) g₀ (2 : ℝ) _
        (threeArmHjoint_add_local (I := I) (M := M) g₀ _ _
          (ricciArmOrder0AACommCoeffField_metricPerturbationPath_threeArmHjoint (I := I) (M := M)
            g₀ T hδ hδZ)
          (threeArmHjoint_sub_local (I := I) (M := M) g₀ _ _
            (threeArmHjoint_add_local (I := I) (M := M) g₀ _ _
              (threeArmHjoint_sub_local (I := I) (M := M) g₀ _ _
                (ricciArmOrder0BackgroundRCommCoeffField_metricPerturbationPath_threeArmHjoint (I := I) (M := M)
                  g₀ T hδ hδZ)
                (threeArmHjoint_const_local (I := I) (M := M) g₀
                  (ricciArmOrder0BackgroundCurvatureCoeffField (I := I) (M := M) g₀ g₀)))
              (threeArmHjoint_const_smul_local (I := I) (M := M) g₀ (1 / 2 : ℝ) _
                (ricciArmSharpGradKoszulResidualField_metricPerturbationPath_threeArmHjoint
                  (I := I) (M := M) g₀ T hδ hδZ)))
            (ricciArmRicciFoldRemainderField_metricPerturbationPath_threeArmHjoint (I := I) (M := M)
              g₀ T hδ hδZ))))
  · intro s hs
    ...
    have hfam : (2 : ℝ) • riemannPalatiniDecompositionC2Family (I := I) (M := M) g₀ T hδ hδZ
      ...
      rw [riemannPalatiniDecompositionC2Family_eq_symmS_kernel (I := I) (M := M) g₀ T hδ hδZ _ _
        ...
        hsymmT, smul_smul]
      rfl
    rw [hfam, operatorFieldApplication_add_left, operatorFieldApplication_smul_left, operatorFieldApplication_smul_left]
    suffices hfold : (1 / 2 : ℝ) •
      ...
      rw [hfold, iteratedCovGrad_smul_real, operatorFieldApplication_smul_right, smul_add, smul_smul] at h2
  ...
  · intro s hs x
    have hQ : riemannianFiberNormSq (I := I) (M := M) g₀ 2 2 x
        ((ricciArmOrder0AACommCoeffField (I := I) (M := M) g₀
          (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s)).toSection x) ≤ ΛQ :=
      ...
      ((2 : ℝ) • ((ricciArmOrder0AACommCoeffField (I := I) (M := M) g₀
        ...
        + ((ricciArmOrder0BackgroundCurvatureCoeffField (I := I) (M := M) g₀
            ...
            + (1 / 2 : ℝ) • ricciArmSharpGradKoszulResidualField (I := I) (M := M) g₀
                (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s) (s • T)
            - ricciArmRicciFoldRemainderField (I := I) (M := M) g₀
                (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s) (s • T))).toSection x))
      ...
      rw [show ((ricciArmOrder0AACommCoeffField (I := I) (M := M) g₀
          ...
          + ((((ricciArmOrder0BackgroundCurvatureCoeffField (I := I) (M := M) g₀
              ...
              + (1 / 2 : ℝ) • (ricciArmSharpGradKoszulResidualField (I := I) (M := M) g₀
                  (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s) (s • T)).toSection x)
            - (ricciArmRicciFoldRemainderField (I := I) (M := M) g₀
                (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s) (s • T)).toSection x) from by
  ...
  · intro i s hs
    ...
    set jq := iteratedCovGrad (I := I) g₀ 2 2 i
      (ricciArmOrder0AACommCoeffField (I := I) (M := M) g₀
        (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s)) with hjq_def
    ...
    set js := iteratedCovGrad (I := I) g₀ 2 2 i
      (ricciArmSharpGradKoszulResidualField (I := I) (M := M) g₀
        (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s) (s • T)) with hjs_def
    set jr := iteratedCovGrad (I := I) g₀ 2 2 i
      (ricciArmRicciFoldRemainderField (I := I) (M := M) g₀
        (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s) (s • T)) with hjr_def
...
theorem riemannPalatiniDecompositionC2Family_riemannianFiberNormSq_le
    ...
    ∀ s ∈ Set.Icc (0 : ℝ) 1, ∀ x : M,
      riemannianFiberNormSq (I := I) (M := M) g₀ 4 2 x
        (((2 : ℝ) • riemannPalatiniDecompositionC2Family (I := I) (M := M) g₀ T hδ hδZ qA qB
          s).toSection x) ≤
      (max (8 * deTurckArmFibreConst (Module.finrank ℝ E) * (δ / (1 - δ))) 0) ^ 2 := by
  classical
  intro s hs x
  have h1mδ : (0 : ℝ) < 1 - δ := by linarith
  obtain ⟨nx, ex, hnx, horthx, hparsx, hexpx, hriemannianFiberNormSqx⟩ :=
  ...
  have hmono_cap : ∀ σp : Equiv.Perm (Fin 4),
    ...
    exact riemannianFiberNormSq_curvatureDecompositionMonomialBiContrFib_le (I := I) (M := M) g₀
      (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s)
  ...
  have hfC_nn : (0 : ℝ) ≤ fC := de_turck_arm_fibre_const_nonneg _
...
theorem exists_riemannPalatiniDecompositionC2Family_l2JetWindow
  ...
  have hδ_lt : δ < 1 := lt_of_le_of_lt hδ_le hδ₀
  refine ⟨riemannPalatiniDecompositionC2Family_riemannianFiberNormSq_le (I := I) (M := M) g₀ T hδ_lt
    hδ_half hδ hδZ
    qA qB hq, ?_⟩
  intro i s hs
  obtain ⟨hs0, hs1⟩ := hs
...
theorem exists_deTurckLieCovariantDerivativeDecompositionC2Family_cap_l2JetWindow
    ...
    ∃ K : ℕ → ℝ, (∀ i, 0 ≤ K i) ∧
      ∀ (T : SmoothCcTensor g₀ 0 2)
        ...
        linearizedRicciThreeArmHjoint (I := I) (M := M) g₀ 4
          (deTurckLieCovariantDerivativeDecompositionC2Family (I := I) (M := M) g₀ T hδ hδZ q ε)
          (δ := δ) (δ' := δ) ∧
        (∀ s ∈ Set.Icc (0 : ℝ) 1, ∀ x : M,
          riemannianFiberNormSq (I := I) (M := M) g₀ 4 2 x
            ((deTurckLieCovariantDerivativeDecompositionC2Family (I := I) (M := M) g₀ T hδ hδZ q ε
        ...
        (∀ i : ℕ, ∀ s ∈ Set.Icc (0 : ℝ) 1,
          ‖iteratedCovGrad (I := I) g₀ 4 2 i
            (deTurckLieCovariantDerivativeDecompositionC2Family (I := I) (M := M) g₀ T hδ hδZ q ε s)‖ ^ 2 ≤
            K i * (1 + ∑ j ∈ Finset.range (i + 2),
              ‖iteratedCovGrad (I := I) g₀ 0 2 j T‖ ^ 2)) := by
  classical
  obtain ⟨K0, hK0_nn, hK0⟩ :=
  ...
  · have hmono : ∀ σp : Equiv.Perm (Fin 4),
      ...
      fun σp => curvatureDecompositionMonomialCoeffField_metricPerturbationPath_jointContMDiffOn
        (I := I) (M := M) g₀ T hδ hδZ
        (ccTensorUnitValueSection (I := I) (M := M) g₀ T)
    ...
    have hsum := jointTotalSpaceRS_add_local (I := I) (M := M) (r := 4) (s := 2)
      (S := metricPerturbationPathDomain (δ := δ) (δ' := δ)) _ _ hsum01 (hpair 2)
    have hfam := jointTotalSpaceRS_smulFun_local (I := I) (M := M) (r := 4) (s := 2)
      (S := metricPerturbationPathDomain (δ := δ) (δ' := δ)) (f := fun t => t) contDiff_id _ hsum
  ...
  · intro s hs x
    ...
    have hmax : max (3 * deTurckArmFibreConst (Module.finrank ℝ E) * (δ / (1 - δ) ^ 2)) 0 =
      ...
      max_eq_left (mul_nonneg (mul_nonneg (by norm_num)
        (de_turck_arm_fibre_const_nonneg _)) (div_nonneg hδ0 (sq_nonneg _)))
    rw [hmax]
    have hsum_nn := riemannianFiberNormSq_nonneg (I := I) (M := M) g₀ 4 2 x
      (ε 0 • ((curvatureActionMonomialCoeffField (I := I) (M := M) g₀
        (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s)
...
theorem exists_deTurckLieCovariantDerivativeArm_curvatureDecomposition_data
  ...
  obtain ⟨Λ, hΛ, KA, hKA, q, ε, hε, hmain⟩ :=
    exists_deTurckLieCovariantDerivativeArm_decomposition_identity_data (I := I) (M := M) g₀ g_bg a
      ha_super hR hδ₀
  obtain ⟨KB, hKB, hfam⟩ :=
  ...
  refine ⟨C0da, deTurckLieCovariantDerivativeDecompositionC2Family (I := I) (M := M) g₀ T hδ hδZ q ε,
...
omit [SigmaCompactSpace M] in
theorem deTurckVF_background_sub_eq_connectionDifference_trace
...
theorem exists_deTurckLieEndoArm_backgroundDifference_perOrder_l2_tameEnvelope_generic
  ...
  have hsubj : deTurckLieEndoArmField (I := I) (M := M) g₀ g₁ g_bg -
    ...
    rw [endoArmField_eq_deTurckLieCovariantDerivativeInsertionCoeffField, endoArmField_eq_deTurckLieCovariantDerivativeInsertionCoeffField]
...
theorem exists_deTurckLieEndoArm_backgroundDifference_order0_data
  ...
  · exact threeArmHjoint_sub_local (I := I) (M := M) g₀ _ _
      (endoArmField_metricPerturbationPath_threeArmHjoint (I := I) (M := M) g₀ T hδ hδZ g_bg)
      (endoArmField_metricPerturbationPath_threeArmHjoint (I := I) (M := M) g₀ T hδ hδZ g₀)
  · intro s hs x
    have h1 : riemannianFiberNormSq (I := I) (M := M) g₀ 2 2 x
        ((deTurckLieEndoArmField (I := I) (M := M) g₀
          (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s) g_bg).toSection x) ≤ Λbg := by
      rw [endoArmField_eq_deTurckLieCovariantDerivativeInsertionCoeffField]
...
theorem covDerivConnectionDifference_metricPerturbationPath_zero_endpoint_eq_smul_covDerivSharp
    ...
    covDerivConnectionDifference (I := I) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0)
        (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s)
        (fun b => X b) (fun b => Y b) (fun b => Z b) x =
      s •
        ((LeviCivita (I := I) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0)).toFun
            (fun b : M =>
              DifferentialGeometry.Geometry.Operator.metricSharp (I := I)
                (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s) b
          ...
          - DifferentialGeometry.Geometry.Operator.metricSharp (I := I)
            ...
            (linearizedKoszulCovec (I := I) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0)
              ...
              (covApply (LeviCivita (I := I) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0))
  ...
  have hs_mem : s ∈ metricPerturbationPathDomain (δ := δ) (δ' := δ) :=
    Icc_subset_metricPerturbationPathDomain hδ_lt hδ_lt hs
  have hexpand : covDerivConnectionDifference (I := I) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0)
      (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s)
      (fun b => X b) (fun b => Y b) (fun b => Z b) x =
      (LeviCivita (I := I) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0)).toFun
          (diffSec (LeviCivita (I := I) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0))
            (LeviCivita (I := I) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s))
            (fun b => Y b) (fun b => Z b)) x (X x)
        - PDE.DeTurck.connectionDifference (I := I) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s)
            (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0) x (Z x)
            (covApply (LeviCivita (I := I) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0))
              (fun b => X b) (fun b => Y b) x)
        - PDE.DeTurck.connectionDifference (I := I) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s)
            (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0) x
            (covApply (LeviCivita (I := I) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0))
              (fun b => X b) (fun b => Z b) x) (Y x) := rfl
  rw [hexpand]
  have hpoint : ∀ (b : M) (u ζ : TangentSpace I b),
      PDE.DeTurck.connectionDifference (I := I) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s)
        (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0) b u ζ =
      s • DifferentialGeometry.Geometry.Operator.metricSharp (I := I)
        (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s) b
        (linearizedKoszulCovec (I := I) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0)
          (realizedVelocityCc (I := I) g₀ T 0 hδ hδZ 0) b u ζ) := by
    intro b u ζ
    have h := connectionDifference_metricPerturbationPath_eq_smul_sharp (I := I) g₀ T 0 hδ_lt hδ hδ_lt hδZ
      h0_mem hs_mem b u ζ
    rwa [sub_zero] at h
  by_cases hs0 : s = 0
  · subst hs0
    have hconn0 : ∀ (b : M) (u ζ : TangentSpace I b),
        PDE.DeTurck.connectionDifference (I := I) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0)
          (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0) b u ζ = 0 := by
      intro b u ζ
      rw [PDE.DeTurck.connectionDifference_self]
      rfl
    have hdz : diffSec (LeviCivita (I := I) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0))
        (LeviCivita (I := I) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0))
        (fun b => Y b) (fun b => Z b) =
        (0 : ℝ) • fun b : M => X b := by
      funext b
    ...
    rw [hconn0 x (Z x) (covApply (LeviCivita (I := I)
      ...
      hconn0 x (covApply (LeviCivita (I := I)
        (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0)) (fun b => X b) (fun b => Z b) x) (Y x)]
    rw [zero_smul, zero_smul]
    simp
  · have hconn_smooth : ContMDiff I (I.prod 𝓘(ℝ, E)) ∞
        (fun b : M => TotalSpace.mk' E (E := fun z : M => TangentSpace I z) b
          (PDE.DeTurck.connectionDifference (I := I) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s)
            (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0) b (Z b) (Y b))) :=
      PDE.DeTurck.connectionDifference_contMDiff (I := I)
        (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0)
    ...
    have hΨ_eq : (fun b : M =>
        ...
        (fun b : M => s⁻¹ •
          PDE.DeTurck.connectionDifference (I := I) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s)
            (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0) b (Z b) (Y b)) := by
      funext b
    ...
    have hΨ_smooth : ContMDiff I (I.prod 𝓘(ℝ, E)) ∞
      ...
      change DifferentialGeometry.Geometry.Operator.metricSharp (I := I)
        (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s) b
          (linearizedKoszulCovec (I := I) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0)
            (realizedVelocityCc (I := I) g₀ T 0 hδ hδZ 0) b (Z b) (Y b)) =
        s⁻¹ • PDE.DeTurck.connectionDifference (I := I) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s)
          (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0) b (Z b) (Y b)
    ...
    have hdiffSec : diffSec (LeviCivita (I := I) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ 0))
        (LeviCivita (I := I) (metricPerturbationPath (I := I) g₀ T 0 hδ hδZ s))
        (fun b => Y b) (fun b => Z b) =
        s • (fun b : M =>
          DifferentialGeometry.Geometry.Operator.metricSharp (I := I)
