...
theorem hasDerivAt_metricPerturbationPath_chartDeTurckVFComp (g₀ : SmoothRiemannianMetric I M)
  ...
  have heq : (fun s : ℝ =>
      ...
      (fun s : ℝ => ∑ a : Fin (Module.finrank ℝ E), ∑ b : Fin (Module.finrank ℝ E),
        chartInvGramOnE (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) x a b y *
          (chartChristoffel (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) x a b k y -
  ...
  have hd : HasDerivAt
      ...
      (∑ a : Fin (Module.finrank ℝ E), ∑ b : Fin (Module.finrank ℝ E),
        ((-(∑ q : Fin (Module.finrank ℝ E), ∑ p : Fin (Module.finrank ℝ E),
              chartInvGramOnE (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s₀) x a p y *
                realizedGramDeriv (I := I) g₀ T T' hδ_lt hδ hδ'_lt hδ' x p q y *
                chartInvGramOnE (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s₀) x q b y)) *
            (chartChristoffel (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s₀) x a b k y -
              chartChristoffel (I := I) g_bg x a b k y) +
          chartInvGramOnE (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s₀) x a b y *
    ...
    have hG := hasDerivAt_metricPerturbationPath_chartInvGramOnE (I := I) g₀ T T' hδ_lt hδ hδ'_lt hδ'
      x a b hy hs₀
    have hΓ := hasDerivAt_metricPerturbationPath_chartChristoffel (I := I) g₀ T T' hδ_lt hδ hδ'_lt hδ'
      x a b k hy hs₀
...
theorem hasDerivAt_metricPerturbationPath_partial_chartDeTurckVFComp (g₀ : SmoothRiemannianMetric I M)
  ...
  have hcomm := hasDerivAt_partialDeriv_comm_at'
    (fun r : ℝ × E =>
      chartDeTurckVFComp (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' r.1) g_bg x k r.2)
    m s₀ y hjoint
  have hderiv_eq : (fun y' => deriv (fun s : ℝ =>
        chartDeTurckVFComp (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g_bg x k y') s₀)
      =ᶠ[nhds y]
      (deTurckVFDerivRaw (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s₀) g_bg x
...
def lieDeTurckChartSlope (g₀ : SmoothRiemannianMetric I M)
    ...
    (s₀ : ℝ) (y : E) : ℝ :=
  lieDeTurckSlopeExprRaw (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s₀) g_bg x
...
theorem hasDerivAt_metricPerturbationPath_chartLieDeTurckComp_chartSlope (g₀ : SmoothRiemannianMetric I M)
  ...
  have hT1 : HasDerivAt
    ...
    have hW := hasDerivAt_metricPerturbationPath_chartDeTurckVFComp (I := I) g₀ T T' hδ_lt hδ hδ'_lt hδ'
      g_bg x k hy hs₀
    have hGr := hasDerivAt_metricPerturbationPath_partial_chartGramOnE (I := I) g₀ T T' hδ_lt hδ hδ'_lt hδ'
      x i j k hy hs₀
    exact hW.mul hGr
  have hT2 : HasDerivAt
      (fun s : ℝ => ∑ k : Fin (Module.finrank ℝ E),
          ...
          chartGramOnE (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s₀) x k j y *
            DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) i
              (deTurckVFDerivRaw (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s₀) g_bg x
                (realizedGramDeriv (I := I) g₀ T T' hδ_lt hδ hδ'_lt hδ' x) k) y)) s₀ := by
    refine HasDerivAt.fun_sum (fun k _ => ?_)
    have hGr := hasDerivAt_metricPerturbationPath_chartGramOnE (I := I) g₀ T T' hδ_lt hδ hδ'_lt hδ'
      x k j y hs₀
    have hW := hasDerivAt_metricPerturbationPath_partial_chartDeTurckVFComp (I := I) g₀ T T'
...
theorem deriv_metricPerturbationPath_chartLieDeTurckComp_eq_chartSlope (g₀ : SmoothRiemannianMetric I M)
    ...
    (hs₀ : s₀ ∈ Set.Ioo (0 : ℝ) 1) :
    deriv (fun s : ℝ =>
        chartLieDeTurckComp (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g_bg x i j
          (extChartAt I x x)) s₀ =
  ...
  exact (hasDerivAt_metricPerturbationPath_chartLieDeTurckComp_chartSlope (I := I) g₀ T T'
    ...
    lieDeTurckChartSlope (I := I) g₀ T T' hδ_lt hδ hδ'_lt hδ' g_bg x i j s₀ y =
      chartDeTurckCorrPrincipalSymbolExprRaw (I := I)
        ...
        lieDeTurckOrder1Raw (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s₀) g_bg x
          (realizedGramDeriv (I := I) g₀ T T' hδ_lt hδ hδ'_lt hδ' x) i j y +
        order0PartRaw (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s₀) g_bg x
