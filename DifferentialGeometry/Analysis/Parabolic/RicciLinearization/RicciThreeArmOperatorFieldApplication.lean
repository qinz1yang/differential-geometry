...
theorem exists_ricciArmOrder1Coeff
  ...
  set R₀ : SmoothCcTensor g₀ 2 2 :=
    pathIntegralCoeffField (I := I) (M := M) g₀ 2 2 Φ₀
      (metricPerturbationPathDomain (δ := δ) (δ' := δ')) hSopen hSI hj0 with hR₀
  set R₁ : SmoothCcTensor g₀ 3 2 :=
    pathIntegralCoeffField (I := I) (M := M) g₀ 3 2 Φ₁
      (metricPerturbationPathDomain (δ := δ) (δ' := δ')) hSopen hSI hj1 with hR₁
  set R₂ : SmoothCcTensor g₀ 4 2 :=
    pathIntegralCoeffField (I := I) (M := M) g₀ 4 2 Φ₂
      (metricPerturbationPathDomain (δ := δ) (δ' := δ')) hSopen hSI hj2 with hR₂
...
theorem ricciTensor_realize_sub_eq_threeArm_operatorFieldApply
  ...
  set A₂ : SmoothCcTensor g₀ 0 2 :=
    operatorFieldApply (I := I) (M := M) g₀ 4 2 R₂ (iteratedCovGrad (I := I) g₀ 0 2 2 (T - T')) with
      hA₂
  rw [operatorFieldApplication_smul_left_local, operatorFieldApplication_smul_left_local, operatorFieldApplication_smul_left_local, ← hA₀, ← hA₁, ← hA₂]
