...
theorem hasDerivAt_metricPerturbationPath_chartGramOnE (g₀ : SmoothRiemannianMetric I M)
    ...
    HasDerivAt (fun s : ℝ => chartGramOnE (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) α i j y)
  ...
  have heq : (fun s : ℝ => chartGramOnE (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) α i j y)
      =ᶠ[nhds s₀] (fun s : ℝ => (1 - s) * F + s * G) := by
    filter_upwards [hSopen.mem_nhds hs₀] with s hs
    rw [hF, hG]
    exact metricPerturbationPath_chartGramOnE (I := I) g₀ T T' hδ_lt hδ hδ'_lt hδ' hs α i j y
...
private lemma realizedGramDeriv_eq_deriv (g₀ : SmoothRiemannianMetric I M)
    ...
    (hs₀ : s₀ ∈ metricPerturbationPathDomain (δ := δ) (δ' := δ')) :
    (fun y => deriv (fun s : ℝ =>
        chartGramOnE (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) α i j y) s₀) =
...
private lemma s_differentiableAt_metricPerturbationPath_chartInvGramOnE (g₀ : SmoothRiemannianMetric I M)
  ...
  have hcomp : (fun s : ℝ => chartInvGramOnE (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) x k l
    y)
      = (fun p : ℝ × E => chartInvGramOnE (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' p.1) x k l
        p.2)
...
theorem hasDerivAt_metricPerturbationPath_chartInvGramOnE (g₀ : SmoothRiemannianMetric I M)
    ...
    (hy : y ∈ interior (extChartAt I x).target) {s₀ : ℝ}
    (hs₀ : s₀ ∈ metricPerturbationPathDomain (δ := δ) (δ' := δ')) :
    HasDerivAt (fun s : ℝ => chartInvGramOnE (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) x k l
      y)
      (-(∑ q : Fin (Module.finrank ℝ E), ∑ p : Fin (Module.finrank ℝ E),
          chartInvGramOnE (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s₀) x k p y *
            ...
            chartInvGramOnE (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s₀) x q l y)) s₀ := by
  classical
  set gs : SmoothRiemannianMetric I M := metricPerturbationPath (I := I) g₀ T T' hδ hδ' s₀ with hgs
  have hSopen : IsOpen (metricPerturbationPathDomain (δ := δ) (δ' := δ')) := metricPerturbationPathDomain_isOpen
  have hsub : (fun s : ℝ => chartInvGramOnE (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) x k l
    y)
    ...
    have := invGramOnE_sub_eq (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) gs x k l hy
    linarith [this]
...
theorem hasDerivAt_metricPerturbationPath_chartChristoffelBracket (g₀ : SmoothRiemannianMetric I M)
  ...
  have h1 := hasDerivAt_metricPerturbationPath_partial_chartGramOnE (I := I) g₀ T T' hδ_lt hδ hδ'_lt hδ' x l j
    i
    hy hs₀
  have h2 := hasDerivAt_metricPerturbationPath_partial_chartGramOnE (I := I) g₀ T T' hδ_lt hδ hδ'_lt hδ' x l i
    j
    hy hs₀
  have h3 := hasDerivAt_metricPerturbationPath_partial_chartGramOnE (I := I) g₀ T T' hδ_lt hδ hδ'_lt hδ' x i j
    l
    hy hs₀
...
theorem hasDerivAt_metricPerturbationPath_chartChristoffel (g₀ : SmoothRiemannianMetric I M)
    ...
    (hy : y ∈ interior (extChartAt I x).target) {s₀ : ℝ}
    (hs₀ : s₀ ∈ metricPerturbationPathDomain (δ := δ) (δ' := δ')) :
    HasDerivAt (fun s : ℝ => chartChristoffel (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) x i j
      k y)
...
theorem hasDerivAt_metricPerturbationPath_chartRiemannTensor (g₀ : SmoothRiemannianMetric I M)
    ...
    (hs₀ : s₀ ∈ metricPerturbationPathDomain (δ := δ) (δ' := δ')) :
    HasDerivAt
      (fun s : ℝ => chartRiemannTensor (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) x i j k l y)
      (deriv (fun s : ℝ =>
...
theorem hasDerivAt_metricPerturbationPath_chartRicciTensor (g₀ : SmoothRiemannianMetric I M)
    ...
    (hs₀ : s₀ ∈ metricPerturbationPathDomain (δ := δ) (δ' := δ')) :
    HasDerivAt
      (fun s : ℝ => chartRicciTensor (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) x i k y)
      (∑ j : Fin (Module.finrank ℝ E),
        deriv (fun s : ℝ =>
          chartRiemannTensor (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) x i j k j y) s₀)
            s₀ := by
  have heq : (fun s : ℝ =>
        chartRicciTensor (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) x i k y) =
      (fun s : ℝ => ∑ j : Fin (Module.finrank ℝ E),
        chartRiemannTensor (I := I) (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) x i j k j y) := by
