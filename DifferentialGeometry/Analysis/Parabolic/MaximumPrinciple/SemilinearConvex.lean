...
private lemma withLp_prod_zero_snd_norm {F : Type*} [NormedAddCommGroup F] (a : F) :
    ‖WithLp.toLp 2 (a, (0 : ℝ))‖ = ‖a‖ := by
  simp

private lemma withLp_prod_fst_norm_le {F : Type*} [NormedAddCommGroup F] (a : F) (b : ℝ) :
    ‖a‖ ≤ ‖WithLp.toLp 2 (a, b)‖ := by
  rw [WithLp.prod_norm_eq_add (by norm_num : 0 < (2 : ENNReal).toReal)]
  simp only [WithLp.toLp_fst, WithLp.toLp_snd, ENNReal.toReal_ofNat]
  rw [← Real.sqrt_eq_rpow]
  have hA : 0 ≤ ‖a‖ ^ (2 : ℝ) + ‖b‖ ^ (2 : ℝ) := by positivity
  have hsq : (‖a‖ : ℝ) ^ 2 ≤ (√(‖a‖ ^ (2 : ℝ) + ‖b‖ ^ (2 : ℝ))) ^ 2 := by
    rw [Real.sq_sqrt hA]
    rw [Real.rpow_two, Real.rpow_two]
    nlinarith [sq_nonneg ‖b‖]
  simpa [abs_of_nonneg] using sq_le_sq.mp hsq
...
theorem closed_convex_heat_reaction_mem_of_timeDep_tangent_lipschitzOnBalls
    ...
    (hK_eq : K = {q : WithLp 2 (F × ℝ) |
      (WithLp.ofLp q).2 ∈ Set.Icc 0 T ∧ (WithLp.ofLp q).1 ∈ C (WithLp.ofLp q).2})
    (hKne : K.Nonempty) (hKclosed : IsClosed K) (hKconvex : Convex Real K)
    (hK0 : ∀ τ : Real, τ ∈ Set.Icc 0 T → (0 : F) ∈ C τ)
    (reaction : Real → M → F → F)
    (u : Real → M → F)
    (hsol : IsInnerProductHeatReactionOn
      (RealTimeInterval.closed 0 T hT) G reaction u)
    (hLip : ∀ R : ℝ, 0 ≤ R → ∃ L : NNReal, ∀ t : Real, t ∈ Set.Ioo 0 T → ∀ x : M,
      LipschitzOnWith L (fun q : WithLp 2 (F × ℝ) =>
        WithLp.toLp 2 (reaction (WithLp.ofLp q).2 x (WithLp.ofLp q).1, (1 : Real)))
  ...
  have hIco : ∀ t : Real, t ∈ Set.Ico 0 T → ∀ x : M, u' t x ∈ K := by
    ...
    have hQsub : Q ⊆ (RealTimeInterval.closed 0 T hT).carrier ×ˢ univ := by
      intro q hq
      exact ⟨⟨hq.1.1, hq.1.2.trans ht.2.le⟩, hq.2⟩
    have hnormCont : ContinuousOn (fun q : Real × M => ‖u' q.1 q.2‖) Q := by
      have hcont : ContinuousOn (fun q : Real × M => u' q.1 q.2) Q :=
        hsol'.jointCont.mono hQsub
      exact continuous_norm.comp_continuousOn hcont
    obtain ⟨qmax, hqmaxQ, hqmaxmax⟩ := hQcompact.exists_isMaxOn hQne hnormCont
    let R₀ : ℝ := ‖u' qmax.1 qmax.2‖
    have hR₀ : 0 ≤ R₀ := norm_nonneg _
    have hR₀bound : ∀ q : Real × M, q ∈ Q → ‖u' q.1 q.2‖ ≤ R₀ := fun q hq => hqmaxmax hq
    obtain ⟨L, hLipAll⟩ := hLip (2 * R₀) (by positivity)
    let KK : Real := (L : Real) + 1
    let d : Real × M → Real := fun q ↦
      Real.exp (-KK * q.1) * Metric.infDist (u' q.1 q.2) K
    have hdcont : ContinuousOn d Q := by
    ...
    have hdistp : ‖u' q₀.1 q₀.2 - p‖ ≤ R₀ := by
      have hzeroK : WithLp.toLp 2 ((0 : F), q₀.1) ∈ K := by
        rw [hK_eq]
        constructor
        · exact ⟨hq₀Q.1.1, hq₀Q.1.2.trans ht.2.le⟩
        · simpa using hK0 q₀.1 ⟨hq₀Q.1.1, hq₀Q.1.2.trans ht.2.le⟩
      have hinf : Metric.infDist (u' q₀.1 q₀.2) K ≤
          dist (u' q₀.1 q₀.2) (WithLp.toLp 2 ((0 : F), q₀.1)) := by
        exact Metric.infDist_le_dist_of_mem hzeroK
      calc
        ‖u' q₀.1 q₀.2 - p‖ = Metric.infDist (u' q₀.1 q₀.2) K := hdist₀.symm
        _ ≤ dist (u' q₀.1 q₀.2) (WithLp.toLp 2 ((0 : F), q₀.1)) := hinf
        _ = ‖u q₀.1 q₀.2‖ := by
          rw [dist_eq_norm]
          have hsub : u' q₀.1 q₀.2 - WithLp.toLp 2 ((0 : F), q₀.1) =
              WithLp.toLp 2 (u q₀.1 q₀.2, (0 : ℝ)) := by
            rw [← WithLp.toLp_ofLp (p := 2) (u' q₀.1 q₀.2 - WithLp.toLp 2 ((0 : F), q₀.1))]
            congr 1
            rw [WithLp.ofLp_sub, WithLp.ofLp_toLp, WithLp.ofLp_toLp]
            dsimp [u']
            exact Prod.ext (by simp) (by simp)
          rw [hsub]
          rw [WithLp.norm_toLp_fst]
        _ ≤ R₀ := by
          exact le_trans (withLp_prod_fst_norm_le (u q₀.1 q₀.2) q₀.1) hu'bound
    have hp'bound : ‖p‖ ≤ 2 * R₀ := by
      calc
        ‖p‖ = ‖(p - u' q₀.1 q₀.2) + u' q₀.1 q₀.2‖ := by
          rw [sub_add_cancel]
        _ ≤ ‖p - u' q₀.1 q₀.2‖ + ‖u' q₀.1 q₀.2‖ := norm_add_le _ _
        _ ≤ R₀ + R₀ := add_le_add (by simpa [norm_sub_rev] using hdistp) hu'bound
        _ = 2 * R₀ := by ring
    have hu'mem : u' q₀.1 q₀.2 ∈ Metric.closedBall 0 (2 * R₀) := by
      rw [mem_closedBall_zero_iff]
      exact le_trans hu'bound (by nlinarith [hR₀])
