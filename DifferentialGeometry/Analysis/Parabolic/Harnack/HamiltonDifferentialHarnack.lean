...
theorem gradientFun_add
  ...
  have hbridge (k : M → ℝ) :
      gradientFun (I := I) g k x = gradFun (I := I) g k x := by
    exact gradient_eq_gradFun (I := I) g k x
  rw [hbridge (f + h), hbridge f, hbridge h]
  exact DifferentialGeometry.Geometry.Operator.gradFun_add g hf hh
...
theorem normGradSq_log_heat_evolution_identity
    ...
    deriv (fun s : ℝ =>
      ...
      laplacianOfSmooth (I := I) g (hNslice t ht) x =
  ...
  have hlog_ev : ∀ s (hs : s ∈ D.regular) y,
      deriv (fun σ : ℝ => Real.log (u σ y)) s =
        laplacianOfSmooth (I := I) g (hlogslice s (D.regular_subset hs)) y + N s y := by
    intro s hs y
  ...
  have hdel_cd : ContMDiff I 𝓘(ℝ, ℝ) ∞
    ...
    change ContMDiff I 𝓘(ℝ, ℝ) ∞
      (laplacianOfSmooth (I := I) g (hlogslice t (D.regular_subset ht)))
    exact laplacianOfSmooth_contMDiff (I := I) g (hlogslice t (D.regular_subset ht))
  ...
  have hbochner :=
    ...
    (I := I) g hslice_ft x
  have hbochner' : laplacianOfSmooth (I := I) g (hNslice t ht) x =
      2 * frobeniusSqGradVector (I := I) g (fun b => gradFun (I := I) g (fun y => f t y) b) x +
        2 * ricciTensor (I := I) g x (gradFun (I := I) g (fun y => f t y) x) (gradFun (I := I) g
          (fun y => f t y) x) +
        2 * g.inner x (gradFun (I := I) g (fun y => f t y) x) (gradFun (I := I) g (laplacianOfSmooth
          (I := I) g hslice_ft) x) := by
    have hcongr : laplacianOfSmooth (I := I) g (hNslice t ht) x =
        laplacianOfSmooth (I := I) g (normGradSqFun_contMDiff (I := I) g hslice_ft) x := by
      ...
      intro y hy
      have hbridge : gradientFun (I := I) g (fun z ↦ f t z) y =
          gradFun (I := I) g (fun z ↦ f t z) y := by
        exact gradient_eq_gradFun (I := I) g (fun z ↦ f t z) y
      change g.inner y (gradientFun (I := I) g (fun z ↦ f t z) y)
          (gradientFun (I := I) g (fun z ↦ f t z) y) =
        g.inner y (gradFun (I := I) g (fun z ↦ f t z) y)
          (gradFun (I := I) g (fun z ↦ f t z) y)
      rw [hbridge]
  ...
  have hmain : deriv (fun s : ℝ => N s x) t -
    ...
    have hΔf_eq : g.inner x (gradientFun (I := I) g (fun y => Real.log (u t y)) x)
          (gradientFun (I := I) g (fun y => laplacianOfSmooth (I := I) g (hlogslice t
            (D.regular_subset ht)) y) x) =
        g.inner x (gradFun (I := I) g (fun y => f t y) x) (gradFun (I := I) g (laplacianOfSmooth
          (I := I) g hslice_ft) x) := by
      rfl
    rw [hΔf_eq]
    ring
...
theorem hamiltonF_ricci_dissipation_of_ricci_lower_bound
  ...
  have hvecg : ∀ h : M → ℝ, gradientFun (I := I) g h x = gradFun (I := I) g h x := by
    intro h
    exact gradient_eq_gradFun (I := I) g h x
...
theorem hamiltonF_evolution_inequality_of_ricci_lower_bound
    ...
    deriv (fun τ : ℝ =>
      ...
      laplacianOfSmooth (I := I) g (hFslice s hs) x ≤
  ...
  have hNid_raw := normGradSq_log_heat_evolution_identity (I := I) (M := M)
    ...
    hs x hNslice
  have hqid' : deriv (fun τ' : ℝ => liYauQuantity g f τ' x) s - laplacianOfSmooth
    (I := I) g hq_slice_cd x =
    ...
    exact hqid_raw
  have hNid' : deriv (fun τ' : ℝ => N τ' x) s - laplacianOfSmooth (I := I) g hN_slice_cd x =
  ...
  have hFtime : deriv (fun τ' : ℝ => F τ' x) s =
    ...
    have hprod : HasDerivAt (fun τ' : ℝ => (1 - Real.exp (c2 * τ')) * N τ' x)
      ...
      have hlin : HasDerivAt (fun τ' : ℝ => c2 * τ') c2 s := by
        convert ((hasDerivAt_const s c2).mul (hasDerivAt_id s)).congr_of_eventuallyEq
          (Filter.Eventually.of_forall fun _ ↦ rfl) using 1
        all_goals first | rfl | ring
      have hdexp : HasDerivAt (fun τ' : ℝ => Real.exp (c2 * τ'))
          (Real.exp (c2 * s) * c2) s := by
        exact ((Real.hasDerivAt_exp (c2 * s)).comp s hlin).congr_of_eventuallyEq
          (Filter.Eventually.of_forall fun _ ↦ rfl)
      have hcoefd : HasDerivAt (fun τ' : ℝ => 1 - Real.exp (c2 * τ'))
          (-(Real.exp (c2 * s) * c2)) s := by
        convert ((hasDerivAt_const s (1 : ℝ)).sub hdexp).congr_of_eventuallyEq
          (Filter.Eventually.of_forall fun _ ↦ rfl) using 1
        all_goals first | rfl | ring
  ...
  have hlapq : laplacianAt (I := I) Gfam s (q s) x = laplacianOfSmooth (I := I) g hq_slice_cd x := by
    rw [laplacianAt_eq_delta (I := I) Gfam s hq_slice_cd hconn]
  have hlapN : laplacianAt (I := I) Gfam s (N s) x = laplacianOfSmooth (I := I) g hN_slice_cd x := by
  ...
  have hFsplit : deriv (fun τ' : ℝ => F τ' x) s - laplacianAt (I := I) Gfam s (F s) x =
      (deriv (fun τ' : ℝ => liYauQuantity g f τ' x) s - laplacianOfSmooth (I := I) g hq_slice_cd x) -
        (1 - Real.exp (c2 * s)) *
          (deriv (fun τ' : ℝ => N τ' x) s - laplacianOfSmooth (I := I) g hN_slice_cd x) +
        c2 * Real.exp (c2 * s) * N s x := by
  ...
  have hdrift : 2 * g.inner x (gradientFun (I := I) g (f s) x) (gradientFun (I := I) g (q s) x) -
    ...
    rw [hinner]
    ring
  let A : ℝ := deriv (fun τ' : ℝ => liYauQuantity g f τ' x) s - laplacianOfSmooth (I := I) g hq_slice_cd x
  let B : ℝ := deriv (fun τ' : ℝ => N τ' x) s - laplacianOfSmooth (I := I) g hN_slice_cd x
  have hsubst : A - (1 - Real.exp (c2 * s)) * B + c2 * Real.exp (c2 * s) * N s x ≤
      -(2 / n) * Real.exp (c2 * s) * (q s x)^2 := by
    change (deriv (fun τ' : ℝ => liYauQuantity g f τ' x) s - laplacianOfSmooth (I := I) g hq_slice_cd x) -
        (1 - Real.exp (c2 * s)) *
          (deriv (fun τ' : ℝ => N τ' x) s - laplacianOfSmooth (I := I) g hN_slice_cd x) +
        c2 * Real.exp (c2 * s) * N s x ≤
  ...
  have hF_delta : laplacianAt (I := I) Gfam s (F s) x =
      laplacianOfSmooth (I := I) g (hFslice s hs) x := by
  ...
  have hG_nonpos : ∀ eps : ℝ, 0 < eps → G eps τ y ≤ 0 := by
    ...
    have hval : deriv (fun τ' : ℝ => G eps τ' x₀) s =
      ...
      have hlin : HasDerivAt (fun τ' : ℝ => τ' * F τ' x₀)
        ...
        convert ((hasDerivAt_id s).mul hFsmooth.hasDerivAt).congr_of_eventuallyEq
          (Filter.Eventually.of_forall fun _ ↦ rfl) using 1
      ...
      have hexp : HasDerivAt (fun τ' : ℝ => Real.exp (2 * K * τ') * (n / 2))
        ...
        have hdexp : HasDerivAt (fun τ' : ℝ => Real.exp (2 * K * τ')) (Real.exp (2 * K * s) *
          ...
          have hinner : HasDerivAt (fun τ' : ℝ => 2 * K * τ') (2 * K) s := by
            convert ((hasDerivAt_const s (2 * K)).mul (hasDerivAt_id s)).congr_of_eventuallyEq
              (Filter.Eventually.of_forall fun _ ↦ rfl) using 1
            all_goals first | rfl | ring
          exact ((Real.hasDerivAt_exp (2 * K * s)).comp s hinner).congr_of_eventuallyEq
            (Filter.Eventually.of_forall fun _ ↦ rfl)
        convert hdexp.mul (hasDerivAt_const s (n / 2)) using 1
        all_goals first | rfl | ring
    ...
    have hFle : deriv (fun τ' : ℝ => F τ' x₀) s - laplacianAt (I := I) Gfam s (F s) x₀ ≤
      ...
      have hlapF_slice : laplacianAt (I := I) Gfam s (F s) x₀ =
          laplacianOfSmooth (I := I) g (hF_s_cd) x₀ := by
    ...
    have hlapG : laplacianAt (I := I) Gfam s (G eps s) x₀ =
      ...
      have hscd : ContMDiff I 𝓘(ℝ, ℝ) ∞ (fun y : M => s • F s y) := by
        rw [show (fun y : M ↦ s • F s y) = (fun _ : M ↦ s) * F s by
          funext y
          simp only [smul_eq_mul, Pi.mul_apply]]
        exact contMDiff_const.mul hF_s_cd
...
theorem heat_solution_hamilton_differential_harnack_of_ricci_lower_bound_on
  ...
  have hvec : ∀ (τ : ℝ) (y : M), gradientFun (I := I) g (f τ) y = gradFun (I := I) g (f τ) y := by
    intro τ y
    exact gradient_eq_gradFun (I := I) g (f τ) y
...
theorem heat_solution_hamilton_differential_harnack_of_ricci_lower_bound
  ...
  let hF : ContMDiff (𝓘(ℝ, ℝ).prod I) 𝓘(ℝ, ℝ) ∞
    ...
    have hexp : ContMDiff (𝓘(ℝ, ℝ).prod I) 𝓘(ℝ, ℝ) ∞
      ...
      rw [show (fun p : ℝ × M ↦ Real.exp ((-(2 * K)) * p.1)) =
        Real.exp ∘ (fun p : ℝ × M ↦ (-(2 * K)) * p.1) by rfl]
      exact (Real.contDiff_exp.contMDiff :
        ContMDiff 𝓘(ℝ, ℝ) 𝓘(ℝ, ℝ) ∞ Real.exp).comp hlin
