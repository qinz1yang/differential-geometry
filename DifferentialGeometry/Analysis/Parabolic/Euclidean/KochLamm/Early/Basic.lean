...
theorem kochLammL1Scale_inv {R : ℝ} (hR : 0 < R) :
    (kochLammL1Scale (V := V) R)⁻¹ =
      ENNReal.ofReal (R ^ Module.finrank ℝ V) := by
  simp [kochLammL1Scale, kochLammL1ScaleR, kochLammDim, Real.rpow_neg hR.le,
    Real.rpow_natCast, ENNReal.ofReal_inv_of_pos (pow_pos hR _)]

omit [NormedSpace ℝ F]
  [CompleteSpace F]
  [Nontrivial V] in
theorem kochLammSourceZero_sourceCarlesonBound {T : ℝ} {A₁ A_q : ℝ≥0}
    {f : ℝ × V → F} (h : KochLammSourceZero T A₁ A_q f) :
    SourceCarlesonBound T (A₁ : ℝ≥0∞) f := by
  refine ⟨?_, ?_⟩
  · simpa [kochLammVolume, spaceTimeVolume] using h.ae
  · intro x R hR hRT
    have hb := h.local_l1 x R hR hRT
    have hs0 : kochLammL1Scale (V := V) R ≠ 0 := by
      exact (ENNReal.ofReal_pos.mpr
        (Real.rpow_pos_of_pos hR (-kochLammDim V))).ne'
...
theorem KochLammSourceZero.earlyHeat_norm {T t : ℝ} {A₁ A_q : ℝ≥0}
  ...
  heatEarly0_norm ht htT f x (kochLammSourceZero_sourceCarlesonBound h)
