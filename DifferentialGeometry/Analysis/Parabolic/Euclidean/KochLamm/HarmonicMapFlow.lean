...
theorem kochLammHarmonicMapFlow_split {T : ℝ} {A₀ A₂ Aₚ ε K : ℝ≥0}
    (A : ℝ × V → G →L[ℝ] H)
    (Q : ℝ × V → G →L[ℝ] G →L[ℝ] F)
    (u : ℝ × V → U) (d : ℝ × V → G)
    (hA : ∀ z, ‖A z‖ ≤ (ε : ℝ))
    ...
    KochLammSourceSplitting T (K * A₂ * A₂) (K * Aₚ * Aₚ) (ε * A₂) (ε * Aₚ)
      (fun z ↦ Q z (d z) (d z)) (fun z ↦ A z (d z)) := by
  refine ⟨?_, ?_⟩
  · apply kochLammBilin_source Q d d hQ hQmeas
    · exact ⟨hd.grad_ae, hd.grad_l2, hd.grad_lp⟩
    · exact ⟨hd.grad_ae, hd.grad_l2, hd.grad_lp⟩
  · apply KochLammSourceOne.clm_apply A d hA hAmeas
    exact ⟨hd.grad_ae, hd.grad_l2, hd.grad_lp⟩
