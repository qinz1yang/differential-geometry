...
Re-keyed onto the intrinsic compact-operator eigenbasis: each occurrence of
`tensorResolventEigenbasisVec h_atlas i` becomes
`tensorResolventEigenbasisVec_ofCompact
  (tensorResolventL2_isCompactOperator_intrinsic g r s) i`, the bundle structure
and every chart-local atom become its `_unconditional` twin, and the
`HasLocallyConstantChartAt` hypothesis disappears entirely. -/

section Unconditional

open DifferentialGeometry.PDE.RicciFlow.IntrinsicSpectral in
/-- Chart-locality-free twin of `sharpDiffPerKBdd`. -/
structure sharpDiffPerKBdd_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (N : ℕ) where
  /-- The structural `MemWkp K'` regularity of the resolvent chart component,
...
private lemma sharpDiffBdd_diff_memWkp_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (N : ℕ)
...
lemma rhsZeroAggregate_le_energy_perK_bdd_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (K N : ℕ)
    (hKN : K + 1 ≤ N)
    (H : sharpDiffPerKBdd_unconditional (I := I) (M := M) g r s α P₀ N) :
  ...
  have hCagg_nn : 0 ≤ Cagg := by
    ...
    exact H.hCeig_nn K
  refine ⟨Cagg, e_max, hCagg_nn, fun i => ?_⟩
  have hvec_norm : ‖tensorResolventEigenbasisVec_ofCompact (I := I) (M := M)
...
private lemma sharpDiffBdd_level_zero_wkpNorm_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (K N : ℕ)
    (hKN : K + 1 ≤ N)
    (H : sharpDiffPerKBdd_unconditional (I := I) (M := M) g r s α P₀ N) :
  ...
  classical
  obtain ⟨Cagg, eAgg, hCagg_nn, hCagg_bd⟩ :=
    rhsZeroAggregate_le_energy_perK_bdd_unconditional (I := I) (M := M)
      g r s α P₀ K N hKN H
  have h_pou : ∀ (i : TensorEigenIdx (I := I) (M := M) g r s)
      (β : M) (Q : TensorCompIdx (E := E) r s),
      MemWkp (d := Module.finrank ℝ E) (K + 1) 2
...
private lemma sharpDiffBdd_recursion_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (N : ℕ)
    (H : sharpDiffPerKBdd_unconditional (I := I) (M := M) g r s α P₀ N) :
    ∀ (m : ℕ) (K : ℕ) (_ : K + m + 1 ≤ N)
  ...
  | zero =>
      ...
      have hK1_le_N : K + 1 ≤ N := by simpa using hKN
      obtain ⟨C, e, hC_nn, hC_bd⟩ :=
        sharpDiffBdd_level_zero_wkpNorm_unconditional (I := I) (M := M)
  ...
  | succ m ih =>
      ...
      have h_prev_mem_succ : ∀ i : TensorEigenIdx (I := I) (M := M) g r s,
          MemWkp (d := Module.finrank ℝ E) (K + 1) 2
            ...
            (chartTargetEuclid (I := I) (M := M) α) := fun i =>
        sharpDiffBdd_diff_memWkp_unconditional (I := I) (M := M) g r s α P₀ N H i
          m (K + 1) hKN_diff (Fin.init l)
      have h_prev_ae_zero : ∀ i : TensorEigenIdx (I := I) (M := M) g r s,
          eigenvectorChartRHSDiff_unconditional (I := I) (M := M)
              g r s i α P₀ m (Fin.init l)
...
theorem eigenvectorChartRHSDiff_wkpNorm_le_chartcpt_sharp_bdd_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (m K : ℕ)
    (l : Fin m → Fin (Module.finrank ℝ E))
    (H : sharpDiffPerKBdd_unconditional (I := I) (M := M)
      g r s α P₀ (K + m + 1)) :
    ∃ (C : ℝ) (e : ℕ), 0 ≤ C ∧
      ∀ i : TensorEigenIdx (I := I) (M := M) g r s,
        wkpNorm (d := Module.finrank ℝ E) K 2
          ...
          ≤ ENNReal.ofReal (C * (i.fst.val)⁻¹ ^ e) *
            ENNReal.ofReal
              ‖tensorResolventEigenbasisVec_ofCompact (I := I) (M := M)
                (tensorResolventL2_isCompactOperator_intrinsic (I := I) (M := M)
                  g r s) i‖ :=
  sharpDiffBdd_recursion_unconditional (I := I) (M := M) g r s α P₀ (K + m + 1)
    H m K (le_refl _) l
