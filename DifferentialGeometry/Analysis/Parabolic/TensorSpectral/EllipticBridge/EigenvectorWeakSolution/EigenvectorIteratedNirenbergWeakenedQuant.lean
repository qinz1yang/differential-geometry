...
theorem eigenvectorChartIteratedPartial_wkpNorm_two_two_le_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) {m : ℕ}
    (directions : Fin m → Fin (Module.finrank ℝ E))
  ...
  have h_first :
      DifferentialGeometry.Analysis.Sobolev.Euclidean.MemWkp
        ...
        (eigenvectorChartIteratedPartial_unconditional (I := I) (M := M)
          g r s i α P₀ m directions)
        (chartTargetEuclid (I := I) (M := M) α) :=
    eigenvectorChartIteratedPartial_memWkp_two_two_unconditional
  ...
  have hD_weak_partial : ∀ j,
    ...
    rw [hP_def]
    change DifferentialGeometry.Analysis.Sobolev.Euclidean.chosenWeakPartial'
        2 j (eigenvectorChartIteratedPartial_unconditional (I := I) (M := M)
  ...
  have h_global_Lp : MemLp P 2
      ((volume : Measure EuclN).restrict
        (chartTargetEuclid (I := I) (M := M) α)) := by
    rw [hP_def]
    exact eigenvectorChartIteratedPartial_unconditional_memLp_volume
      (I := I) (M := M) g r s i α P₀ m directions
  have h_ae_zero :
      P =ᵐ[(volume : Measure EuclN).restrict
        (chartTargetEuclid (I := I) (M := M) α \ K)] 0 := by
    rw [hP_def, hK_def]
    exact eigenvectorChartIteratedPartial_unconditional_ae_zero_off_chartPouKernel
      (I := I) (M := M) g r s i α P₀ m directions
  obtain ⟨K_prom, hK_prom_pos, hK_prom_bound⟩ :=
    DifferentialGeometry.Analysis.Sobolev.Euclidean.wkpNorm_le_of_memWkp_precompact_of_ae_zero_off_compact
      (d := Module.finrank ℝ E) (k := 2) (p := 2)
...
theorem eigenvectorChartIteratedPartial_wkpNorm_two_two_le_uniform_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) {m : ℕ} :
    ∃ C : ℝ, 0 < C ∧
      ∀ (i : TensorEigenIdx (I := I) (M := M) g r s)
