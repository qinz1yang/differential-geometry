...
The same `W^{k,2}` regularity propagators, re-keyed onto the intrinsic
compact-operator eigenbasis, without the `HasLocallyConstantChartAt` hypothesis.
-/

/-- Chart-locality-free twin of `eigenvectorChartIteratedStep_memWkp_K_two`. -/
theorem eigenvectorChartIteratedStep_memWkp_K_two_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (m K : ℕ)
    (dirs : Fin m → Fin (Module.finrank ℝ E))
...
/-- Chart-locality-free twin of `eigenvectorChartIteratedStep_memW1p_two`. -/
theorem eigenvectorChartIteratedStep_memW1p_two_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (m : ℕ)
    (dirs : Fin m → Fin (Module.finrank ℝ E))
  ...
  have h_comp' : MemWkp (d := Module.finrank ℝ E) (m + 2 + 1) 2
    ...
    have h_eq : m + 2 + 1 = m + 3 := by ring
    rw [h_eq]; exact h_comp
  have h_mem := eigenvectorChartIteratedStep_memWkp_K_two_unconditional (I := I) (M := M)
