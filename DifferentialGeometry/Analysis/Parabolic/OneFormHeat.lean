...
structure IsEvolvingOneFormOn
    ...
    (phi : Real -> (x : M) ->
      Tensor0SSpace (𝕜 := Real) (E := E) (H := H) (I := I) (M := M) 1 x) : Prop where
  realizes :
    forall t : RealTimeInterval.FlowTime D, forall x : M,
      Nabla2OneFormRealizesAt (I := I) (G.connection (t : Real))
        (h (t : Real)) (nablaH (t : Real)) x (nabla2H (t : Real) x)
...
lemma IsHeatOneFormOn.toEvolving
    ...
    IsEvolvingOneFormOn (I := I) G h nablaH nabla2H
      (fun t x => roughLap0STensor (I := I) (G.metric t) (s := 1) (nabla2H t x)) where
  realizes := hHeat.realizes
  equation := hHeat.equation
  jointSmooth := hHeat.jointSmooth
  jointSmoothNabla := hHeat.jointSmoothNabla
...
theorem evolvingOneForm_nablaNormSq_jointContMDiffOn
    [IsManifold I 1 M] [IsManifold I 2 M] [IsManifold I ((∞ : WithTop ℕ∞) + 1) M]
    {D : RealTimeInterval}
    {G : RealizedMetricFamilyOn (I := I) (M := M) D}
    (hG : MetricFamilySmoothOn (I := I) (M := M) D G)
    ...
    {nabla2H : ℝ → (x : M) →
      Tensor0SSpace (𝕜 := ℝ) (E := E) (H := H) (I := I) (M := M) 3 x}
    {phi : ℝ → (x : M) →
      Tensor0SSpace (𝕜 := ℝ) (E := E) (H := H) (I := I) (M := M) 1 x}
    (hEvol : IsEvolvingOneFormOn (I := I) G h nablaH nabla2H phi) :
    ContMDiffOn (𝓘(ℝ, ℝ).prod I) 𝓘(ℝ, ℝ) ∞
      (fun p : ℝ × M => normSq0S (I := I) (G.metric p.1) p.2 2 (nablaH p.1 p.2))
      (D.regular ×ˢ (Set.univ : Set M)) := by
  refine Tensor0SBundle.normSq0S_jointContMDiffOn (fun t => G.metric t) 2 (fun t x => nablaH t x)
  ...
  · exact hG.frameCompSmooth
  · intro Idx _ frame u hframe m
    refine (hEvol.jointSmoothNabla frame hframe (m 0) (m 1)).congr (fun p _ => ?_)
    change nablaH p.1 p.2 (fun a : Fin 2 => frame (m a) p.2)
        = nablaH p.1 p.2 (vec2 (frame (m 0) p.2) (frame (m 1) p.2))
    congr 1
    funext a
