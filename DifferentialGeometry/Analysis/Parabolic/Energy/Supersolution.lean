...
theorem localized_energy_differential_of_supersolution
  ...
  have hcross_cont : Continuous (fun x : M =>
    ...
    have hinner := contMDiff_g_inner_of_smooth_sections (I := I) (M := M) g
      (gradG (I := I) g cutoff.toContMDiffMap) (gradG (I := I) g ut.toContMDiffMap)
    have hinner' : Continuous (fun x : M =>
        g.inner x
          (gradientFun (I := I) g cutoff.toFun x)
          (gradientFun (I := I) g ut.toFun x)) := by
      change Continuous (fun x : M =>
        g.inner x
          (gradG (I := I) g cutoff.toContMDiffMap x)
          (gradG (I := I) g ut.toContMDiffMap x))
      exact hinner.continuous
    exact cutoff.smooth.continuous.mul hinner'
  have hlap_cont : Continuous (fun x : M => ΔG (I := I) g ut.toContMDiffMap x) :=
  ...
  have hlap_int : Integrable (fun x : M =>
      cutoff.toFun x ^ 2 * ΔG (I := I) g ut.toContMDiffMap x) μ :=
  ...
  have htime_int : Integrable (fun x : M =>
    ...
    ((cutoff.smooth.continuous.pow 2).mul htime_cont)
      |>.integrable_of_hasCompactSupport (HasCompactSupport.of_compactSpace _)
  have hpointwise : ∀ x : M,
      cutoff.toFun x ^ 2 * ΔG (I := I) g ut.toContMDiffMap x +
  ...
  have htime_le :
      (∫ x, cutoff.toFun x ^ 2 * ΔG (I := I) g ut.toContMDiffMap x ∂μ) +
  ...
  change (∫ x, g.inner x
      ...
      (gradFun (I := I) g ut.toFun x)
        ∂(riemannianVolumeMeasure (I := I) (M := M) g)) =
    -∫ x, test.toFun x * ΔG (I := I) g ut.toContMDiffMap x
    ...
    have hgreen' :
        2 * ∫ x, cutoff.toFun x *
            g.inner x
              (gradientFun (I := I) g cutoff.toFun x)
              (gradientFun (I := I) g ut.toFun x) ∂μ =
          -∫ x, cutoff.toFun x ^ 2 * ΔG (I := I) g ut.toContMDiffMap x ∂μ := by
      calc
        2 * ∫ x, cutoff.toFun x *
              g.inner x
                (gradientFun (I := I) g cutoff.toFun x)
        ...
        _ = -∫ x, cutoff.toFun x ^ 2 * ΔG (I := I) g ut.toContMDiffMap x ∂μ := by
              simpa only [μ, test, ut, smoothScalarSlice_toFun,
                hgradient] using hgreen
    linarith
