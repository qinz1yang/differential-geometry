...
private lemma chart_christoffel_expansion_nabla_W_alpha_chartBasis
  ...
  have hW_at : MDiffAt (T% fun y => W y) x := W.mdifferentiableAt
  set v : TangentSpace I x := DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α j x with hv_def
  have h_trivToE_v : trivToE (I := I) α x v = (DifferentialGeometry.Tensor.Coordinates.chartModelBasis E) j := by
    rw [hv_def]
    ...
    rw [heq, trivToE_trivFromE (I := I) α hx_base ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E) j)]
  have hLC_apply :
  ...
  · rw [h_trivToE_v]
    rw [show ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E).repr
          (fderiv ℝ (chartESectionRepr (I := I) α (W : ∀ x : M, TangentSpace I x) ∘
            (extChartAt I α).symm) (extChartAt I α x) ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E) j))) k =
        (((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E).coord k).toContinuousLinearMap)
          (fderiv ℝ (chartESectionRepr (I := I) α (W : ∀ x : M, TangentSpace I x) ∘
            (extChartAt I α).symm) (extChartAt I α x) ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E) j)) from by
      rw [← Module.Basis.coord_apply]; rfl]
    rw [← ContinuousLinearMap.comp_apply]
    rw [← ContinuousLinearMap.fderiv (((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E).coord k).toContinuousLinearMap)]
          ...
          (∑ i' : Fin (Module.finrank ℝ E),
            ∑ j' : Fin (Module.finrank ℝ E),
              ∑ k' : Fin (Module.finrank ℝ E),
                (((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E).repr ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E) j)) i' *
                    ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E).repr
                      (chartESectionRepr (I := I) α
                        (W : ∀ x : M, TangentSpace I x) x)) j' *
                    chartChristoffel (I := I) g α i' j' k' (extChartAt I α x)) •
                  (DifferentialGeometry.Tensor.Coordinates.chartModelBasis E) k')) k =
        ∑ i' : Fin (Module.finrank ℝ E),
          ∑ j' : Fin (Module.finrank ℝ E),
            ∑ k' : Fin (Module.finrank ℝ E),
              (((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E).repr ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E) j)) i' *
                  ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E).repr
                    (chartESectionRepr (I := I) α
                      (W : ∀ x : M, TangentSpace I x) x)) j' *
                  chartChristoffel (I := I) g α i' j' k' (extChartAt I α x)) *
                ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E).repr ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E) k')) k from by
            ...
            (((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E).repr ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E) j)) i' *
                ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E).repr
                  (chartESectionRepr (I := I) α
                    (W : ∀ x : M, TangentSpace I x) x)) j' *
                chartChristoffel (I := I) g α i' j' k' (extChartAt I α x)) *
              ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E).repr ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E) k')) k) =
        ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E).repr ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E) j)) i' *
          chartChristoffel (I := I) g α i' j' k (extChartAt I α x) *
          ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E).repr
            (chartESectionRepr (I := I) α
...
private lemma chartLieDerivMetricMatrix_alpha_algebraic
  ...
  have hreshape_H1 :
    ...
    rw [show (∑ k : Fin (Module.finrank ℝ E),
          chartCoeff (I := I) α W k x *
            chartChristoffel (I := I) g α k i l (extChartAt I α x) *
            DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g α x l j) =
        DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g α x l j *
  ...
  have hreshape_H2 :
    ...
    refine Finset.sum_congr rfl (fun l _ => ?_)
    rw [hgram l i]
    rw [show DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g α x l i =
          DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g α x i l from by
    ...
    rw [show (∑ k : Fin (Module.finrank ℝ E),
          chartCoeff (I := I) α W k x *
            chartChristoffel (I := I) g α k j l (extChartAt I α x) *
            DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g α x i l) =
        DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g α x i l *
...
theorem chartLieDerivMetricMatrix_eq_lieDerivMetric_chartBasis
  ...
  have hRHS2 :
      ...
      ∑ i' : Fin (Module.finrank ℝ E),
        ∑ j' : Fin (Module.finrank ℝ E),
          ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E).repr (trivToE (I := I) α x vα)) i' *
            ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E).repr
              (trivToE (I := I) α x
  ...
  have h_trivToE_vα : trivToE (I := I) α x vα = (DifferentialGeometry.Tensor.Coordinates.chartModelBasis E) i := by
    ...
    rw [heq, trivToE_trivFromE (I := I) α hx_base ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E) i)]
  have h_trivToE_wα : trivToE (I := I) α x wα = (DifferentialGeometry.Tensor.Coordinates.chartModelBasis E) j := by
    rw [hwα_def]
    have heq : DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α j x =
        trivFromE (I := I) α x ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E) j) := rfl
    rw [heq, trivToE_trivFromE (I := I) α hx_base ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E) j)]
