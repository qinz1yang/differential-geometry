...
@[simp] lemma curvatureGradContractionSection_toSection
...
lemma curvatureCommutatorRemainderSection_slice_eq
...
lemma curvatureCommutatorRemainderSection_slice_eval_value_local
    (g : SmoothRiemannianMetric I M) (s : ℕ) (S : SmoothCcTensor g 0 s) (x : M)
  ...
  rw [show (show Tensor0SSpace 0 I x →L[ℝ] Tensor0SSpace s I x from
    ...
    curvatureCommutatorRemainderSection_slice_eq (I := I) (M := M) g s S x a]
...
lemma curvatureCommutatorRemainderSection_value_local
  ...
  rw [curvatureCommutatorRemainderSection_slice_eval_value_local (I := I) (M := M) g s S₁ x a m,
    curvatureCommutatorRemainderSection_slice_eval_value_local (I := I) (M := M) g s S₂ x a m]
...
omit [I.Boundaryless] in
lemma curvatureGradContractionSection_toSection_add
  ...
  rw [curvatureGradContractionSection_toSection, curvatureGradContractionSection_toSection, curvatureGradContractionSection_toSection]
  rw [show (W₁ + W₂).toSection x = W₁.toSection x + W₂.toSection x from by
    rw [SmoothCcTensor.toSection_add, ContMDiffSection.coe_add, Pi.add_apply]]
  rw [map_add]

omit [I.Boundaryless] in
lemma curvatureGradContractionSection_toSection_smul
  ...
  rw [curvatureGradContractionSection_toSection, curvatureGradContractionSection_toSection]
  rw [show (c • W).toSection x = c • W.toSection x from by
    rw [SmoothCcTensor.toSection_smul, ContMDiffSection.coe_smul, Pi.smul_apply]]
  rw [map_smul]

omit [I.Boundaryless] in
lemma curvatureGradContractionSection_value_local
    (g : SmoothRiemannianMetric I M) (s : ℕ) (W₁ W₂ : SmoothCcTensor g 0 (s + 1)) (x : M)
    (hx : W₁.toSection x = W₂.toSection x) :
    (curvatureGradContractionSection (I := I) (M := M) g s W₁).toSection x =
      (curvatureGradContractionSection (I := I) (M := M) g s W₂).toSection x := by
  rw [curvatureGradContractionSection_toSection, curvatureGradContractionSection_toSection, hx]

omit [I.Boundaryless] in
theorem exists_curvatureGradContractionSection_homTensorRSFieldApply (g : SmoothRiemannianMetric I M) (s : ℕ) :
  ...
  exists_value_local_homTensorRSFieldApply (I := I) (M := M) g 0 (s + 1) (s + 1)
    (fun W => curvatureGradContractionSection (I := I) (M := M) g s W)
    (fun W₁ W₂ x => curvatureGradContractionSection_toSection_add (I := I) (M := M) g s W₁ W₂ x)
    (fun c W x => curvatureGradContractionSection_toSection_smul (I := I) (M := M) g s c W x)
    (fun W₁ W₂ x hW => curvatureGradContractionSection_value_local (I := I) (M := M) g s W₁ W₂ x hW)
...
lemma curvatureCommutatorRemainderSection_toSection_add
    (g : SmoothRiemannianMetric I M) (s : ℕ) (S₁ S₂ : SmoothCcTensor g 0 s) (x : M) :
  ...
  rw [covGrad_add (I := I) (M := M) g 0 s S₁ S₂,
    curvatureGradContractionSection_toSection_add (I := I) (M := M) g s
      (covGrad (I := I) (M := M) g 0 s S₁) (covGrad (I := I) (M := M) g 0 s S₂) x]
  abel

lemma curvatureCommutatorRemainderSection_toSection_smul
    (g : SmoothRiemannianMetric I M) (s : ℕ) (c : ℝ) (S : SmoothCcTensor g 0 s) (x : M) :
  ...
  rw [covGrad_smul (I := I) (M := M) g 0 s c S,
    curvatureGradContractionSection_toSection_smul (I := I) (M := M) g s c
      (covGrad (I := I) (M := M) g 0 s S) x]
  rw [smul_sub]

theorem exists_curvatureCommutatorRemainderSection_homTensorRSFieldApply (g : SmoothRiemannianMetric I M) (s : ℕ) :
  ...
  exists_value_local_homTensorRSFieldApply (I := I) (M := M) g 0 s (s + 1)
    (fun S => curvatureCommutatorRemainderSection (I := I) (M := M) g s S)
    (fun S₁ S₂ x => curvatureCommutatorRemainderSection_toSection_add (I := I) (M := M) g s S₁ S₂ x)
    (fun c S x => curvatureCommutatorRemainderSection_toSection_smul (I := I) (M := M) g s c S x)
    (fun S₁ S₂ x hS => curvatureCommutatorRemainderSection_value_local (I := I) (M := M) g s S₁ S₂ x hS)
...
theorem exists_pointwiseTensorCurv_firstOrder_homField_section
  ...
  obtain ⟨H_R, hH_R⟩ := exists_curvatureGradContractionSection_homTensorRSFieldApply (I := I) (M := M) (E := E) g s
  obtain ⟨H_dR, hH_dR⟩ := exists_curvatureCommutatorRemainderSection_homTensorRSFieldApply (I := I) (M := M) (E := E) g s
