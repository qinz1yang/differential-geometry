...
private noncomputable def coBchange
  ...
  Matrix.of fun i k => (DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x).repr (B i) k
...
@[simp] private lemma coBchange_apply
    ...
    coBchange (I := I) (x := x) B i k =
      (DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x).repr (B i) k := rfl

private noncomputable def modelGramMatrix :
    Matrix (Fin (Module.finrank ℝ E)) (Fin (Module.finrank ℝ E)) ℝ :=
  Matrix.of fun k l =>
    g.inner x ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k) ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l)

omit [NeZero (Module.finrank ℝ E)] [SigmaCompactSpace M] [T2Space M] [BoundarylessManifold I M] in
@[simp] private lemma modelGramMatrix_apply
    (k l : Fin (Module.finrank ℝ E)) :
    modelGramMatrix (I := I) g x k l =
      g.inner x ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k) ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l) := rfl

omit [NeZero (Module.finrank ℝ E)] [SigmaCompactSpace M] [T2Space M] [BoundarylessManifold I M] in
private lemma modelGramMatrix_eq_chartGramMatrix :
    modelGramMatrix (I := I) g x = DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g x x := by
  classical
  ext k l
  rw [modelGramMatrix_apply, DifferentialGeometry.Tensor.Coordinates.chartGramMatrix_apply]
...
private lemma decompose_in_modelBasis
    ...
    B i = ∑ k : Fin (Module.finrank ℝ E),
      coBchange (I := I) (x := x) B i k •
        ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k : TangentSpace I x) := by
  classical
  have h := (DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x).sum_repr (B i)
  exact h.symm
...
private lemma bilin_expand_modelBasis
      ...
      ∑ k : Fin (Module.finrank ℝ E), ∑ l : Fin (Module.finrank ℝ E),
        coBchange (I := I) (x := x) B i k *
          coBchange (I := I) (x := x) B j l *
            Hb ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k) ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l) := by
  classical
  have hHb_first : Hb (B i) =
      ∑ k : Fin (Module.finrank ℝ E),
        coBchange (I := I) (x := x) B i k •
          Hb ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k) := by
    have hi := decompose_in_modelBasis (I := I) (x := x) B i
    rw [show Hb (B i) = Hb (∑ k : Fin (Module.finrank ℝ E),
            coBchange (I := I) (x := x) B i k •
              ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k : TangentSpace I x))
          from congrArg Hb hi]
    rw [map_sum]
  ...
  have hHb_second : Hb ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k) (B j) =
      ∑ l : Fin (Module.finrank ℝ E),
        coBchange (I := I) (x := x) B j l *
          Hb ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k) ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l) := by
    have hj := decompose_in_modelBasis (I := I) (x := x) B j
    rw [show Hb ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k) (B j) =
          Hb ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k)
            (∑ l : Fin (Module.finrank ℝ E),
              coBchange (I := I) (x := x) B j l •
                ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l : TangentSpace I x))
        from congrArg (Hb ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k)) hj]
    rw [map_sum]
    refine Finset.sum_congr rfl ?_
    intro l _
    have hsmul : Hb ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k)
        (coBchange (I := I) (x := x) B j l •
          ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l : TangentSpace I x)) =
        coBchange (I := I) (x := x) B j l •
          Hb ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k)
            ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l : TangentSpace I x) :=
      (Hb ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k)).map_smul
        (coBchange (I := I) (x := x) B j l)
        ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l : TangentSpace I x)
...
private lemma sum_coBchange_eq_invGram
  ...
  have heq : (coBchange (I := I) (x := x) B).transpose *
      coBchange (I := I) (x := x) B =
        (DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g x x)⁻¹ := by
    rw [h]
...
theorem orthonormal_basis_bilin_trace
  ...
  have h_expand : ∀ i : Fin (Module.finrank ℝ E),
      Hb (B i) (B i) =
        ∑ k : Fin (Module.finrank ℝ E), ∑ l : Fin (Module.finrank ℝ E),
          coBchange (I := I) (x := x) B i k *
            coBchange (I := I) (x := x) B i l *
              Hb ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k) ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l) :=
  ...
  rw [show ∑ i : Fin (Module.finrank ℝ E), Hb (B i) (B i) =
      ∑ i : Fin (Module.finrank ℝ E),
        ∑ k : Fin (Module.finrank ℝ E), ∑ l : Fin (Module.finrank ℝ E),
          coBchange (I := I) (x := x) B i k *
            coBchange (I := I) (x := x) B i l *
              Hb ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k) ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l) from
  ...
  rw [show (∑ i : Fin (Module.finrank ℝ E),
      ...
      (∑ i : Fin (Module.finrank ℝ E),
        coBchange (I := I) (x := x) B i k *
          coBchange (I := I) (x := x) B i l) *
        Hb ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k) ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l) from by
    rw [Finset.sum_mul]]
...
private lemma finBasis_repr_eq_invGram_inner_sum
    ...
    (DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x).repr X k =
      ∑ l : Fin (Module.finrank ℝ E),
        chartInvGramMatrix (I := I) g x x k l *
          g.inner x X ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l) := by
  classical
  have hX_decomp : X = ∑ p : Fin (Module.finrank ℝ E),
      (DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x).repr X p •
        ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) p : TangentSpace I x) :=
    ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x).sum_repr X).symm
  have h_inner_decomp : ∀ l : Fin (Module.finrank ℝ E),
      g.inner x X ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l) =
        ∑ p : Fin (Module.finrank ℝ E),
          (DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x).repr X p *
            DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g x x p l := by
    intro l
    rw [g.symm x X ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l)]
    rw [show g.inner x ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l) X =
          g.inner x ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l) (∑ p : Fin (Module.finrank ℝ E),
            (DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x).repr X p •
              ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) p : TangentSpace I x)) from
      congrArg (g.inner x ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l)) hX_decomp]
    rw [map_sum]
    refine Finset.sum_congr rfl ?_
    intro p _
    show g.inner x ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l)
        (((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x).repr X) p •
          ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) p : TangentSpace I x)) =
      ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x).repr X) p *
        DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g x x p l
    rw [show g.inner x ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l)
      ...
      ContinuousLinearMap.map_smul (g.inner x ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l)) _ _]
    rw [smul_eq_mul]
    rw [g.symm x ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l) ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) p)]
    rw [show g.inner x ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) p) ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l) =
          DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g x x p l from by
  ...
  rw [show (∑ l : Fin (Module.finrank ℝ E),
      ...
      ∑ l : Fin (Module.finrank ℝ E),
        chartInvGramMatrix (I := I) g x x k l *
          (∑ p : Fin (Module.finrank ℝ E),
            (DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x).repr X p *
              DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g x x p l) from
  ...
  have h_invGram_gram_eq : ∀ p : Fin (Module.finrank ℝ E),
    ...
    have h_gram_symm : ∀ l : Fin (Module.finrank ℝ E),
        DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g x x p l =
        DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g x x l p := by
      intro l
    ...
    rw [show (∑ l : Fin (Module.finrank ℝ E),
        ...
        ∑ l : Fin (Module.finrank ℝ E),
          chartInvGramMatrix (I := I) g x x k l *
            DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g x x l p from by
      refine Finset.sum_congr rfl ?_
      intro l _
      rw [h_gram_symm]]
    ...
    have heval : (chartInvGramMatrix (I := I) g x x *
        DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g x x) k p =
  ...
  rw [show (∑ p : Fin (Module.finrank ℝ E),
      ...
      (DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x).repr X k from by
...
private lemma linearMap_trace_eq_invGram_bilin_sum
  ...
  rw [LinearMap.trace_eq_matrix_trace ℝ
        (DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) T]
  unfold Matrix.trace
  refine Finset.sum_congr rfl ?_
  intro k _
  ...
  exact finBasis_repr_eq_invGram_inner_sum (I := I) g x
    (T ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k)) k
  ...
  have hT_trace_xy : LinearMap.trace ℝ (TangentSpace I x) T = g.inner x X Y := by
    ...
    rw [show (∑ k : Fin (Module.finrank ℝ E),
        ...
        ∑ k : Fin (Module.finrank ℝ E),
          ∑ l : Fin (Module.finrank ℝ E),
            chartInvGramMatrix (I := I) g x x k l *
              (g.inner x X ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k) *
                g.inner x ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l) Y) from by
      refine Finset.sum_congr rfl ?_
      intro k _
      refine Finset.sum_congr rfl ?_
      intro l _
    ...
    rw [show (∑ k : Fin (Module.finrank ℝ E),
          ∑ l : Fin (Module.finrank ℝ E),
            chartInvGramMatrix (I := I) g x x k l *
              (g.inner x X ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k) *
                g.inner x ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l) Y)) =
        ∑ k : Fin (Module.finrank ℝ E),
          g.inner x X ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k) *
            (∑ l : Fin (Module.finrank ℝ E),
              chartInvGramMatrix (I := I) g x x k l *
    ...
    rw [show (∑ k : Fin (Module.finrank ℝ E),
        ...
        ∑ k : Fin (Module.finrank ℝ E),
          g.inner x X ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k) *
            (DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x).repr Y k from by
      refine Finset.sum_congr rfl ?_
      intro k _
      have h := finBasis_repr_eq_invGram_inner_sum (I := I) g x Y k
      have h' : (DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x).repr Y k =
          ∑ l : Fin (Module.finrank ℝ E),
            chartInvGramMatrix (I := I) g x x k l *
              g.inner x ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l) Y := by
        rw [h]
        refine Finset.sum_congr rfl ?_
        intro l _
        rw [g.symm x Y ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) l)]
      rw [h']]
    have hY_decomp : Y = ∑ k : Fin (Module.finrank ℝ E),
        (DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x).repr Y k •
          ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k : TangentSpace I x) :=
      ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x).sum_repr Y).symm
    rw [show g.inner x X Y = g.inner x X
            (∑ k : Fin (Module.finrank ℝ E),
              (DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x).repr Y k •
                ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k : TangentSpace I x)) from
    ...
    rw [show g.inner x X
            ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x).repr Y k •
              ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k : TangentSpace I x)) =
          (DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x).repr Y k *
            g.inner x X ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k) from by
      rw [show ((g.inner x) X)
              (((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x).repr Y k) •
                ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k : TangentSpace I x)) =
            ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x).repr Y k) •
              ((g.inner x) X) ((DifferentialGeometry.Tensor.Coordinates.centeredChartTangentBasis (I := I) x) k) from
