...
theorem ricciEigen3_ordered
  ...
  let l2 : Real := hT.eigenvalues hdim 1
  let l3 : Real := hT.eigenvalues hdim 2
  have horth : OrthonormalBasisAt (I := I) g x basis := by
    intro i j
    have hinner :
        Inner.inner Real (ob i) (ob j) = D.inner (ob i) (ob j) :=
      MetricFiberData.toCore_inner D (ob i) (ob j)
    ...
    rw [← hinner]
    simpa [delta3] using hob
  have hdiag : RicciDiagAt (I := I) Ric (ricciEigenScalar3 l1 l2 l3)
      l1 l2 l3 basis := by
    constructor
    · rfl
    · intro i j
  ...
  have h21 : l2 <= l1 := by
    dsimp [l1, l2]
    exact hT.eigenvalues_antitone hdim (by decide : (0 : Fin 3) <= 1)
  have h32 : l3 <= l2 := by
    dsimp [l2, l3]
    exact hT.eigenvalues_antitone hdim (by decide : (1 : Fin 3) <= 2)
  exact ⟨basis, l1, l2, l3, horth, h21, h32, hdiag⟩
