...
theorem exists_iterated_covariant_derivative_add_norm_bound
  ...
  obtain ⟨Cc, hCc0, hcorII⟩ :=
    exists_iterated_covariant_derivative_norm_comparison_bound (I := I) hu g₀ g₁ δ₁ p eps0 heps0_0 heps0_1 hequiv hgK
  ...
  have hinv := DifferentialGeometry.Tensor0SBundle.metricInverseInBasis_of_orthonormal (I := I) g₀ basis hON
      ...
      ∀ {M' : Type u} [TopologicalSpace M'] [ChartedSpace H M']
        ...
        ∀ x ∈ u, ∀ r, 0 < r → r ≤ p →
          Real.sqrt (Tensor0SBundle.normSq0S (I := I) g₀ x (2 + r)
            (iterCov (I := I) g₀ 2 (δ₀ + δ₁) r x)) ≤ eps0 + eps1 * Cp := by
  classical
  obtain ⟨Cc, hCc0, hcorII⟩ := exists_uniform_iterated_covariant_derivative_norm_comparison_bound (E := E) (H := H) (I := I) 2 p
  have hCp0 : (0 : Real) ≤ Real.sqrt ((2 : Real) ^ (2 + p)) * (1 + Cc * (p : Real)) :=
