...
def componentL2SqRS
  ...
  ∑ upper : Fin r -> Idx, ∑ lower : Fin s -> Idx,
    (componentRSField (I := I) basis A upper lower) ^ 2
...
theorem normSqRS_one_two_identity_eq_sum
    ...
    normSqRS (I := I) (g := g) (x := x) 1 2 A =
      ∑ k : Idx, ∑ i : Idx, ∑ j : Idx,
        (componentRSField (I := I) basis A (fun _ : Fin 1 => k)
...
theorem componentRS_sq_le_componentL2SqRS
  ...
  have h_lower :
      (componentRSField (I := I) basis A upper lower) ^ 2 <=
        ∑ lower' : Fin s -> Idx,
          (componentRSField (I := I) basis A upper lower') ^ 2 := by
...
theorem sqrt_normSqRS_apply
  ...
  have hout :
    ...
    apply Finset.sum_congr rfl
    intro lower _
    rw [componentRSField_apply_input_eq_sum (I := I) basis A input lower]
  have hinput :
