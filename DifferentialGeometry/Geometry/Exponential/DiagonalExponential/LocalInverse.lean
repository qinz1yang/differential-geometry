...
section DiagonalExponentialLocalInverse
...
attribute [-instance] Tensor0SBundle.tangentSpaceNormedAddCommGroup
  Tensor0SBundle.tangentSpaceNormedSpace in
private lemma exists_chartedDiagExp_contDiffOn
    [PseudoEMetricSpace M] [IsRiemannianManifold I M] [CompleteSpace M]
    [T2Space (TangentBundle I M)]
    [IsContinuousRiemannianBundle E (fun (x : M) ↦ TangentSpace I x)]
    (g : SmoothRiemannianMetric I M)
...
attribute [-instance] Tensor0SBundle.tangentSpaceNormedAddCommGroup
  Tensor0SBundle.tangentSpaceNormedSpace in
theorem diagExp_hasFDerivAt_zero_linearEquiv
    [PseudoEMetricSpace M] [IsRiemannianManifold I M] [CompleteSpace M]
    [T2Space (TangentBundle I M)]
    [IsContinuousRiemannianBundle E (fun (x : M) ↦ TangentSpace I x)]
    (g : SmoothRiemannianMetric I M)
    ...
    (p : M) (n : ℕ) (hn : 1 ≤ n) :
    HasFDerivAt (chartedDiagExp (I := I) g hEnorm p)
      (DifferentialGeometry.PhaseFlow.freeDiagCLE (E := E) :
        (E × E) →L[ℝ] (E × E)) (diagExpZeroPt (I := I) p) := by
  rw [DifferentialGeometry.PhaseFlow.freeDiagCLE_coe]
  exact diagExp_hasFDerivAt_zero (I := I) g hEnorm p n hn
...
private def diagExpIFT
  ...
  ContDiffAt.toOpenPartialHomeomorph (𝕂 := ℝ)
    ...
    (f' := DifferentialGeometry.PhaseFlow.freeDiagCLE (E := E))
    (chartedDiagExp_cdaOne (I := I) g hEnorm p)
    (diagExp_hasFDerivAt_zero_linearEquiv (I := I) g hEnorm p 1 le_rfl)
    one_ne_zero
...
attribute [-instance] Tensor0SBundle.tangentSpaceNormedAddCommGroup
  Tensor0SBundle.tangentSpaceNormedSpace in
private lemma exists_diagExpIFT_symm_contDiffOn
    [PseudoEMetricSpace M] [IsRiemannianManifold I M] [CompleteSpace M]
    [T2Space (TangentBundle I M)]
    [IsContinuousRiemannianBundle E (fun (x : M) ↦ TangentSpace I x)]
    (g : SmoothRiemannianMetric I M)
  ...
  classical
  obtain ⟨U, hU_open, hcU, hfU⟩ := exists_chartedDiagExp_contDiffOn (I := I) g hEnorm p
  ...
  have hfd_eq : fderiv ℝ (chartedDiagExp (I := I) g hEnorm p)
    ...
    (diagExp_hasFDerivAt_zero_linearEquiv (I := I) g hEnorm p 1 le_rfl).fderiv
  have hinv_nhds :
      {L : (E × E) →L[ℝ] (E × E) | ∃ e : (E × E) ≃L[ℝ] (E × E), e = L} ∈
        𝓝 (fderiv ℝ (chartedDiagExp (I := I) g hEnorm p)
          (diagExpZeroPt (I := I) p)) := by
    rw [hfd_eq]
    exact (DifferentialGeometry.PhaseFlow.freeDiagCLE (E := E)).nhds
  ...
  have hsrc : diagExpZeroPt (I := I) p ∈
    ...
    ContDiffAt.mem_toOpenPartialHomeomorph_source
      (chartedDiagExp_cdaOne (I := I) g hEnorm p)
      (diagExp_hasFDerivAt_zero_linearEquiv (I := I) g hEnorm p 1 le_rfl) one_ne_zero
...
theorem diagExpInv_contMDiffAt
  ...
  have hsymm_eq : (diagExpIFT (I := I) g hEnorm p).symm =
      (chartedDiagExp_cdaOne (I := I) g hEnorm p).localInverse
        (diagExp_hasFDerivAt_zero_linearEquiv (I := I) g hEnorm p 1 le_rfl) one_ne_zero := rfl
...
theorem diagExpInv_contMDiffAt_order
  ...
  have hsymm_eq : (diagExpIFT (I := I) g hEnorm p).symm =
      (chartedDiagExp_contDiffAt (I := I) g hEnorm p n).localInverse
        (diagExp_hasFDerivAt_zero_linearEquiv (I := I) g hEnorm p 1 le_rfl) hn0 := rfl
  have hsymm_cd : ContDiffAt ℝ (n : ℕ∞) (diagExpIFT (I := I) g hEnorm p).symm
    ...
    exact (chartedDiagExp_contDiffAt (I := I) g hEnorm p n).to_localInverse
      (diagExp_hasFDerivAt_zero_linearEquiv (I := I) g hEnorm p 1 le_rfl) hn0
...
attribute [-instance] Tensor0SBundle.tangentSpaceNormedAddCommGroup
  Tensor0SBundle.tangentSpaceNormedSpace in
theorem exists_diagonalInverse_smooth_domain
    [PseudoEMetricSpace M] [IsRiemannianManifold I M] [CompleteSpace M]
    [T2Space (TangentBundle I M)]
    [IsContinuousRiemannianBundle E (fun (x : M) ↦ TangentSpace I x)]
    (g : SmoothRiemannianMetric I M)
  ...
  obtain ⟨V, hV_open, hcenterV, hsymmV, hVprop⟩ :=
    exists_diagExpIFT_symm_contDiffOn (I := I) g hEnorm p
...
attribute [-instance] Tensor0SBundle.tangentSpaceNormedAddCommGroup
  Tensor0SBundle.tangentSpaceNormedSpace in
private theorem exists_standardDiagonalInverseBranch
    [PseudoEMetricSpace M] [IsRiemannianManifold I M] [CompleteSpace M]
    [T2Space (TangentBundle I M)]
    [IsContinuousRiemannianBundle E (fun (x : M) ↦ TangentSpace I x)]
    (g : SmoothRiemannianMetric I M)
  ...
  have hhomEq : EqOn (fun u ↦ H u) (diagExp (I := I) g hEnorm) H.source := by
    ...
    rw [hleft'] at hright
    exact hright.symm
  have hinvSmooth : ContMDiffOn (I.prod I) I.tangent ∞ H.symm H.target := by
    rw [hHinv]
  ...
  let B : DiagonalInverseBranch (I := I) g hEnorm p :=
    { hom := H
      zero_mem := hzeroH
      hom_eq := hhomEq
      inv_contMDiffOn := hinvSmooth }
  refine ⟨B, ?_⟩
  exact hHinv

attribute [-instance] Tensor0SBundle.tangentSpaceNormedAddCommGroup
  Tensor0SBundle.tangentSpaceNormedSpace in
noncomputable def standardDiagonalInverseBranch
    [PseudoEMetricSpace M] [IsRiemannianManifold I M] [CompleteSpace M]
    [T2Space (TangentBundle I M)]
    [IsContinuousRiemannianBundle E (fun (x : M) ↦ TangentSpace I x)]
    (g : SmoothRiemannianMetric I M)
  ...
  Classical.choose (exists_standardDiagonalInverseBranch (I := I) g hEnorm p)

attribute [-instance] Tensor0SBundle.tangentSpaceNormedAddCommGroup
  Tensor0SBundle.tangentSpaceNormedSpace in
theorem standardDiagonalInverseBranch_inv
    [PseudoEMetricSpace M] [IsRiemannianManifold I M] [CompleteSpace M]
    [T2Space (TangentBundle I M)]
    [IsContinuousRiemannianBundle E (fun (x : M) ↦ TangentSpace I x)]
    (g : SmoothRiemannianMetric I M)
    (hEnorm : IsMetricNorm (I := I) (M := M) g)
    (p : M) :
    (standardDiagonalInverseBranch (I := I) g hEnorm p).inv =
      diagExpInv (I := I) g hEnorm p :=
  Classical.choose_spec (exists_standardDiagonalInverseBranch (I := I) g hEnorm p)

end DiagonalExponentialLocalInverse
