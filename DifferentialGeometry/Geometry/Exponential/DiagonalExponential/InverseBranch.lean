...
structure DiagonalInverseBranch
  ...
  inv_contMDiffOn : ContMDiffOn (I.prod I) I.tangent ∞ hom.symm hom.target
...
theorem inv_vector_contMDiffOn_at_fixed_target
    {g : SmoothRiemannianMetric I M}
    {hEnorm : ∀ (x : M) (w : TangentSpace I x),
      ‖w‖₊ = ENNReal.ofReal (Real.sqrt (g.inner x w w))}
    {p pt : M} (B : DiagonalInverseBranch (I := I) g hEnorm p) {S : Set M}
    (hdom : ∀ y ∈ S, (y, pt) ∈ B.dom) :
  ...
  have hinv : ContMDiffOn I I.tangent ∞
    ...
    have hcomp := B.inv_contMDiffOn.comp hpair hdom
...
theorem inv_contMDiffOn_at_fixed_base
    {g : SmoothRiemannianMetric I M}
    {hEnorm : IsMetricNorm (I := I) (M := M) g}
    {p pt : M} (B : DiagonalInverseBranch (I := I) g hEnorm p) {S : Set M}
    (hdom : ∀ y ∈ S, (pt, y) ∈ B.dom) :
...
theorem inv_fiberCoordinates_contMDiffOn_at_fixed_base
    {g : SmoothRiemannianMetric I M}
    {hEnorm : IsMetricNorm (I := I) (M := M) g}
    {c p : M} (B : DiagonalInverseBranch (I := I) g hEnorm c) {S : Set M}
    (hdom : ∀ z ∈ S, (p, z) ∈ B.dom) :
    ContMDiffOn I 𝓘(Real, E) ∞
      (fun z : M => ((B.inv (p, z)).snd : E)) S := by
  have hinv : ContMDiffOn I I.tangent ∞
      (fun z : M => B.inv (p, z)) S :=
    B.inv_contMDiffOn_at_fixed_base hdom
  have hmaps :
...
noncomputable def fixed
    ...
    ExponentialInverseBranch (I := I) g hEnorm p := by
  classical
  ...
  let Φ : PartialDiffeomorph 𝓘(Real, E) I E M ∞ :=
    { toFun := fun u : E =>
      ...
      contMDiffOn_invFun :=
        B.inv_fiberCoordinates_contMDiffOn_at_fixed_base (S := {y : M | (p, y) ∈ B.dom})
          (fun y hy => hy) }
  exact ⟨Φ, fun _ _ => rfl⟩
...
@[simp] theorem fixed_source
    {g : SmoothRiemannianMetric I M}
    {hEnorm : IsMetricNorm (I := I) (M := M) g}
    {c p : M} (B : DiagonalInverseBranch (I := I) g hEnorm c) (u : E) :
...
@[simp] theorem fixed_target
    {g : SmoothRiemannianMetric I M}
    {hEnorm : IsMetricNorm (I := I) (M := M) g}
    {c p : M} (B : DiagonalInverseBranch (I := I) g hEnorm c) (y : M) :
...
theorem inv_eq_normal_lt
    ...
    {g : SmoothRiemannianMetric I M}
    {hEnorm : IsMetricNorm (I := I) (M := M) g}
    {p y q : M} (B : DiagonalInverseBranch (I := I) g hEnorm p)
    (hy : (y, q) ∈ B.dom)
    (hsmall : Real.sqrt
