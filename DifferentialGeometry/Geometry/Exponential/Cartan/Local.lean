...
theorem cartanMap_smooth
  ...
  have hinv : ContMDiffOn I 𝓘(ℝ, E) ∞ invf U := by
    simpa only [U, invf] using
      B.inv_fiberCoordinates_contMDiffOn_at_fixed_base (S := U) (fun x hx => hx)
...
omit [CompleteSpace E] [T2Space (TangentBundle I M)]
  [T2Space (TangentBundle I' M')] [ConnectedSpace M] [ConnectedSpace M'] in
theorem cartanPartialDiffeomorph_coe
    {g : SmoothRiemannianMetric I M}
    {hEnorm : ∀ (x : M) (w : TangentSpace I x),
      ‖w‖ₑ = ENNReal.ofReal (Real.sqrt (g.inner x w w))}
    {p : M} (B : DiagonalInverseBranch (I := I) g hEnorm p)
    ...
    {p' : M'} (B' : DiagonalInverseBranch (I := I') g' hEnorm' p')
    (i : E ≃L[ℝ] E) :
    (cartanPartialDiffeomorph B B' i : M → M') =
      cartanMap B p g' hEnorm' p' i :=
  rfl

omit [CompleteSpace E] [T2Space (TangentBundle I M)]
  [T2Space (TangentBundle I' M')] [ConnectedSpace M] [ConnectedSpace M'] in
theorem cartanPartialDiffeomorph_center_mem_source
    {g : SmoothRiemannianMetric I M}
    {hEnorm : ∀ (x : M) (w : TangentSpace I x),
      ‖w‖ₑ = ENNReal.ofReal (Real.sqrt (g.inner x w w))}
    {p : M} (B : DiagonalInverseBranch (I := I) g hEnorm p)
    ...
    {p' : M'} (B' : DiagonalInverseBranch (I := I') g' hEnorm' p')
    (i : E ≃L[ℝ] E) :
    p ∈ (cartanPartialDiffeomorph B B' i).source := by
  change
