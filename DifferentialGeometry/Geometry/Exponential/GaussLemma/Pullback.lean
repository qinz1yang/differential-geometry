...
private lemma exists_metricCoerciveConst (g : SmoothRiemannianMetric I M) (p : M) :
    ∃ c : ℝ, 0 < c ∧
  ...
  let : Nontrivial E := Module.nontrivial_of_finrank_pos hfin_pos
  ...
  · intro d hd
    ...
    have hdu₀ : d ≤ B u₀ u₀ := by
      have hdu := hd u₀
      rw [hu₀_norm, one_pow, mul_one] at hdu
      exact hdu
    simpa only [hc_def, hQ] using hdu₀

def metricCoerciveConst (g : SmoothRiemannianMetric I M) (p : M) : ℝ :=
  Classical.choose (exists_metricCoerciveConst (I := I) g p)

omit [I.Boundaryless] [CompleteSpace E] [T2Space (TangentBundle I M)] in
lemma metricCoerciveConst_pos (g : SmoothRiemannianMetric I M) (p : M) :
    0 < metricCoerciveConst (I := I) g p :=
  (Classical.choose_spec (exists_metricCoerciveConst (I := I) g p)).1

omit [I.Boundaryless] [CompleteSpace E] [T2Space (TangentBundle I M)] in
lemma metricCoerciveConst_le (g : SmoothRiemannianMetric I M) (p : M) (x : E) :
    metricCoerciveConst (I := I) g p * ‖x‖ ^ 2 ≤ g.inner p x x :=
  (Classical.choose_spec (exists_metricCoerciveConst (I := I) g p)).2.1 x

omit [I.Boundaryless] [CompleteSpace E] [T2Space (TangentBundle I M)] in
lemma le_metricCoerciveConst (g : SmoothRiemannianMetric I M) (p : M) {c : ℝ}
    (hc : ∀ x : E, c * ‖x‖ ^ 2 ≤ g.inner p x x) :
    c ≤ metricCoerciveConst (I := I) g p :=
  (Classical.choose_spec (exists_metricCoerciveConst (I := I) g p)).2.2 hc
...
lemma metricCoerciveExpRadius_pos (g : SmoothRiemannianMetric I M) (p : M) :
...
lemma norm_lt_exp_div
    ...
    (hn : 0 < n)
    (hx : Real.sqrt (g.inner p x x) < metricCoerciveExpRadius (I := I) g p / n) :
    ‖x‖ < expMapC2Radius (I := I) g p / n := by
  have hc_pos : 0 < metricCoerciveConst (I := I) g p :=
    metricCoerciveConst_pos (I := I) g p
  have hsq :
      g.inner p x x < (metricCoerciveExpRadius (I := I) g p / n) ^ 2 :=
    Real.lt_sq_of_sqrt_lt hx
  have hR : (metricCoerciveExpRadius (I := I) g p / n) ^ 2 =
      metricCoerciveConst (I := I) g p *
        (expMapC2Radius (I := I) g p / n) ^ 2 := by
    rw [metricCoerciveExpRadius, div_pow, mul_pow, Real.sq_sqrt hc_pos.le, div_pow]
    ring
  rw [hR] at hsq
  have hcoerc : metricCoerciveConst (I := I) g p * ‖x‖ ^ 2 ≤
      g.inner p x x := metricCoerciveConst_le (I := I) g p x
  have hlt : metricCoerciveConst (I := I) g p * ‖x‖ ^ 2 <
      metricCoerciveConst (I := I) g p *
...
private lemma velocityChartRep_differentiableAt_of_contMDiffAt2
  ...
  unfold chartRepAt
  exact MFDerivAlongCurve.velocity_coord_diff (I := I) γ t₀ hγC2
...
private def expMapModel (g : SmoothRiemannianMetric I M) (p : M) (b : E) : M :=
  expMap (I := I) g p ((tangentSpaceModelContinuousLinearEquiv (I := I) p).symm b)
...
lemma radialCurve_launch_velocity
  ...
  have hexp_mdiff : MDifferentiableAt 𝓘(ℝ, E) I
      (expMapModel (I := I) g p) ((fun u : ℝ => u • a) 0) := by
    rw [hexpModel]
  ...
  have hcomp : (fun u : ℝ => (expMap (I := I) g p (show TangentSpace I p from (u • a)) : M))
      = expMapModel (I := I) g p ∘ (fun u : ℝ => u • a) := by
    funext u
    exact (congrFun hexpModel (u • a)).symm
  ...
  have hlaunch : mfderiv 𝓘(ℝ, ℝ) 𝓘(ℝ, E) (fun u : ℝ => u • a) 0 (1 : ℝ) = a := by
    ...
    have h : HasFDerivAt (fun u : ℝ => u • a)
      ...
      exact (hasFDerivAt_id (0 : ℝ)).smul_const a
    rw [h.fderiv]
    change (ContinuousLinearMap.smulRight (1 : ℝ →L[ℝ] ℝ) a) (1 : ℝ) = a
    rw [ContinuousLinearMap.smulRight_apply, one_apply_eq_self, one_smul]
  have hexp0 : mfderiv 𝓘(ℝ, E) I (expMapModel (I := I) g p) 0 =
      ContinuousLinearMap.id ℝ E := by
    rw [hexpModel]
    exact mfderiv_expMap_at_zero (I := I) g p
  have hexpModel0 : expMapModel (I := I) g p 0 = p := by
    change expMap (I := I) g p
      ((tangentSpaceModelContinuousLinearEquiv (I := I) p).symm 0) = p
    rw [map_zero, expMap_zero]
  rw [zero_smul, hexpModel0, hexp0]
  apply (tangentSpaceModelContinuousLinearEquiv (I := I) p).injective
  change mfderiv 𝓘(ℝ, ℝ) 𝓘(ℝ, E) (fun u : ℝ => u • a) 0 (1 : ℝ) = a
  exact hlaunch
...
private lemma launchSpeedSq_s_hasDerivAt
  ...
  have hd : HasDerivAt (fun s : ℝ => B v v + s * (B v w + B w v) + s ^ 2 * (B w w))
      (B v w + B w v) 0 := by
    have h := (hd1.add hd2).add hd3
    have hfun :
        (((fun _ : ℝ => B v v) + fun s : ℝ => s * (B v w + B w v)) +
            fun s : ℝ => s ^ 2 * (B w w)) =
          fun s : ℝ => B v v + s * (B v w + B w v) + s ^ 2 * (B w w) := by
      funext s
      rfl
  ...
  have hslice_v_ev : ∀ᶠ v' in nhds t₀, ContMDiffAt 𝓘(ℝ, ℝ) I 2 (fun u : ℝ => F u v') 0 := by
    ...
    have hbase : ContMDiffAt 𝓘(ℝ, ℝ) 𝓘(ℝ, E) 2
      ...
      have hMD : ContMDiff 𝓘(ℝ, ℝ) 𝓘(ℝ, E) ∞
        ...
        (contMDiff_const (I := 𝓘(ℝ, ℝ)) (I' := 𝓘(ℝ, ℝ)) (c := v')).smul
