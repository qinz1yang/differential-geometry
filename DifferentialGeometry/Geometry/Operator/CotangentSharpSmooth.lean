...
theorem cotangentSharp_gen_eq_metricSharp
    ...
    cotangentSharpGen (I := I) g x β =
      metricSharp (I := I) g x (cotangentToDualGen (I := I) β) := by
  apply metricFlatLinear_injective (I := I) g x
  ext w
  change g.inner x (cotangentSharpGen (I := I) g x β) w =
    g.inner x (metricSharp (I := I) g x (cotangentToDualGen (I := I) β)) w
  rw [cotangentSharp_inner_gen (I := I) g x β w,
    inner_metricSharp (I := I) g x (cotangentToDualGen (I := I) β) w]
...
theorem cotangentSharp_gen_contMDiff_total (g : SmoothRiemannianMetric I M)
    ...
    ContMDiff I (I.prod 𝓘(ℝ, E)) ∞
      (fun b : M =>
        TotalSpace.mk' E b (cotangentSharpGen (I := I) g b (β b))) := by
  ...
  have hsection_eq :
      (fun b : M =>
          TotalSpace.mk' E b (cotangentSharpGen (I := I) g b (β b))) =
        fun b : M =>
          TotalSpace.mk' E b (metricSharp (I := I) g b (cv b)) := by
    funext b
    change TotalSpace.mk' E b (cotangentSharpGen (I := I) g b (β b)) =
      TotalSpace.mk' E b (metricSharp (I := I) g b (cotangentToDualGen (I := I) (β b)))
