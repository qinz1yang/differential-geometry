...
theorem isMIntegralCurveAt_geodesicVectorFieldChart_eventuallyEq
    ...
    (hα_source : (f₁ t₀).proj ∈ (chartAt H α).source)
...
theorem isGeodesicAt_eventuallyEq
    ...
    {f₁ f₂ : ℝ → TangentBundle I M}
    (hα_source : γ₁ t₀ ∈ (chartAt H α).source)
    ...
    (h0 : f₁ t₀ = f₂ t₀) :
    γ₁ =ᶠ[𝓝 t₀] γ₂ := by
  have hα_source' : (f₁ t₀).proj ∈ (chartAt H α).source := by
    rw [hproj₁ t₀]; exact hα_source
  have hproj_eq :=
