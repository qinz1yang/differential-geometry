...
theorem exists_partial_diffeomorph_of_is_local_diffeomorph_on_inj_on [Nonempty M]
...
theorem exists_exponential_ball_partial_diffeomorph_of_lt
    ...
    ∃ Φ : PartialDiffeomorph 𝓘(ℝ, E) I E M 1,
      ...
      EqOn Φ (fun v : E => (expMap (I := I) g p (show TangentSpace I p from v) : M))
        (Metric.ball (0 : E) r) :=
  exists_exponential_ball_partial_diffeomorph (I := I) g p hrinj
    (exponential_map_is_local_diffeomorph_on_ball (I := I) g p hrC2)
...
theorem framed_exponential_map_is_local_diffeomorph_on_ball
    (g : SmoothRiemannianMetric I M) (p : M) {r : ℝ}
    (hr : r ≤ metricCoerciveExpRadius (I := I) g p) :
    IsLocalDiffeomorphOn 𝓘(ℝ, E) I 1
      (framedExpMap (I := I) g p)
      (Metric.ball (0 : E) r) := by
  have : IsManifold I 1 M := by
  ...
  rintro ⟨x, hx⟩
  rw [Metric.mem_ball, dist_zero_right] at hx
  have hxGp : ‖x‖ < metricCoerciveExpRadius (I := I) g p := lt_of_lt_of_le hx hr
...
theorem exists_framed_exponential_ball_partial_diffeomorph_of_lt
    (g : SmoothRiemannianMetric I M) (p : M) {r : ℝ}
    (hrinj : ENNReal.ofReal r < framedInjRadius (I := I) g p)
    (hrC2 : r ≤ metricCoerciveExpRadius (I := I) g p) :
    ∃ Φ : PartialDiffeomorph 𝓘(ℝ, E) I E M 1,
      Φ.source = Metric.ball (0 : E) r ∧
      Φ.target = framedExpMap (I := I) g p '' Metric.ball (0 : E) r ∧
      EqOn Φ (framedExpMap (I := I) g p)
        (Metric.ball (0 : E) r) :=
  exists_framed_exponential_ball_partial_diffeomorph (I := I) g p hrinj
    (framed_exponential_map_is_local_diffeomorph_on_ball (I := I) g p hrC2)
