...
theorem transDens_eq_rigid
    ...
    (heq : curveDensity (I := I) g (intrinsicGeodesic (I := I) g hEnorm x v)
      ...
      hyperbolicDensity (q * Real.sqrt (g.inner x v v)) (Module.finrank ℝ E - 1) 1) :
    ...
    (∀ t ∈ Set.Ioo (0 : ℝ) 1,
      curveDensity (I := I) g γ V t / hyperbolicDensity (q * ℓ) d t = 1) ∧
    ∀ t ∈ Set.Ioo (0 : ℝ) 1,
  ...
  let d := Module.finrank ℝ E - 1
  let ℓ := Real.sqrt (g.inner x v v)
  let R := fun t => curveDensity (I := I) g γ V t / hyperbolicDensity (q * ℓ) d t
  ...
  have hanti : AntitoneOn R (Set.Ioo (0 : ℝ) 1) := by
    simpa only [R, γ, V, d, ℓ] using intrinsicRatioOfFrame (I := I) g hEnorm x v q 1
  ...
  have hden₁ : 0 < hyperbolicDensity (q * ℓ) d 1 := hyperbolicDensity_pos hqℓ (by norm_num)
  have hR₁ : R 1 = 1 := by
    simp only [R, γ, V, d, ℓ]
  ...
  have hlim₁ : Tendsto R (𝓝[<] (1 : ℝ)) (𝓝 1) := by
    have hc : ContinuousAt R 1 :=
      (curveDensity_jacobiFrame_continuousAt (I := I) g hEnorm x v w 1).div
        (hyperbolicDen_continuous (q * ℓ) d).continuousAt hden₁.ne'
  ...
  have hVdiff (t : ℝ) (i : Fin d) :=
    (intrinsicJacobi_diff (I := I) g hEnorm x v (w i) t).1
  have hDVdiff (t : ℝ) (i : Fin d) :=
    (intrinsicJacobi_diff (I := I) g hEnorm x v (w i) t).2
  have hVperp (t : ℝ) (ht : t ≠ 0) (i : Fin d) :
      g.inner (γ t) (curveVelocity (I := I) γ t) (V i t) = 0 := by
    simpa only [γ, V] using intrinsicJacobi_perp_ne (I := I) g hEnorm x v (w i) ht (hperp i)
  have hDVperp (t : ℝ) (ht : t ≠ 0) (i : Fin d) :
      g.inner (γ t) (curveVelocity (I := I) γ t)
        (covDerivAlong (I := I) g γ (V i) t) = 0 := by
    simpa only [γ, V] using intrinsicJacobi_dperp (I := I) g hEnorm x v (w i) ht (hperp i)
  ...
  have hmean : ∀ t ∈ Set.Ioo (0 : ℝ) 1,
      curveMean (I := I) g γ V t = hyperbolicMeanCurv (q * ℓ) d t := by
    intro t ht
    ...
    change R t * (curveMean (I := I) g γ V t - hyperbolicMeanCurv (q * ℓ) d t) = 0 at hz
  ...
  have hlocal : (curveMean (I := I) g γ V) =ᶠ[𝓝 t] hyperbolicMeanCurv (q * ℓ) d := by
    filter_upwards [Ioo_mem_nhds ht.1 ht.2] with s hs
    exact hmean s hs
  have heqDer := hm.1.unique
    ((hasDerivAt_hyperbolicMean hqℓ ht.1 hd).congr_of_eventuallyEq hlocal)
