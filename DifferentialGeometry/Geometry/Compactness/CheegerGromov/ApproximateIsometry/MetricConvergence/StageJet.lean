...
theorem HasStageJetConvergence.coeff_tail
  ...
  have hshape := hdata
  dsimp only [HasSupportedCenterMapConvergence] at hshape
  rcases hshape with
  ...
  have hjetAll : ∀ alpha : LiveSlot L inp.pack r,
      HasStageJetTail (I := I) inp P L hr phi hphi C0 R 1
...
theorem HasStageJetConvergence.chart_convergence
    ...
    MapCInfConvergenceOnCompacts V
      (fun n z ↦
        ...
        chiL (stageComparisonMap inp P Lphi r hr (kn n) (ln n)
          (chiK.symm z))) id := by
