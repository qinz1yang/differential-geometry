  ...
  have mkPre : ∀ (K' : Set M') (Φcoe : M' → M')
    ...
    · intro a ha1 _ x _
      ...
      change Real.sqrt (Tensor0SBundle.normSq0S (I := I) g x (a' + 1 + 2)
        ...
        ≤ 0
      rw [tensor02CovDeriv_metric_eq_zero]
      change Real.sqrt (Tensor0SBundle.normSq0S (I := I) g x (a' + 1 + 2)
...
theorem partial_diffeomorph_metric_approximation_refl
    ...
    Nonempty (PartialDiffeomorphMetricApproximation (I := I) K ε p
      (PartialDiffeomorph.refl (I := I) M') g g) := by
  obtain ⟨D⟩ := partial_diffeomorph_metric_approximation_bounds_refl (I := I) K g p
  exact ⟨D.toMetricApproximation hε hε1 hε.le hε.le⟩
