...
theorem trans_bounds_on
  ...
  · intro k
    ...
    have hovl' :
      ...
      simpa only [BoundedGeometryNormalChartData.chartOverlapOn] using hovl k
    have hUrad :
        U ⊆ Metric.ball (0 : E) (d.chart k (x k)).radius :=
      fun z hz => (hovl' z hz).1
    simpa only [BoundedGeometryNormalChartData.chartMetric] using
      (d.chart k (x k)).metric_cont_diff_on (X.obj k).metric hU
  ...
  · intro k
    ...
    simpa only [BoundedGeometryNormalChartData.chartMetric] using
      (d.chart k (y k)).metric_cont_diff_on (X.obj k).metric hV
  ...
  · intro k z hz u v
    ...
    have hovl' :
        (d.chart k (x k)).OverlapOn (d.chart k (y k)) U := by
      simpa only [BoundedGeometryNormalChartData.chartOverlapOn] using hovl k
    simpa only [BoundedGeometryNormalChartData.chartMetric, BoundedGeometryNormalChartData.chartTransition] using
...
theorem exists_transition_limit_subsequence
  ...
  have hJ : ∀ k, ContDiffOn Real (⊤ : ℕ∞)
    ...
    have hovl' :
        (d.chart k (x k)).OverlapOn (d.chart k (y k)) U := by
      simpa only [BoundedGeometryNormalChartData.chartOverlapOn] using hovlJ k
    simpa only [BoundedGeometryNormalChartData.chartTransition] using
      (d.chart k (x k)).transition_smooth (d.chart k (y k)) hovl'
