...
def phaseRadius
    ...
    (d : BoundedGeometryNormalChartData (I := I) X hd) (R : Real) : Real :=
  d.ratio * hd.mu R / 4
...
def phaseK
    {X : PointedRiemannianSeq.{u, uE, uH} (I := I)}
    {hd : InjectivityRadiusDecay (I := I) X}
    (d : BoundedGeometryNormalChartData (I := I) X hd) (R : NNReal) : NNReal where
...
theorem phaseErr_lt_ev
  ...
  with_unfolding_all
    exact chartPhaseErr_lt_ev (I := I) Y.metric b₀ heps
