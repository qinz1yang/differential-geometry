...
theorem uniformTwoIdRegularityOn {s : Set M} {USeq : ℕ → ℕ → ι → Set M}
    ...
    (hpts :
      ...
      ∀ i : ι, ∀ ε > 0, ∃ N : ℕ, ∀ k ≥ N, ∀ l ≥ N, ∀ x : M, x ∈ s →
        x ∈ USeq k l i → dist x (pointsSeq k l x i) < ε) :
    letI : RiemannianBundle (fun x : M => TangentSpace I x) :=
      ⟨g.toRiemannianMetric⟩
    letI : IsContinuousRiemannianBundle E (fun x : M => TangentSpace I x) :=
      ⟨g.inner, g.contMDiff.continuous, fun _ _ _ => rfl⟩
  ...
  refine uniformTwoIdOfFillOn (I := I) (g := g) (join := join) hcomplete hr hqstar
...
theorem uniformTwoIdDataOn {s : Set M} {USeq : ℕ → ℕ → ι → Set M}
    {μSeq : ℕ → ℕ → M → ι → ℝ} {pointsSeq : ℕ → ℕ → M → ι → M}
    {pSeq : ℕ → ℕ → M → M} {rSeq : ℕ → ℕ → M → ℝ}
    (hcomplete :
      letI : RiemannianBundle (fun x : M => TangentSpace I x) :=
...
theorem uniformTwoIdDataSelf {s : Set M} {USeq : ℕ → ℕ → ι → Set M}
    ...
    (hactive_mem :
      ...
      letI : MetricSpace M := HopfRinow.riemMetricSpace (I := I) (M := M)
      ∀ k l : ℕ, ∀ x : M, x ∈ s → ∀ i : ι, μSeq k l x i ≠ 0 →
        dist x (pointsSeq k l x i) < rSeq k l x)
    (hdata : ∀ k l : ℕ, ∀ x : M, x ∈ s →
      ((∀ i : ι, 0 ≤ μSeq k l x i) ∧
        (∃ i : ι, 0 < μSeq k l x i) ∧
          ∑ i : ι, μSeq k l x i = 1) ∧
      ...
      dist x
        (centerAverageOn (I := I) g s (μSeq k l)
          ...
          (fun x hx => inputOfFillSelf (I := I) (g := g) (μ := μSeq k l)
            (points := pointsSeq k l) (join := join) (r := rSeq k l)
...
theorem equation_local (x : X)
    (hdiffSummands :
      ...
      letI : IsContinuousRiemannianBundle E (fun x : M => TangentSpace I x) :=
        ⟨g.inner, g.contMDiff.continuous, fun _ _ _ => rfl⟩
      letI : MetricSpace M := HopfRinow.riemMetricSpace (I := I) (M := M)
      ∀ i : ι, MDifferentiableAt I 𝓘(ℝ, ℝ) (CenterOfMass.halfSqDist (points x i))
        (centerAverage (I := I) g μ points join p r h x))
    (hsrc :
      letI : RiemannianBundle (fun x : M => TangentSpace I x) :=
        ⟨g.toRiemannianMetric⟩
      letI : IsContinuousRiemannianBundle E (fun x : M => TangentSpace I x) :=
      ...
      ∀ i : ι, points x i ∈
        (NormalCoordinates.normalChartAt (I := I) g
          (centerAverage (I := I) g μ points join p r h x)).source) :
    letI : RiemannianBundle (fun x : M => TangentSpace I x) :=
      ⟨g.toRiemannianMetric⟩
    letI : IsContinuousRiemannianBundle E (fun x : M => TangentSpace I x) :=
      ⟨g.inner, g.contMDiff.continuous, fun _ _ _ => rfl⟩
    letI : MetricSpace M := HopfRinow.riemMetricSpace (I := I) (M := M)
    ∃ ρ : ℝ, 0 < ρ ∧
      ((∀ i : ι, points x i ≠ centerAverage (I := I) g μ points join p r h x →
        Real.sqrt
          (g.inner (centerAverage (I := I) g μ points join p r h x)
            (NormalCoordinates.normalChartAt (I := I) g
              (centerAverage (I := I) g μ points join p r h x) (points x i) : E)
            (NormalCoordinates.normalChartAt (I := I) g
              (centerAverage (I := I) g μ points join p r h x) (points x i) : E)) < ρ) →
        ∑ i : ι, μ x i •
          (show TangentSpace I (centerAverage (I := I) g μ points join p r h x) from
            NormalCoordinates.normalChartAt (I := I) g
              (centerAverage (I := I) g μ points join p r h x) (points x i)) = 0) := by
  unfold centerAverage
  exact centerOfMass.expInv_equation_local (I := I) (g := g) (μ := μ x) (points := points x)
...
theorem equation_local_on {s : Set X} {qstar : X → M}
    ...
    (hsrc :
      ...
      ∀ i : ι, points x i ∈
        (NormalCoordinates.normalChartAt (I := I) g
          (centerAverageOn (I := I) g s μ points join p r qstar hOn x)).source) :
    letI : RiemannianBundle (fun x : M => TangentSpace I x) :=
      ⟨g.toRiemannianMetric⟩
    letI : IsContinuousRiemannianBundle E (fun x : M => TangentSpace I x) :=
      ⟨g.inner, g.contMDiff.continuous, fun _ _ _ => rfl⟩
    ...
    ∃ ρ : ℝ, 0 < ρ ∧
      ((∀ i : ι,
        points x i ≠ centerAverageOn (I := I) g s μ points join p r qstar hOn x →
        Real.sqrt
          (g.inner (centerAverageOn (I := I) g s μ points join p r qstar hOn x)
            (NormalCoordinates.normalChartAt (I := I) g
              (centerAverageOn (I := I) g s μ points join p r qstar hOn x)
              (points x i) : E)
            (NormalCoordinates.normalChartAt (I := I) g
        ...
        ∑ i : ι, μ x i •
          (show TangentSpace I
              (centerAverageOn (I := I) g s μ points join p r qstar hOn x) from
            NormalCoordinates.normalChartAt (I := I) g
              (centerAverageOn (I := I) g s μ points join p r qstar hOn x)
              (points x i)) = 0) := by
  let : RiemannianBundle (fun x : M => TangentSpace I x) :=
  ...
  have hcm :
      centerAverageOn (I := I) g s μ points join p r qstar hOn x =
        centerOfMass (I := I) g (μ x) (points x) join (p x) (r x) (hOn x hx) :=
    on_eq (I := I) (g := g) (μ := μ) (points := points) (join := join) (p := p)
      (r := r) hOn hx
  have hdiff :
      ∀ i : ι, MDifferentiableAt I 𝓘(ℝ, ℝ) (CenterOfMass.halfSqDist (points x i))
  ...
  have hsrc' :
      ∀ i : ι, points x i ∈
        (NormalCoordinates.normalChartAt (I := I) g
          (centerOfMass (I := I) g (μ x) (points x) join (p x) (r x)
            (hOn x hx))).source := by
    intro i
