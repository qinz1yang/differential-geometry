...
private theorem chartGramMatrix_mul_invGramPullbackCandidate [I.Boundaryless]
  ...
  have hGram : ∀ k m : Fin (Module.finrank ℝ E),
      DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g α p k m =
        ∑ a, ∑ b, J a k * J b m *
          DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g β p a b := by
    intro k m
    ...
    have hLHS : chartGramOnE (I := I) g α k m x = DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g α p k m := by
  ...
  have hGβinvR : ∀ a d : Fin (Module.finrank ℝ E),
      ∑ b, DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g β p a b * Gβinv b d =
    ...
    rw [hGβinv_def]
    rw [show (∑ b, DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g β p a b *
  ...
  calc ∑ m, DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g α p k m *
    ...
    _ = ∑ a, ∑ b, ∑ j,
          (J a k * DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g β p a b) * (K l j * Gβinv b j) := by
        refine Finset.sum_congr rfl ?_; intro a _
        refine Finset.sum_congr rfl ?_; intro b _
        rw [Finset.sum_comm]
        refine Finset.sum_congr rfl ?_; intro j _
        rw [show (∑ i, (J a k * DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g β p a b) *
    ...
    _ = ∑ a, J a k * K l a := by
        ...
        have hstep : ∀ j : Fin (Module.finrank ℝ E),
            (∑ b, (J a k * DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g β p a b) * (K l j * Gβinv b j)) =
              (if a = j then J a k * K l j else 0) := by
          intro j
          have : (∑ b, (J a k * DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g β p a b) *
                (K l j * Gβinv b j)) =
              J a k * K l j *
                (∑ b, DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g β p a b * Gβinv b j) := by
            rw [Finset.mul_sum]
            refine Finset.sum_congr rfl ?_; intro b _
            ring
          rw [this, hGβinvR a j]
...
lemma partialDeriv_mul (k : Fin (Module.finrank ℝ E)) (u v : E → ℝ) {y : E}
    ...
    DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) k (fun z => u z * v z) y =
      DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) k u y * v y + u y * DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) k v y := by
  unfold DifferentialGeometry.Tensor.Coordinates.partialDeriv
...
lemma partialDeriv_finset_sum {ι : Type*} (s : Finset ι)
    ...
    DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) k (fun z => ∑ i ∈ s, f i z) y =
      ∑ i ∈ s, DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) k (f i) y := by
  unfold DifferentialGeometry.Tensor.Coordinates.partialDeriv
...
lemma partialDeriv_double_finset_sum {ι κ : Type*} (s : Finset ι) (t : Finset κ)
    ...
    DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) k (fun z => ∑ a ∈ s, ∑ b ∈ t, f a b z) y =
      ∑ a ∈ s, ∑ b ∈ t, DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) k (f a b) y := by
...
theorem partialDeriv_chartGramOnE_eq_sum_chartTransition [I.Boundaryless]
  ...
  have h_pd_eq : DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) k (chartGramOnE (I := I) g α i j) y =
      DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) k
        (fun z => ∑ a : Fin (Module.finrank ℝ E),
          ∑ b : Fin (Module.finrank ℝ E),
            chartTransitionJacEntry (I := I) α β z a i *
  ...
  have hexp : chartTransitionAt (I := I) α β x v =
      ∑ i : Fin (Module.finrank ℝ E),
        (DifferentialGeometry.Tensor.Coordinates.chartModelBasis E).repr v i •
          chartTransitionAt (I := I) α β x ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E) i) := by
...
lemma partialDeriv_chartTransitionJacEntry_swap [I.Boundaryless]
  ...
  rw [hsymm ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E) i) ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E) l)]
...
lemma chartInvGramBeta_mul_chartGramOnE_diag
  ...
  have hG : ∀ d : Fin (Module.finrank ℝ E),
      chartGramOnE (I := I) g β d b
          (chartTransitionMap (I := I) α β (extChartAt I α p)) =
        DifferentialGeometry.Tensor.Coordinates.chartGramMatrix (I := I) g β p d b := by
    intro d
...
lemma chartChristoffelNumerator_transitionIdentity [I.Boundaryless]
  ...
  set dGb : Fin (Module.finrank ℝ E) → Fin (Module.finrank ℝ E) →
      Fin (Module.finrank ℝ E) → ℝ :=
    fun t a b => DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) t (chartGramOnE (I := I) g β a b) Tx with hdGb
  ...
  have hTerm1 :
      (∑ l, K l d * DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) i (chartGramOnE (I := I) g α l j) x) =
        A1 + BB + C1 := by
    have hstep : ∀ l, K l d * DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) i (chartGramOnE (I := I) g α l j) x =
    ...
    · rw [hC1]
      ...
      rw [Finset.sum_comm]
  have hTerm2 :
      (∑ l, K l d * DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) j (chartGramOnE (I := I) g α l i) x) =
        A2 + BB + C2 := by
    have hstep : ∀ l, K l d * DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) j (chartGramOnE (I := I) g α l i) x =
    ...
    · rw [hC2]
      ...
      ring
  have hTerm3 :
      (∑ l, K l d * DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) l (chartGramOnE (I := I) g α i j) x) =
        A1 + A2 + C3 := by
    have hstep : ∀ l, K l d * DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) l (chartGramOnE (I := I) g α i j) x =
  ...
  have hSigma :
    ...
    rw [show (∑ l, K l d *
        ...
        (∑ l, K l d * DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) i (chartGramOnE (I := I) g α l j) x)
          + (∑ l, K l d * DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) j (chartGramOnE (I := I) g α l i) x)
          - (∑ l, K l d * DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) l (chartGramOnE (I := I) g α i j) x) from by
  ...
  rw [show (∑ l, chartTransitionJacEntry (I := I) β α Tx l d *
      ...
      (∑ l, K l d *
        (DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) i (chartGramOnE (I := I) g α l j) x +
          DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) j (chartGramOnE (I := I) g α l i) x -
          DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) l (chartGramOnE (I := I) g α i j) x)) from rfl]
  rw [hSigma]
  have hRHS_S :
...
theorem chartChristoffel_transform [I.Boundaryless]
  ...
  set S : Fin (Module.finrank ℝ E) → ℝ :=
    fun l => DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) i (chartGramOnE (I := I) g α l j) x +
      DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) j (chartGramOnE (I := I) g α l i) x -
      DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) l (chartGramOnE (I := I) g α i j) x with hS
  set Sbar : Fin (Module.finrank ℝ E) → Fin (Module.finrank ℝ E) →
      Fin (Module.finrank ℝ E) → ℝ :=
    fun a b d => DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) a (chartGramOnE (I := I) g β d b) Tx +
      DifferentialGeometry.Tensor.Coordinates.partialDeriv (E := E) b (chartGramOnE (I := I) g β d a) Tx -
...
lemma chartCoord_chartChristoffelContraction
  ...
  have hbasis : ∀ k : Fin (Module.finrank ℝ E),
      (DifferentialGeometry.Tensor.Coordinates.chartModelBasis E).repr ((DifferentialGeometry.Tensor.Coordinates.chartModelBasis E) k) l = if k = l then 1 else 0 :=
    fun k => (DifferentialGeometry.Tensor.Coordinates.chartModelBasis E).repr_self_apply k l
  calc
    ∑ k : Fin (Module.finrank ℝ E),
...
theorem chartChristoffelContraction_transform [I.Boundaryless]
  ...
  refine (DifferentialGeometry.Tensor.Coordinates.chartModelBasis E).ext_elem (fun l => ?_)
...
lemma fderiv_chartTransitionAt_apply_eq_pushCorrection [I.Boundaryless]
  ...
  classical
  refine (DifferentialGeometry.Tensor.Coordinates.chartModelBasis E).ext_elem (fun a => ?_)
