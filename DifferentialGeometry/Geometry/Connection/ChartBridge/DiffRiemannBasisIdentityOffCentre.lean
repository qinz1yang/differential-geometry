...
private lemma nablaCurvSec_chartBasisVec_alpha_leadingTerm
    ...
    (hXp_eq : ∀ y ∈ U, Xp y = DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α p y)
    (hXq_eq : ∀ y ∈ U, Xq y = DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α q y)
    (hXr_eq : ∀ y ∈ U, Xr y = DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α r y)
    (hXs_eq : ∀ y ∈ U, Xs y = DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α s y) :
  ...
  have hXp_x : Xp x = DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α p x := hXp_eq x hxU
  ...
  set term : Fin (Module.finrank ℝ E) → Π y : M, TangentSpace I y :=
    fun m y => Rc m y • DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α m y with hterm_def
  ...
  have hsum_apply :
      (cov.toFun (fun y : M => ∑ m : Fin (Module.finrank ℝ E), term m y) x)
        ...
        ∑ m : Fin (Module.finrank ℝ E),
          (cov.toFun (term m) x) (DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α p x) := by
    have hfun :
    ...
    exact leviCivita_finset_sum_apply (I := I) g
      ...
      (DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α p x) hterm_diff
  rw [hsum_apply]
  have hleib : ∀ m : Fin (Module.finrank ℝ E),
      (cov.toFun (term m) x) (DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α p x) =
    ...
    have hleibniz := cov.isCovariantDerivativeOnUniv.leibniz
      (σ := fun y : M => DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α m y) (g := Rc m) (x := x)
      (hframe_diff m) (hRc_diff m)
    have hterm_eq : term m = (Rc m) • (fun y : M => DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α m y) := by
      funext y; rfl
    rw [hterm_eq]
    have happ := congr($(hleibniz) (DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α p x))
...
private lemma riemannOp_chartFrame_triple
    ...
    riemannOp (cov := LeviCivita (I := I) g) x
        (DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α m' x) (DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α r x)
...
private lemma covApply_chartFrame_value
  ...
  have hchart_q_diff : MDiffAt (T% (fun z : M => DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α q z)) x :=
    chartBasisVec_alpha_mdifferentiableAt (I := I) α q hx
  have hXq_ev : (fun z : M => Xq z) =ᶠ[𝓝 x]
      (fun z : M => DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α q z) := by
    filter_upwards [hU_open.mem_nhds hxU] with z hz using hXq_eq z hz
  have hcov_congr :
      cov.toFun Xq x = cov.toFun (fun z : M => DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α q z) x :=
...
private lemma nablaCurvSec_chartBasisVec_alpha_corr_firstAntisym
  ...
  set er : TangentSpace I x := DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α r x with her_def
  set es : TangentSpace I x := DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α s x with hes_def
  have hexpand :
      riemannOp (cov := cov) x
          (∑ m : Fin (Module.finrank ℝ E),
            chartChristoffel (I := I) g α p q m (extChartAt I α x) •
              DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α m x) er es =
        ∑ m : Fin (Module.finrank ℝ E),
          chartChristoffel (I := I) g α p q m (extChartAt I α x) •
            riemannOp (cov := cov) x (DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α m x) er es := by
    rw [map_sum]
  ...
  have hper : ∀ m : Fin (Module.finrank ℝ E),
      chartChristoffel (I := I) g α p q m (extChartAt I α x) •
          riemannOp (cov := cov) x (DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α m x) er es =
        ∑ l : Fin (Module.finrank ℝ E),
          (chartChristoffel (I := I) g α p q m (extChartAt I α x) *
            chartRiemannTensor (I := I) g α s m r l (extChartAt I α x)) •
            DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α l x := by
...
private lemma nablaCurvSec_chartBasisVec_alpha_corr_secondAntisym
  ...
  rw [hXq_eq x hxU,
    ...
    hXs_eq x hxU]
  set eq' : TangentSpace I x := DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α q x with heq'_def
  set es : TangentSpace I x := DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α s x with hes_def
  have hexpand :
      riemannOp (cov := cov) x eq'
          (∑ m : Fin (Module.finrank ℝ E),
            chartChristoffel (I := I) g α p r m (extChartAt I α x) •
              DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α m x) es =
        ∑ m : Fin (Module.finrank ℝ E),
          chartChristoffel (I := I) g α p r m (extChartAt I α x) •
            riemannOp (cov := cov) x eq' (DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α m x) es := by
    rw [map_sum]
  ...
  have hper : ∀ m : Fin (Module.finrank ℝ E),
      chartChristoffel (I := I) g α p r m (extChartAt I α x) •
          riemannOp (cov := cov) x eq' (DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α m x) es =
        ∑ l : Fin (Module.finrank ℝ E),
          (chartChristoffel (I := I) g α p r m (extChartAt I α x) *
            chartRiemannTensor (I := I) g α s q m l (extChartAt I α x)) •
            DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α l x := by
...
private lemma nablaCurvSec_chartBasisVec_alpha_corr_acted
  ...
  have hexpand :
      riemannOp (cov := cov) x eq' er
        ...
        ∑ m : Fin (Module.finrank ℝ E),
          chartChristoffel (I := I) g α p s m (extChartAt I α x) •
            riemannOp (cov := cov) x eq' er (DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α m x) := by
    rw [map_sum]
    refine Finset.sum_congr rfl (fun m _ => ?_)
    rw [map_smul]
  rw [hexpand]
  have hper : ∀ m : Fin (Module.finrank ℝ E),
      chartChristoffel (I := I) g α p s m (extChartAt I α x) •
          riemannOp (cov := cov) x eq' er (DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α m x) =
        ∑ l : Fin (Module.finrank ℝ E),
          (chartChristoffel (I := I) g α p s m (extChartAt I α x) *
            chartRiemannTensor (I := I) g α m q r l (extChartAt I α x)) •
            DifferentialGeometry.Tensor.Coordinates.chartBasisVecFiber (I := I) α l x := by
