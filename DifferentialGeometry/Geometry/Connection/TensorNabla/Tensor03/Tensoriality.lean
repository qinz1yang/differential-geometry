...
local instance tensorialityTangentDualSMulCommClass (x : M) :
...
noncomputable local instance tensorialityTangentBilinearNormedSpace (x : M) :
  ...
  @ContinuousLinearMap.toNormedSpace ℝ ℝ (TangentSpace I x)
    (TangentSpace I x →L[ℝ] ℝ) _ _ _ _ _ _ (RingHom.id ℝ) _ ℝ _
      (tensorialityTangentDualNormedSpace (I := I) x)
        (tensorialityTangentDualSMulCommClass (I := I) x)

local instance tensorialityTangentBilinearSMulCommClass (x : M) :
    SMulCommClass ℝ ℝ
...
noncomputable local instance tensorialityTangentTrilinearNormedAddCommGroup (x : M) :
  ...
  @ContinuousLinearMap.toNormedAddCommGroup ℝ ℝ (TangentSpace I x)
    (TangentSpace I x →L[ℝ] TangentSpace I x →L[ℝ] ℝ)
      _ (tensorialityTangentBilinearNormedAddCommGroup (I := I) x) _ _ _
        (tensorialityTangentBilinearNormedSpace (I := I) x) (RingHom.id ℝ) _
...
noncomputable local instance tensorialityTangentTrilinearNormedSpace (x : M) :
  ...
  @ContinuousLinearMap.toNormedSpace ℝ ℝ (TangentSpace I x)
    (TangentSpace I x →L[ℝ] TangentSpace I x →L[ℝ] ℝ)
      _ (tensorialityTangentBilinearNormedAddCommGroup (I := I) x).toSeminormedAddCommGroup
        _ _ _ (tensorialityTangentBilinearNormedSpace (I := I) x) (RingHom.id ℝ) _ ℝ _
          (tensorialityTangentBilinearNormedSpace (I := I) x)
            (tensorialityTangentBilinearSMulCommClass (I := I) x)
...
lemma tensor03Scalar_tensorialAt_X
  ...
  smul {f X} _hf _hX := by
    ...
    rw [hfX]
    rw [(mvfderiv (I := I) (fun b => T b (Y b) (Z b) (W b)) x).map_smul]
    rw [(cov.toFun Y x).map_smul, (cov.toFun Z x).map_smul, (cov.toFun W x).map_smul]
    rw [(T x).map_smul, smul_apply, smul_apply,
  ...
  add {X X'} _hX _hX' := by
    ...
    rw [(T x).map_add, add_apply, add_apply,
...
lemma tensor03Scalar_tensorialAt_Y
  ...
  smul {g Y} hg hY := by
    ...
    have hfun : (fun b : M => T b ((g • Y) b) (Z b) (W b)) = (fun b : M => g b * h b) := by
      ...
      rw [this, ContinuousLinearMap.map_smul, smul_apply,
        smul_apply, smul_eq_mul]
    change mvfderiv (I := I) (fun b => T b ((g • Y) b) (Z b) (W b)) x (X x)
  ...
  add {Y Y'} hY hY' := by
    classical
    change mvfderiv (I := I) (fun b => T b ((Y + Y') b) (Z b) (W b)) x (X x)
    ...
    have h2 : MDifferentiableAt I 𝓘(ℝ, ℝ) (fun b : M => T b (Y' b) (Z b) (W b)) x :=
      mdifferentiableAt_tensor03_pairing hT hY' hZ hW
    have hext_add : mvfderiv (I := I) (fun b => T b ((Y + Y') b) (Z b) (W b)) x =
...
lemma tensor03Scalar_tensorialAt_Z
  ...
  smul {g Z} hg hZ := by
    ...
    change mvfderiv (I := I) (fun b => T b (Y b) ((g • Z) b) (W b)) x (X x)
  ...
  add {Z Z'} hZ hZ' := by
    classical
    change mvfderiv (I := I) (fun b => T b (Y b) ((Z + Z') b) (W b)) x (X x)
    ...
    have h2 : MDifferentiableAt I 𝓘(ℝ, ℝ) (fun b : M => T b (Y b) (Z' b) (W b)) x :=
      mdifferentiableAt_tensor03_pairing hT hY hZ' hW
    have hext_add : mvfderiv (I := I) (fun b => T b (Y b) ((Z + Z') b) (W b)) x =
        mvfderiv (I := I) (fun b => T b (Y b) (Z b) (W b)) x +
        mvfderiv (I := I) (fun b => T b (Y b) (Z' b) (W b)) x := by
      rw [hadd_fun, mvfderiv_add h1 h2]
    rw [hext_add]
...
lemma tensor03Scalar_tensorialAt_W
  ...
  smul {g W} hg hW := by
    ...
    change mvfderiv (I := I) (fun b => T b (Y b) (Z b) ((g • W) b)) x (X x)
      ...
      g x • (mvfderiv (I := I) h x (X x)
        ...
        - T x (Y x) (cov.toFun Z x (X x)) (W x)
        - T x (Y x) (Z x) (cov.toFun W x (X x)))
    rw [hfun, mvfderiv_mul_apply hg' hh, covOn.leibniz hW hg (Set.mem_univ x)]
  ...
  add {W W'} hW hW' := by
    classical
    change mvfderiv (I := I) (fun b => T b (Y b) (Z b) ((W + W') b)) x (X x)
    ...
    have h2 : MDifferentiableAt I 𝓘(ℝ, ℝ) (fun b : M => T b (Y b) (Z b) (W' b)) x :=
      mdifferentiableAt_tensor03_pairing hT hY hZ hW'
    have hext_add : mvfderiv (I := I) (fun b => T b (Y b) (Z b) ((W + W') b)) x =
        mvfderiv (I := I) (fun b => T b (Y b) (Z b) (W b)) x +
        mvfderiv (I := I) (fun b => T b (Y b) (Z b) (W' b)) x := by
      rw [hadd_fun, mvfderiv_add h1 h2]
    rw [hext_add]
...
private noncomputable def tensor03XSlot
  ...
  map_add' v v' := by
    ...
    unfold tensor03Scalar
    rw [h1, h2, h3]
    rw [(mvfderiv (I := I) (fun b => T b (Y b) (Z b) (W b)) x).map_add]
