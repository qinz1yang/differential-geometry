...
private theorem deriv_repr_comp_nat
  ...
  have hu_hd : HasDerivAt u (deriv u t) t := by
    ...
    rw [hu_def]
    change HasDerivAt (fun s => extChartAt I a (gamma s))
      (deriv (fun s => extChartAt I a (gamma s)) t) t
    exact (mdifferentiableAt_iff_differentiableAt.mp hchart).hasDerivAt
...
private theorem triv_mfderiv_nat
  ...
  have hfixed' :
      mfderiv I (modelWithCornersSelf ℝ F) (fun y : M => extChartAt J b (Phi y)) p
          (trivFromE (I := I) a p v) =
        fderiv ℝ (fun z : E => extChartAt J b (Phi ((extChartAt I a).symm z)))
          (extChartAt I a p) v := by
    simpa [writtenInExtChartAt, Function.comp_def, chartAt_self_eq] using hfixed
  exact hcomp'.trans hfixed'
...
theorem chartRep_map_diff
  ...
  have hu : DifferentiableAt ℝ u t := by
    have hchart : MDifferentiableAt (modelWithCornersSelf ℝ ℝ)
        ...
        (fun s => extChartAt I a (gamma s)) t :=
      (mdifferentiableAt_extChartAt (I := I) (mem_chart_source H (gamma t))).comp t hgamma
    change DifferentiableAt ℝ (fun s => extChartAt I a (gamma s)) t
...
private theorem covAlong_map_zero
  ...
  have hleftCoord :
    ...
    simpa [psi, u, a, b, chartCurve, writtenInExtChartAt, Function.comp_def] using
      (triv_mfderiv_nat (I := I) (J := J) Phi a a b haSource hbSource (deriv w t))
  have hrightCoord :
...
private theorem covAlong_nat_cross
  ...
  have hRdiff : DifferentiableAt ℝ (chartRepAt (I := I) gamma R t) t := by
    ...
    exact hV.sub hYdiff
  have hRt : R t = 0 := by
    rw [show R t = V t - Yalong t by rfl]
    rw [show Yalong t = linearExtensionTangent (I := I) (gamma t) (V t) (gamma t) by rfl]
