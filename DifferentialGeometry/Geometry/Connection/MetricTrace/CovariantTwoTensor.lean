...
@[simp] theorem freezeMiddle04Field_apply
    ...
    freezeMiddle04Field (I := I) (M := M) A Y Z x =
      freezeFirstTwo0S (I := I)
        ((A x).domDomCongr covariantFourTracePerm)
        (vec2 (I := I) (Y x) (Z x)) := by
  unfold freezeMiddle04Field
  rfl

theorem differential_metricTrace_eq_trace_totalNabla_inBasis
...
theorem differential_metricTrace_eq_metricTrace_nabla
    [T2Space M] [IsManifold I 1 M]
    [IsManifold I ((∞ : WithTop ℕ∞) + 1) M]
    (cov : CovariantDerivative I E (TangentSpace I : M -> Type _))
    (g : SmoothRiemannianMetric I M)
  ...
  have htrace :=
    differential_metricTrace_eq_trace_totalNabla_inBasis (I := I) (M := M) cov g hmc A basis gInv hinv (X x)
  have htot :
