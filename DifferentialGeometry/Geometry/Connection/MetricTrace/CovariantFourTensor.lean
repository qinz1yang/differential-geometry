...
def covariantFourTracePerm : Equiv.Perm (Fin 4) where
...
private def covariantFourTraceSlots
...
private theorem trace04Slots_apply {x₀ y : M}
    ...
    (fun q : Fin 4 =>
      metricTraceInput (I := I)
        ...
        (covariantFourTracePerm q)) =
      fun q : Fin 4 =>
        coordinateFrameAt (I := I) x₀ (covariantFourTraceSlots i j tail q) y := by
  funext q
  fin_cases q
  · simp [covariantFourTracePerm, metricTraceInput, covariantFourTraceSlots]
  · change
...
private theorem covariantFourTrace_eventually
    ...
    (fun y : M =>
        metricTraceFirstTwo0STensor (I := I) g
          ((A y).domDomCongr covariantFourTracePerm)
          (fun q : Fin 2 =>
...
private theorem metricTraceCovariantFour_coeff
    (g : SmoothRiemannianMetric I M)
    (A : Tensor0SField (𝕜 := Real) (E := E) (H := H) (I := I) (M := M)
      (n := (∞ : WithTop ℕ∞)) 4)
    (x₀ : M) (tail : Fin 2 -> CoordinateIdx (𝕜 := Real) E) :
  ...
  have hRhs :
          ...
          ∑ i : CoordinateIdx (𝕜 := Real) E,
            ∑ j : CoordinateIdx (𝕜 := Real) E,
              inverseMetricFlatModelInChartComponent (I := I) g x₀ i j
                ...
                A y
                  (fun q : Fin 4 =>
                    coordinateFrameAt (I := I) x₀ (covariantFourTraceSlots i j tail q) y))
        x₀ := by
    ...
    exact (gInvComp_contMDiffAt (I := I) g x₀ i j).mul
      (DifferentialGeometry.Tensor.Coordinates.tensor0S_eval_coordinateFrame_contMDiffAt
        (𝕜 := Real) (I := I) (M := M) A x₀
        (covariantFourTraceSlots i j tail))
  exact hRhs.congr_of_eventuallyEq
    (covariantFourTrace_eventually (I := I) g A x₀ tail)

def metricTraceCovariantFourField
    (g : SmoothRiemannianMetric I M)
    (A : Tensor0SField (𝕜 := Real) (E := E) (H := H) (I := I) (M := M)
      (n := (∞ : WithTop ℕ∞)) 4) :
  ...
  let F : (p : M) ->
    ...
    fun p : M =>
      metricTraceFirstTwo0STensor (I := I) g
        ((A p).domDomCongr covariantFourTracePerm)
  refine ⟨F, ?_⟩
  ...
  refine (contMDiff_multilinearSection_iff_coord (TangentSpace I)
    (∞ : WithTop ℕ∞) b F).mpr ?_
  intro σ x₀
  have hcoeff := metricTraceCovariantFour_coeff (I := I) g A x₀ σ
...
@[simp] theorem trace04Field_apply
    ...
    (A : Tensor0SField (𝕜 := Real) (E := E) (H := H) (I := I) (M := M)
      (n := (∞ : WithTop ℕ∞)) 4)
    (x : M) :
    metricTraceCovariantFourField (I := I) (M := M) g A x =
      metricTraceFirstTwo0STensor (I := I) g
      ((A x).domDomCongr covariantFourTracePerm) := by
  rfl
