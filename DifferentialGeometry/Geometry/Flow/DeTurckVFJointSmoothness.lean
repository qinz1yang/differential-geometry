...
/-! ## Joint-smoothness keystone for the DeTurck vector field

This file builds the joint-`(s, y)` smoothness of the scalar chart component
`chartDeTurckVFComp (g_DT s) g_bg α k (extChartAt I α ·)` from the joint chart-Gram
hypothesis, by assembling the joint Cramer inverse, the joint chart-Christoffel symbol,
and joint spatial partial derivatives. Each step is the joint-`(s, y)` analogue of the
single-metric facts in `Geometry/Operator/HessianTrace.lean` and
`Analysis/Parabolic/DeTurckLinearization/ChartVectorField.lean`. The keystone then feeds
the bundle-section readout `deTurckVF_jointContMDiffOn_Ioo` on the open time slab. -/

namespace DeTurckVFJointKeystone
...
abbrev Idx (E : Type*) [NormedAddCommGroup E] [NormedSpace ℝ E]
...
entries. -/
lemma contDiffOn_jointDet {s : Set (ℝ × E)}
    {A : ℝ × E → Matrix n n ℝ}
...
entries. -/
lemma contDiffOn_jointAdjugate {s : Set (ℝ × E)}
    {A : ℝ × E → Matrix n n ℝ}
...
lemma contDiffOn_jointMatrixInv_entry {s : Set (ℝ × E)}
    {A : ℝ × E → Matrix n n ℝ}
    (hA : ∀ i j, ContDiffOn ℝ ∞ (fun p : ℝ × E => A p i j) s)
...
lemma contDiffOn_jointPartialDeriv {sI : Set ℝ} {U : Set E}
...
lemma symm_mem_baseSet_of_mem_interior {α : M} {y : E}
...
lemma gramOnE_matrix_eq (g : SmoothRiemannianMetric I M) (α : M) (y : E) :
...
`chartGramOnE` matrix. -/
lemma chartInvGramOnE_eq_matrixInv
...
`C^∞`. -/
lemma chartInvGramOnE_joint_contDiffOn
...
lemma chartChristoffel_joint_contDiffOn
    (g_DT : ℝ → SmoothRiemannianMetric I M) (α : M) (T : ℝ)
    (h_gram_E : ∀ i j : Fin (Module.finrank ℝ E),
      ContDiffOn ℝ ∞
        (fun p : ℝ × E => chartGramOnE (I := I) (g_DT p.1) α i j p.2)
...
lemma contDiffOn_snd_of_contDiffOn_interior {α : M} (T : ℝ)
...
lemma chartDeTurckVFComp_joint_contDiffOn_E
...
lemma chartGramOnE_joint_contDiffOn_of_manifold
    (g_DT : ℝ → SmoothRiemannianMetric I M) (α : M) (T : ℝ)
...
lemma chartDeTurckVFComp_joint_contMDiffOn
    (g_DT : ℝ → SmoothRiemannianMetric I M) (g_bg : SmoothRiemannianMetric I M)
    (α : M) (T : ℝ)
    (h_gDT : ∀ (i j : Fin (Module.finrank ℝ E)),
      ContMDiffOn (𝓘(ℝ, ℝ).prod I) 𝓘(ℝ, ℝ) ∞
...
lemma deTurckVF_trivSnd_eq_chartModelBasis_sum
    [I.Boundaryless]
...
/-- `C⁰` joint determinant. -/
lemma continuousOn_jointDet {s : Set (ℝ × E)}
    {A : ℝ × E → Matrix n n ℝ}
...
lemma continuousOn_jointAdjugate {s : Set (ℝ × E)}
    {A : ℝ × E → Matrix n n ℝ}
...
lemma continuousOn_jointMatrixInv_entry {s : Set (ℝ × E)}
    {A : ℝ × E → Matrix n n ℝ}
    (hA : ∀ i j, ContinuousOn (fun p : ℝ × E => A p i j) s)
    (hdet : ∀ p ∈ s, (A p).det ≠ 0) (a b : n) :
...
lemma chartInvGramOnE_joint_continuousOn
    (g_DT : ℝ → SmoothRiemannianMetric I M) (α : M) (T : ℝ)
...
lemma chartChristoffel_joint_continuousOn
    (g_DT : ℝ → SmoothRiemannianMetric I M) (α : M) (T : ℝ)
    (h_gram0 : ∀ i j : Fin (Module.finrank ℝ E),
      ContinuousOn (fun p : ℝ × E => chartGramOnE (I := I) (g_DT p.1) α i j p.2)
        (Set.Icc (0 : ℝ) T ×ˢ interior (extChartAt I α).target))
...
lemma chartDeTurckVFComp_joint_continuousOn_E
...
lemma chartDeTurckVFComp_joint_continuousOn_M
    (g_DT : ℝ → SmoothRiemannianMetric I M) (g₀ : SmoothRiemannianMetric I M)
    (α : M) (T : ℝ)
    (h_gram0 : ∀ i j : Fin (Module.finrank ℝ E),
      ContinuousOn (fun p : ℝ × E => chartGramOnE (I := I) (g_DT p.1) α i j p.2)
...
lemma partialDeriv_mul_eq {f h : E → ℝ} {y : E}
    (l : Fin (Module.finrank ℝ E))
...
lemma partialDeriv_chartInvGramOnE_joint_continuousOn
    (g_DT : ℝ → SmoothRiemannianMetric I M) (α : M) (T : ℝ)
    (h_gram0 : ∀ i j : Fin (Module.finrank ℝ E),
      ContinuousOn (fun p : ℝ × E => chartGramOnE (I := I) (g_DT p.1) α i j p.2)
        (Set.Icc (0 : ℝ) T ×ˢ interior (extChartAt I α).target))
...
lemma partialDeriv_const_mul_eq {f : E → ℝ} {y : E} (c : ℝ)
...
lemma partialDeriv_finset_sum_eq {ι : Type*} (t : Finset ι)
...
lemma partialDeriv_add_eq {f h : E → ℝ} {y : E}
    (l : Fin (Module.finrank ℝ E))
    (hf : DifferentiableAt ℝ f y) (hh : DifferentiableAt ℝ h y) :
    partialDeriv (E := E) l (fun x => f x + h x) y
...
lemma partialDeriv_sub_eq {f h : E → ℝ} {y : E}
    (l : Fin (Module.finrank ℝ E))
    (hf : DifferentiableAt ℝ f y) (hh : DifferentiableAt ℝ h y) :
    partialDeriv (E := E) l (fun x => f x - h x) y
      = partialDeriv (E := E) l f y - partialDeriv (E := E) l h y := by
...
lemma partialDeriv_chartChristoffel_joint_continuousOn
    (g_DT : ℝ → SmoothRiemannianMetric I M) (α : M) (T : ℝ)
    (h_gram0 : ∀ i j : Fin (Module.finrank ℝ E),
      ContinuousOn (fun p : ℝ × E => chartGramOnE (I := I) (g_DT p.1) α i j p.2)
        (Set.Icc (0 : ℝ) T ×ˢ interior (extChartAt I α).target))
...
lemma partialDeriv_chartDeTurckVFComp_joint_continuousOn
    (g_DT : ℝ → SmoothRiemannianMetric I M) (g₀ : SmoothRiemannianMetric I M)
    (α : M) (T : ℝ)
    (h_gram0 : ∀ i j : Fin (Module.finrank ℝ E),
      ContinuousOn (fun p : ℝ × E => chartGramOnE (I := I) (g_DT p.1) α i j p.2)
...
lemma fderiv_eq_partialDeriv_sum (F : E → ℝ) (y : E) :
...
lemma fderiv_chartDeTurckVFComp_joint_continuousOn
    (g_DT : ℝ → SmoothRiemannianMetric I M) (g₀ : SmoothRiemannianMetric I M)
    (α : M) (T : ℝ)
    (h_gram0 : ∀ i j : Fin (Module.finrank ℝ E),
      ContinuousOn (fun p : ℝ × E => chartGramOnE (I := I) (g_DT p.1) α i j p.2)
...
theorem deTurckVF_jointContMDiffOn_Ioo
  ...
  have hcoeff := fun p =>
    DeTurckVFJointKeystone.chartDeTurckVFComp_joint_contMDiffOn
  ...
  have hsum_within : ContMDiffWithinAt (𝓘(ℝ, ℝ).prod I) 𝓘(ℝ, E) ∞
    ...
    refine ContMDiffWithinAt.sum (fun p _ => ?_)
  ...
  have heqOn : Set.EqOn
      ...
      (Set.Ioo (0 : ℝ) T ×ˢ chartLeviCivitaGoodSet (I := I) α) := by
    intro q hq
    exact DeTurckVFJointKeystone.deTurckVF_trivSnd_eq_chartModelBasis_sum
