...
theorem rm13_apply_eq_rm04_raise
    ...
    Rm13 β (vec3 (I := I) X Y Z) =
      Rm04 (vec4 (I := I) X Y Z (cotangentSharp (I := I) g x β)) := by
  have hL := hLower X Y Z (cotangentSharp (I := I) g x β)
  rw [hL]
  have hβ :
      dualToCotangent (I := I)
          ((tangentFlatLinear (I := I) g x) (cotangentSharp (I := I) g x β)) = β := by
    refine cotangentToDualLinear_injective (I := I) (x := x) ?_
    rw [cotangentToDualLinear_apply, cotangentToDualLinear_apply,
      cotangentToDual_dualToCotangent]
    ext X
    rw [tangentFlatLinear_apply, cotangentToDual_apply,
      ...
      -∑ q : Fin 5,
        S.base.rm04 t x₀
          (vec4 (I := I) (coordinateFrameAt (I := I) x₀ a x₀)
            ...
            (cotangentSharp (I := I) (S.base.metric t) x₀
