...
theorem exists_lMinimizingVector
...
theorem lMinDomain_down
  ...
  · have hvec := (mem_lMinDomain S T x Z tau).1 hmin
    ...
    have hgammaC1 : ContMDiffOn (modelWithCornersSelf Real Real) I 1 gamma
      ...
      simpa only [gamma] using lRegularizedCurve_c1On S hS T x Z htauDom
    have hregTau : ∀ s ∈ Icc (0 : Real) (Real.sqrt tau),
        T - s ^ 2 ∈ D.regular := by
      intro s hs
      exact lExpPosDom_regularity S T x Z htauPos hs
    ...
    have htimeTau : Icc (T - tau) T ⊆ D.carrier := by
      ...
      have hregR := lExpPosDom_regularity S T x Z htauPos hsqrtMem
      have heq : T - (Real.sqrt (T - r)) ^ 2 = r := by
        rw [Real.sq_sqrt hnonneg]
        ring
      exact D.regular_subset (by simpa only [heq] using hregR)
    have hcostTau : lRegularizedAction S T gamma 0 (Real.sqrt tau) =
        lRegularizedCostC1 S T 0 (Real.sqrt tau) x (gamma (Real.sqrt tau)) := by
      calc
        ...
        _ = lRegularizedCostC1 S T 0 (Real.sqrt tau) x
            (lExp S T x Z tau) := lCost_eq_regularity (I := I) S T x
    ...
    have hminTau : ∀ delta : Real → M,
      ...
      exact lRegularizedCostC1_le (I := I) S hS T (T - tau) T 0
        ...
        (hd0.trans (by simp only [gamma, lRegularizedCurve_zero])) hdt hregTau
    have hMet : MetricFamilySmoothOn (I := I) (M := M) D S.family.metric :=
      hS.smoothMetric
    have hSc : ScalarSTContOn (I := I) (M := M) S := ⟨hS.scalarCont⟩
    have hminSigma := lRegularized_prefix_min (I := I) S hMet hSc T 0
    ...
    have hgammaSigma : ContMDiffOn (modelWithCornersSelf Real Real) I 1 gamma
      ...
      simpa only [gamma] using lRegularizedCurve_c1On S hS T x Z hsigmaDom
    have hregSigma : ∀ s ∈ Icc (0 : Real) (Real.sqrt sigma),
        T - s ^ 2 ∈ D.regular := by
      intro s hs
      exact lExpPosDom_regularity S T x Z hsigmaPos hs
    ...
    have htimeSigma : Icc (T - sigma) T ⊆ D.carrier := by
      ...
      have hregR := lExpPosDom_regularity S T x Z hsigmaPos hsqrtMem
      have heq : T - (Real.sqrt (T - r)) ^ 2 = r := by
        rw [Real.sq_sqrt hnonneg]
        ring
      exact D.regular_subset (by simpa only [heq] using hregR)
    have hcostSigma : lRegularizedAction S T gamma 0 (Real.sqrt sigma) =
        lRegularizedCostC1 S T 0 (Real.sqrt sigma) x
          (gamma (Real.sqrt sigma)) := by
      apply lRegularizedCostC1_eq_on (I := I) S hS T (T - sigma) T 0
