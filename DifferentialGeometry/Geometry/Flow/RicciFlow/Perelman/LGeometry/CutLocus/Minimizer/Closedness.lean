...
theorem lMinimizingVector_lim
  ...
  have hactLim : Tendsto
      ...
      atTop (nhds (lRegularizedAction S T gamma 0 b)) := by
    have hcontinuous := continuousAt_lRegularizedAction_lRegularizedCurve (I := I) S hS T x hb hbDom
  ...
  have hactEq (n : Nat) :
    ...
    calc
      lRegularizedAction S T (lRegularizedCurve S T x (Z n)) 0 b =
        ...
        simpa only [b] using
          (lLength_squareRootReparametrization_eq_lRegularizedAction (I := I) S T (lRegularizedCurve S T x (Z n)) tau htau.le).symm
  ...
  have hgammaC1 : ContMDiffOn (modelWithCornersSelf Real Real) I 1 gamma
      (Icc (0 : Real) b) := by
    simpa only [gamma, b] using lRegularizedCurve_c1On S hS T x Z₀ hbDom
  have hmin0 : ∀ delta : Real → M,
    ...
    by_contra hnot
    have hlt : lRegularizedAction S T delta 0 b < lRegularizedAction S T gamma 0 b :=
      lt_of_not_ge hnot
    let A : Real :=
    ...
    dsimp only [A] at hle
    linarith
  have hcostEq : lRegularizedAction S T gamma 0 b =
      lRegularizedCostC1 S T 0 b x y :=
    lRegularizedCostC1_eq_on (I := I) S hS T (T - tau) T 0 b hb htime
      hback x y gamma hgammaC1 (by simp only [gamma, lRegularizedCurve_zero]) rfl
      hreg hmin0
  ...
  calc
    ...
    _ = lRegularizedCostC1 S T 0 b x y := hcostEq
    _ = lCost S T x (lExp S T x Z₀ tau) tau := by
      simpa only [b, y] using
        (lCost_eq_regularity (I := I) S T x (lExp S T x Z₀ tau) tau htau.le).symm
