...
theorem lRegAction_minimizer_contMDiffOn_one_of_two_chart_pieces
  ...
  have hnode : (extChartAt I (p 0)).symm
    ...
    rw [hrecover 0 (partitionIntervalLength t 0)
        ⟨by simpa only [partitionIntervalLength, hfin0s] using sub_nonneg.mpr (hpos 0).le,
          le_rfl⟩,
  ...
  have hreg0 : ∀ r, r ∈ Icc (0 : Real) (partitionIntervalLength t 0) →
    ...
    have hr2 : r ≤ t 1 - t 0 := by
      simpa only [partitionIntervalLength, hfin0c, hfin0s] using hr.2
  ...
  have hp0der : derivWithin ((extChartAt I (p 0)) ∘ gamma)
      ...
      derivWithin (u 0).toFun
        (Icc (0 : Real) (partitionIntervalLength t 0)) (partitionIntervalLength t 0) := by
    have hshift := chartDeriv_shift
      (a := t (0 : Fin 2).castSucc) (b := t (0 : Fin 2).succ)
      (r := partitionIntervalLength t 0) ((extChartAt I (p 0)) ∘ gamma) (u 0)
      (by
        intro r hr
        change (u 0).toFun r =
    ...
    simpa only [hfin0c, hfin0s, partitionIntervalLength, add_sub_cancel] using hshift
  have hp1der : derivWithin ((extChartAt I (p 1)) ∘ gamma)
    ...
    simpa only [hfin1c, hfin1s, partitionIntervalLength, add_zero] using hshift
