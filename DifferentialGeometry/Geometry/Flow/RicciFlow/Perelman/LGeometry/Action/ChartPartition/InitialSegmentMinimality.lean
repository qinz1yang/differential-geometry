...
theorem lChartAct_initial_pair_le_of_lRegAction_minimizer
  ...
  have hvhtar : ∀ i, MapsTo (vh i).toFun
    ...
    · change MapsTo (vh 0).toFun
        (Icc (0 : Real) (lSegLen th 0)) (extChartAt I (ph 0)).target
      rw [hvh0]
      simpa [th, ph, lSegLen] using hvtar0
    · change MapsTo (vh 1).toFun
        (Icc (0 : Real) (lSegLen th 1)) (extChartAt I (ph 1)).target
      rw [hvh1]
      simpa [th, ph, lSegLen] using hvtar1
  have hvhnode : (extChartAt I (ph 0)).symm
        ((vh 0).toFun (lSegLen th 0)) =
      (extChartAt I (ph 1)).symm ((vh 1).toFun 0) := by
    rw [hvh0, hvh1]
    simpa [th, ph, lSegLen] using hvnode
  ...
  have hrepBase : ∀ i, EqOn (uf i).toFun
    ...
    · change EqOn (uf 0).toFun
        ...
        (Icc (0 : Real) (lSegLen tf 0))
      rw [huf0]
      simpa [tf, pf, lSegLen] using hrep0
    · change EqOn (uf 1).toFun
        (fun r ↦ extChartAt I (pf 1) (gamma (tf 1 + r)))
        (Icc (0 : Real) (lSegLen tf 1))
      rw [huf1]
      simpa [tf, pf, lSegLen] using hrep1
    · change EqOn (uf 2).toFun
        (fun r ↦ extChartAt I (pf 2) (gamma (tf 2 + r)))
        (Icc (0 : Real) (lSegLen tf 2))
      rw [huf2]
      simpa [tf, pf, lSegLen] using hrep2
