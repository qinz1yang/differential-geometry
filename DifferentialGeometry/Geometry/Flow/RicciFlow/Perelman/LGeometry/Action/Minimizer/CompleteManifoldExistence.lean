...
theorem lRmChartH1_fin
  ...
  let B : Real := A - C * (b - a)
  have hspeed (n : Nat) :
      (∫ s in a..b, lRegularizedSpeedSq S T (alpha n) s) ≤ 2 * B := by
    exact lRegularizedKinetic_le (I := I) S T (alpha n) a b A C hab
  ...
  have hhalfInt (n : Nat) : IntervalIntegrable
      (fun s ↦ (1 / 2 : Real) * lRegularizedSpeedSq S T (alpha n) s) volume a b :=
    (hkin n).const_mul (1 / 2 : Real)
  have hhalf (n : Nat) :
      (∫ s in a..b, (1 / 2 : Real) * lRegularizedSpeedSq S T (alpha n) s) ≤ B := by
    rw [intervalIntegral.integral_const_mul]
    linarith [hspeed n]
  have hnonneg (n : Nat) :
      ∀ᵐ s ∂volume.restrict (Ioc a b),
        0 ≤ (1 / 2 : Real) * lRegularizedSpeedSq S T (alpha n) s :=
  ...
  have hpiece (i : Fin m) (n : Nat) :
      (∫ s in t i.castSucc..t i.succ,
        (1 / 2 : Real) * lRegularizedSpeedSq S T (alpha n) s) ≤ B := by
  ...
  have hchart (i : Fin m) (n : Nat) :
    ...
    rw [show (∫ r in (0 : Real)..t i.succ - t i.castSucc,
        ...
        ((u i n).deriv r)) =
        ∫ s in t i.castSucc..t i.succ,
          (1 / 2 : Real) * lRegularizedSpeedSq S T (alpha n) s by
      calc
  ...
  have htimeRegularity (i : Fin m) : MapsTo
      (fun r : Real ↦ T - (t i.castSucc + r) ^ 2)
  ...
  exact chartH1_fin (I := I) hMet p
    ...
    (fun i r ↦ T - (t i.castSucc + r) ^ 2) htimeCont htimeRegularity
...
theorem lRmAction_subseq
  ...
  have hequi : Equicontinuous (fun n ↦ (f n : Icc a b → M)) := by
    have huniform : UniformEquicontinuous (fun n ↦ (f n : Icc a b → M)) := by
      ...
      intro ε hε
      obtain ⟨ρ, hρ, htoDist⟩ :=
        dist_lt_riedist_compact (I := I) (S.base.metric T) Cpt hCpt hε
  ...
  obtain ⟨phi, g, hphi, hconv⟩ :=
    DifferentialGeometry.Analysis.arzela_subseq_compact Cpt hCpt f hval
      hequi
...
theorem exists_lRegularizedMin_rm
    (S : SolutionOn (I := I) (M := M) D) (hS : IsSolutionOn (I := I) S)
    (K T : Real)
    (hg : RiemannianMetricComplete (I := I) (S.base.metric T))
  ...
  have hcosts_bdd : BddBelow costs := by
    ...
    have hkin := intervalIntegrable_lRegularizedSpeedSq_of_contMDiffOn_one (I := I) S hMet hSc T a b hab.le alpha
      halpha.contMDiffOn hregBack
    have hLag := intervalIntegrable_lRegularizedLagrangian_of_contMDiffOn_one (I := I) S hMet hSc T a b hab.le alpha
      halpha.contMDiffOn hregBack
    have hbound := lRegularizedKinetic_le (I := I) S T alpha a b
      (lRegularizedAction S T alpha a b) C hab.le
      (fun s hs ↦ lRegularizedPot_lower_rm (I := I) S K T b hb hRm s
  ...
  have hkin (n : Nat) : IntervalIntegrable
      (lRegularizedSpeedSq S T (alpha n)) volume a b :=
    intervalIntegrable_lRegularizedSpeedSq_of_contMDiffOn_one (I := I) S hMet hSc T a b hab.le (alpha n)
      (halpha n).contMDiffOn hregBack
  have hLag (n : Nat) : IntervalIntegrable
      (lRegularizedLagrangian S T (alpha n)) volume a b :=
    intervalIntegrable_lRegularizedLagrangian_of_contMDiffOn_one (I := I) S hMet hSc T a b hab.le (alpha n)
      (halpha n).contMDiffOn hregBack
  ...
  have hsubseq : Tendsto
    ...
    rw [show (fun n ↦ lRegularizedAction S T (alpha (chi n)) a b) =
      (fun n ↦ v (chi n)) by
      funext n
      exact hval (chi n)]
    exact h
  ...
  have hsol : ∀ s ∈ Ioo a b,
    ...
    lMinCurve_regularity (I := I) S hS T a b hab t htmono ht0 htlast p gamma
      hgamma uLim hsrc hrep hregBack hmin'
...
theorem exists_lMin_rm
  ...
  obtain ⟨gamma, hgamma, hc1, hga, hgb, hcost, hmin, hsol⟩ :=
    exists_lRegularizedMin_rm (I := I) S hS K T hg 0 (Real.sqrt tau) le_rfl
  ...
  have hregBack : ∀ s ∈ Icc (0 : Real) (Real.sqrt tau),
    ...
    constructor <;> linarith [sq_nonneg s]
  obtain ⟨alpha, halpha, e, he, halphaSolution⟩ :=
    exists_lRegularizedExtOn (I := I) S hS T 0 (Real.sqrt tau) hsqrt gamma hc1
      hregBack hsol
  have ha0 : alpha 0 = x :=
    (halpha ⟨le_rfl, hsqrt.le⟩).trans hga
  have hat : alpha (Real.sqrt tau) = y :=
    (halpha ⟨hsqrt.le, le_rfl⟩).trans hgb
  have haction : lRegularizedAction S T alpha 0 (Real.sqrt tau) =
      lRegularizedAction S T gamma 0 (Real.sqrt tau) := by
    apply lRegularizedAction_congr (I := I) S T alpha gamma 0 (Real.sqrt tau)
    intro s hs
  ...
  have hcurve : IsLRegularizedCurveOn S T alpha
      (Icc (0 : Real) (Real.sqrt tau)) x Z :=
    ⟨ha0, hvel, fun s hs ↦ halphaSolution s
      ⟨by linarith [hs.1], by linarith [hs.2]⟩⟩
  have hcurveOpen : IsLRegularizedCurveOn S T alpha
      (Ioo (-e) (Real.sqrt tau + e)) x Z := by
    refine ⟨ha0, hvel, ?_⟩
    simpa only [IsLRegularizedGeodesicOn, zero_sub] using halphaSolution
  have h0Open : (0 : Real) ∈ Ioo (-e) (Real.sqrt tau + e) := by
    constructor <;> linarith
  have htOpen : Real.sqrt tau ∈ Ioo (-e) (Real.sqrt tau + e) := by
    constructor <;> linarith
  have htDom : Real.sqrt tau ∈ lRegularizedDomain S T x Z :=
    ⟨alpha, Ioo (-e) (Real.sqrt tau + e), isOpen_Ioo, isPreconnected_Ioo,
      h0Open, htOpen, hcurveOpen⟩
  have hmax : Set.EqOn (lRegularizedCurve S T x Z) alpha
      (Icc (0 : Real) (Real.sqrt tau)) :=
    lRegularizedCurve_eqIcc S hS T (Real.sqrt tau) e hsqrt.le he hcurveOpen
  ...
  · calc
      ...
      _ = lRegularizedCostC1 S T 0 (Real.sqrt tau) x y := haction.trans hcost
      _ = lCost S T x y tau :=
...
theorem exists_lMinimizingVector_rm
    (S : SolutionOn (I := I) (M := M) D) (hS : IsSolutionOn (I := I) S)
    (K T : Real)
    (hg : RiemannianMetricComplete (I := I) (S.base.metric T))
    (tau : Real) (htau : 0 < tau)
  ...
  calc
    lLength S T (fun r : Real ↦ lExp S T x Z r) 0 tau =
      ...
      change lLength S T
          (fun r : Real ↦ lRegularizedCurve S T x Z (Real.sqrt r)) 0 tau = _
    ...
    _ = lRegularizedAction S T alpha 0 (Real.sqrt tau) := by
    ...
    _ = lLength S T (squareRootReparametrization alpha) 0 tau :=
      (lLength_squareRootReparametrization_eq_lRegularizedAction (I := I) S T alpha tau htau.le).symm
