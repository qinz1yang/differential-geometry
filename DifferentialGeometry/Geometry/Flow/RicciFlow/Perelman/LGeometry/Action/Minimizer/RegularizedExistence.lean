...
theorem exists_lRegularizedMinOn
    ...
    ∃ (alpha : Real → M) (Z : TangentSpace I x),
      IsLRegularizedCurveOn S T alpha (Icc (0 : Real) b) x Z ∧
        b ∈ lRegularizedDomain S T x Z ∧
          Set.EqOn (lRegularizedCurve S T x Z) alpha (Icc (0 : Real) b) ∧
            alpha b = y ∧
            lRegularizedAction S T alpha 0 b = lRegularizedCostC1 S T 0 b x y ∧
            ∀ delta : Real → M,
              ContMDiff (modelWithCornersSelf Real Real) I 1 delta →
              delta 0 = x → delta b = y →
              lRegularizedAction S T alpha 0 b ≤ lRegularizedAction S T delta 0 b := by
  obtain ⟨gamma, m, t, p, uLim, _beta, _u, hgamma, hga, hgb, hcost,
    ...
    exists_lRegularizedMinC1 (I := I) S hS T t0 t1 0 b hb.le htime hback x y
  ...
  have hsol : ∀ s ∈ Ioo (0 : Real) b,
    ...
    lMinCurve_regularity (I := I) S hS T 0 b hb t htmono ht0 htlast p gamma
      hgamma uLim hsrc hrep hreg hmin'
  obtain ⟨alpha, halpha, e, he, halphaSolution⟩ :=
    exists_lRegularizedExtOn (I := I) S hS T 0 b hb gamma hc1 hreg hsol
  have ha0 : alpha 0 = x :=
    (halpha ⟨le_rfl, hb.le⟩).trans hga
  have hab : alpha b = y :=
    (halpha ⟨hb.le, le_rfl⟩).trans hgb
  have haction : lRegularizedAction S T alpha 0 b = lRegularizedAction S T gamma 0 b := by
    apply lRegularizedAction_congr (I := I) S T alpha gamma 0 b
    intro s hs
  ...
  have hcurve : IsLRegularizedCurveOn S T alpha (Icc (0 : Real) b) x Z :=
    ⟨ha0, hvel, fun s hs ↦ halphaSolution s
      ⟨by linarith [hs.1], by linarith [hs.2]⟩⟩
  have hcurveOpen :
      IsLRegularizedCurveOn S T alpha (Ioo (-e) (b + e)) x Z := by
    refine ⟨ha0, hvel, ?_⟩
  ...
  have hmax : Set.EqOn (lRegularizedCurve S T x Z) alpha (Icc (0 : Real) b) :=
    lRegularizedCurve_eqIcc S hS T b e hb.le he hcurveOpen
