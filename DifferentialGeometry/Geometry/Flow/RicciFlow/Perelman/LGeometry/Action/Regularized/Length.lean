...
theorem lRegDensity_eq_lRegLagrangian_squareReparametrization
    (S : SolutionOn (I := I) (M := M) D) (T : Real)
    (gamma : Real → M) (s : Real) :
    lRegDensity S T gamma s = lRegLagrangian S T (squareReparametrization gamma) s := by
  rfl

omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [T2Space M] [SigmaCompactSpace M] in
theorem lLength_eq_lRegAction_squareReparametrization
    (S : SolutionOn (I := I) (M := M) D) (T : Real)
    ...
    lLength S T gamma tau1 tau2 =
      lRegAction S T (squareReparametrization gamma)
        (Real.sqrt tau1) (Real.sqrt tau2) := by
  simpa only [lRegAction, lRegDensity_eq_lRegLagrangian_squareReparametrization] using
    lLength_squareReparametrization (I := I) S T gamma tau1 tau2 htau1 htau2 hgamma

omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [T2Space M] [SigmaCompactSpace M] in
theorem lLength_eq_lRegAction_squareReparametrization_ae
    (S : SolutionOn (I := I) (M := M) D) (T : Real)
    (gamma : Real → M) (tau1 tau2 : Real)
    (htau1 : 0 ≤ tau1) (htau2 : 0 ≤ tau2) :
    lLength S T gamma tau1 tau2 =
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [T2Space M] [SigmaCompactSpace M] in
theorem lLength_squareRootReparametrization_eq_lRegAction
    (S : SolutionOn (I := I) (M := M) D) (T : Real)
  ...
  have hsq := lLength_eq_lRegAction_squareReparametrization_ae (I := I) S T (squareRootReparametrization alpha)
    0 tau (by norm_num) htau
  have hsq' : lLength S T (squareRootReparametrization alpha) 0 tau =
      lRegAction S T (squareReparametrization (squareRootReparametrization alpha)) 0
        (Real.sqrt tau) := by
