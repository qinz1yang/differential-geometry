...
private theorem lRegularizedIndex_eq_boundary_sub_integral_variation
    ...
    (hgeo : IsLRegularizedCurveOn S T (f 0) (uIcc a b) x Z) :
  ...
  have hIint := intervalIntegrable_lRegularizedIndexIntegrand_of_contMDiff (I := I) S hS T a b alpha Y Y
    hY2 hY2 ht
  have hJint : IntervalIntegrable
      (fun s : Real ↦ lRegularizedJacobiPair S T alpha Y s (Y s))
      MeasureTheory.volume a b := by
    simpa only [alpha, Y] using
      (continuousOn_lRegularizedJacobiPair_variation (I := I) S hS T f hf a b x Z hgeo).intervalIntegrable
  simpa only [alpha, Y] using
...
omit [CompactSpace M] in
omit [NeZero (Module.finrank ℝ E)] [SigmaCompactSpace M] in
private theorem hasDerivAt_deriv_piecewise_lRegularizedAction
    (S : SolutionOn (I := I) (M := M) D)
    (hS : IsSolutionOn (I := I) S) (T : Real)
    (f0 f1 : Real → Real → M)
    ...
    (hgeo0 : IsLRegularizedCurveOn S T (f0 0) (uIcc a c) x Z)
    (hgeo1 : IsLRegularizedCurveOn S T (f1 0) (uIcc c b) x Z)
    (hcenter : f0 0 = f1 0)
    ...
    HasDerivAt
      (fun u : Real ↦ deriv
        (fun v : Real ↦
          lRegularizedAction S T (f0 v) a c + lRegularizedAction S T (f1 v) c b) u)
      (2 *
        (lRegularizedIndex S T (f0 0)
            (fun s : Real ↦
              lVelocity (I := I) (fun u : Real ↦ f0 u s) 0)
            (fun s : Real ↦
              lVelocity (I := I) (fun u : Real ↦ f0 u s) 0) a c +
          lRegularizedIndex S T (f1 0)
            (fun s : Real ↦
              lVelocity (I := I) (fun u : Real ↦ f1 u s) 0)
            (fun s : Real ↦
              lVelocity (I := I) (fun u : Real ↦ f1 u s) 0) c b)) 0 := by
  let L : Real → Real := fun u ↦
    lRegularizedAction S T (f0 u) a c + lRegularizedAction S T (f1 u) c b
  ...
  let E0 : Real → Real := fun u ↦
    ∫ s in a..c, -lRegularizedEulerPair S T (f0 u) s
      (lVelocity (I := I) (fun v : Real ↦ f0 v s) u)
  let E1 : Real → Real := fun u ↦
    ∫ s in c..b, -lRegularizedEulerPair S T (f1 u) s
  ...
  have hderivEq (u : Real) : deriv L u = P0 u - P1 u + E0 u + E1 u := by
    ...
    have hYb : lVelocity (I := I) (fun v : Real ↦ F1 v b) 0 = 0 := by
      ...
      simp only [lVelocity, mfderiv_const]
      rfl
    have hfirst0 := lRegularizedAction_first_variation (I := I) S hS T F0 hF0 a c ht0
    have hfirst1 := lRegularizedAction_first_variation (I := I) S hS T F1 hF1 c b ht1
    rw [hYa] at hfirst0
    rw [hYb] at hfirst1
  ...
  have hpairs : HasDerivAt (fun u : Real ↦ P0 u - P1 u)
    ...
    rw [hsym0, hsym1]
    ring
  have heuler0 := hasDerivAt_integral_lRegularizedEulerPair_variation (I := I) S hS T f0 hf0 a c x Z hgeo0
  have heuler1 := hasDerivAt_integral_lRegularizedEulerPair_variation (I := I) S hS T f1 hf1 c b x Z hgeo1
  have htotal := (hpairs.add heuler0).add heuler1
  have hgreen0 := lRegularizedIndex_eq_boundary_sub_integral_variation (I := I) S hS T f0 hf0 a c x Z hgeo0
  have hgreen1 := lRegularizedIndex_eq_boundary_sub_integral_variation (I := I) S hS T f1 hf1 c b x Z hgeo1
...
omit [CompactSpace M] in
omit [NeZero (Module.finrank ℝ E)] [SigmaCompactSpace M] in
theorem lRegularizedIndex_piecewise_nonneg
    (S : SolutionOn (I := I) (M := M) D)
    (hS : IsSolutionOn (I := I) S) (T : Real)
    (gamma : Real → M) (a c b : Real) (hac : a < c) (hcb : c < b)
    (x : M) (Z : TangentSpace I x)
    (hgeo : IsLRegularizedCurveOn S T gamma (uIcc a b) x Z)
    (hmin : ∀ delta : Real → M,
    ...
    0 ≤ lRegularizedIndex S T gamma Y0 Y0 a c +
      lRegularizedIndex S T gamma Y1 Y1 c b := by
  ...
  have hgeo0 : IsLRegularizedCurveOn S T (f0 0) (uIcc a c) x Z := by
    rw [hcenter0]
  ...
  have hgeo1 : IsLRegularizedCurveOn S T (f1 0) (uIcc c b) x Z := by
    rw [hcenter1]
    refine ⟨hgeo.1, hgeo.2.1, fun s hs ↦ hgeo.2.2 s ?_⟩
  ...
  have hLglobal : ∀ u : Real, L 0 ≤ L u := by
    ...
    obtain ⟨alpha, _w, halpha, halphaa, halphab, _hsrcAlpha,
        _hrepAlpha, _hw, _huniform, haction⟩ :=
    ...
    have hgammaInt0 := intervalIntegrable_lRegularizedLagrangian_of_contMDiffOn_one (I := I) S hS.smoothMetric
      ⟨hS.scalarCont⟩ T a c hac.le gamma hgamma0 hreg0
    have hgammaInt1 := intervalIntegrable_lRegularizedLagrangian_of_contMDiffOn_one (I := I) S hS.smoothMetric
      ⟨hS.scalarCont⟩ T c b hcb.le gamma hgamma1 hreg1
    have hetaInt0 := intervalIntegrable_lRegularizedLagrangian_of_contMDiffOn_one (I := I) S hS.smoothMetric
      ⟨hS.scalarCont⟩ T a c hac.le eta hetaHead hreg0
    have hetaInt1 := intervalIntegrable_lRegularizedLagrangian_of_contMDiffOn_one (I := I) S hS.smoothMetric
      ⟨hS.scalarCont⟩ T c b hcb.le eta hetaTail hreg1
    have hgammaAdd := lRegularizedAction_add (I := I) S T gamma a c b
      hgammaInt0 hgammaInt1
    have hetaAdd := lRegularizedAction_add (I := I) S T eta a c b
      hetaInt0 hetaInt1
    have hetaEq0 : lRegularizedAction S T eta a c =
        lRegularizedAction S T (f0 u) a c := by
    ...
    have hetaEq1 : lRegularizedAction S T eta c b =
        lRegularizedAction S T (f1 u) c b := by
      apply lRegularizedAction_congr (I := I) S T
      intro s hs
  ...
  have hfirst0 := hasDerivAt_lRegularizedAction (I := I) S hS T f0 hf0 a c hreg0
  have hfirst1 := hasDerivAt_lRegularizedAction (I := I) S hS T f1 hf1 c b hreg1
  have hfirst : HasDerivAt L 0 0 := by
    have hraw := hfirst0.add hfirst1
    exact hraw.congr_deriv (hLmin.hasDerivAt_eq_zero hraw)
  have hsecond := hasDerivAt_deriv_piecewise_lRegularizedAction (I := I) S hS T f0 f1 hf0 hf1
  ...
  rw [hcenter0, hcenter1,
    lRegularizedIndex_congr_of_eqOn (I := I) S T gamma
      (fun s : Real ↦ lVelocity (I := I) (fun u : Real ↦ f0 u s) 0)
      (fun s : Real ↦ lVelocity (I := I) (fun u : Real ↦ f0 u s) 0)
      Y0 Y0 a c hEq0 hEq0,
    lRegularizedIndex_congr_of_eqOn (I := I) S T gamma
