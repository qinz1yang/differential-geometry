...
theorem parabolicNablaRmNormSq
    ...
    nablaKRm04NormSqIntrinsic
      ...
      (R⁻¹) ^ 3 *
        nablaKRm04NormSqIntrinsic S 1 (parabolicTime τ R s) x := by
  ...
  have hrm :
      (parabolicSolution (I := I) S τ R hR hτ).base.rm04 s =
        R • S.base.rm04 (parabolicTime τ R s) := by
    apply DFunLike.ext
    intro y
    exact parabolicSolution_rm04 (I := I) S τ R hR hτ s y
  have hfield :
      nablaKRm04Field (I := I)
          (parabolicSolution (I := I) S τ R hR hτ) s 1 x =
        R • nablaKRm04Field (I := I) S (parabolicTime τ R s) 1 x := by
    change
      totalNabla0SFun (𝕜 := Real) (E := E) (H := H) (I := I) (M := M) 4
        ...
        R • totalNabla0SFun
          ...
          (S.base.rm04 (parabolicTime τ R s)) x
    rw [hconn, hrm]
    exact totalNabla0SFun_smul
      (S.family.connection (parabolicTime τ R s)) R
      (S.base.rm04 (parabolicTime τ R s)) x
