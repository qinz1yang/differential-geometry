...
omit [SigmaCompactSpace M] in
theorem sum_orderedSectionalCurvaturesAt_basis_independent
    {D : RealTimeInterval} (S : SolutionOn (I := I) (M := M) D)
    {t : Real} {x : M}
...
omit [SigmaCompactSpace M] in
theorem scalar_ge_two_mul_negative_sectional_of_convex_barrier
    {D : RealTimeInterval} (S : SolutionOn (I := I) (M := M) D)
    {t t0 K : Real} {x : M}
    (basis : Module.Basis (Fin 3) Real (TangentSpace I x))
    (horth : OrthonormalBasisAt (I := I) (S.base.metric t) x basis)
...
theorem sectional_curvature_asymptotic_pinching_of_barrier
    {D : RealTimeInterval} (S : SolutionOn (I := I) (M := M) D)
...
theorem scalar_curvature_lower_bound
  ...
  scalar_curvature_lower_bound_of_curvature_operator_lower_bound
...
omit [SigmaCompactSpace M] in
theorem scalar_curvature_lower_and_negative_barrier_of_curvatureOperatorRegionPropagationOn
    {D : RealTimeInterval} (S : SolutionOn (I := I) (M := M) D)
    {t0 T K : Real}
  ...
  · intro t ht x hneg
    ...
    have hsectionalNeg : orderedSectionalCurvaturesAt (I := I) x basis A 2 < 0 := by
      rw [← hνeq]
      simpa [A] using hneg
    have hmain := scalar_ge_two_mul_negative_sectional_of_convex_barrier
      (I := I) (M := M) S basis horth hsectionalNeg hbar'
    simpa [A, hνeq] using hmain

omit [SigmaCompactSpace M] in
theorem hamilton_ivey_pinching_of_curvatureOperatorRegionPropagationOn
    {D : RealTimeInterval} (S : SolutionOn (I := I) (M := M) D)
    {t0 T K : Real}
    (hprop : curvatureOperatorRegionPropagationOn (I := I) (M := M) S K t0 T) :
    (∀ t : Real, t ∈ Set.Icc t0 (t0 + T) → ∀ x : M,
  ...
  scalar_curvature_lower_and_negative_barrier_of_curvatureOperatorRegionPropagationOn
    (I := I) (M := M) S hprop

omit [SigmaCompactSpace M] in
theorem hamilton_ivey_asymptotic_pinching_of_curvatureOperatorRegionPropagationOn
    {D : RealTimeInterval} (S : SolutionOn (I := I) (M := M) D)
  ...
  simpa [A] using sectional_curvature_asymptotic_pinching_of_barrier
    (I := I) (M := M) S (K := K) (δ := δ) (ν :=
      leastCurvatureOperatorEigenvalueAt (I := I) (S.base.metric t) x A)
      hK hδ hτ hbarrier

omit [SigmaCompactSpace M] in
theorem hamilton_ivey_pinching_k_one_of_curvatureOperatorRegionPropagationOn
    {D : RealTimeInterval} (S : SolutionOn (I := I) (M := M) D)
    {t0 T : Real}
  ...
  have hmain := hamilton_ivey_pinching_of_curvatureOperatorRegionPropagationOn
    (I := I) (M := M) S (K := 1) hprop
  constructor
  · intro t ht x
