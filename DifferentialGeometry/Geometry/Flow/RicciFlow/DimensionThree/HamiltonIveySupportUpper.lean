...
omit [SigmaCompactSpace M] [T2Space M] [IsManifold I 2 M] in
theorem hamiltonIveySupportUpperReactBlockPoly_not_nonneg_of_scalar_lower :
    ¬ (∀ a C l0 l1 l2 : Real, 0 < a -> 0 < C ->
      Real.exp (a + 2) = 1000 ->
      C + (1 + a) / (2 * a) * (l0 + l1 + l2) ≤ l0 ->
      l1 ≤ l0 -> l2 ≤ l0 ->
      -6 * (a * C / Real.exp (a + 2)) ≤ l0 + l1 + l2 ->
      0 ≤ hamiltonIveySupportUpperReactBlockPoly a C (2 * a * C / Real.exp (a + 2))
        (l0 - l1) (l0 - l2) 0) := by
  intro h
  let a : Real := Real.log 1000 - 2
  have hExp2 : Real.exp 2 < 1000 := by
  ...
  have hexp : Real.exp (a + 2) = 1000 := by
    ...
    rw [show Real.log 1000 - 2 + 2 = Real.log 1000 by ring]
    rw [Real.exp_log (by norm_num : (0:ℝ) < 1000)]
  let l0 : Real := 10061 / 10
  let l1 : Real := 988
  let l2 : Real := -1984
  have hl0 : 1000 + ((1 + a) / (2 * a)) * (l0 + l1 + l2) ≤ l0 := by
    dsimp [l0, l1, l2]
    have ha_ne : a ≠ 0 := ne_of_gt ha
    field_simp [ha_ne]
    ring_nf
  ...
  have hl20 : l2 ≤ l0 := by
    dsimp [l0, l2]
    norm_num
  have hscalar : -6 * (a * 1000 / Real.exp (a + 2)) ≤ l0 + l1 + l2 := by
    rw [hexp]
    dsimp [l0, l1, l2]
    norm_num
    nlinarith [ha49]
  have hle := h a 1000 l0 l1 l2 ha (by norm_num) hexp hl0 hl10 hl20 hscalar
  have hpoly_lt : hamiltonIveySupportUpperReactBlockPoly a 1000
      (2 * a * 1000 / Real.exp (a + 2)) (l0 - l1) (l0 - l2) 0 < 0 := by
    rw [hexp]
    dsimp [l0, l1, l2]
    have ha_ne : a ≠ 0 := ne_of_gt ha
    have ha3 : a + 3 ≠ 0 := by nlinarith [ha]
    unfold hamiltonIveySupportUpperReactBlockPoly
    field_simp [ha_ne, ha3]
    ring_nf
    nlinarith [ha49]
  exact (not_le_of_gt hpoly_lt) hle
