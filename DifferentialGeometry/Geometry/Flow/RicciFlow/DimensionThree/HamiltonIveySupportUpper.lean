...
private lemma hamiltonIveyExpQuarticLower
    {a : ℝ} (ha : 0 < a) :
    Real.exp (a + 2) * (4 * a + 3) / 4 ≥ a ^ 2 * (a + 3) := by
  have hx : 0 ≤ a + 2 := by positivity
  have hsum := Real.sum_le_exp_of_nonneg hx 5
  have hpoly : 1 + (a + 2) + (a + 2)^2 / 2 + (a+2)^3 / 6 + (a+2)^4 / 24 ≤ Real.exp (a + 2) := by
    norm_num [Finset.sum_range_succ] at hsum ⊢
    exact hsum
  have hpoly_mul : (1 + (a + 2) + (a + 2)^2 / 2 + (a+2)^3 / 6 + (a+2)^4 / 24) * (4*a+3)/4 ≥ a^2*(a+3) := by
    have hdiff : (1 + (a + 2) + (a + 2)^2 / 2 + (a+2)^3 / 6 + (a+2)^4 / 24) * (4*a+3)/4 - a^2*(a+3)
        = (4*a^5 + 51*a^4 + 180*a^3 + 500*a^2 + 1128*a + 504)/96 := by
      ring
    have ha_nonneg : 0 ≤ a := le_of_lt ha
    have hnonneg : 0 ≤ (4*a^5 + 51*a^4 + 180*a^3 + 500*a^2 + 1128*a + 504)/96 := by
      nlinarith [sq_nonneg a, sq_nonneg (a^2), sq_nonneg (a^3)]
    linarith
  have hmul := mul_le_mul_of_nonneg_left hpoly (by positivity : 0 ≤ (4*a+3)/4)
  nlinarith

private lemma hamiltonIveyExpAddOneLower
    {a : ℝ} : a + 3 ≤ Real.exp (a + 2) := by
  have := Real.add_one_le_exp (a + 2)
  linarith

private lemma hamiltonIveySupportUpperReactMq_nonneg
    {a q x y z E : ℝ}
    (ha : 0 < a) (hq : 0 < q) (hE : E = Real.exp (a + 2))
    (hxy : x * y ≥ z ^ 2) :
    0 ≤ (E^2 * (a + 1)^2 * q^2 - E * a^2 * (a + 3) * q^2 - E * (4*a^2 + 2*a) * q * (x + y)
      + a^2 * (a+1)^2 * (x+y)^2 + a^2 * (3 - 2*a - a^2) * (x-y)^2
      + 4*a^2 * (3 - 2*a - a^2) * z^2) := by
  have hEpos : 0 < E := by rw [hE]; exact Real.exp_pos _
  have hqnonneg : 0 ≤ q := le_of_lt hq
  let u : ℝ := x + y
  let v : ℝ := x - y
  let w : ℝ := z
  have hconstr : u^2 - v^2 - 4 * w^2 ≥ 0 := by
    dsimp [u, v, w]
    nlinarith
  have hmain : 0 ≤ E^2 * (a + 1)^2 * q^2 - E * a^2 * (a + 3) * q^2 - E * (4*a^2 + 2*a) * q * u
      + a^2 * (a+1)^2 * u^2 + a^2 * (3 - 2*a - a^2) * v^2
      + 4*a^2 * (3 - 2*a - a^2) * w^2 := by
    by_cases ha1 : 1 ≤ a
    · have hB_nonpos : a^2 * (3 - 2*a - a^2) ≤ 0 := by
        have h : 3 - 2*a - a^2 ≤ 0 := by nlinarith [sq_nonneg (a - 1)]
        exact mul_nonpos_of_nonneg_of_nonpos (sq_nonneg a) h
      have hC_nonpos : 4 * (a^2 * (3 - 2*a - a^2)) ≤ 0 := by nlinarith
      have hconst : 0 ≤ E^2 * (4*a + 3) / 4 - E * a^2 * (a + 3) := by
        have h := hamiltonIveyExpQuarticLower ha
        have hmul := mul_le_mul_of_nonneg_left h (le_of_lt (Real.exp_pos (a + 2)))
        have h' : 0 ≤ Real.exp (a + 2)^2 * (4*a + 3) / 4 - Real.exp (a + 2) * a^2 * (a + 3) := by
          nlinarith
        rw [← hE] at h'
        exact h'
      have hquad : 0 ≤ 4*a^2 * (u - q * E * (4*a^2 + 2*a) / (8*a^2))^2 := by positivity
      have hid : E^2 * (a + 1)^2 * q^2 - E * a^2 * (a + 3) * q^2 - E * (4*a^2 + 2*a) * q * u + 4*a^2*u^2
          = q^2 * (E^2 * (4*a + 3) / 4 - E * a^2 * (a + 3)) + 4*a^2 * (u - q * E * (4*a^2 + 2*a) / (8*a^2))^2 := by
        field_simp [show (8*a^2) ≠ 0 by positivity]
        ring
      have hbase : 0 ≤ E^2 * (a + 1)^2 * q^2 - E * a^2 * (a + 3) * q^2 - E * (4*a^2 + 2*a) * q * u + 4*a^2*u^2 := by
        rw [hid]
        positivity
      have hrel : E^2 * (a + 1)^2 * q^2 - E * a^2 * (a + 3) * q^2 - E * (4*a^2 + 2*a) * q * u
          + a^2 * (a+1)^2 * u^2 + a^2 * (3 - 2*a - a^2) * v^2
          + 4*a^2 * (3 - 2*a - a^2) * w^2 -
          (E^2 * (a + 1)^2 * q^2 - E * a^2 * (a + 3) * q^2 - E * (4*a^2 + 2*a) * q * u + 4*a^2*u^2)
          = a^2 * (a - 1) * (a + 3) * (u^2 - v^2 - 4*w^2) := by
        ring
      nlinarith
    · have ha_le : a ≤ 1 := le_of_not_ge ha1
      have hB_nonneg : 0 ≤ a^2 * (3 - 2*a - a^2) := by
        have h : 0 ≤ 3 - 2*a - a^2 := by nlinarith [sq_nonneg (1 - a)]
        exact mul_nonneg (sq_nonneg a) h
      have hC_nonneg : 0 ≤ 4 * (a^2 * (3 - 2*a - a^2)) := by positivity
      have hEge : a + 3 ≤ E := by rw [hE]; exact hamiltonIveyExpAddOneLower
      have hratio1 : (a^2 + 4*a + 2) / (a + 1)^2 ≥ 1 := by
        have hden : 0 < (a + 1)^2 := by positivity
        rw [ge_iff_le]
        rw [one_le_div hden]
        nlinarith
      have hE_ratio : a + 3 ≤ E * (a^2 + 4*a + 2) / (a + 1)^2 := by
        have hden : 0 < (a + 1)^2 := by positivity
        rw [le_div_iff₀ hden]
        have h1 : (a + 3) * (a + 1)^2 ≤ (a + 3) * (a^2 + 4*a + 2) := by
          nlinarith
        have h2 : (a + 3) * (a^2 + 4*a + 2) ≤ E * (a^2 + 4*a + 2) := by
          exact mul_le_mul_of_nonneg_right hEge (by nlinarith)
        nlinarith
      have hmin2 : 0 ≤ E^2 * ((a+1)^2 - (2*a+1)^2 / (a+1)^2) - E * a^2 * (a + 3) := by
        have hfac : (a+1)^2 - (2*a+1)^2 / (a+1)^2 = a^2 * (a^2 + 4*a + 2) / (a+1)^2 := by
          have hden : (a+1)^2 ≠ 0 := by positivity
          field_simp [hden]
          ring
        have hmain2 : 0 ≤ E * (a^2 + 4*a + 2) / (a+1)^2 - (a + 3) := by linarith
        have hrewrite : E^2 * ((a+1)^2 - (2*a+1)^2 / (a+1)^2) - E * a^2 * (a + 3)
            = E * (E * (a^2 + 4*a + 2) / (a+1)^2 - (a + 3)) * a^2 := by
          rw [hfac]
          field_simp [show (a+1)^2 ≠ 0 by positivity]
        rw [hrewrite]
        positivity
      have hquad : 0 ≤ a^2 * (a+1)^2 * (u - q * E * (4*a^2 + 2*a) / (2 * a^2 * (a+1)^2))^2 := by positivity
      have hid : E^2 * (a + 1)^2 * q^2 - E * a^2 * (a + 3) * q^2 - E * (4*a^2 + 2*a) * q * u
          + a^2 * (a+1)^2 * u^2
          = q^2 * (E^2 * ((a+1)^2 - (2*a+1)^2 / (a+1)^2) - E * a^2 * (a + 3))
            + a^2 * (a+1)^2 * (u - q * E * (4*a^2 + 2*a) / (2 * a^2 * (a+1)^2))^2 := by
        field_simp [show (2 * a^2 * (a+1)^2) ≠ 0 by positivity]
        ring
      have hbase : 0 ≤ E^2 * (a + 1)^2 * q^2 - E * a^2 * (a + 3) * q^2 - E * (4*a^2 + 2*a) * q * u
          + a^2 * (a+1)^2 * u^2 := by
        rw [hid]
        positivity
      have hrel : E^2 * (a + 1)^2 * q^2 - E * a^2 * (a + 3) * q^2 - E * (4*a^2 + 2*a) * q * u
          + a^2 * (a+1)^2 * u^2 + a^2 * (3 - 2*a - a^2) * v^2
          + 4*a^2 * (3 - 2*a - a^2) * w^2 -
          (E^2 * (a + 1)^2 * q^2 - E * a^2 * (a + 3) * q^2 - E * (4*a^2 + 2*a) * q * u + a^2 * (a+1)^2 * u^2)
          = a^2 * (3 - 2*a - a^2) * (v^2 + 4*w^2) := by
        ring
      nlinarith
  dsimp [u, v, w] at hmain
  exact hmain

theorem hamiltonIveySupportUpperReactBlockPoly_nonneg_of_psd_null
    {a C q x y z E : ℝ}
    (ha : 0 < a) (hq : 0 < q) (hE : E = Real.exp (a + 2))
    (hC : C = q * E / (2 * a))
    (hxy : x * y ≥ z ^ 2) :
    0 ≤ hamiltonIveySupportUpperReactBlockPoly a C q (C - x) (C - y) (-z) := by
  have hnum := hamiltonIveySupportUpperReactMq_nonneg (a := a) (q := q) (x := x) (y := y)
    (z := z) (E := E) ha hq hE hxy
  have hden : 0 < 2 * a^3 * (a + 3) := by positivity
  have hpoly_eq : hamiltonIveySupportUpperReactBlockPoly a C q (C - x) (C - y) (-z)
      = (E^2 * (a + 1)^2 * q^2 - E * a^2 * (a + 3) * q^2 - E * (4*a^2 + 2*a) * q * (x + y)
            + a^2 * (a+1)^2 * (x+y)^2 + a^2 * (3 - 2*a - a^2) * (x-y)^2
            + 4*a^2 * (3 - 2*a - a^2) * z^2) / (2 * a^3 * (a + 3)) := by
    unfold hamiltonIveySupportUpperReactBlockPoly
    rw [hC]
    field_simp [ha.ne', show (a + 3) ≠ 0 by nlinarith]
    ring_nf
  rw [hpoly_eq]
