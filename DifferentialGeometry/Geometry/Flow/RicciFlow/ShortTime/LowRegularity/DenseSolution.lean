...
def deTurckRemainderOnLowerState
...
theorem deTurckRemainderOnSmoothCore_outer_bound
    (hDim : Module.finrank ℝ E = 3)
  ...
  have hyrep : coreRep g₀ yQ = coreRep g₀ y := by
    ...
    rw [coreRep_spec, coreRep_spec]
  have hxN :
      deTurckRemainderOnSmoothCore (I := I) (M := M) g₀ g_bg hδ₀_lt hrealQ xQ =
        deTurckRemainderOnSmoothCore (I := I) (M := M) g₀ g_bg hδ₀_lt
...
theorem deTurckRemainderOnLowerState_outer_bound
    (hDim : Module.finrank ℝ E = 3)
    (g₀ g_bg : SmoothRiemannianMetric I M)
    {δ₀ : ℝ} (hδ₀_nonneg : 0 ≤ δ₀) (hδ₀_lt : δ₀ < 1) :
    ∃ ρ Ctop : ℝ, ∃ B0 B1 : ℝ → ℝ,
      ...
      ∀ {Q R : ℝ} (_hQ : 0 ≤ Q) (_hQρ : Q ≤ ρ)
        ...
        (hrealQ : ∀ T : SmoothCcTensor g₀ 0 2,
          ‖smoothCcToTensorHs (I := I) (M := M) g₀ (((1 : ℕ) : ℝ) + 1) T‖ ≤ Q →
            metricCauchySchwarzBound (I := I) (M := M) g₀
              (ccTensorBilinSymm (I := I) g₀ T) δ₀),
        Continuous (deTurckRemainderOnLowerState (I := I) (M := M) g₀ g_bg hR hδ₀_lt
          (realizeOfLE (I := I) (M := M) g₀ hRQ hrealQ)) ∧
        Continuous (deTurckRemainderOnSmoothCore (I := I) (M := M) g₀ g_bg hδ₀_lt
          (realizeOfLE (I := I) (M := M) g₀ hRQ hrealQ)) ∧
...
structure LowRegularityTimeSolution
...
theorem exists_lowRegularity_partial_solution
    (hDim : Module.finrank ℝ E = 3)
    (g₀ g_bg : SmoothRiemannianMetric I M) :
    Nonempty (LowRegularityPartialSolution (I := I) (M := M) g₀ g_bg) := by
  let θ : ℝ := deTurckRemainderContractionThreshold (Module.finrank ℝ E)
  have hθ : 0 < θ := de_turck_remainder_contraction_threshold_pos (Module.finrank ℝ E)
  have hθlt : θ < 1 :=
    de_turck_remainder_contraction_threshold_lt_one_of_ne_zero (Module.finrank ℝ E)
  obtain ⟨ρ, Ctop, B0, B1, hρ, hCtop, hB0, hB1, houter⟩ :=
    deTurckRemainderOnLowerState_outer_bound (I := I) (M := M) hDim g₀ g_bg hθ.le hθlt
  ...
  have hcorecont : Continuous
      (deTurckRemainderOnSmoothCore (I := I) (M := M) g₀ g_bg hθlt hrealR) := by
