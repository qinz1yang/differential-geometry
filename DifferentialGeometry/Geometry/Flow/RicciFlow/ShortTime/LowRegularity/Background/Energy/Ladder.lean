...
def HasGalerkinApproximationEnergyFiveBoundBackground (g₀ g_bg : SmoothRiemannianMetric I M)
    ...
    (solution : MaximalRegularitySolutionSpace (I := I) (M := M) (g := g₀) (r := 0) (s := 2)
      ((1 : ℕ) : ℝ) T)
    (fLo : timeL2 (TensorHs (I := I) (M := M) g₀ 0 2 ((1 : ℕ) : ℝ)) T)
    (hsol : IsBackgroundLowRegularitySolutionAt (I := I) (M := M) g₀ g_bg K hT hT1 solution fLo Rcap)
...
theorem exists_galerkin_approximation_energy_five_bound_background
    ...
    (hlo : IsAdaptedBackgroundLowRegularitySolution (I := I) (M := M) g₀ g_bg K hT hT1
      solution fLo Rcap Ctop₂ Kr2 Kr1 Kcap) :
    ∃ (fseq : ℕ → timeL2 (TensorHs (I := I) (M := M) g₀ 0 2 ((1 : ℕ) : ℝ)) T),
      HasGalerkinApproximationEnergyFiveBoundBackground (I := I) (M := M) g₀ g_bg K solution fLo
  ...
  obtain ⟨fseq, _hconv, hmode, hpack⟩ :=
    exists_galerkin_projected_forcing_sequence_with_mode_convergence_background (I := I) (M := M) g₀ g_bg K hT hT1 solution fLo hsol
  ...
  have hstate : 0 ≤ lowRegularityStateRadius K.top K.slope K.outer K.realize :=
    (lowRegularityStateRadius_pos K.top_nonneg K.slope_nonneg K.outer_pos K.realize_pos).le
  have hdom3 := energyLadder_absorption_coefficient_le hA3 hB3 hsol.threshold_nonneg hstate
  have habs3 :
  ...
  obtain ⟨Φ3, hE3⟩ := exists_uniform_galerkin_energy_three_bound_of_integral_bound_background (I := I) (M := M) g₀ g_bg hT
    K.threshold_lt hsol.threshold_nonneg hsol.threshold_le_third K.top_nonneg K.base_nonneg K.slope_nonneg K.outer_pos K.realize_pos
    hsol.metric_realization hsol.smoothCore_continuous hsol.remainder_lipschitz fseq (fun N => (hpack N).2.1)
  ...
  have hUderiv : ∀ N, ∀ t ∈ Set.Ico (0 : ℝ) T,
      ...
      HasDerivWithinAt (fun s => U N s i)
        (-(TensorEigenIdx.lambda (I := I) (M := M) i) * U N t i +
          galerkinTameForce (I := I) (M := M) g₀ 1 hstate
            (boundedDeTurckRemainderOnLowerState (I := I) (M := M) g₀ g_bg K.threshold_lt K.top_nonneg K.slope_nonneg
              K.outer_pos K.realize_pos hsol.metric_realization)
    ...
    · exact galerkinProjectedForce_mode_continuous (I := I) (M := M) g₀ g_bg K.threshold_lt K.top_nonneg K.base_nonneg
        K.slope_nonneg K.outer_pos K.realize_pos hsol.metric_realization hsol.remainder_lipschitz N (U N)
  ...
  have hdom4 := energyLadder_absorption_coefficient_le hA4 hB4 hsol.threshold_nonneg hstate
  have habs4 :
  ...
  obtain ⟨Φ4, hE4⟩ := hord4.2.2.2.2
    ...
    K.threshold_lt hsol.threshold_nonneg hsol.threshold_le_third K.top_nonneg K.slope_nonneg K.outer_pos K.realize_pos
    hsol.metric_realization hsol.smoothCore_continuous hUcont hUderiv hUinit hR3 hE3cap hε habs4
  ...
  have hdom5 := energyLadder_absorption_coefficient_le hA5 hB5 hsol.threshold_nonneg hstate
  have habs5 :
  ...
  obtain ⟨Φ5, hE5⟩ := hord5.2.2.2.2
    ...
    K.threshold_lt hsol.threshold_nonneg hsol.threshold_le_third K.top_nonneg K.slope_nonneg K.outer_pos K.realize_pos
    hsol.metric_realization hsol.smoothCore_continuous hUcont hUderiv hUinit hR3 hE3cap hR4 hE4cap hε habs5
  refine ⟨fseq, ?_⟩
