...
import DifferentialGeometry.Analysis.Parabolic.RicciLinearization.ThreeArm.CorrectionField.TameEnvelope
    ...
    ∃ C : ℝ, 0 ≤ C ∧
      ∀ (Φ : SmoothCcTensor g r c) (W : SmoothCcTensor g p r) (A B : ℝ),
        ...
        (∑ j ∈ Finset.range 2,
          ...
          (C * A * B) ^ 2 := by
  obtain ⟨C, hC, happ⟩ := operator_field_composition_h1_h2_to_h1_bound (I := I) (M := M) hDim g p r c
  refine ⟨C, hC, ?_⟩
      ...
      ∀ (Φ : SmoothCcTensor g r c) (W : SmoothCcTensor g p r) (A B : ℝ),
        ...
        (∑ j ∈ Finset.range 2,
          ‖iteratedCovGrad (I := I) g p c j
            (ccOperatorFieldComp (I := I) (M := M) g p r c Φ W)‖ ^ 2) ≤
          (C * A * B) ^ 2 := by
  obtain ⟨C, hC, happ⟩ := operator_field_composition_h2_h1_to_h1_bound (I := I) (M := M) hDim g p r c
  refine ⟨C, hC, ?_⟩
  intro Φ W A B hA hB hΦ hW
  have hnorm := happ Φ W A B hA hB hΦ hW
  have hsquare := pow_le_pow_left₀
...
theorem slotIter_riemannianFiberNormSq
...
theorem slotIter_l2
  ...
  have hsq := normSq_le_integral_of_pointwise_fiberNormSq_le_rs
    ...
    F hF (fun x => slotIter_riemannianFiberNormSq (I := I) (M := M) g r s w Φ i x)
  have hint : (∫ x,
      riemannianFiberNormSq (I := I) (M := M) g r (s + i) x
        ((iteratedCovGrad (I := I) g r s i Φ).toSection x)
      ∂(riemannianVolumeMeasure (I := I) (M := M) g)) =
      ...
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        ...
        (∑ i ∈ Finset.range 3,
          ‖iteratedCovGrad (I := I) g₀ (p + 2) p i
            (lieCorrectionZeroTr (I := I) (M := M) g₀ g₁ p σ)‖ ^ 2) ≤ (B A) ^ 2 := by
  classical
  obtain ⟨C, hC, hpt⟩ :=
    lieCorrectionZeroTr_pointwise_antidiagonalGrid_le (I := I) (M := M) p g₀ hδ₀
  obtain ⟨B, hB_nn, hB⟩ := h2_of_grid_low (I := I) (M := M)
    (r := p + 2) (s := p) hDim g₀ C hC
  refine ⟨B, hB_nn, ?_⟩
  intro g₁ P htie δ hδ_le hδ_nonneg hbound σ A hA hP
  refine hB P (lieCorrectionZeroTr (I := I) (M := M) g₀ g₁ p σ) A hA hP ?_
  intro i hi x
      ...
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        ...
        (∑ i ∈ Finset.range 3,
          ‖iteratedCovGrad (I := I) g₀ 4 2 i
            (lieCorrectionZeroTr (I := I) (M := M) g₀ g₁ 2 σ)‖ ^ 2) ≤ (B A) ^ 2 := by
...
theorem connLow_h2
      ...
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        ...
        (∑ i ∈ Finset.range 3,
          ‖iteratedCovGrad (I := I) g₀ 0 3 i
            (metricLoweredConnectionDifferenceCoefficient (I := I) g₀ g₁)‖ ^ 2) ≤ (B A) ^ 2 := by
  classical
  obtain ⟨C, hC, hpt⟩ :=
  ...
  refine hB P (metricLoweredConnectionDifferenceCoefficient (I := I) g₀ g₁) A hA hP ?_
  intro i hi x
...
theorem connLow_tame
  ...
  refine hB P (metricLoweredConnectionDifferenceCoefficient (I := I) g₀ g₁) R A hR hA hP2 htop ?_
  intro i hi x
  rw [metricLoweredConnectionDifferenceCoefficient_fiber_norm_sq_eq (I := I) (M := M) g₀ g₁ i x]
  simpa only [lowJetGrid, Combinatorics.antidiagonalTupleGrid] using
    hpt g₁ P htie hδ_le hδ_nonneg hbound i x
...
theorem kappaSelf_h1
  ...
  classical
  rw [lieCorrectionZeroKappa_self_eq_koszulCovecCc (I := I) (M := M) g₀ g₁ P htie]
...
theorem kappaSelf_h2
    ...
    (∑ i ∈ Finset.range 3,
      ‖iteratedCovGrad (I := I) g₀ 0 3 i
        (lieCorrectionZeroKappa (I := I) (M := M) g₀ g₁ g₀)‖ ^ 2) ≤ (4 * A) ^ 2 := by
  classical
  rw [lieCorrectionZeroKappa_self_eq_koszulCovecCc (I := I) (M := M) g₀ g₁ P htie]
...
theorem kappa_self_pair_h2
    ...
    (∑ i ∈ Finset.range 3,
        ‖iteratedCovGrad (I := I) g 0 3 i
          (lieCorrectionZeroKappa (I := I) (M := M) g gT g -
            lieCorrectionZeroKappa (I := I) (M := M) g gU g)‖ ^ 2) ≤
      10 * (∑ j ∈ Finset.range 4,
  ...
  have hsub :
    ...
    rw [lieCorrectionZeroKappa_self_eq_koszulCovecCc (I := I) (M := M) g gT T hTtie,
      lieCorrectionZeroKappa_self_eq_koszulCovecCc (I := I) (M := M) g gU U hUtie,
...
theorem pbLow_h2
    ...
    ∃ B : ℝ → ℝ,
      ...
      ∀ (P : SmoothCcTensor g₀ 0 2) (A : ℝ), 0 ≤ A →
        ...
        (∑ i ∈ Finset.range 3,
          ‖iteratedCovGrad (I := I) g₀ 0 3 i
            (lieCorrectionZeroPbLow (I := I) (M := M) g₀ P g₀ gB)‖ ^ 2) ≤ (B A) ^ 2 := by
  classical
  obtain ⟨C, hC, hprod⟩ := operator_field_composition_h2_h2_to_h2_bound
    (I := I) (M := M) hDim g₀ 1 1 2
  let SF : ℝ := ∑ j ∈ Finset.range 3,
    ‖iteratedCovGrad (I := I) g₀ 1 2 j
      (lieCorrectionZeroFixCd (I := I) (M := M) g₀ gB)‖ ^ 2
  ...
  have hout := hprod (lieCorrectionZeroFixCd (I := I) (M := M) g₀ gB) W AF A
      ...
      ∑ i ∈ Finset.range 3,
        ‖iteratedCovGrad (I := I) g₀ 1 2 i
          (ccOperatorFieldComp (I := I) (M := M) g₀ 1 1 2
            (lieCorrectionZeroFixCd (I := I) (M := M) g₀ gB) W)‖ ^ 2 := by
    apply Finset.sum_congr rfl
    intro i hi
    simpa only [W] using
    ...
    ∃ C : ℝ, 0 ≤ C ∧
      ∀ (P : SmoothCcTensor g₀ 0 2) (A : ℝ), 0 ≤ A →
        ...
        (∑ i ∈ Finset.range 3,
          ...
          (C * A) ^ 2 := by
  classical
  obtain ⟨Ca, hCa, hprod⟩ := operator_field_composition_h2_h2_to_h2_bound
    (I := I) (M := M) hDim g₀ 1 1 2
  let SF : ℝ := ∑ j ∈ Finset.range 3,
    ‖iteratedCovGrad (I := I) g₀ 1 2 j
      (lieCorrectionZeroFixCd (I := I) (M := M) g₀ gB)‖ ^ 2
...
theorem kappaBackground_h1
    ...
    ∃ B : ℝ → ℝ,
      ...
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        ...
        (∑ i ∈ Finset.range 2,
          ‖iteratedCovGrad (I := I) g₀ 0 3 i
            (lieCorrectionZeroKappa (I := I) (M := M) g₀ g₁ gB)‖ ^ 2) ≤ (B R) ^ 2 := by
  classical
  obtain ⟨BP, hBP_nn, hBP⟩ := pbLow_h2 (I := I) (M := M) hDim g₀ gB
  let SF : ℝ := ∑ i ∈ Finset.range 2,
    ‖iteratedCovGrad (I := I) g₀ 0 3 i
      (metricLoweredConnectionDifferenceCoefficient (I := I) g₀ gB)‖ ^ 2
  ...
  have hpb : (∑ i ∈ Finset.range 2,
      ‖iteratedCovGrad (I := I) g₀ 0 3 i
        (lieCorrectionZeroPbLow (I := I) (M := M) g₀ P g₀ gB)‖ ^ 2) ≤ (BP R) ^ 2 := by
  ...
  have hfactor :
      (∑ i ∈ Finset.range 2,
        ...
        3 * ((∑ i ∈ Finset.range 2,
          ‖iteratedCovGrad (I := I) g₀ 0 3 i
            (lieCorrectionZeroKappa (I := I) (M := M) g₀ g₁ g₀)‖ ^ 2) + SF +
          (∑ i ∈ Finset.range 2,
          ‖iteratedCovGrad (I := I) g₀ 0 3 i
            (lieCorrectionZeroPbLow (I := I) (M := M) g₀ P g₀ gB)‖ ^ 2)) := by
  ...
  have hagg : 3 * ((∑ i ∈ Finset.range 2,
        ...
        (∑ i ∈ Finset.range 2,
          ‖iteratedCovGrad (I := I) g₀ 0 3 i
            (lieCorrectionZeroPbLow (I := I) (M := M) g₀ P g₀ gB)‖ ^ 2)) ≤ Q R := by
    dsimp only [Q]
...
theorem kappaBackground_tame
    ...
    ∃ B : ℝ → ℝ → ℝ,
      ...
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        ...
        (∑ i ∈ Finset.range 3,
          ‖iteratedCovGrad (I := I) g₀ 0 3 i
            (lieCorrectionZeroKappa (I := I) (M := M) g₀ g₁ gB)‖ ^ 2) ≤ (B R A) ^ 2 := by
  classical
  obtain ⟨BP, hBP_nn, hBP⟩ := pbLow_h2 (I := I) (M := M) hDim g₀ gB
  ...
  have hagg : 3 * ((∑ i ∈ Finset.range 3,
        ...
        (∑ i ∈ Finset.range 3,
          ‖iteratedCovGrad (I := I) g₀ 0 3 i
            (lieCorrectionZeroPbLow (I := I) (M := M) g₀ P g₀ gB)‖ ^ 2)) ≤ Q R A := by
    dsimp only [Q]
    refine mul_le_mul_of_nonneg_left ?_ (by norm_num)
    nlinarith
  refine hsum.trans (hagg.trans ?_)
...
theorem kappaBackground_h2
    ...
    ∃ B : ℝ → ℝ,
      ...
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        ...
        (∑ i ∈ Finset.range 3,
          ‖iteratedCovGrad (I := I) g₀ 0 3 i
            (lieCorrectionZeroKappa (I := I) (M := M) g₀ g₁ gB)‖ ^ 2) ≤ (B A) ^ 2 := by
  obtain ⟨Bt, hBt_nn, hBt⟩ := kappaBackground_tame (I := I) (M := M) hDim g₀ gB
...
theorem linearizedRicciConnectionDifferenceOrder0CoeffField_h1_bound
    (hDim : Module.finrank ℝ E = 3)
    (g₀ : SmoothRiemannianMetric I M) {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ B : ℝ → ℝ,
      (∀ A : ℝ, 0 ≤ A → 0 ≤ B A) ∧
...
theorem exists_deTurckLieConnectionDifferenceDerivativeCoefficient_covariantJetNormSq_one_bound
    (hDim : Module.finrank ℝ E = 3)
...
theorem exists_deTurckLieCovariantDerivativeInsertion_backgroundDifference_covariantJetNormSq_one_bound
    (hDim : Module.finrank ℝ E = 3)
    (g₀ g_bg : SmoothRiemannianMetric I M)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ B : ℝ → ℝ,
...
theorem lieCorrectionZeroRiemann_h1_bound
    (hDim : Module.finrank ℝ E = 3)
    (g₀ : SmoothRiemannianMetric I M) {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ B : ℝ → ℝ,
      (∀ A : ℝ, 0 ≤ A → 0 ≤ B A) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        ...
        (∑ i ∈ Finset.range 2,
          ‖iteratedCovGrad (I := I) g₀ 2 2 i
            (lieCorrectionZeroRiemann (I := I) (M := M) g₀ g₁)‖ ^ 2) ≤ (B A) ^ 2 := by
  classical
  ...
  have hRf : (∑ j ∈ Finset.range 3,
      ‖iteratedCovGrad (I := I) g₀ 2 4 j
        (lieCorrectionZeroRiemRestField (I := I) (M := M) g₀)‖ ^ 2) ≤ Br ^ 2 := by
  ...
  have hTr : (∑ i ∈ Finset.range 3,
      ‖iteratedCovGrad (I := I) g₀ 4 2 i
        (lieCorrectionZeroTr (I := I) (M := M) g₀ g₁ 2 lieCorrectionZeroRiemPerm2)‖ ^ 2) ≤
      (Bt A) ^ 2 :=
    htrace g₁ P htie hδ_le hδ_nonneg hbound
      lieCorrectionZeroRiemPerm2 A hA hP3
  have hApp := happ
  ...
  have hAppJet : (∑ i ∈ Finset.range 2,
    ...
    calc
      ...
      _ ≤ (Cp * Bt A * Br) ^ 2 := hApp
      _ = (B A) ^ 2 := by rfl
  rw [lieCorrectionZeroRiemann_eq_field (I := I) (M := M) g₀ g₁]
  simpa only [lieCorrectionZeroRiemannField, iteratedCovGrad_smul_real, norm_smul,
    Real.norm_eq_abs, abs_neg, abs_one, one_mul] using hAppJet

theorem lieCorrectionZeroVectorBundleField_h1_bound
    (hDim : Module.finrank ℝ E = 3)
    (g₀ : SmoothRiemannianMetric I M) {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ B : ℝ → ℝ → ℝ,
      (∀ R : ℝ, 0 ≤ R → ∀ A : ℝ, 0 ≤ A → 0 ≤ B R A) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        ...
        (∑ i ∈ Finset.range 2,
          ‖iteratedCovGrad (I := I) g₀ 2 2 i
            (lieCorrectionZeroVectorBundleField (I := I) (M := M) g₀ g₁)‖ ^ 2) ≤ (B R A) ^ 2 := by
  classical
  obtain ⟨Bt1, hBt1, htr1⟩ := trace_h2 (I := I) (M := M) 1 hDim g₀ hδ₀
  obtain ⟨Bt2, hBt2, htr2⟩ := trace_h2 (I := I) (M := M) 2 hDim g₀ hδ₀
  obtain ⟨Cv, hCv, hvprod⟩ := operator_field_composition_h2_h2_to_h2_bound
    (I := I) (M := M) hDim g₀ 0 3 1
  obtain ⟨Ci, hCi, hiprod⟩ := operator_field_composition_h2_h2_to_h2_bound
  ...
  let T1 : SmoothCcTensor g₀ 3 1 :=
    lieCorrectionZeroTr (I := I) (M := M) g₀ g₁ 1 (Equiv.refl _)
  ...
  have hT1i : (∑ i ∈ Finset.range 3,
    ...
    simpa only [T1i] using htr1 g₁ P htie hδ_le hδ_nonneg hbound
      lieCorrectionZeroIVPerm R hR hP2
  have hT2 : (∑ i ∈ Finset.range 3,
    ...
    simpa only [T2] using htr2 g₁ P htie hδ_le hδ_nonneg hbound
      lieCorrectionZeroVectorBundleTracePermutation R hR hP2
...
theorem lieCorrectionZeroMixedConnectionField_h1_bound
    (hDim : Module.finrank ℝ E = 3)
    ...
    ∃ B : ℝ → ℝ → ℝ,
      ...
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        ...
        (∑ i ∈ Finset.range 2,
          ‖iteratedCovGrad (I := I) g₀ 2 2 i
            (lieCorrectionZeroMixedConnectionField (I := I) (M := M) g₀ g₁ gB)‖ ^ 2) ≤
          (B R A) ^ 2 := by
  classical
  obtain ⟨Bt2, hBt2, htr2⟩ := trace_h2 (I := I) (M := M) 2 hDim g₀ hδ₀
  obtain ⟨Bt3, hBt3, htr3⟩ := trace_h2 (I := I) (M := M) 3 hDim g₀ hδ₀
  obtain ⟨Bt4, hBt4, htr4⟩ := trace_h2 (I := I) (M := M) 4 hDim g₀ hδ₀
  obtain ⟨BK, hBK, hkbg⟩ := kappaBackground_h1 (I := I) (M := M) hDim g₀ gB
  obtain ⟨Cq, hCq, hqprod⟩ := operator_field_composition_h2_h2_to_h2_bound
  ...
  let T2a : SmoothCcTensor g₀ 4 2 :=
    lieCorrectionZeroTr (I := I) (M := M) g₀ g₁ 2 lieCorrectionZeroMixedConnectionPermutationCycleZeroTwoOne
  let T2b : SmoothCcTensor g₀ 4 2 :=
    lieCorrectionZeroTr (I := I) (M := M) g₀ g₁ 2
  ...
  have hT2b : (∑ i ∈ Finset.range 3,
    ...
    simpa only [T2b] using htr2 g₁ P htie hδ_le hδ_nonneg hbound
      (lieCorrectionZeroSwapOutPerm * lieCorrectionZeroMixedConnectionPermutationCycleZeroTwoOne) R hR hP2
...
theorem lieCorrectionZeroVectorBundle_h1_bound
    (hDim : Module.finrank ℝ E = 3)
    (g₀ : SmoothRiemannianMetric I M) {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ B : ℝ → ℝ → ℝ,
      (∀ R : ℝ, 0 ≤ R → ∀ A : ℝ, 0 ≤ A → 0 ≤ B R A) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        ...
        (∑ i ∈ Finset.range 2,
          ‖iteratedCovGrad (I := I) g₀ 2 2 i
            (lieCorrectionZeroVectorBundle (I := I) (M := M) g₀ g₁)‖ ^ 2) ≤ (B R A) ^ 2 := by
  obtain ⟨B, hB, hform⟩ := lieCorrectionZeroVectorBundleField_h1_bound (I := I) (M := M) hDim g₀ hδ₀
  refine ⟨B, hB, ?_⟩
  intro g₁ P htie δ hδ_le hδ_nonneg hbound R A hR hA hP2 hP3
  rw [lieCorrectionZeroVectorBundle_eq_field (I := I) (M := M) g₀ g₁]
  exact hform g₁ P htie hδ_le hδ_nonneg hbound R A hR hA hP2 hP3

theorem lieCorrectionZeroMixedConnection_h1_bound
    (hDim : Module.finrank ℝ E = 3)
    (g₀ gB : SmoothRiemannianMetric I M) {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ B : ℝ → ℝ → ℝ,
      (∀ R : ℝ, 0 ≤ R → ∀ A : ℝ, 0 ≤ A → 0 ≤ B R A) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        ...
        (∑ i ∈ Finset.range 2,
          ...
          (B R A) ^ 2 := by
  obtain ⟨B, hB, hform⟩ := lieCorrectionZeroMixedConnectionField_h1_bound (I := I) (M := M) hDim g₀ gB hδ₀
  refine ⟨B, hB, ?_⟩
  intro g₁ P htie δ hδ_le hδ_nonneg hbound R A hR hA hP2 hP3
...
theorem deTurckLieLowerOrderTail_decomposition
  ...
  rw [deTurckLieLowerOrderTail_split (I := I) (M := M) g₀ g₁ g_bg,
    tail_base_split (I := I) (M := M) g₀ g₁ g_bg]

omit [NeZero (Module.finrank ℝ E)] in
theorem deTurckLieLowerOrderTail_h1_bound_of_component_bounds
    (g₀ g₁ g_bg : SmoothRiemannianMetric I M)
    (Bd Bi Bv Ba Br : ℝ)
    (hD : (∑ i ∈ Finset.range 2,
      ‖iteratedCovGrad (I := I) g₀ 2 2 i
        (deTurckLieCovariantDerivativeInsertionField (I := I) (M := M) g₀ g₁ g_bg -
          deTurckLieCovariantDerivativeInsertionField (I := I) (M := M) g₀ g₁ g₀)‖ ^ 2) ≤ Bd ^ 2)
    (hI : (∑ i ∈ Finset.range 2,
      ‖iteratedCovGrad (I := I) g₀ 2 2 i
        (lieCorrectionZeroInsertion (I := I) (M := M) g₀ g₁ g_bg -
          lieCorrectionZeroInsertion (I := I) (M := M) g₀ g₁ g₀)‖ ^ 2) ≤ Bi ^ 2)
    (hV : (∑ i ∈ Finset.range 2,
      ‖iteratedCovGrad (I := I) g₀ 2 2 i
        (lieCorrectionZeroVectorBundle (I := I) (M := M) g₀ g₁)‖ ^ 2) ≤ Bv ^ 2)
    (hA : (∑ i ∈ Finset.range 2,
      ‖iteratedCovGrad (I := I) g₀ 2 2 i
        (lieCorrectionZeroMixedConnection (I := I) (M := M) g₀ g₁ g_bg)‖ ^ 2) ≤ Ba ^ 2)
    (hR : (∑ i ∈ Finset.range 2,
      ‖iteratedCovGrad (I := I) g₀ 2 2 i
        (lieCorrectionZeroRiemann (I := I) (M := M) g₀ g₁)‖ ^ 2) ≤ Br ^ 2) :
    (∑ i ∈ Finset.range 2,
      ‖iteratedCovGrad (I := I) g₀ 2 2 i
        (deTurckLieCovariantDerivativeInsertionField (I := I) (M := M) g₀ g₁ g_bg +
  ...
  let Ins : SmoothCcTensor g₀ 2 2 :=
    lieCorrectionZeroInsertion (I := I) (M := M) g₀ g₁ g_bg -
      lieCorrectionZeroInsertion (I := I) (M := M) g₀ g₁ g₀
  let VB : SmoothCcTensor g₀ 2 2 := lieCorrectionZeroVectorBundle (I := I) (M := M) g₀ g₁
  let AM : SmoothCcTensor g₀ 2 2 := lieCorrectionZeroMixedConnection (I := I) (M := M) g₀ g₁ g_bg
  let RF : SmoothCcTensor g₀ 2 2 := lieCorrectionZeroRiemann (I := I) (M := M) g₀ g₁
  ...
  have htail :
    ...
    simpa only [Df, Ins, VB, AM, RF] using
      deTurckLieLowerOrderTail_decomposition (I := I) (M := M) g₀ g₁ g_bg
  have hinside : 0 ≤ 5 *
      (Bd ^ 2 + Bi ^ 2 + Bv ^ 2 + Ba ^ 2 + Br ^ 2) := by positivity
  rw [htail, Real.sq_sqrt hinside]
  exact hsum

theorem exists_rhs_zero_coefficient_sobolev_one_bound
    (hDim : Module.finrank ℝ E = 3)
    (g₀ g_bg : SmoothRiemannianMetric I M)
          ...
          ‖iteratedCovGrad (I := I) g₀ 2 2 i
            (deTurckLieCovariantDerivativeInsertionField (I := I) (M := M) g₀
              ...
              lieCorrectionZeroField (I := I) (M := M) g₀
                (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g_bg)‖ ^ 2) ≤ Lt ^ 2 →
        (∑ i ∈ Finset.range 2,
          ‖iteratedCovGrad (I := I) g₀ 2 2 i
            (ricciDeTurckRemainderZeroOrderCoefficient (I := I) (M := M) g₀ g_bg T T' hδ hδ' s)‖ ^ 2) ≤
          (Real.sqrt (4 *
            (4 * (BR (C * R)) ^ 2 + (BD (C * R)) ^ 2 + Lt ^ 2))) ^ 2 := by
  classical
  obtain ⟨C, hC, hpath⟩ := convex_h3_jet (I := I) (M := M) g₀
  obtain ⟨BR, hBR, hric⟩ := linearizedRicciConnectionDifferenceOrder0CoeffField_h1_bound (I := I) (M := M) hDim g₀ hδ₀_lt
  obtain ⟨BD, hBD, hdla⟩ := exists_deTurckLieConnectionDifferenceDerivativeCoefficient_covariantJetNormSq_one_bound (I := I) (M := M) hDim g₀ g_bg hδ₀_lt
  ...
  have hinside : 0 ≤ 4 *
    ...
    positivity
  have hrhs : ricciDeTurckRemainderZeroOrderCoefficient (I := I) (M := M) g₀ g_bg T T' hδ hδ' s =
      (-2 : ℝ) • Ric + ((DLa + Tail) + (0 : SmoothCcTensor g₀ 2 2)) := by
    simp only [ricciDeTurckRemainderZeroOrderCoefficient, linearizedRicciConnectionDifferenceOrder0Coeff,
...
theorem ricciDeTurckRemainderZeroOrderCoefficient_h1_bound_of_component_bounds
    (g₀ g_bg : SmoothRiemannianMetric I M)
    (T T' : SmoothCcTensor g₀ 0 2)
    {δ δ' : ℝ}
    ...
    (hRic : (∑ i ∈ Finset.range 2,
      ‖iteratedCovGrad (I := I) g₀ 2 2 i
        (linearizedRicciConnectionDifferenceOrder0CoeffField (I := I) (M := M) g₀
          (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s))‖ ^ 2) ≤ Br ^ 2)
    (hDLa : (∑ i ∈ Finset.range 2,
      ‖iteratedCovGrad (I := I) g₀ 2 2 i
        (deTurckLieConnectionDifferenceDerivCoeffField (I := I) (M := M) g₀
          (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g_bg)‖ ^ 2) ≤ Bd ^ 2)
    (hTail : (∑ i ∈ Finset.range 2,
      ‖iteratedCovGrad (I := I) g₀ 2 2 i
        (deTurckLieCovariantDerivativeInsertionField (I := I) (M := M) g₀
            (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g_bg +
          lieCorrectionZeroField (I := I) (M := M) g₀
            (metricPerturbationPath (I := I) g₀ T T' hδ hδ' s) g_bg)‖ ^ 2) ≤ Bt ^ 2) :
    (∑ i ∈ Finset.range 2,
      ‖iteratedCovGrad (I := I) g₀ 2 2 i
        (ricciDeTurckRemainderZeroOrderCoefficient (I := I) (M := M) g₀ g_bg T T' hδ hδ' s)‖ ^ 2) ≤
...
theorem rhs_one_coefficient_sobolev_two_bound
    ...
    (∑ i ∈ Finset.range 3,
      ‖iteratedCovGrad (I := I) g₀ 3 2 i
        (ricciDeTurckRemainderFirstOrderCoefficient (I := I) (M := M) g₀ g_bg T T' hδ hδ' s)‖ ^ 2) ≤
