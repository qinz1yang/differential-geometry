...
theorem ricciDeTurckRemainderZeroOrderCoefficient_h1_bound
  ...
  obtain ⟨Bdlb, _hBdlb, hdlb⟩ :=
    exists_deTurckLieCovariantDerivativeInsertion_backgroundDifference_covariantJetNormSq_one_bound (I := I) (M := M) hDim g₀ g_bg hδ₀_lt
  obtain ⟨Bins, _hBins, hins⟩ :=
    insert_h1 (I := I) (M := M) hDim g₀ g_bg hδ₀_lt
  obtain ⟨Bvb, _hBvb, hvb⟩ := lieCorrectionZeroVectorBundle_h1_bound (I := I) (M := M) hDim g₀ hδ₀_lt
  obtain ⟨Bam, _hBam, ham⟩ := lieCorrectionZeroMixedConnection_h1_bound (I := I) (M := M) hDim g₀ g_bg hδ₀_lt
  obtain ⟨Briem, _hBriem, hriem⟩ := lieCorrectionZeroRiemann_h1_bound (I := I) (M := M) hDim g₀ hδ₀_lt
  obtain ⟨Caux, BRic, BDla, _hCaux, _hBRic, _hBDla, haux⟩ :=
    exists_rhs_zero_coefficient_sobolev_one_bound (I := I) (M := M) hDim g₀ g_bg hδ₀_nonneg hδ₀_lt
  ...
  have htail : (∑ i ∈ Finset.range 2,
      ...
      (L R) ^ 2 := by
    simpa only [L] using deTurckLieLowerOrderTail_h1_bound_of_component_bounds (I := I) (M := M) g₀ g₁ g_bg
...
theorem ricciDeTurckRemainderZeroOrderPathIntegral_h1_bound
    (hDim : Module.finrank ℝ E = 3)
    (g₀ g_bg : SmoothRiemannianMetric I M)
    {δ₀ : ℝ} (hδ₀_nonneg : 0 ≤ δ₀) (hδ₀_lt : δ₀ < 1) :
    ∃ B : ℝ → ℝ,
  ...
  obtain ⟨B, hB, hcoeff⟩ :=
    ricciDeTurckRemainderZeroOrderCoefficient_h1_bound (I := I) (M := M) hDim g₀ g_bg hδ₀_nonneg hδ₀_lt
  refine ⟨B, hB, ?_⟩
