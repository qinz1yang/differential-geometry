...
private theorem rhs_raw_eq
    ...
    tensorChartComponentRaw (I := I) (M := M) gBase 0 2
        (deTurckRHSSectionBackground (I := I) gBase g) α Idx Jdx b =
      chartDeTurckRHSComp (I := I) gBase g α (Jdx 0) (Jdx 1)
        (extChartAt I α b) := by
  simpa only [chartDeTurckRHSComp_def] using
    tensorChartComponentRaw_deTurckRHSSectionBackground_eq_chartRicciLie
...
private theorem rhs_raw_sub_eq
    ...
    (Jdx : Fin 2 → Fin (Module.finrank ℝ E)) :
    tensorChartComponentRaw (I := I) (M := M) gBase 0 2
        (deTurckRHSSectionBackground (I := I) gBase g₁ -
          deTurckRHSSectionBackground (I := I) gBase g₂) α Idx Jdx b =
      chartDeTurckRHSComp (I := I) gBase g₁ α (Jdx 0) (Jdx 1)
          (extChartAt I α b) -
        chartDeTurckRHSComp (I := I) gBase g₂ α (Jdx 0) (Jdx 1)
    ...
    tensorChartComponentRaw (I := I) (M := M) gBase 0 3
      ...
      partialDeriv (E := E) (Kdx 0)
        ...
        covDerivLowerOrderTerm (I := I) (M := M) gBase 0 2
          (deTurckRHSSectionBackground (I := I) gBase g) α (Kdx 0)
    ...
    calc
      (∑ p : (Fin 0 → Fin (Module.finrank ℝ E)) ×
          ...
          |covDerivLowerOrderCoeff (I := I) (M := M) gBase 0 2 α d
            ...
            tensorChartComponentRaw (I := I) (M := M) gBase 0 2
              (deTurckRHSSectionBackground (I := I) gBase (gSeq k₁) -
                deTurckRHSSectionBackground (I := I) gBase (gSeq k₂)) α p.1 p.2
...
theorem deTurck_rhs_sobolev_one_uniform_bound {ι : Type*}
    (gBase : SmoothRiemannianMetric I M)
    (gSeq : ι → SmoothRiemannianMetric I M) (D : LowRegularityCoefficientBounds)
    (hD : HasLowRegularityCoefficientBounds (I := I) gBase gSeq D) :
    ∃ C : ℝ, 0 ≤ C ∧ ∀ k : ι,
      ‖ccTensorToHs (I := I) (M := M) gBase 2 (1 : ℝ)
        (deTurckRHSSectionBackground (I := I) gBase (gSeq k))‖ ≤ C := by
  classical
