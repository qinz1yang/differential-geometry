...
theorem exists_deTurckLieConnectionDifferenceDerivativeCoefficient_covariantJetNormSq_one_tame_bound
    (hDim : Module.finrank ℝ E = 3)
...
theorem exists_deTurckLieCovariantDerivativeInsertion_backgroundDifference_covariantJetNormSq_one_tame_bound
    (hDim : Module.finrank ℝ E = 3)
    (g₀ g_bg : SmoothRiemannianMetric I M)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ B0 B1 : ℝ → ℝ,
...
theorem lieCorrectionZeroRiemann_h1_tame_bound
    (hDim : Module.finrank ℝ E = 3)
    (g₀ : SmoothRiemannianMetric I M) {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ B0 B1 : ℝ → ℝ,
      (∀ R : ℝ, 0 ≤ R → 0 ≤ B0 R) ∧
  ...
  have hTr : (∑ i ∈ Finset.range 3,
    ...
    htrace g₁ P htie hδ_le hδ_nonneg hbound
      lieCorrectionZeroRiemPerm2 R hR hP2
  have hApp := happ
  ...
  simpa only [lieCorrectionZeroRiemannField, B1, zero_mul, add_zero,
...
theorem lieCorrectionZeroVectorBundle_h1_tame_bound
    (hDim : Module.finrank ℝ E = 3)
    (g₀ : SmoothRiemannianMetric I M) {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ B0 B1 : ℝ → ℝ,
      (∀ R : ℝ, 0 ≤ R → 0 ≤ B0 R) ∧
...
theorem lieCorrectionZeroMixedConnection_h1_tame_bound
    (hDim : Module.finrank ℝ E = 3)
...
theorem deTurckLieLowerOrderTail_h1_tame_bound
    (hDim : Module.finrank ℝ E = 3)
    (g₀ g_bg : SmoothRiemannianMetric I M)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ B0 B1 : ℝ → ℝ,
  ...
  obtain ⟨Bd0, Bd1, hBd0, hBd1, hdlb⟩ :=
    exists_deTurckLieCovariantDerivativeInsertion_backgroundDifference_covariantJetNormSq_one_tame_bound (I := I) (M := M) hDim g₀ g_bg hδ₀
  obtain ⟨Bi, hBi, hins⟩ :=
    insert_h1 (I := I) (M := M) hDim g₀ g_bg hδ₀
  obtain ⟨Bv0, Bv1, hBv0, hBv1, hvb⟩ :=
    lieCorrectionZeroVectorBundle_h1_tame_bound (I := I) (M := M) hDim g₀ hδ₀
  obtain ⟨Ba0, Ba1, hBa0, hBa1, ham⟩ :=
    lieCorrectionZeroMixedConnection_h1_tame_bound (I := I) (M := M) hDim g₀ g_bg hδ₀
  obtain ⟨Br0, Br1, hBr0, hBr1, hriem⟩ :=
    lieCorrectionZeroRiemann_h1_tame_bound (I := I) (M := M) hDim g₀ hδ₀
  let S0 : ℝ → ℝ := fun R ↦
  ...
  have hRjet := hriem g₁ P htie hδ_le hδ_nonneg hbound
    R A hR hA hP2 htop
  have hraw := deTurckLieLowerOrderTail_h1_bound_of_component_bounds (I := I) (M := M) g₀ g₁ g_bg
...
theorem ricciDeTurckRemainderZeroOrderCoefficient_h1_tame_bound_of_segment_bounds
    (hDim : Module.finrank ℝ E = 3)
    (g₀ g_bg : SmoothRiemannianMetric I M)
    ...
    ∃ B0 B1 : ℝ → ℝ,
      ...
      ∀ (T T' : SmoothCcTensor g₀ 0 2)
        ...
        ∀ s : ℝ, s ∈ Set.Icc (0 : ℝ) 1 →
          (∑ i ∈ Finset.range 2,
            ‖iteratedCovGrad (I := I) g₀ 2 2 i
              (ricciDeTurckRemainderZeroOrderCoefficient (I := I) (M := M) g₀ g_bg
                T T' hδ hδ' s)‖ ^ 2) ≤
            (B0 R + B1 R * A) ^ 2 := by
  classical
  obtain ⟨Br0, Br1, hBr0, hBr1, hric⟩ :=
    linearizedRicciConnectionDifferenceOrder0CoeffField_h1_tame_bound (I := I) (M := M) hDim g₀ hδ₀_lt
  obtain ⟨Bd0, Bd1, hBd0, hBd1, hdla⟩ :=
    exists_deTurckLieConnectionDifferenceDerivativeCoefficient_covariantJetNormSq_one_tame_bound (I := I) (M := M) hDim g₀ g_bg hδ₀_lt
  obtain ⟨Bt0, Bt1, hBt0, hBt1, htail⟩ :=
    deTurckLieLowerOrderTail_h1_tame_bound (I := I) (M := M) hDim g₀ g_bg hδ₀_lt
...
theorem ricciDeTurckRemainderZeroOrderCoefficient_h1_tame_bound
    (hDim : Module.finrank ℝ E = 3)
    (g₀ g_bg : SmoothRiemannianMetric I M)
    {δ₀ : ℝ} (hδ₀_nonneg : 0 ≤ δ₀) (hδ₀_lt : δ₀ < 1) :
    ∃ B0 B1 : ℝ → ℝ,
