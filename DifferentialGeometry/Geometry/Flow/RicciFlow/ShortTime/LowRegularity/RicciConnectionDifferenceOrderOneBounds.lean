...
private theorem core_h2
    ...
    (hA : (∑ i ∈ Finset.range 3,
      ‖iteratedCovGrad (I := I) g₀ 0 3 i
        (metricLoweredConnectionDifferenceCoefficient (I := I) g₀ g₁)‖ ^ 2) ≤ A ^ 2) :
    (∑ i ∈ Finset.range 3,
    ...
    calc
      ...
      _ = 3 * ‖iteratedCovGrad (I := I) g₀ 0 3 i
        ...
        calc
          _ = Real.sqrt (3 : ℝ) ^ 2 *
              ‖iteratedCovGrad (I := I) g₀ 0 3 i
                (metricLoweredConnectionDifferenceCoefficient (I := I) g₀ g₁)‖ := by ring_nf
          _ = _ := by rw [hs]
  have hsq : ∀ i : ℕ,
      ‖iteratedCovGrad (I := I) g₀ 3 4 i
          (connectionDifferenceContravariantInsertionField (I := I) g₀ g₁)‖ ^ 2 ≤
...
private def slotPermutationCoefficient
...
private theorem kernel_h2
    ...
    (hA : (∑ i ∈ Finset.range 3,
      ‖iteratedCovGrad (I := I) g₀ 3 4 i
        (connectionDifferenceContravariantInsertionField (I := I) g₀ g₁)‖ ^ 2) ≤ A ^ 2) :
    (∑ i ∈ Finset.range 3,
      ‖iteratedCovGrad (I := I) g₀ 3 4 i
...
theorem exists_linearizedRicciConnectionDifferenceOrderOneCoefficient_covariantJetNormSq_two_tame_bound
    (hDim : Module.finrank ℝ E = 3)
  ...
  have hkernel' : (∑ i ∈ Finset.range 3,
      ‖iteratedCovGrad (I := I) g₀ 3 4 i
        (linearizedRicciConnectionDifferenceOrder1KernelField
          (I := I) g₀ g₁)‖ ^ 2) ≤
...
theorem exists_uniform_linearizedRicciConnectionDifferenceOrderOneCoefficient_covariantJetNormSq_two_tame_bound
  ...
  obtain ⟨Capp, hCapp, happ⟩ :=
    operatorFieldComposition_h2_h2_to_h2_uniform_bound (I := IU) (M := MU) hDim gBase hΛ 3 4 2
  obtain ⟨Bt, hBt, htrace⟩ :=
    trace2_h2_uniform (I := IU) (M := MU) hDim gBase hΛ0 hδ₀
  obtain ⟨Bc0, Bc1, hBc0, hBc1, hconn⟩ :=
    connLow_tame_uniform (I := IU) (M := MU) hDim gBase hΛ0 hδ₀
  let B0 : ℝ → ℝ := fun R => Capp * (2 * Bt R) * (15 * Bc0 R)
  let B1 : ℝ → ℝ := fun R => Capp * (2 * Bt R) * (15 * Bc1 R)
  refine ⟨B0, B1, fun R hR => by
    exact mul_nonneg
  ...
  have hpure : (∑ i ∈ Finset.range 3,
    ...
    calc
      _ = ∑ i ∈ Finset.range 3,
          ‖iteratedCovGrad (I := IU) g₀ 4 2 i
            (reindexedPureTrace (I := IU) (M := MU) g₀ g₁ 2
              (Equiv.refl (Fin 4)))‖ ^ 2 := by
        apply Finset.sum_congr rfl
        intro i _
  ...
  have hout := happ g₀ hEq hjet1 hjet2
      (ricciCometricFourTraceCastG0 (I := IU) g₀ g₁)
      (linearizedRicciConnectionDifferenceOrder1KernelField (I := IU) g₀ g₁)
      (2 * Bt R) (15 * (Bc0 R + Bc1 R * A))
      (mul_nonneg (by norm_num) (hBt R hR))
      (mul_nonneg (by norm_num)
        (add_nonneg (hBc0 R hR) (mul_nonneg (hBc1 R hR) hA)))
...
theorem exists_linearizedRicciConnectionDifferenceOrderOneCoefficient_covariantJetNormSq_two_bound
    (hDim : Module.finrank ℝ E = 3)
    (g₀ : SmoothRiemannianMetric I M) {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ B : ℝ → ℝ,
      (∀ A : ℝ, 0 ≤ A → 0 ≤ B A) ∧
