...
theorem kappaDiff_h2
  ...
  have hfix : (∑ i ∈ Finset.range 3,
      ‖iteratedCovGrad (I := I) g₀ 0 3 i
        (metricLoweredConnectionDifferenceCoefficient (I := I) g₀ gB)‖ ^ 2) ≤ AF ^ 2 := by
    change SF ≤ AF ^ 2
    rw [show AF ^ 2 = SF by simp only [AF, Real.sq_sqrt hSF]]
  have hp' := hp P R hR hP
  rw [lieCorrectionZeroKappa_self_sub_eq_connectionDifferenceLowered_sub_pbLow
    (I := I) (M := M) g₀ g₁ gB P htie]
  have hle := jet_sub (I := I) (M := M) g₀ 0 3 3
    (metricLoweredConnectionDifferenceCoefficient (I := I) g₀ gB)
    (lieCorrectionZeroPbLow (I := I) (M := M) g₀ P g₀ gB) AF (BP R) hfix hp'
  change _ ≤ (B R) ^ 2
  rw [show (B R) ^ 2 = Q R by
...
theorem insert_h1
  ...
  obtain ⟨Civ, hCiv, hiprod⟩ :=
    operator_field_composition_h2_h2_to_h2_bound (I := I) (M := M) hDim g₀ 2 3 1
  ...
  let ND := lieCorrectionZeroNEndoSec (I := I) (M := M) g₀ g₁ gB -
  ...
  have hTi : (∑ i ∈ Finset.range 3,
    ...
    simpa only [Ti] using htr g₁ P htie hδ_le hδ_nonneg hbound
      lieCorrectionZeroIVPerm R hR hP
  ...
  have hID : (∑ i ∈ Finset.range 3,
    ...
    dsimp only [ID]
    rw [lieCorrectionZeroIVField_sub_eq_trace_comp_slotExtend_vflat_sub
      (I := I) (M := M) g₀ g₁ gB]
  ...
  have hCsec : (∑ i ∈ Finset.range 2,
      ‖iteratedCovGrad (I := I) g₀ 1 2 i
        (connectionDifferenceSection (I := I) g₁ g₀)‖ ^ 2) ≤ (BC R) ^ 2 :=
    hc g₁ P htie hδ_le hδ_nonneg hbound R hR hP
  have hCD : (∑ i ∈ Finset.range 2,
      ‖iteratedCovGrad (I := I) g₀ 1 1 i CD‖ ^ 2) ≤ (BD R) ^ 2 := by
    dsimp only [CD]
    rw [lieCorrectionZeroCdVField_sub_eq_comp_connectionDifference (I := I) (M := M) g₀ g₁ gB]
    simpa only [ID, BD] using
      hcdvprod ID (connectionDifferenceSection (I := I) g₁ g₀) (BI R) (BC R)
        (hBI R hR) (hBC R hR) hID hCsec
  have hN0 : (∑ i ∈ Finset.range 2,
      ‖iteratedCovGrad (I := I) g₀ 1 1 i
        (endoSlotZeroCcTensor (I := I) (M := M) g₀ 0 ND)‖ ^ 2) ≤
        (BD R) ^ 2 := by
    rw [lieCorrectionZeroNEndoSec_sub_insert_eq_lieCorrectionZeroCdVField_sub
      (I := I) (M := M) g₀ g₁ gB]
  ...
  have hSS : (∑ i ∈ Finset.range 2,
    ...
    rw [hSS_eq]
    exact hS
  rw [lieCorrectionZeroInsertion_eq_lieCorrectionZeroInsertionField (I := I) (M := M) g₀ g₁ gB,
    lieCorrectionZeroInsertion_eq_lieCorrectionZeroInsertionField (I := I) (M := M) g₀ g₁ g₀,
    lieCorrectionZeroInsertionField_sub_eq_nEndoInsert (I := I) (M := M) g₀ g₁ gB]
