...
theorem compactnessSolution
  ...
  let hgeom0 : SeqBoundedGeometry (I := I) (X.atZero (I := I)) :=
    FlowCurvatureBoundedOnCompactWindows.atZeroGeomOpen (I := I) h0 X hD hcomplete hcurv
  have hconn0 : ∀ k : Nat,
  ...
  obtain ⟨d, hcompleteL⟩ :=
    exists_smooth_flow_limit_subsequence_of_canonical_metric_compactness (I := I) canon h0 hD hcomplete hcurv
  let mc' : MetricCompactLimit (I := I) (X.atZero (I := I)) :=
